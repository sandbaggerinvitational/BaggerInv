-- Preview-only participant Playing Handicap read-projection consistency.
-- Published HCP uses the same per-side, player-ID precedence as /matches.
-- Missing/null values fall back to the snapshot; numeric zero is preserved.
-- No arithmetic, stored value changes, schema-shape changes or scoring changes.
...
begin
  ...
  -- Match and participant identity select the same published values as
