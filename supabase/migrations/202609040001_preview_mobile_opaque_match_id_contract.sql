-- Additive isolated Preview repair: mobile Match IDs are exact opaque strings.
-- Canonical contract: 1..200 Unicode scalar values; no NUL, malformed surrogate
-- sequences, or exact dot-segments ('.' / '..'). UTF8 PostgreSQL text/jsonb reject
-- NUL and malformed Unicode before the RPC runs. char_length counts scalars,
-- including one per non-BMP character. No ID trimming or normalization is allowed.
-- No table, RLS, import, scoring, or mutation authority is changed.
