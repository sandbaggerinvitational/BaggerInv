-- Preview-only participant-scoped My Match read.
-- The signed Player Passport remains the identity authority; this RPC is
-- service/server-only and receives the already trusted stable Player ID.

create index if not exists scoring_authority_match_participants_player_idx
  on scoring_authority.match_participants (player_id, match_id);
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'scores', coalesce((select jsonb_agg(to_jsonb(hs) order by hs.hole_number)
