-- Isolated Preview-only Scramble Team HCP read-projection consistency.
-- Match Detail uses the same published tournamentMatchDisplay Team Playing
-- Handicap as /matches, with identical nullish snapshot fallback. Exact Match
-- keys and numeric zero are preserved. No arithmetic or stored data changes.
-- Public mobile schemas, team strokes, participant data and privileges unchanged.

begin;

-- Fail before any definition changes if the database cannot implement the
-- approved Unicode scalar-value contract.
do $encoding$
begin
  if pg_catalog.current_setting('server_encoding') <> 'UTF8' then
    raise exception 'Opaque mobile Match IDs require UTF8 PostgreSQL encoding.';
  end if;
end;
$encoding$;
...
begin
  if pg_catalog.upper(pg_catalog.btrim(coalesce(input->>'environment', ''))) <> 'PREVIEW'
      ...
      or pg_catalog.jsonb_typeof(input->'match_id') is distinct from 'string'
      or pg_catalog.char_length(target_match) not between 1 and 200
      or target_match collate pg_catalog."C" in ('.', '..')
      or not exists (
        select 1
  ...
  where value.match_id collate pg_catalog."C" = target_match collate pg_catalog."C"
  ...
  where value.snapshot_id = match_row.scoring_snapshot_id
    and value.tournament_id = target_tournament
    and value.match_id collate pg_catalog."C" = target_match collate pg_catalog."C";
  if not found then
    return pg_catalog.jsonb_build_object(
      'ok', false, 'code', 'MATCH_DETAIL_AUTHORITY_UNAVAILABLE');
  end if;

  -- The collection's read_tournament_live_view exposes this same published
  ...
  if (select pg_catalog.count(*) from scoring_authority.teams team
      ...
      or (select pg_catalog.count(*) from scoring_authority.match_participants participant
        where participant.match_id collate pg_catalog."C" = target_match collate pg_catalog."C") not between 2 and 4
      or (select pg_catalog.count(distinct participant.team_side)
        from scoring_authority.match_participants participant
        where participant.match_id collate pg_catalog."C" = target_match collate pg_catalog."C") <> 2
      or exists (
        select 1
        from scoring_authority.match_participants participant
        where participant.match_id collate pg_catalog."C" = target_match collate pg_catalog."C"
      ...
      or (select pg_catalog.count(*) from scoring_authority.match_holes hole
        where hole.match_id collate pg_catalog."C" = target_match collate pg_catalog."C") <> 18
      or (select pg_catalog.count(*) from scoring_authority.hole_scores score
        where score.match_id collate pg_catalog."C" = target_match collate pg_catalog."C") > 18
      or exists (
    select 1
    from scoring_authority.hole_scores score
    where score.match_id collate pg_catalog."C" = target_match collate pg_catalog."C"
      and (
  ...
  where hole.match_id collate pg_catalog."C" = target_match collate pg_catalog."C";
  ...
  if exists (
      ...
      left join scoring_authority.game_center_presentations candidate_presentation
        on candidate_presentation.match_id collate pg_catalog."C" = candidate.match_id collate pg_catalog."C"
  ...
  with ordered as (
    ...
    select * from ordered where match_id collate pg_catalog."C" = target_match collate pg_catalog."C"
  ), my_match as (
    select candidate.match_id
    from ordered candidate
    join scoring_authority.match_participants participant
      on participant.match_id collate pg_catalog."C" = candidate.match_id collate pg_catalog."C"
  ...
  select pg_catalog.jsonb_build_object(
    ...
    'is_my_match', selected.match_id collate pg_catalog."C" = my_match.match_id collate pg_catalog."C"
  ...
  return pg_catalog.jsonb_build_object(
    ...
    'snapshot', pg_catalog.jsonb_build_object(
      ...
      'team_configuration', pg_catalog.jsonb_build_object(
        'team_1_playing_handicap', coalesce(
          nullif(published_match_display->'team1PlayingHcp', 'null'::jsonb),
          snapshot_row.team_configuration->'team_1_playing_handicap'),
        'team_2_playing_handicap', coalesce(
          nullif(published_match_display->'team2PlayingHcp', 'null'::jsonb),
          snapshot_row.team_configuration->'team_2_playing_handicap'),
