-- Correct the isolated fixture cleanup dependency order discovered by the service gate.
-- Match-owned rows are removed before the tournament-owned scoring snapshot.
...
as $$
declare
  fixture_year integer;
  fixture_tournament text;
  fixture_match text;
  first_snapshot uuid;
...
begin
  ...
  select candidate into fixture_year from generate_series(9000, 9999) candidate
  where not exists (select 1 from scoring_authority.tournaments where tournament_year = candidate)
  order by candidate limit 1;
  if fixture_year is null then
    return jsonb_build_object('ok', false, 'code', 'ISOLATED_FIXTURE_YEAR_UNAVAILABLE');
  end if;
  fixture_tournament := 'ARCHIVE-FIXTURE-' || fixture_year;
  fixture_match := fixture_year || '-R3-1';

  insert into scoring_authority.tournaments (tournament_id, tournament_year, name, source_workbook_id, scoring_authority)
  values (fixture_tournament, fixture_year, 'Archive Fixture', 'preview-fixture-workbook', 'SUPABASE');
  ...
  insert into scoring_authority.tournament_players (
    ...
    (fixture_tournament, 'ARCHIVE-FIXTURE-P1', 'FIXTURE-T1', 1, 'ACTIVE', fixture_year || ':P1'),
    (fixture_tournament, 'ARCHIVE-FIXTURE-P2', 'FIXTURE-T2', 2, 'ACTIVE', fixture_year || ':P2');
  ...
  delete from scoring_authority.matches where tournament_id = fixture_tournament;
  delete from scoring_authority.tournaments where tournament_id = fixture_tournament;
  delete from scoring_authority.players p where p.player_id in ('ARCHIVE-FIXTURE-P1','ARCHIVE-FIXTURE-P2')
    and not exists (select 1 from scoring_authority.tournament_players tp where tp.player_id = p.player_id);
  return result_value;
exception when others then
  if fixture_tournament is not null then
    delete from scoring_authority.matches where tournament_id = fixture_tournament;
    delete from scoring_authority.tournaments where tournament_id = fixture_tournament;
  end if;
  delete from scoring_authority.players p where p.player_id in ('ARCHIVE-FIXTURE-P1','ARCHIVE-FIXTURE-P2')
