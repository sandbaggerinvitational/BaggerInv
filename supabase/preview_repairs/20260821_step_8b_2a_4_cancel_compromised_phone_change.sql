-- Step 8B.2A.4 Preview-only cancellation of the compromised real phone-change OTP.
--
-- The provider accepted the authenticated updateUser({ phone }) request, but
-- the application rejected the pending state before verifyOtp ran. Supabase
-- exposes no supported Admin API that cancels a pending phone-change OTP. This
-- transaction therefore clears only the exact, unverified auth.users
-- phone_change residue after proving every ownership and attempt invariant.
-- It sends no SMS, stores/prints no PII or OTP, and never changes Auth UUID,
-- email, Player ownership, scoring authority, or verification status.
...
begin
  ...
  if target_attempt.status <> 'SEND_FAILED'
    ...
    raise exception 'Preview cancellation aborted: latest attempt is not the audited compromised send.';
  end if;
  ...
  if not found then
    raise exception 'Preview cancellation aborted: pending Auth phone change changed concurrently.';
  end if;

  update participant_identity.participant_phone_otp_attempts
  set status = 'CANCELLED',
      safe_reason = 'COMPROMISED_OTP_SCREENSHOT',
      updated_at = now()
  where attempt_id = target_attempt.attempt_id
    and status = 'SEND_FAILED'
    and safe_reason = 'PHONE_OTP_AUTH_MISMATCH'
    and provider_called
    and verify_failure_count = 0
    and verified_at is null
    and used_at is null;
  if not found then
    raise exception 'Preview cancellation aborted: compromised attempt changed concurrently.';
  end if;
  ...
  insert into participant_identity.identity_audit_events (
    ...
    'Step 8B.2A.4 Preview cancellation', target_attempt.attempt_id::text,
    jsonb_build_object(
      ...
      'providerOtpAcceptedByApplication', false,
...
-- PII-safe post-cancellation evidence for the SQL Editor result grid.
...
select jsonb_build_object(
  ...
  'activeAttempts', (select count(*) from participant_identity.participant_phone_otp_attempts attempt
    where attempt.auth_user_id = target.auth_user_id and attempt.status in ('REQUESTING', 'SENT')),
