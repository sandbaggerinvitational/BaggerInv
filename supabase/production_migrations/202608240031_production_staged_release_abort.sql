-- Abort only an inert, pre-cutover staged Production release.
--
-- This forward migration is installation-inert. The service-role-only RPC below
-- can clear the exact staged candidate only while every Production authority,
-- ingress, worker, publication, mirror, Google-write, and pending-work boundary
-- still proves the original Google/Passport STATIC_BACKEND state.
begin;

-- Every enabled Odds RPC holds this shared transaction lock from its scope
-- assertion through commit. Shared locks remain mutually compatible, so normal
-- Odds work stays concurrent. The staged-release abort takes the exclusive
-- counterpart and therefore cannot race a request that already passed its
-- runtime check.
