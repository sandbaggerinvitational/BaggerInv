...
- `Design/` contains the semantic native Bagger colors, typography, spacing, surfaces, controls, statuses, state treatments, and identity-image conventions shared throughout the participant app. `ios/DESIGN_SYSTEM.md` is the practical contract for later polish steps.
...
- **Step 2J.0 — COMPLETE:** a bundled native identity-asset catalog, audited canonical ID/key manifest, typed fail-closed lookup API, deterministic unit coverage, and a Debug-only visual gallery are implemented without redesigning or wiring artwork into participant screens.
...
### Matches contract authority and boundaries

The additive Match-intelligence projection supplies stable canonical team IDs, participant Playing Handicap and strokes received, Scramble side/team Playing Handicap and team strokes received, and a participant-facing display Match number where available. `HCP` always means Playing Handicap; it is never relabeled Course Handicap. Best Ball and Singles use canonical `No strokes` or `+N stroke(s)`, while Scramble uses compact `Team HCP … · No strokes/+N strokes` presentation with full Team Playing Handicap/team-strokes accessibility wording. These are Tournament Live/PWA pass-through values: Swift performs validation and formatting only.

The server orders each Round by canonical `match_sort_order` with its stable server tiebreak, and the mobile adapter plus native presenter preserve that sequence exactly. Neither opaque `matchId` nor `displayMatchNumber` is parsed or sorted by Swift. The authenticated golfer's Match remains in its natural canonical position. Participant-visible richer live differential language such as `2 UP`, dormie, or holes remaining is still outside `/matches` and is not invented locally. An empty Round absent from the collection is not manufactured, and tee-time clock/timezone values do not authorize native date inference. Owned-Match scoring context remains separately supplied by Step 2E's no-store scoring-current service.
...
## Step 2J.0 native identity-asset foundation

Step 2J.0 adds a curated `Assets.xcassets` foundation for identity artwork only. The bundled catalog contains the primary Bagger mark, 27 Player portraits, 17 team marks, 30 canonical Course-ID mappings backed by 25 unique course logos, and 10 annual tournament logos. The shared Course aliases are explicit reviewed manifest entries; Swift does not deduplicate or select them by a participant-facing name. Source files were curated from the companion `BaggerInv` website repository, primarily `public/images/players`, `public/images/teams/logos`, `public/images/courses/logos`, `public/images/tournaments/logos`, and `public/images/sandbagger-logo.png`.

`BaggerAssetManifest` maps reviewed canonical Player, Team, and Course IDs—and canonical tournament year/key pairs—to internal catalog names. `BaggerAsset` accepts those identities plus the exact server-projected asset key when one is available. A supplied key must exactly match the manifest entry. An unknown ID, an ID/key mismatch, or an unreviewed entry resolves to no image and preserves the controlled fallback: initials for Players and teams, `flag.fill` for Courses, and the primary Bagger mark for tournaments. Display names, inferred slugs, arbitrary paths, and remote URLs never become catalog authority.

This foundation does not redesign a screen or change participant navigation, layout, accessibility meaning, or mobile authority. The catalog is not yet wired into Today, Matches, Leaders, Passport, Guide, or History presentation. Existing authoritative text, initials, and SF Symbols therefore remain the participant-facing experience. Remote Course/tournament photography, Guide hero/profile imagery, and other editorial media remain deferred; Step 2J.0 adds no remote-image fetcher, remote image cache, or interpretation of Guide asset keys as filesystem or network paths.

Debug builds alone accept `--bagger-asset-gallery`, which opens a deterministic gallery of the primary mark, annual logos, team marks, Player portraits, unique Course logos, and every fallback class. The gallery is not linked from participant navigation and is compiled out of Release builds. Focused tests verify manifest counts and intentional shared Course resources, exact ID/key resolution, strict mismatch and hostile-key fallback, bundled catalog presence, representative rendering, and accessibility-XXXL fallback usability. Simulator and physical-iPhone smoke validation confirmed representative clarity, crop, transparency, scrolling, and every fallback class.

The 80 selected source image files total **7,675,855 bytes** before asset-catalog compilation. A clean Debug Simulator build compiles them into a **6,127,592-byte `Assets.car`** and increases the packaged app from **33,748 KiB to 40,156 KiB**: **6,408 KiB (19.0%)**. The eventual distribution archive still requires release/thinning measurement. Bundling also records technical provenance, not legal clearance. Player portraits, team marks, Course logos, tournament logos, and the primary mark require an explicit ownership/licensing and likeness-rights review before external distribution; any asset that is not cleared must be removed or replaced while retaining the same fail-closed fallback behavior.

## Step 2J.1 global native design system

Step 2J.1 turns the existing palette and card helpers into a semantic visual contract for the whole participant app. It defines the shared use of warm paper, evergreen, restrained gold, readable neutral text, Dynamic Type serif/sans hierarchy, tabular competition numerics, spacing, radii, borders, restrained shadows, button/control roles, status semantics, and initial-loading/empty/error/offline treatment. The full rules and later-screen boundaries are documented in `ios/DESIGN_SYSTEM.md`.

Shared identity wrappers consume only the Step 2J.0 typed resolver and preserve its canonical-ID lookup and controlled fallback behavior. Portrait and logo images do not infer identity from display names, and consuming rows own the final accessibility label so VoiceOver does not announce one identity repeatedly. The design layer formats presentation but does not decide scoring authority, publication, standings, identity, or financial results.

The DEBUG-only gallery extends the existing asset proof with deterministic colors, type roles, surfaces, controls, statuses, state treatments, rows, identity wrappers, and long-content/accessibility cases. It remains outside participant navigation and Release. Step 2J.1 performs only limited shared-surface adoption; the certified hierarchy and behavior of Today, Matches, Score, Leaders, More, Auth, and the scoring queue remain unchanged.

## Step 2J.2 Today visual and UX polish

Today uses a compact app-level header—The Bagger, the centered Today title, and the authenticated Player portrait leading to native Player Passport—above a separate branded tournament masthead. The masthead uses the canonical tournament mark and existing tournament/year/Round/status fields, keeps Preview visible as environment context, and does not invent location data that Today does not project.

The current-Match hero now uses canonical Course and team identities when their IDs resolve through the audited asset catalog, retains text/initial fallbacks, and opens the exact native Match Detail from the full card or its explicit action. Personal Match rows remain compact and text-first while opening their exact Match Details. Tournament Score remains server-projected, presents both canonical team stacks with golf half-point formatting, and opens the Score product in native Leaders. The immediate Schedule remains canonical and opens full native Schedule; generic events retain semantic SF Symbols because the Schedule contract does not expose a canonical Course identity.

The polish is presentation and native-navigation only. It does not change Today repositories, protected-read caching, identity authority, scoring state, network transport, backend contracts, or Production configuration. Dynamic Type/VoiceOver, long names, missing assets, cached/offline states, native navigation, full unit/UI regression, and physical-iPhone presentation were validated before completion.

## Step 2J.3A Matches index visual and UX polish

The Matches index now carries the shared The Bagger header and tournament masthead, retains canonical Round selection, and presents a prominent Your Match hero followed by a dense complete Match list. Canonical team and Course IDs resolve through the audited asset catalog with controlled fallbacks. Rows preserve server-projected display Match numbers, format-specific Playing Handicap/stroke language, lifecycle/result values, and canonical Match order across Best Ball, Scramble, and Singles.

The full Your Match card and each All Matches row navigate by canonical Match ID to the existing native read-only Match Detail. Returning preserves the selected Round, and the authenticated golfer's Match remains in its natural server position with restrained, non-reordering emphasis. Match Detail itself was deliberately left unchanged for separate Step 2J.3B physical review.
