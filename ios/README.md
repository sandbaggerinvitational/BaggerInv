...
- **Next — Step 2J.3:** Matches and Match Detail visual and UX polish using the shared design and identity foundations while preserving canonical Match authority, cache behavior, and navigation.
...
**Step 2J.3 — Matches and Match Detail visual and UX polish** is next. It should apply the shared design and canonical identity foundations to Round selection, Your Match, all Matches, and exact Match Detail while preserving server-owned Match state, cached-first behavior, offline presentation, and native navigation. It must not alter scoring authority, begin another tab's redesign, or enable Production. Final distribution-archive measurement and rights clearance remain explicit release prerequisites rather than inferred from the completed visual foundation.
