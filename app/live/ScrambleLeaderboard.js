...
function PairingSheet({ row, players, matches, round, roundLabel, courseName, tournament, returnTo, onClose }) {
  ...
  const breakdown = participantRoundBreakdown({ number: round, format: "SC", matches }, row.playerIds, row.points, tournament);
  return <RoundLeaderboardSheet title="Scramble Pairing" identity={<ScrambleTeamIdentity playerIds={row.playerIds} players={players} large />} roundLabel={roundLabel} formatLabel="Scramble" courseName={courseName} rank={row.displayRank} holes={row.holes} gross={row.gross} net={row.net} netToPar={toPar(row.netToPar)} points={row.points} breakdown={breakdown} officialFinal={row.officialFinal} matchId={match?.id} returnTo={returnTo} onClose={onClose} />;
}
  ...
  return <section className={styles.board} aria-label="Scramble pairing leaderboard">
    ...
    {!ranked.length ? <div className={styles.empty}><strong>{matches.length ? "Scores pending." : "Round not started."}</strong><span>{matches.length ? "The leaderboard will update as official scores are recorded." : "Pairings and scores will appear when the round opens."}</span></div> : <>
      ...
      <div className={styles.entries}>{ranked.map((row) => {
        ...
        return <LeaderboardEntry rank={row.displayRank} current={isCurrent} identity={<ScrambleTeamIdentity playerIds={row.playerIds} players={players} />} metrics={<LeaderboardMetrics metrics={[{ label: "THRU", value: thru, emphasis: final ? "" : "live" }, { label: "Gross", value: row.gross, secondary: true }, { label: "Net", value: row.net, emphasis: final ? "final" : "live" }, { label: "Net +/-", value: toPar(row.netToPar), emphasis: "live" }, { label: "Points", value: row.points === null ? "—" : formatPlayerPoints(row.points), emphasis: "points" }]} />} state={final ? "final" : "live"} onClick={() => setSelectedId(row.id)} label={`Open ${name}, rank ${row.displayRank}, ${final ? "final" : `through ${row.holes}`}, ${row.points === null ? "points pending" : `${formatPlayerPoints(row.points)} points`}, net ${row.net}, ${toPar(row.netToPar)}${isCurrent ? ", your pairing" : ""}`} key={`${row.round}-${row.id}`} />;
      })}</div>
    </>}
