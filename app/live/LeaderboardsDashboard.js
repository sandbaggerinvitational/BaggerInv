...
function NetSkinsBoard({ data, currentPlayer }) {
  ...
  const currentInWaiting = Boolean(currentPlayer?.id && waitingField.some((row) => row.playerIds?.includes(currentPlayer.id)));
  const currentWaitingEntry = currentPlayer?.id ? waitingField.find((row) => row.playerIds?.includes(currentPlayer.id)) : null;
  useEffect(() => setWaitingExpanded(false), [selectedRound]);
  const rankLabel = (row) => {
  ...
  const renderEntry = (row, ranked = false) => {
    ...
    const displayRank = ranked ? rankLabel(row) : "";
    const placement = Number(String(displayRank).match(/\d+/)?.[0]);
    const identity = isScramble
      ? <span className={skinsStyles.identityWrap}><ScrambleTeamIdentity playerIds={row.playerIds} players={data.players || []} />{isCurrent ? <em className={skinsStyles.currentBadge} aria-label="Pairing containing current player">YOU</em> : null}</span>
      : <span className={skinsStyles.identityWrap}><span className={skinsStyles.teamIdentity}><span className={skinsStyles.avatars} aria-hidden="true">{teamMembers(row).map((player) => <span key={player.id}><PlayerAvatar player={player} fallbackClassName={skinsStyles.avatarFallback} /></span>)}</span><b>{teamMembers(row).map((player) => <span key={player.id}>{player.name}</span>)}</b></span>{isCurrent ? <em className={skinsStyles.currentBadge} aria-label="Current player">YOU</em> : null}</span>;
    return <div className={skinsStyles.entry} data-ranked={ranked || undefined} data-zero={!ranked || undefined} data-leader={ranked && placement === 1 || undefined} data-current={isCurrent || undefined} key={`${row.id}-${row.skinsWon}-${row.totalWinnings}`}>
      <button type="button" className={skinsStyles.row} aria-label={`Open details for ${teamName(row)}, ${displayRank ? `${netSkinsRankAccessibleLabel(displayRank)}, ` : ""}${netSkinsCountLabel(row.skinsWon)}, ${currency(row.totalWinnings)} winnings${isCurrent ? isScramble ? ", pairing containing current player" : ", current player" : ""}`} onClick={() => setSelectedTeamId(row.id)}>
        <strong>{displayRank}</strong>{identity}<span className={skinsStyles.outcomes} data-animate="true"><span className={skinsStyles.skinMetric}><strong>{row.skinsWon}</strong><small><i className={skinsStyles.skinCoin} aria-hidden="true">S</i>{row.skinsWon === 1 ? "Skin" : "Skins"}</small></span><span className={skinsStyles.winningsMetric}><strong>{currency(row.totalWinnings)}</strong><small>Winnings</small></span></span>
      </button>
    </div>;
  };
  ...
  const stories = round ? [
    leaders.length ? { label: "Current Skin Leaders", copy: leaders.length === 1 ? `${teamName(leaders[0])} ${isScramble ? "lead" : "leads"} with ${leaders[0].skinsWon} ${leaders[0].skinsWon === 1 ? "skin" : "skins"}.` : leaders.length === 2 ? `${leaders.map(teamName).join(" and ")} share the lead with ${topSkinCount} ${topSkinCount === 1 ? "skin" : "skins"}.` : `${leaders.length} ${isScramble ? "teams" : "golfers"} share the lead with ${topSkinCount} ${topSkinCount === 1 ? "skin" : "skins"}.` } : null,
  ...
  return <section className={skinsStyles.experience} aria-label="Net Skins standings">
    ...
    <section className={skinsStyles.board}>
      ...
      {!round.leaderboard.length ? <div className={styles.empty}><strong>No eligible participants yet.</strong><span>The leaderboard will calculate automatically from official net scores.</span></div> : <div>
        ...
        <div className={skinsStyles.rankedEntries} aria-label={`${skinHolders.length} ${isScramble ? "teams" : "golfers"} holding skins`}>{skinHolders.map((row) => renderEntry(row, true))}</div>
        {waitingField.length ? <section className={skinsStyles.waitingSection}>
          <button type="button" className={skinsStyles.noSkinsToggle} aria-expanded={waitingExpanded} aria-controls={`net-skins-waiting-round-${round.round}`} onClick={() => setWaitingExpanded((value) => !value)}>
            <span><strong>No Skins Yet</strong><small>{waitingField.length} {isScramble ? waitingField.length === 1 ? "team" : "teams" : waitingField.length === 1 ? "golfer" : "golfers"}{currentInWaiting ? waitingExpanded ? " · You included" : " · Your row shown" : ""}</small></span><i aria-hidden="true">⌄</i>
          </button>
          <div className={skinsStyles.waitingEntries} id={`net-skins-waiting-round-${round.round}`} data-featured-current={!waitingExpanded && currentWaitingEntry ? "true" : undefined}>{waitingExpanded ? waitingField.map((row) => renderEntry(row)) : currentWaitingEntry ? renderEntry(currentWaitingEntry) : null}</div>
        </section> : null}
      </div>}
