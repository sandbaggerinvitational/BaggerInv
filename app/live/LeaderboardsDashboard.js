        ...
        {ranked.map((row) => {
          ...
          return <div className={styles.playerEntry} data-current={isCurrent || undefined} key={row.id}>
            <button className={styles.overallRow} type="button" aria-expanded={isOpen} aria-label={`${row.player}, rank ${row.displayRank || "unranked"}, record ${row.record}, ${metricValue(row, metric)}${isCurrent ? ", your position" : ""}`} onClick={() => setExpanded((current) => current === row.id ? "" : row.id)}>
              ...
              <span className={styles.playerIdentity}><span className={styles.playerPhoto}><PlayerAvatar filename={row.photo} name={row.player} fallbackClassName={styles.playerFallback} /></span><span><span className={badgeStyles.identityLine}><b className={badgeStyles.playerName} title={row.player}>{row.player}</b>{isCurrent ? <em className={badgeStyles.badge} aria-label="Current player">YOU</em> : null}</span><small><TeamMark filename={row.teamLogo} name={row.team} />{row.team}</small></span></span>
  ...
  const renderEntry = (row, ranked = false) => {
    ...
    return <div className={skinsStyles.entry} data-current={isCurrent || undefined} key={`${row.id}-${row.skinsWon}-${row.totalWinnings}`}>
      <button type="button" className={skinsStyles.row} aria-label={`Open details for ${teamName(row)}, ${row.skinsWon} skins, ${currency(row.totalWinnings)} winnings${isCurrent ? ", your entry" : ""}`} onClick={() => setSelectedTeamId(row.id)}>
        <strong>{ranked ? rankLabel(row) : ""}</strong><span className={skinsStyles.teamIdentity}><span className={skinsStyles.avatars}>{teamMembers(row).map((player) => <span key={player.id}><PlayerAvatar player={player} fallbackClassName={skinsStyles.avatarFallback} /></span>)}</span><b>{teamMembers(row).map((player) => <span key={player.id}>{player.name}</span>)}</b></span><span data-animate="true">{row.skinsWon}</span><strong data-animate="true">{currency(row.totalWinnings)}</strong>
      </button>
    </div>;
  };
