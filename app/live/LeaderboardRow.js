...
export function PlayerLeaderboardIdentity({ player, current = false, team = "", meta = "", large = false }) {
  return <span className={styles.identity} data-individual="true" data-large={large || undefined}>
    ...
    <span className={styles.names}><span className={styles.nameLine}><strong>{player?.name || player?.player}</strong>{current ? <em aria-label="Current player">YOU</em> : null}</span>{team || meta ? <small>{[team, meta].filter(Boolean).join(" · ")}</small> : null}</span>
  </span>;
}
...
export function LeaderboardEntry({ rank, identity, metrics, state = "live", variant = "round", podium, current = false, onClick, label, expanded, children }) {
  const Component = onClick ? "button" : "div";
  return <Component type={onClick ? "button" : undefined} className={styles.entry} data-state={state} data-variant={variant} data-podium={podium || undefined} data-current={current || undefined} onClick={onClick} aria-label={label} aria-expanded={expanded}>
