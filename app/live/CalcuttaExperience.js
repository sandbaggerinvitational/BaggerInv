...
function GolferSheet({ golfer, close, tournamentComplete }) {
  return <div className={styles.sheetLayer} role="presentation"><button type="button" className={styles.backdrop} onClick={close} aria-label="Close Calcutta golfer details" /><section className={styles.sheet} role="dialog" aria-modal="true" aria-labelledby="calcutta-golfer-name">
    ...
    <div className={styles.metrics}><p><small>Purchase Price</small><strong>{money(golfer.purchasePrice)}</strong></p><p><small>Guaranteed Winnings</small><strong>{money(golfer.guaranteedWinnings)}</strong>{!golfer.guaranteedWinnings ? <span className={styles.metricHelper}>Updates after official round results.</span> : null}</p><p><small>{tournamentComplete ? "Final Tournament Winnings" : "Projected Tournament Value"}</small><strong>{money(golfer.currentPayoutValue)}</strong>{!tournamentComplete ? <span className={styles.metricHelper}>{golfer.currentPayoutValue ? "If the tournament ended today." : "Updates as official results are published."}</span> : null}</p><p><small>{roiLabel(tournamentComplete)}</small><strong data-positive={golfer.roi > 0 || undefined}>{percent(golfer.roi)}</strong>{roiHelper(tournamentComplete) ? <span className={styles.metricHelper}>{roiHelper(tournamentComplete)}</span> : null}</p><p><small>Calcutta Points</small><strong>{golfer.totalPoints.toFixed(2).replace(/\.00$/, "")}</strong></p></div>
    ...
    <section className={styles.roundDetails}><header><span>Round Performance</span></header>{[1,2,3].map((round) => { const result = golfer.rounds[round]; return <article key={round}><h4>Round {round}{result?.format ? ` • ${formatName(result.format)}` : ""}</h4>{result ? <div><p><small>Gross</small><strong>{result.gross}</strong></p><p><small>Net</small><strong>{result.net}</strong></p><p><small>Finish</small><strong>{ordinalPlace(result.place)}{result.tieSize > 1 ? " · Tied" : ""}</strong></p><p><small>Calcutta Points</small><strong>{result.points.toFixed(2).replace(/\.00$/, "")}</strong></p><p><small>Guaranteed</small><strong>{money(result.guaranteedWinnings)}</strong></p></div> : <p><strong>Round not yet completed.</strong><span>Results will appear once official.</span></p>}</article>; })}</section>
    <footer><span>{tournamentComplete ? "Final Tournament Winnings" : "Projected Tournament Value"}</span><strong>{money(golfer.currentPayoutValue)}</strong>{!tournamentComplete ? <small>If the tournament ended today.</small> : null}</footer>
  </section></div>;
}
...
function OwnerSheet({ portfolio, close, tournamentComplete }) {
  ...
  return <div className={styles.sheetLayer} role="presentation"><button type="button" className={styles.backdrop} onClick={close} aria-label="Close Calcutta portfolio details" /><section className={styles.sheet} role="dialog" aria-modal="true" aria-labelledby="calcutta-owner-name">
    ...
    <section className={styles.portfolioSummary} aria-label="Portfolio Summary"><header><span>Portfolio Summary</span></header><div className={styles.metrics}><p><small>Golfers Owned</small><strong>{portfolio.investments.length}</strong></p><p><small>Purchase Cost</small><strong>{money(portfolio.purchaseCost)}</strong></p><p><small>Guaranteed Winnings</small><strong>{money(portfolio.guaranteedWinnings)}</strong>{!portfolio.guaranteedWinnings ? <span className={styles.metricHelper}>Updates after official round results.</span> : null}</p><p><small>{tournamentComplete ? "Final Tournament Winnings" : "Projected Tournament Value"}</small><strong>{money(portfolio.currentPayoutValue)}</strong>{!tournamentComplete ? <span className={styles.metricHelper}>{portfolio.currentPayoutValue ? "If the tournament ended today." : "Updates as official results are published."}</span> : null}</p><p><small>Net Profit</small><strong>{money(portfolio.netProfit)}</strong></p><p><small>{roiLabel(tournamentComplete)}</small><strong data-positive={portfolio.roi > 0 || undefined}>{percent(portfolio.roi)}</strong>{roiHelper(tournamentComplete) ? <span className={styles.metricHelper}>{roiHelper(tournamentComplete)}</span> : null}</p></div></section>
    <section className={styles.portfolioPerformance} aria-label="Portfolio Performance"><header><span>Portfolio Performance</span></header><div><p><small>Purchase Cost</small><strong>{money(portfolio.purchaseCost)}</strong></p><p><small>Current Value</small><strong>{money(portfolio.currentPayoutValue)}</strong></p><p><small>{roiLabel(tournamentComplete)}</small><strong data-positive={portfolio.roi >= 0 || undefined} data-negative={portfolio.roi < 0 || undefined}>{portfolio.roi >= 0 ? "▲" : "▼"} {percent(portfolio.roi)}</strong><span>{tournamentComplete ? "Since Opening Auction" : "If the tournament ended today."}</span></p></div></section>
    <section className={styles.investments}><header><span>Investments</span></header>{portfolio.investments.length ? portfolio.investments.map((investment) => <article key={investment.playerId}><span><Portrait player={investment.player} /><b>{investment.player.name}</b><small>{payoutPercent(investment.ownership)} ownership</small></span><span><small>Purchase</small><strong>{money(investment.purchasePrice)}</strong></span><span><small>Guaranteed</small><strong>{money(investment.guaranteedWinnings)}</strong></span><span><small>{tournamentComplete ? "Final" : "Projected"}</small><strong>{money(investment.currentPayoutValue)}</strong></span><span><small>{roiLabel(tournamentComplete)}</small><strong>{percent(investment.roi)}</strong></span></article>) : <p className={styles.emptyDetail}>Investments will appear after the opening auction is published.</p>}</section>
...
export default function CalcuttaExperience({ model }) {
  ...
  return <section className={styles.experience} aria-label="Calcutta">
    ...
    <section className={styles.board} aria-label={tab === "golfers" ? "Calcutta golfer standings" : "Calcutta portfolio standings"}>
      ...
      <div className={styles.row} data-header="true"><span>Rank</span><span>{tab === "golfers" ? "Golfer" : "Owner"}</span><span>{tab === "golfers" ? "Purchase" : "Cost"}</span><span>{model.tournamentComplete ? "Final" : "Projected"}</span><span>{roiLabel(model.tournamentComplete)}</span></div>
