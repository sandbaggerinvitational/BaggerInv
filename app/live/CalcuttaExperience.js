...
function GolferSheet({ golfer, close, tournamentComplete }) {
  return <div className={styles.sheetLayer} role="presentation"><button type="button" className={styles.backdrop} onClick={close} aria-label="Close Calcutta golfer details" /><section className={styles.sheet} role="dialog" aria-modal="true" aria-labelledby="calcutta-golfer-name">
    ...
    <div className={styles.metrics}><p><small>Purchase Price</small><strong>{money(golfer.purchasePrice)}</strong></p><p><small>Guaranteed Winnings</small><strong>{money(golfer.guaranteedWinnings)}</strong>{!golfer.guaranteedWinnings ? <span className={styles.metricHelper}>Updates after official round results.</span> : null}</p><p><small>{tournamentComplete ? "Final Tournament Value" : "Projected Tournament Value"}</small><strong>{money(golfer.currentPayoutValue)}</strong>{!tournamentComplete ? <span className={styles.metricHelper}>{golfer.currentPayoutValue ? "If the tournament ended today." : "Updates as official results are published."}</span> : null}</p><p><small>{roiLabel(tournamentComplete)}</small><strong {...financialState(golfer.roi)}>{percent(golfer.roi)}</strong>{roiHelper(tournamentComplete) ? <span className={styles.metricHelper}>{roiHelper(tournamentComplete)}</span> : null}</p><p><small>Calcutta Points</small><strong>{golfer.totalPoints.toFixed(2).replace(/\.00$/, "")}</strong></p></div>
...
export default function CalcuttaExperience({ model }) {
  ...
  return <section className={styles.experience} aria-label="Calcutta">
    ...
    <section className={styles.board} aria-label={tab === "golfers" ? "Calcutta golfer standings" : "Calcutta portfolio standings"}>
      ...
      <div className={styles.row} data-header="true" data-portfolio={tab === "portfolios" || undefined}><span>Rank</span><span>{tab === "golfers" ? "Golfer" : "Owner"}</span><span>{tab === "golfers" ? "Purchase" : "Cost"}</span><span>{model.tournamentComplete ? "Final" : "Projected"}</span>{tab === "portfolios" ? <span>Net Profit</span> : null}<span>ROI</span></div>
      {rows.map((row) => { const portfolio = tab === "portfolios"; const person = portfolio ? row.owner : row.player; const cost = portfolio ? row.purchaseCost : row.purchasePrice; return <button type="button" className={styles.row} data-portfolio={portfolio || undefined} onClick={() => setSelected(row)} aria-label={`Open Calcutta details for ${person.name}`} key={portfolio ? row.ownerId : row.playerId}><strong>{row.rank}</strong><span className={styles.person}><Portrait player={person} /><b>{person.name}</b></span><span>{money(cost)}</span><span>{money(row.currentPayoutValue)}</span>{portfolio ? <span {...financialState(row.netProfit)}>{signedMoney(row.netProfit)}</span> : null}<span {...financialState(row.roi)}>{percent(row.roi)}</span></button>; })}
    </section>
