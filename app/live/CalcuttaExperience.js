...
function GolferSheet({ golfer, close, tournamentComplete }) {
  return <div className={styles.sheetLayer} role="presentation"><button type="button" className={styles.backdrop} onClick={close} aria-label="Close Calcutta golfer details" /><section className={styles.sheet} role="dialog" aria-modal="true" aria-labelledby="calcutta-golfer-name">
    ...
    <section className={styles.roundDetails}><header><span>Round Performance</span></header>{[1,2,3].map((round) => { const result = golfer.rounds[round]; return <article key={round}><h4>Round {round}{result?.format ? ` • ${formatName(result.format)}` : ""}</h4>{result ? <div><p><small>Gross</small><strong>{result.gross}</strong></p><p><small>Net</small><strong>{result.net}</strong></p><p><small>Place</small><strong>{ordinalPlace(result.place)}{result.tieSize > 1 ? " · Tied" : ""}</strong></p><p><small>Points</small><strong>{result.points.toFixed(2).replace(/\.00$/, "")}</strong></p><p><small>Guaranteed Dollars</small><strong>{money(result.guaranteedWinnings)}</strong></p></div> : <p>Results have not been posted.</p>}</article>; })}</section>
