...
function Readiness({ data, busy, onStage }) {
  ...
  const scope = { targetTournamentId: data.selectedTournament.tournamentId, tournamentYear: data.selectedTournament.tournamentYear };
  return <section className={styles.card}><header><span>Owner-governed lifecycle</span><h3>Activation, Close & Archive readiness</h3></header><div className={styles.readiness}><article><State value={readiness.readyForActivation ? "READY_FOR_ACTIVATION" : "NEEDS_ATTENTION"} /><strong>{readiness.readyForActivation ? "Authoritative readiness passed" : "Setup needs attention"}</strong><span>{blockers.length ? `${blockers.length} authoritative blocker(s)` : "No readiness blockers returned."}</span></article>{blockers.map((item, index) => <article className={styles.blocker} key={`${item.code}:${index}`}><strong>{pretty(item.section || item.code)}</strong><span>{item.message || item.code}</span></article>)}</div><ReasonAction allowed={runtime.capabilities.markReady && readiness.readyForActivation && Boolean(readiness.fingerprint)} busy={busy} label="Review Ready for Activation" onStage={(reason) => onStage("mark-ready-v2", { ...scope, expectedRevision: data.selectedTournament.revision, readinessFingerprint: readiness.fingerprint, reason }, `Mark ${data.selectedTournament.tournamentYear} Ready for Activation using the current certified fingerprint`)} /><section className={styles.activation}><span>Owner confirmation required</span><h4>Activate tournament</h4><p>Activation atomically compares the current pointer and this exact readiness fingerprint. It is never inferred from dates.</p><ReasonAction allowed={runtime.capabilities.activateTournament && data.selectedTournament.lifecycle === "READY_FOR_ACTIVATION" && readiness.readyForActivation} busy={busy} label="Review activation" onStage={(reason) => onStage("activate", { ...scope, expectedRevision: data.selectedTournament.lifecycleRevision, expectedPointerRevision: runtime.currentTournament.pointerRevision, readinessFingerprint: readiness.fingerprint, reason }, `Activate ${data.selectedTournament.tournamentYear} and atomically advance the current-tournament pointer`)} /></section><section className={styles.activation}><span>Explicit post-competition lifecycle</span><h4>Close / Archive plan</h4><p>Close requires complete competition evidence. Archive execution remains unavailable; this phase creates only a reviewed immutable promotion plan.</p><ReasonAction allowed={runtime.capabilities.closeTournament} busy={busy} label="Review Close" onStage={(reason) => onStage("close", { ...scope, expectedRevision: data.selectedTournament.lifecycleRevision, completionFingerprint: readiness.fingerprint, reason }, `Close ${data.selectedTournament.tournamentYear} after certified completion`)} /><ReasonAction allowed={runtime.capabilities.prepareArchivePlan} busy={busy} label="Prepare archive plan" onStage={(reason) => onStage("prepare-archive-plan", { ...scope, expectedRevision: data.selectedTournament.lifecycleRevision, reason }, `Prepare the completed-history archive plan for ${data.selectedTournament.tournamentYear}`)} />{runtime.archivePlan ? <p><State value={runtime.archivePlan.status} /> Plan revision {runtime.archivePlan.planRevision} · {pretty(runtime.archivePlan.promotionStatus)}</p> : null}<button type="button" disabled>Archive execution unavailable</button></section></section>;
}
...
export default function ProductionFutureYearAdministrationPanel() {
  ...
  return <section className={styles.shell} aria-labelledby="future-tournaments-title"><header className={styles.hero}><div><span>Production Owner annual administration</span><h2 id="future-tournaments-title">Future Tournaments</h2><p>Prepare a future annual tournament without changing the current tournament’s lifecycle.</p></div><State value={selected?.lifecycle || "CATALOG"} /></header><div className={styles.layout}><FutureList data={data} target={target} onSelect={select} /><main className={styles.main}>{!selected ? <CreateTournament onStage={stage} allowed={data.capabilities.createTournament} busy={busy} /> : <><div className={styles.selected}><div><span>Selected annual record</span><h3>{selected.name || `Tournament ${selected.tournamentYear}`}</h3><p>{selected.destination || "Destination pending"} · Revision {selected.revision}</p></div><State value={selected.lifecycle} /></div><Basics tournament={selected} allowed={data.capabilities.editTournament} busy={busy} onStage={stage} /><Teams data={data} allowed={data.capabilities.configureTeams} busy={busy} onStage={stage} /><Roster data={data} allowed={data.capabilities.replaceRoster} busy={busy} onStage={stage} /><Rounds data={data} allowed={data.capabilities.configureRounds || data.capabilities.assignExistingCourse} busy={busy} onStage={stage} /><MatchGeneration data={data} allowed={data.capabilities.generateMatchStructure} busy={busy} onStage={stage} /><GlobalCourseLibrary data={data} busy={busy} onStage={stage} /><RuntimePromotion data={data} busy={busy} onStage={stage} /><FutureHandicaps data={data} busy={busy} onStage={stage} /><FutureRuntimeMatches data={data} busy={busy} onStage={stage} /><Readiness data={data} busy={busy} onStage={stage} /><AuditTimeline audit={data.audit} /></>}{review ? <Review review={review} confirmed={confirmed} setConfirmed={setConfirmed} busy={phase === "submitting"} onCancel={() => { setReview(null); setConfirmed(false); }} onCommit={commit} /> : null}{message ? <p className={styles.message} role="status">{message}</p> : null}{receipt ? <p className={styles.receipt}><strong>{pretty(receipt.operation)} confirmed</strong><span>Revision {receipt.revision}{receipt.idempotent ? " · safe retry" : ""}</span></p> : null}</main></div></section>;
}
