...
export function NetSkinsManagement({ operations, busy, save, initialContext = {} }) {
  ...
  const toggleRound = (round) => setDraftEligibility((current) => ({ ...current, [round]: !current[round] }));
  return <form className={styles.managementPanel} onSubmit={(event) => { event.preventDefault(); if (!pending.length) return; save("net-skins-eligibility", { playerId, updates: pending.map((item) => ({ round: item.round, eligible: Boolean(draftEligibility[item.round]) })) }); }}><label>Player<select value={playerId} onChange={(event) => selectPlayer(event.target.value)}>{playerIds.map((id) => <option value={id} key={id}>{operations.players.find((item) => item.id === id)?.name || id}</option>)}</select></label><div className={styles.skinsBulkEditor}>{configuredRounds.map((item) => { const eligible = Boolean(draftEligibility[item.round]); return <article key={item.round}><div><strong>Round {item.round} • {item.format}</strong><span>{eligible ? "Eligible" : "Ineligible"}</span></div><button className={styles.eligibilitySwitch} type="button" role="switch" aria-checked={eligible} aria-label={`Round ${item.round} ${item.format} eligibility`} disabled={Boolean(busy)} onClick={() => toggleRound(item.round)}><i aria-hidden="true" /><b>{eligible ? "ON" : "OFF"}</b></button></article>; })}</div>{pending.length ? <div className={styles.unsavedChanges} role="status"><strong>Unsaved Changes</strong><span>{pending.length} update{pending.length === 1 ? "" : "s"} pending.</span></div> : null}<button className={styles.saveSkinsButton} disabled={Boolean(busy) || !pending.length} type="submit">Save Changes</button></form>;
}
