...
export function RoundPairingsManagement({ operations, busy, save, onDirtyChange }) {
  ...
  return <form className={`${styles.managementPanel} ${styles.roundPairingsEditor}`} onSubmit={submit}>
    ...
    <div className={styles.roundPairingList}>{roundMatches.map((match) => {
      ...
      const sideFields = (side) => <fieldset><legend>{operations.teamNames?.[side] || `Team ${side}`}</legend>{Array.from({ length: slots }, (_, index) => { const field = `Team ${side} Player ${index + 1}`; return <label key={field}>{slots > 1 ? `Player ${index + 1}` : <span className={styles.srOnly}>Golfer</span>}<select aria-label={`${operations.teamNames?.[side] || `Team ${side}`} ${slots > 1 ? `Player ${index + 1}` : "Golfer"} for Match ${match.match}`} value={draft[match.id]?.[field] || ""} disabled={Boolean(busy) || finalRound} onChange={(event) => update(match.id, field, event.target.value)}><option value="">Unassigned</option>{optionsFor(side, match.id, field).map((player) => <option value={player.id} key={player.id}>{player.name}</option>)}</select></label>; })}</fieldset>;
      return <article className={styles.roundPairingCard} key={match.id} data-changed={changedMatches.some((item) => item.id === match.id) ? "true" : undefined}>
        <header><div><small>Match {match.match} • {match.teeTime || "Tee time pending"}</small><strong>{course?.name || match.courseId || "Course pending"}</strong></div>{finalRound ? <span>Finalized</span> : null}</header>
        <div className={styles.roundPairingSides}>{sideFields(1)}<b className={styles.pairingVersus} aria-hidden="true">VS</b>{sideFields(2)}</div>
      </article>;
    })}</div>
