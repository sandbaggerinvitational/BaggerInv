...
export default function DirectorDashboard({ directorName }) {
  ...
  const [shadowBenchmarkResult, setShadowBenchmarkResult] = useState(null);
  const [phase2DryRunResult, setPhase2DryRunResult] = useState(null);
  const [phase2AuthorityResult, setPhase2AuthorityResult] = useState(null);
  const [phase2Epoch, setPhase2Epoch] = useState(null);
  const [phase2MirrorEvents, setPhase2MirrorEvents] = useState([]);
  const [phase2MirrorEventId, setPhase2MirrorEventId] = useState("");
  ...
  const runShadowBenchmark = async (action) => {
    setBusy(`shadow-benchmark-${action}`); setMessage("");
    try {
      const response = await directorFetch("/api/director/scoring-shadow/benchmark", { method: "POST", credentials: "same-origin", headers: { "content-type": "application/json" }, body: JSON.stringify({ action }) });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error || "Scoring shadow benchmark failed.");
      setShadowBenchmarkResult(payload);
      recordOperation(`Shadow benchmark · ${action}`, "success", `${payload.requestMs || 0} ms`);
      setMessage(`Shadow benchmark ${action} complete · ${payload.requestMs || 0} ms`);
    } catch (error) {
      recordOperation(`Shadow benchmark · ${action}`, "failed", error.message);
      setMessage(error.message);
    } finally { setBusy(""); }
  };
  const runPhase2DryRun = async (action) => {
    setBusy(`phase2-dry-run-${action}`); setMessage("");
    try {
      const response = await directorFetch("/api/director/scoring-shadow/phase2-dry-run", { method: "POST", credentials: "same-origin", headers: { "content-type": "application/json" }, body: JSON.stringify({ action }) });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error || "Phase 2 dry run failed.");
      setPhase2DryRunResult(payload);
      recordOperation(`Phase 2 dry run · ${action}`, "success", `${payload.requestMs || 0} ms`);
      setMessage(`Phase 2 dry run ${action} complete · ${payload.requestMs || 0} ms`);
    } catch (error) {
      recordOperation(`Phase 2 dry run · ${action}`, "failed", error.message);
      setMessage(error.message);
    } finally { setBusy(""); }
  };
  const runPhase2Authority = async (action, input = {}) => {
    setBusy(`phase2-authority-${action}`); setMessage("");
    try {
      const response = await directorFetch("/api/director/scoring-authority", { method: "POST", credentials: "same-origin", headers: { "content-type": "application/json" }, body: JSON.stringify({ action, ...input }) });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error || "Phase 2 authority rehearsal failed.");
      setPhase2AuthorityResult(payload);
      if (action === "prepare-cutover" || action === "prepare-rollback") {
        setPhase2Epoch({ type: action === "prepare-cutover" ? "CUTOVER" : "ROLLBACK", id: payload.result?.prepared?.epoch_id || "" });
      }
      if (action === "commit-cutover" || action === "commit-rollback") setPhase2Epoch(null);
      const mirrorEvents = payload.result?.operations?.events || payload.result?.after?.operations?.events;
      if (Array.isArray(mirrorEvents)) {
        setPhase2MirrorEvents(mirrorEvents);
        setPhase2MirrorEventId((current) => mirrorEvents.some((event) => event.id === current) ? current : mirrorEvents[0]?.id || "");
      }
      recordOperation(`Phase 2 authority · ${action}`, "success", `${payload.requestMs || 0} ms`);
      setMessage(`Phase 2 authority ${action} complete`);
    } catch (error) {
      recordOperation(`Phase 2 authority · ${action}`, "failed", error.message);
      setMessage(error.message);
    } finally { setBusy(""); }
  };
  ...
  const selectedMirrorEvent = phase2MirrorEvents.find((event) => event.id === phase2MirrorEventId) || null;
  return <section className={styles.shell}>
    ...
    {data.qaTools ? <OperationsSection id="preview-tools" eyebrow="Preview only" title="Preview Tools" summary="Impersonate a golfer, inspect the scoring shadow, or reset Dress Rehearsal runtime state."><section className={styles.qaTools} aria-labelledby="qa-tools-title"><header><span>Preview only</span><h2 id="qa-tools-title">QA Tools</h2></header><label><span>Preview As</span><select value={testPlayerId} disabled={Boolean(busy)} onChange={(event) => { const playerId = event.target.value; setTestPlayerId(playerId); previewAsPlayer(playerId); }}>{data.qaTools.players.map((player) => <option value={player.id} key={player.id}>{player.name}</option>)}</select></label><p>{busy === "impersonation" ? "Opening player preview…" : "Preview the app as the selected golfer."}</p>{resetComplete ? <div className={styles.resetComplete} role="status"><strong>Preview Tournament Reset Complete</strong><span>Ready for Dress Rehearsal.</span></div> : null}<div className={styles.shadowUtility}><div><strong>Scoring Shadow Diagnostics</strong><span>Inspect, replay, or rebuild Google-verified observations. This never writes Google.</span></div><label><span>Match ID</span><input value={shadowSelection.matchId} onChange={(event) => setShadowSelection((current) => ({ ...current, matchId: event.target.value }))} /></label><label><span>Hole</span><input inputMode="numeric" value={shadowSelection.holeNumber} onChange={(event) => setShadowSelection((current) => ({ ...current, holeNumber: event.target.value }))} /></label><div className={styles.shadowActions}><button disabled={Boolean(busy)} onClick={inspectShadow}>{busy === "shadow-inspect" ? "Inspecting…" : "Inspect Shadow"}</button><button disabled={Boolean(busy) || !shadowInspection?.observation} onClick={replayShadow}>{busy === "shadow-replay" ? "Replaying…" : "Replay Exact Observation"}</button><button disabled={Boolean(busy)} onClick={rebuildShadow}>{busy === "shadow-rebuild" ? "Rebuilding…" : "Rebuild Full Shadow"}</button></div><div className={styles.shadowActions} aria-label="Phase 1 benchmark stages"><button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("baseline")}>Baseline</button><button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("corrections")}>Corrections</button><button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("replay")}>Replay Benchmark</button><button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("gate-a")}>Gate A</button><button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("burst")}>11-Hole Burst</button><button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("concurrency")}>12-Match Concurrency</button><button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("gate-b")}>Gate B</button><button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("google-failure")}>Google Failure</button><button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("final-rebuild")}>Final Rebuild</button></div>{shadowInspection ? <div className={styles.shadowResult} role="status"><strong>{shadowInspection.observation ? `${shadowInspection.observation.match_id} · Hole ${shadowInspection.observation.hole_number} · ${shadowInspection.observation.comparison_status}` : "No matching observation"}</strong><span>Events {shadowInspection.counts.score_mirror_events} · Holes {shadowInspection.counts.hole_score_mirror} · Matches {shadowInspection.counts.live_match_mirror} · Runs {shadowInspection.counts.mirror_reconciliation_runs} · Delivery {shadowInspection.observation?.delivery_count || 0}</span>{shadowInspection.observation?.comparison_status === "DIVERGENCE" ? <span>{JSON.stringify(shadowInspection.observation.comparison_diagnostics || {})}</span> : null}{shadowRebuildResult ? <><span>Rebuild {shadowRebuildResult.summary?.pass ? "PASS" : "DIVERGENCE"} · {shadowRebuildResult.timings?.totalRequestDurationMs || 0} ms</span><span>Google {shadowRebuildResult.timings?.googleReadDurationMs || 0} ms · Normalize {shadowRebuildResult.timings?.normalizationDurationMs || 0} ms · Supabase {shadowRebuildResult.timings?.supabaseRebuildDurationMs || 0} ms · Reconcile {shadowRebuildResult.timings?.reconciliationDurationMs || 0} ms · Total {shadowRebuildResult.timings?.totalDurationMs || 0} ms</span></> : null}</div> : null}{shadowBenchmarkResult ? <div className={styles.shadowResult} role="status"><strong>Benchmark {shadowBenchmarkResult.action}</strong><span>{shadowBenchmarkResult.requestMs} ms · Authorization {shadowBenchmarkResult.authorizationMs} ms</span><span>{JSON.stringify(shadowBenchmarkResult.result)}</span></div> : null}</div><div className={styles.resetUtility}><div><strong>Dress Rehearsal</strong><span>Clear runtime tournament results while preserving setup and content.</span></div><button disabled={Boolean(busy)} onClick={() => { setResetComplete(false); setResetOpen(true); }}>🔄 Reset Preview Tournament</button></div></section></OperationsSection> : null}
    {data.qaTools ? <OperationsSection id="phase2-authority-dry-run" eyebrow="Preview only" title="Phase 2 Authority-Equivalent Dry Run" summary="Exercise isolated Postgres transactions. Google remains authoritative and participant scoring never reads these fixtures."><section className={styles.shadowUtility}><div className={styles.shadowActions}><button disabled={Boolean(busy)} onClick={() => runPhase2DryRun("seed")}>Seed 24 Fixtures</button><button disabled={Boolean(busy)} onClick={() => runPhase2DryRun("equivalence")}>Formats Equivalence</button><button disabled={Boolean(busy)} onClick={() => runPhase2DryRun("corrections")}>Revision Corrections</button><button disabled={Boolean(busy)} onClick={() => runPhase2DryRun("concurrency")}>12 Writable Matches</button><button disabled={Boolean(busy)} onClick={() => runPhase2DryRun("same-match")}>Same-Match Races</button><button disabled={Boolean(busy)} onClick={() => runPhase2DryRun("post-clinch")}>Post-Clinch 18</button><button disabled={Boolean(busy)} onClick={() => runPhase2DryRun("finalization")}>Finalization Race</button><button disabled={Boolean(busy)} onClick={() => runPhase2DryRun("reads")}>Read Benchmarks</button><button disabled={Boolean(busy)} onClick={() => runPhase2DryRun("failures")}>Failure Injection</button><button disabled={Boolean(busy)} onClick={() => runPhase2DryRun("diagnostics")}>Dry-Run Diagnostics</button></div><div className={styles.shadowActions} aria-label="Authority-equivalent mutation samples">{Array.from({ length: 30 }, (_, index) => <button disabled={Boolean(busy)} key={`phase2-baseline-${index + 1}`} onClick={() => runPhase2DryRun(`baseline-${index + 1}`)}>Mutation {index + 1}</button>)}</div><div className={styles.shadowActions} aria-label="Separated Google timing samples">{Array.from({ length: 10 }, (_, index) => <button disabled={Boolean(busy)} key={`phase2-google-${index + 1}`} onClick={() => runPhase2DryRun(`google-timing-${index + 1}`)}>Google Timing {index + 1}</button>)}</div>{phase2DryRunResult ? <div className={styles.shadowResult} role="status"><strong>Phase 2 dry run · {phase2DryRunResult.action}</strong><span>{phase2DryRunResult.requestMs} ms · Authorization {phase2DryRunResult.authorizationMs} ms</span><span>{JSON.stringify(phase2DryRunResult.result)}</span></div> : null}</section></OperationsSection> : null}
    {data.qaTools ? <OperationsSection id="participant-identity-foundation" eyebrow="Preview only · Protected identity" title="Player / Participant Identity" summary="Review canonical email ownership, Auth links, and eligible mobile authentication identifiers. Mobile entry never sends SMS or verifies a phone."><ParticipantIdentityFoundationPanel /></OperationsSection> : null}
    {data.qaTools ? <OperationsSection id="phase2-preview-authority" eyebrow="Preview only · Controlled authority" title="Phase 2 Preview Authority Readiness" summary="Verify parity, enforce ingress leases, and operate the explicit Preview cutover or rollback epoch."><section className={styles.shadowUtility}><div><strong>Controlled authority operations</strong><span>Prepare pauses scoring ingress. Commit changes only the canonical Preview epoch; the server environment remains separately controlled.</span></div><div className={styles.shadowActions}><button disabled={Boolean(busy)} onClick={() => runPhase2Authority("preflight")}>Preflight</button><button disabled={Boolean(busy)} onClick={() => runPhase2Authority("import")}>Import Canonical State</button><button disabled={Boolean(busy)} onClick={() => runPhase2Authority("reconcile")}>Reconcile Google ↔ Supabase</button><button disabled={Boolean(busy)} onClick={() => runPhase2Authority("readiness")}>Readiness Gate</button><button disabled={Boolean(busy)} onClick={() => runPhase2Authority("mirror-diagnostics")}>Inspect Mirror Outbox</button><select aria-label="Select mirror outbox event" value={phase2MirrorEventId} disabled={Boolean(busy) || !phase2MirrorEvents.length} onChange={(event) => setPhase2MirrorEventId(event.target.value)}><option value="">Select mirror event</option>{phase2MirrorEvents.map((event) => <option key={event.id} value={event.id}>{event.match_id} · {event.event_type} · revision {event.match_revision} · {event.status}</option>)}</select><button disabled={Boolean(busy) || !selectedMirrorEvent?.claimable} onClick={() => runPhase2Authority("deliver-mirror-event", { eventId: selectedMirrorEvent.id, matchId: selectedMirrorEvent.match_id, confirmDelivery: true })}>Deliver Selected Mirror Event</button><button disabled={Boolean(busy)} onClick={() => runPhase2Authority("refresh-game-center-presentations")}>Refresh Game Center Presentation</button><button disabled={Boolean(busy)} onClick={() => runPhase2Authority("game-center-parity")}>Verify Game Center Parity</button><button disabled={Boolean(busy)} onClick={() => runPhase2Authority("migrate-preview-scoring-lock-schema")}>Migrate Preview Scoring Lock</button><button disabled={Boolean(busy) || !reopenId} onClick={() => runPhase2Authority("repair-finalization-parity", { matchId: reopenId })}>Repair Selected Final Parity</button><button disabled={Boolean(busy)} onClick={() => runPhase2Authority("prepare-cutover")}>Pause + Prepare Cutover</button><button disabled={Boolean(busy) || phase2Epoch?.type !== "CUTOVER" || !phase2Epoch.id} onClick={() => runPhase2Authority("commit-cutover", { epochId: phase2Epoch.id })}>Commit Cutover Epoch</button><button disabled={Boolean(busy)} onClick={() => runPhase2Authority("prepare-rollback")}>Pause + Prepare Rollback</button><button disabled={Boolean(busy) || phase2Epoch?.type !== "ROLLBACK" || !phase2Epoch.id} onClick={() => runPhase2Authority("commit-rollback", { epochId: phase2Epoch.id })}>Drain + Commit Rollback</button></div>{phase2AuthorityResult ? <div className={styles.shadowResult} role="status"><strong>Phase 2 authority · {phase2AuthorityResult.action}</strong><span>{phase2AuthorityResult.requestMs} ms</span><span>{JSON.stringify(phase2AuthorityResult.result)}</span></div> : null}</section></OperationsSection> : null}
    {data.qaTools ? <OperationsSection id="shadow-benchmark-suite" eyebrow="Preview only" title="Scoring Shadow Benchmark Suite" summary="Run the authorized Phase 1 stages against reversible Preview test data."><section className={styles.shadowUtility}><div className={styles.shadowActions}><button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("preflight")}>Preflight</button><button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("baseline")}>Single-Hole Baseline</button><button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("corrections")}>Corrections</button><button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("replay")}>Idempotent Replay</button><button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("gate-a")}>Gate A</button><button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("burst")}>11-Hole Burst</button><button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("concurrency")}>12-Match Concurrency</button><button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("two-device")}>Two-Device Conflict</button><button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("finalization-race")}>Finalization Gate</button><button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("gate-b")}>Gate B</button><button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("supabase-failure")}>Supabase Failure Isolation</button><button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("google-failure")}>Google Failure Isolation</button><button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("final-rebuild")}>Final Rebuild</button></div>{shadowBenchmarkResult ? <div className={styles.shadowResult} role="status"><strong>Benchmark {shadowBenchmarkResult.action}</strong><span>{shadowBenchmarkResult.requestMs} ms · Authorization {shadowBenchmarkResult.authorizationMs} ms</span><span>{JSON.stringify(shadowBenchmarkResult.result)}</span></div> : null}</section></OperationsSection> : null}
    {data.qaTools ? <OperationsSection id="shadow-benchmark-batches" eyebrow="Preview only" title="Benchmark Sample Batches" summary="Short, Vercel-safe reversible requests for percentile collection."><section className={styles.shadowUtility}><div className={styles.shadowActions}><button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("repair-interrupted-baseline")}>Restore Interrupted Hole 3</button>{Array.from({ length: 10 }, (_, index) => <button disabled={Boolean(busy)} key={`baseline-${index + 1}`} onClick={() => runShadowBenchmark(`baseline-${index + 1}`)}>Baseline {index + 1}</button>)}{Array.from({ length: 6 }, (_, index) => <button disabled={Boolean(busy)} key={`correction-${index + 1}`} onClick={() => runShadowBenchmark(`correction-${index + 1}`)}>Correction {index + 1}</button>)}</div>{shadowBenchmarkResult ? <div className={styles.shadowResult} role="status"><strong>Benchmark {shadowBenchmarkResult.action}</strong><span>{shadowBenchmarkResult.requestMs} ms · Authorization {shadowBenchmarkResult.authorizationMs} ms</span><span>{JSON.stringify(shadowBenchmarkResult.result)}</span></div> : null}</section></OperationsSection> : null}
    {data.qaTools ? <button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("repair-interrupted-baseline")}>Restore Latest Interrupted Benchmark Sample</button> : null}
    {data.qaTools ? <OperationsSection id="shadow-burst-segments" eyebrow="Preview only" title="Burst and Concurrency Segments" summary="Quota-safe orchestration for the authorized stress workload."><section className={styles.shadowUtility}><div className={styles.shadowActions}>{Array.from({ length: 4 }, (_, index) => <button disabled={Boolean(busy)} key={`burst-forward-${index + 1}`} onClick={() => runShadowBenchmark(`burst-forward-${index + 1}`)}>Burst Forward {index + 1}</button>)}{Array.from({ length: 4 }, (_, index) => <button disabled={Boolean(busy)} key={`burst-restore-${index + 1}`} onClick={() => runShadowBenchmark(`burst-restore-${index + 1}`)}>Burst Restore {index + 1}</button>)}<button disabled={Boolean(busy)} onClick={() => runShadowBenchmark("supabase-concurrency")}>Supabase 12-Match Replay</button></div>{shadowBenchmarkResult ? <div className={styles.shadowResult} role="status"><strong>Benchmark {shadowBenchmarkResult.action}</strong><span>{shadowBenchmarkResult.requestMs} ms · Authorization {shadowBenchmarkResult.authorizationMs} ms</span><span>{JSON.stringify(shadowBenchmarkResult.result)}</span></div> : null}</section></OperationsSection> : null}
    ...
    {data.qaTools ? <OperationsSection id="read-parity-certification" eyebrow="Preview only · Read-only" title="Live Read Parity Certification" summary="Compare the existing Google presentation contract with the canonical Supabase tournament and leaderboard projections."><section className={styles.shadowUtility}><div className={styles.shadowActions}><button disabled={Boolean(busy)} onClick={() => runPhase2Authority("tournament-parity")}>Verify Tournament Parity</button><button disabled={Boolean(busy)} onClick={() => runPhase2Authority("leaderboards-core-parity")}>Verify Leaderboards Parity</button></div></section></OperationsSection> : null}
