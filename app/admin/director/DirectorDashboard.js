...
export default function DirectorDashboard({ directorName }) {
  ...
  const runPhase2Authority = async (action, input = {}) => {
    ...
    try {
      ...
      setMessage(`Phase 2 authority ${action} complete · Google authority unchanged`);
    } catch (error) {
  ...
  return <section className={styles.shell}>
    ...
    {data.qaTools ? <OperationsSection id="phase2-preview-authority" eyebrow="Preview only · Authority stays Google" title="Phase 2 Preview Authority Readiness" summary="Import and reconcile the canonical aggregate, exercise durable Google outbox delivery, and rehearse cutover/rollback against an isolated tournament."><section className={styles.shadowUtility}><div><strong>Pre-cutover controls</strong><span>These controls cannot activate authority for the 2026 Preview tournament. SCORING_AUTHORITY remains Google.</span></div><div className={styles.shadowActions}><button disabled={Boolean(busy)} onClick={() => runPhase2Authority("preflight")}>Preflight</button><button disabled={Boolean(busy)} onClick={() => runPhase2Authority("import")}>Import Canonical State</button><button disabled={Boolean(busy)} onClick={() => runPhase2Authority("reconcile")}>Reconcile Google ↔ Supabase</button><button disabled={Boolean(busy)} onClick={() => runPhase2Authority("outbox-rehearsal", { cycles: 2 })}>Google Outbox Rehearsal</button><button disabled={Boolean(busy)} onClick={() => runPhase2Authority("outbox-failures")}>Outbox Failure Recovery</button><button disabled={Boolean(busy)} onClick={() => runPhase2Authority("cutover-rollback-rehearsal")}>Isolated Cutover + Rollback</button><button disabled={Boolean(busy)} onClick={() => runPhase2Authority("readiness")}>Readiness Gate</button></div>{phase2AuthorityResult ? <div className={styles.shadowResult} role="status"><strong>Phase 2 authority · {phase2AuthorityResult.action}</strong><span>{phase2AuthorityResult.requestMs} ms · Google remains authoritative</span><span>{JSON.stringify(phase2AuthorityResult.result)}</span></div> : null}</section></OperationsSection> : null}
