  ...
  return <section className={styles.shell}>
    ...
    <section className={styles.command} aria-labelledby="command-title">
      ...
      <div className={styles.commandGrid}>
        ...
        {data.timelineAvailable ? <div><small>Next Event</small><strong>{data.nextEvent?.title || "No remaining scheduled events today."}</strong><span>{data.nextEvent?.countdown || ""}</span></div> : null}
      </div>
    </section>

    {data.timelineAvailable ? <section className={styles.nextEvent} aria-labelledby="next-event-title"><header><span>Tournament countdown</span><h2 id="next-event-title">Next Event</h2></header>{data.nextEvent ? <div><div><strong>{data.nextEvent.title}</strong><span>{data.nextEvent.subtitle}</span><small>{data.nextEvent.startTime}</small></div><div><b>{data.nextEvent.countdown}</b><span>{data.nextEvent.status}</span></div></div> : <p>No remaining scheduled events today.</p>}</section> : null}
