...
function FieldEditor({ field, value, onChange }) {
  ...
  return <label><span>{field.label}</span><input type={type} value={value || ""} onChange={(event) => onChange(event.target.value)} /></label>;
}
...
export function StandingsPanel({ secret, year }) {
  ...
  return <section className={styles.manager}><header className={styles.heading}><div><p>Official results</p><h2>Standings</h2><span>Calculated live from finalized matches. Public pages continue to use the same source rows.</span></div><button type="button" onClick={load}>Recalculate</button></header>{status ? <div className={styles.notice}>{status}</div> : null}{data ? <><div className={styles.teamStandings}>{data.teams.map((team) => <article key={team.side}><span>{team.name}</span><strong>{team.points.toFixed(1)}</strong></article>)}</div><div className={styles.standingsTable}><div><b>Rank</b><b>Player</b><b>Record</b><b>Points</b></div>{data.players.map((player, index) => <div key={player.id}><span>{index + 1}</span><strong>{player.name}</strong><span>{player.wins}-{player.losses}-{player.halves}</span><b>{player.points.toFixed(2)}</b></div>)}</div></> : null}</section>;
}
