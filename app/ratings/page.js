...
export default async function RatingsPage(){const env=await applicationPageEnvironment();const useSupabase=isSupabaseSecondaryHistory(env);const secondaryHistory=useSupabase?await loadSecondaryHistoryModel({env}):null;if(!useSupabase)await refreshHistoricalData();const ratings=useSupabase?secondaryHistory.calculations.getSandbaggerRatings():getSandbaggerRatings();return <main data-secondary-history-source={useSupabase?"supabase":"google"}><Header/><section className={styles.hero}><p>PLAYER POWER INDEX</p><h1>Sandbagger Rating</h1><span>The official player-strength metric of the Sandbagger Invitational.</span></section><section className={styles.content}><div className={styles.explainer}><h2>The Sandbagger Rating (SBR)</h2><p>The Sandbagger Rating is our proprietary player rating system designed specifically for the Sandbagger Invitational. Ratings use an enhanced Elo model that considers match results, opponent strength, and tournament performance. Separate ratings are maintained for every competition format, allowing players to develop distinct strengths while every match also contributes to an independent Overall SBR.</p><p>Every player begins at <strong>{ELO_START}</strong>. Halves count as <strong>0.5</strong>, and the base update factor is <strong>K={ELO_K}</strong>. Overall updates after every match; format ratings update only when that format is played.</p></div><RatingsTable ratings={ratings}/></section><Footer/></main>}
