...
export default function OddsCenter({ snapshots, error }) {
  ...
  if (isTournamentRecapPhase(current.phase)) {
    ...
    return <><section className={styles.hero}><p>SBI Analytics</p><h1>Tournament Recap</h1><span>The official conclusion to this year’s Championship Projection story.</span></section><section className={styles.shell}>
      <nav className={styles.timeline}>{snapshots.map((snapshot)=><button className={snapshot.phase===current.phase?styles.active:""} onClick={()=>setSelectedPhase(snapshot.phase)} key={snapshot.phase}><i/><span>{projectionPresentationLabel(snapshot.phase)}</span><small>{new Date(snapshot.publishedAt).toLocaleDateString()}</small></button>)}</nav>
  ...
  return <>
    ...
    <section className={styles.shell}>
      <nav className={styles.timeline}>{snapshots.map(s=><button className={s.phase===current.phase?styles.active:""} onClick={()=>setSelectedPhase(s.phase)} key={s.phase}><i/><span>{projectionPresentationLabel(s.phase)}</span><small>{new Date(s.publishedAt).toLocaleDateString()}</small></button>)}</nav>
      ...
      <div className={styles.teamGrid}>{current.teams.map(t=><article key={t.side}><span>Championship Odds</span><h3>{t.name}</h3><strong>{t.probability.toFixed(1)}%</strong><b>{t.americanOdds}</b><p>Expected points · Out of {current.totalPointsAvailable || 72} <em>{t.expectedPoints.toFixed(2)}</em></p></article>)}</div>
      ...
      <section className={styles.playerTimeline}><span>Player Projection Timeline</span>{current.players.slice(0,8).map(p=><div key={p.id}><b>{p.name}</b>{snapshots.map(s=>{const x=s.players.find(v=>v.id===p.id);return <span key={s.phase}><small>{projectionPresentationLabel(s.phase)}</small><strong>{s.phase==="Final Results"&&x?.probability===100?"Winner":x?.americanOdds||"—"}</strong></span>})}</div>)}</section>
    </section>
  </>;
}
