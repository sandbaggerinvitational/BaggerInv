...
export default function OddsCenter({ snapshots, error, portraits = {} }) {
  ...
  return <>
    ...
    <section className={styles.shell}>
      ...
      <ResponsiveDisclosure className={styles.playerHistory} desktopOpen><summary>View Projection History</summary><section className={styles.playerTimeline}><span>Player Projection Timeline</span>{current.players.slice(0,8).map(p=><div key={p.id}><b>{p.name}</b>{snapshots.map(s=>{const x=s.players.find(v=>v.id===p.id);return <span key={s.phase}><small>{projectionPresentationLabel(s.phase)}</small><strong>{s.phase==="Final Results"&&x?.probability===100?"Winner":formatChampionshipOdds(x?.americanOdds)}</strong></span>})}</div>)}</section></ResponsiveDisclosure>
    </section>
  </>;
}
