...
export function LeaderboardPlayer({ name, slug, photo, compact = false, linked = true, prefetch, participantPresentation = false }) {
  const label = linked && slug ? <Link href={`${participantPresentation ? "/app/players" : "/players"}/${slug}`} prefetch={prefetch}>{name}</Link> : <strong>{name}</strong>;
...
export default function TournamentLeaderboard({ rows = [], pointsTracked = true, prefetchPlayerLinks, emptyMessage = "No completed matches have been recorded yet.", participantPresentation = false }) {
  ...
  return <div className={styles.table} data-points={pointsTracked ? "true" : "false"}>
    ...
    {normalized.length ? normalized.map((row) => {
      ...
      return <div className={styles.row} data-place={place || undefined} key={row.id}>
        ...
        <LeaderboardPlayer name={row.name} slug={row.slug} photo={row.photo} prefetch={prefetchPlayerLinks} participantPresentation={participantPresentation} />
