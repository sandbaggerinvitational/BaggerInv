  ...
  try {
    ...
    const response = await client.responses.create({
      ...
      instructions: [
        ...
        "Write a sharp, matchup-specific scouting briefing in 190-280 words; every sentence must use a distinct supplied metric, player, pairing sample, stroke allocation, or simulation output.",
        "Return short paragraphs separated by blank lines and start them with these exact labels: THE VERDICT:, MODEL RECONCILIATION:, PLAYER IMPACT:, PAIRING CHEMISTRY:, STROKE STORY:, UPSET PATH:, CAPTAIN'S MOVES:.",
        "If simulation data is present, also include PROJECTED MATCH FLOW:. If it is absent, omit that paragraph and do not speculate about finish holes.",
        "Use evidence labels naturally: strong evidence, moderate evidence, limited evidence, or no reliable sample. Never describe missing history as even.",
        "Give a distinct captain recommendation for each team. Never recommend a lineup change unless optimizer data supports it.",
        "Explicitly reconcile opposing factors so the explanation agrees with the displayed favorite and probabilities.",
        "Calibrate favorite language to the probability gap: under 3 points toss-up, under 8 slight edge, under 16 clear edge, under 26 strong favorite, otherwise heavy favorite.",
        "Keep each paragraph concise. Do not use markdown headings, tables, generic golf advice, or run-on sentences.",
        "Vary the opening language, cadence, golf metaphors, and strategic angle from matchup to matchup.",
        "Avoid stock phrases including viable path, match play remains volatile, gets the nod, not a walkover, this is not automatic, real upset path, and the model says.",
