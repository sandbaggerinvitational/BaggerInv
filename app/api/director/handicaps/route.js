...
function safeFailure(error) {
  return {
    ...
    code: /^(?:PRODUCTION_)?HANDICAP_[A-Z0-9_]{3,120}$/.test(clean(error?.code))
      ? clean(error.code)
...
export async function GET(request) {
  ...
  try {
    ...
    }, () => readProductionHandicapManagement(actor(access.identity)));
...
export async function POST(request) {
  ...
  if (!new Set(["stage", "validate", "approve"]).has(action)) {
    return NextResponse.json({
