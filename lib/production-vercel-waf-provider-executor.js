...
export const PRODUCTION_VERCEL_WAF_EXECUTOR_ACTIONS = Object.freeze([
  "INSTALL", "REATTEST", "RESTORE", "RECOVER_REJECTED", "RETIRE_REJECTED",
]);
...
function recoveredRejectedEpochContract(epoch) {
  const nonce = lower(epochField(epoch, "runOwnedRuleNonce", "run_owned_rule_nonce"));
  const ruleName = epochField(epoch, "runOwnedRuleName", "run_owned_rule_name");
  const candidateAliasOrigin = lower(epochField(
    epoch, "candidateAliasOrigin", "candidate_alias_origin",
  ));
  const candidateImmutableOrigin = lower(epochField(
    epoch, "candidateImmutableOrigin", "candidate_immutable_origin",
  ));
  const insert = buildProductionGoogleWriterCriticalWindowVercelRuleInsert({
  ...
  if (insert.runOwnedRuleFingerprint !== lower(epochField(
    epoch, "runOwnedRuleFingerprint", "run_owned_rule_fingerprint",
  )) || insert.runOwnedInsertDocumentFingerprint !== lower(epochField(
    epoch,
    "runOwnedInsertDocumentFingerprint",
    "run_owned_insert_document_fingerprint",
  ))) {
    throw executorError(
      "STEP11_6_VERCEL_WAF_EXECUTOR_RECOVERED_CONTRACT_INVALID",
      "The recovered WAF epoch did not match its exact historical rule contract.",
      {}, 409,
    );
  }
  return Object.freeze({ nonce, ruleName, insert });
...
function publicEpoch(epoch = {}, providerReadback = null) {
  ...
  return Object.freeze({
    ...
    recoverableRejected: epoch.recoverableRejected === true ||
      epoch.recoverable_rejected === true,
...
async function executeRetireRejected(context) {
  const { control, provider, credentials, now, epochId, candidate,
    environment, operationRequestId } = context;
  const epoch = await control.recoverRejectedCriticalWafEpoch({
    environment,
  ...
  const contract = recoveredRejectedEpochContract(epoch);
  const evidenceCandidate = Object.freeze({
    ...candidate,
    aliasOrigin: lower(epochField(
      epoch, "candidateAliasOrigin", "candidate_alias_origin",
    )),
    immutableOrigin: lower(epochField(
      epoch, "candidateImmutableOrigin", "candidate_immutable_origin",
    )),
  });
  const request = evidenceRequest({
...
async function executeRecoverRejected({ control, environment }) {
  const epoch = await control.recoverRejectedCriticalWafEpoch({ environment });
  return Object.freeze({
    ok: true,
    action: "recover-rejected-vercel-waf-provider-epoch",
...
export async function executeProductionVercelWafProviderAction(
  ...
  if (action === "RECOVER_REJECTED") {
    return executeRecoverRejected({ control, environment });
  }
