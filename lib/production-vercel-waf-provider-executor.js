...
function providerClient({ candidate, credentials, fetchImpl }) {
  ...
  return Object.freeze({
    read: async () => {
      ...
      return assertProviderScope(result.payload, candidate);
    },
...
function evidenceRequest({
  ...
  return Object.freeze({
    ...
    evidenceRequestId: derivedUuid(epochId, `${stage}-evidence-request`),
    wafEpochId: epochId,
    transitionRequestId: transitionRequestId ||
      derivedUuid(epochId, `${stage}-transition-request`),
    stage,
    ...
    criticalEvidenceId: restored ? derivedUuid(epochId, "critical-active-evidence") : null,
    ...
    criticalSemanticFingerprint: restored ? lower(epochField(
...
async function executeInstall(context) {
  ...
  const insertMutation = Object.freeze({
    ...
    perform: () => provider.mutate("/draft", contract.insert.body),
  });
...
export async function executeProductionVercelWafProviderAction(
  ...
  return action === "INSTALL"
    ? executeInstall(context)
    : executeRestore(context, input);
}
