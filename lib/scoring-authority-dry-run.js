...
const formatCode = (value) => {
  ...
  if (["BB", "BESTBALL"].includes(normalized)) return "BB";
  if (["SC", "SCRAMBLE"].includes(normalized)) return "SC";
  if (["SI", "SINGLES", "SINGLE"].includes(normalized)) return "SI";
  return normalized;
};
const courseKey = (value) => clean(value).toUpperCase().replace(/[^A-Z0-9]/g, "");

export function resolveScoringAuthorityCourseSnapshot({ historicalMatch = {}, currentMatch = {}, courses = [], courseHoles = [] } = {}) {
  const year = number(historicalMatch.Year || currentMatch.Year);
  const format = formatCode(historicalMatch.Format || currentMatch.Format);
  const requestedCourseId = clean(historicalMatch["Course ID"] || currentMatch["Course ID"]);
  const requestedCourseKey = courseKey(requestedCourseId);
  const yearMatches = (item) => !item.Year || number(item.Year) === year;
  const course = courses.find((item) => requestedCourseKey && courseKey(item["Course ID"]) === requestedCourseKey && yearMatches(item))
    || courses.find((item) => yearMatches(item) && formatCode(item.Format) === format)
    || {};
  const courseId = clean(requestedCourseId || course["Course ID"]);
  const tee = clean(historicalMatch.Tee || historicalMatch["Tee Played"] || currentMatch.Tee || currentMatch["Tee Played"] || course["Tee Played"] || course.Tee);
  const resolvedMatch = { ...historicalMatch, "Course ID": courseId, Tee: tee };
  const holes = courseHoles.filter((item) => courseKey(item["Course ID"]) === courseKey(courseId) && (!tee || !item.Tee || clean(item.Tee).toUpperCase() === tee.toUpperCase()));
...
export function mergeScoringAuthorityHoleResults(existingHoles = [], newest = {}) {
  const newestHole = number(newest.holeNumber || newest.hole_number);
  return [
    ...existingHoles
      .filter((hole) => number(hole.holeNumber || hole.hole_number) !== newestHole)
      .map((hole) => ({
        holeNumber: number(hole.holeNumber || hole.hole_number),
        winner: clean(hole.winner || hole.hole_winner),
      })),
    newest,
  ].sort((left, right) => number(left.holeNumber || left.hole_number) - number(right.holeNumber || right.hole_number));
}

export function scoringAuthorityDryRunFixtureFromRow(row = {}) {
  return {
    ...
    scoring_rules_version: clean(row.scoring_rules_version || row.scoring_snapshot?.scoring_rules_version),
    status: "LIVE",
    scoring_locked: false,
    permission_revision: Math.max(1, number(row.permission_revision, 1)),
    match_revision: 0,
