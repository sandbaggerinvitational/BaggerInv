...
function singlesOutcome(results) {
  ...
  let team2 = 0;
  let decided = null;
  for (let holeNumber = 1; holeNumber <= currentHole; holeNumber += 1) {
    const result = unique.get(holeNumber);
    if (result.winner === "Team 1") team1 += 1;
    if (result.winner === "Team 2") team2 += 1;
    const remainingAtHole = 18 - holeNumber;
    const leadAtHole = Math.abs(team1 - team2);
    if (!decided && leadAtHole > remainingAtHole) {
      decided = {
        winner: team1 > team2 ? "Team 1" : "Team 2",
        lead: leadAtHole,
        holesRemaining: remainingAtHole,
      };
    }
  }
  if (decided) return { ...decided, complete: true, currentHole, scorecardComplete: currentHole === 18 };
  const lead = Math.abs(team1 - team2);
  const complete = currentHole === 18;
  const winner = !complete ? "" : team1 === team2 ? "Halved" : team1 > team2 ? "Team 1" : "Team 2";
  return { winner, complete, currentHole, holesRemaining, lead, scorecardComplete: currentHole === 18 };
}

export function isScorecardComplete(holeResults = []) {
  const holes = new Set(holeResults
    .map((item) => Number(item.holeNumber))
    .filter((holeNumber) => Number.isInteger(holeNumber) && holeNumber >= 1 && holeNumber <= 18));
  return holes.size === 18 && Array.from({ length: 18 }, (_, index) => index + 1).every((holeNumber) => holes.has(holeNumber));
}
...
export function hashAccessToken(token, salt) {
  if (!clean(token) || !clean(salt)) throw new Error("Access token and salt are required.");
  return createHash("sha256").update(`${salt}:TOKEN:${clean(token)}`).digest("hex");
}

export function accessTokenMatches(token, expectedHash, salt) {
  if (!expectedHash) return false;
  const actual = Buffer.from(hashAccessToken(token, salt), "hex");
  const expected = Buffer.from(clean(expectedHash), "hex");
  return actual.length === expected.length && timingSafeEqual(actual, expected);
}
