...
function canonicalHalfPoint(value) {
  if (!finite(value)) return null;
  const pointValue = Number(value);
  return Number.isInteger(pointValue * 2) ? pointValue : null;
}

function completedPoint(value) {
...
/**
 * Audit the read-only 2023 Matches authority without consulting or rounding a
 * separately stored Tournament Final. Every accepted allocation must be an
 * exact half-point increment and the complete 6/6/12 match population must be
 * present before a participant-facing total can be projected.
 */
export function auditCanonical2023TournamentPoints({ matches = [] } = {}) {
  const rows = matches
    .filter((match) => Number(value(match, "Year", "year")) === TARGET_YEAR)
    .map((match) => ({
      matchId: clean(value(match, "Match ID", "matchId", "id")),
      round: roundNumber(match),
      match: Number(value(match, "Match", "match", "matchNumber")),
      team1Points: canonicalHalfPoint(value(match, "Team 1 Points", "team1Points")),
      team2Points: canonicalHalfPoint(value(match, "Team 2 Points", "team2Points")),
      result: clean(firstValue(match, "Matchup Winner", "matchupWinner", "18-Hole Winner", "overallWinner")),
    }))
    .sort((left, right) => left.round - right.round || left.match - right.match);
  const uniqueMatchIds = new Set(rows.map((row) => row.matchId).filter(Boolean));
  const expectedRoundCounts = new Map([[1, 6], [2, 6], [3, 12]]);
  const rounds = [...expectedRoundCounts].map(([round, expectedMatches]) => {
    const roundRows = rows.filter((row) => row.round === round);
    return {
      round,
      expectedMatches,
      matches: roundRows.length,
      team1Points: roundRows.reduce((total, row) => total + (row.team1Points ?? 0), 0),
      team2Points: roundRows.reduce((total, row) => total + (row.team2Points ?? 0), 0),
      complete: roundRows.length === expectedMatches && roundRows.every((row) =>
        row.matchId && row.team1Points !== null && row.team2Points !== null && row.result
      ),
    };
  });
  const complete = rows.length === 24 &&
    uniqueMatchIds.size === 24 &&
    rows.every((row) => row.team1Points !== null && row.team2Points !== null && row.result) &&
    rounds.every((round) => round.complete);
  return {
    year: TARGET_YEAR,
    rows,
    rounds,
    complete,
    team1Points: complete ? rows.reduce((total, row) => total + row.team1Points, 0) : null,
    team2Points: complete ? rows.reduce((total, row) => total + row.team2Points, 0) : null,
  };
}

/**
 * Restore a reconciled 2023 Tournament Final as derived presentation only.
 * The tournament and match source rows remain untouched; incomplete or
 * ambiguous team identity fails closed to the separately supplied source row.
 */
export function projectCanonical2023TournamentFinal({ tournament, matches = [] } = {}) {
  ...
  if (!audit.complete || ![1, 2].includes(championSide) || ![1, 2].includes(runnerUpSide) || championSide === runnerUpSide) {
    return { tournament, audit, applied: false };
  }
  const pointsForSide = (side) => side === 1 ? audit.team1Points : audit.team2Points;
  const finalScore = `${completedPoint(pointsForSide(championSide))} - ${completedPoint(pointsForSide(runnerUpSide))}`;
  return {
