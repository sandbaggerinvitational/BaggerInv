...
export function history2026ReadEnvironment(env = process.env) {
  ...
  const approvedProject = projectRef === PREVIEW_HISTORY_2026_SUPABASE_PROJECT_REF;
  ...
  const supabaseEligible = previewDeployment && previewWorkbook && approvedProject &&
  ...
  const reason = resolved === "supabase" ? "preview-supabase-2026-history"
...
export function isSupabaseHistory2026(year, env = process.env) {
  ...
  return Number(year) === PREVIEW_HISTORY_2026_TOURNAMENT_YEAR &&
    state.previewDeployment && state.requested !== "google";
}
