...
export function normalizeTournamentTimeline({ values, rows, activeYear, tournamentStatus, timeZone, sheetState, now = new Date(), previewDate = "", previewEnabled = false } = {}) {
  ...
  const events = parsed.rows.map((row, index) => {
    ...
    return { ...event, status: timelineEventStatus(event, { now: effectiveNow, tournamentStatus, timeZone }) };
