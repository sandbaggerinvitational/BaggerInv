...
function roundNumber(value) {
  ...
  const canonicalLabel = source.match(/^Round ([1-9][0-9]?)$/i);
  return integer(canonicalLabel ? canonicalLabel[1] : value, { minimum: 1, maximum: 99 });
}
...
function roundFormatDto(row = {}, catalog = {}) {
  ...
  return {
    roundNumber: roundNumber(row.Round),
    format,
...
export function mobileGuideDataFromProjection(read = {}, identity = {}, dependencies = {}) {
  ...
  requireValue(isCanonicalReviewer(identity?.context) || (identifier(identity?.playerId) && identity.context?.membership?.active === true));
  return guideGolfData(read, identity, dependencies);
}

// Pure published golf presentation. The public caller applies its independent
// allowlist and must never expose contacts/account/administrative products.
export function mobilePublicGuideGolfData(read) {
  return guideGolfData(read, {tournamentId: "2026"}, {});
}
function guideGolfData(read, identity, dependencies) {
  const tournamentId = identifier(identity.tournamentId);
