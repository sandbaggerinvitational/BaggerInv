...
export const rehearseSupabaseOddsSnapshot = (input, options = {}) => publishSupabaseOddsSnapshot({ ...input, rehearsal: true }, options);
export const claimSupabaseOddsGoogleMirror = (input, options = {}) => scoringShadowRpc("claim_preview_championship_odds_google_mirror", { input }, { ...options, timeoutMs: options.timeoutMs || 15_000 });
...
export const readSupabaseOddsPublicationDiagnostics = (tournamentId = "", options = {}) => scoringShadowRpc("read_preview_championship_odds_publication_diagnostics", { target_tournament_id: clean(tournamentId) }, { ...options, timeoutMs: options.timeoutMs || 10_000 });
...
function matchRows(view = {}, calculated = {}) {
  ...
  return (view.matches || []).map((entry) => {
    ...
    const lifecycle = clean(match.status).toUpperCase();
    const final = lifecycle === "FINAL";
    const row = {
      Year: number(view.tournament?.tournament_year), Round: number(match.round_number), Format: clean(match.format),
      "Match ID": clean(match.match_id), "Match Status": lifecycle,
      // A reopened match retains its scorecard but has no official result.
      // Supplying retained live points here would make the unchanged Odds
      // engine re-award an invalidated Final during a later milestone.
      "Team 1 Points": final ? live.team1Points : null,
      "Team 2 Points": final ? live.team2Points : null,
    };
