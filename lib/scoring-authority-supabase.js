...
export function buildCanonicalScoringAuthorityImport({ sheets = {}, sourceWorkbookId, requestedBy = "Phase 2 import" } = {}) {
  ...
  const activeTeams = [1, 2].map((side) => {
    ...
    return {
      ...
      source_payload: { Year: year, "Team Side": `Team ${side}`, "Team ID": clean(row["Team ID"]), "Team Names": clean(row["Team Names"] || row["Team Name"]) },
    };
  });
  ...
  for (const current of liveMatches) {
    ...
    const state = canonicalMatchState(current, matchHoles, fixture);
    ...
    matches.push({
      ...
      scoring_snapshot_id: snapshotId, status: state.status, scoring_locked: truthy(current["Scoring Locked"]),
...
export function reconcileCanonicalScoringAuthority(expectedImport, currentState = {}) {
  ...
  const snapshotDivergence = expected.snapshots.filter((wanted) => !currentState.snapshots?.some((row) => row.snapshot_id === wanted.snapshot_id && row.canonical_hash === wanted.canonical_hash)).map((row) => row.match_id);
