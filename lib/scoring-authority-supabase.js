...
async function productionOnlyScoringOperationsRpc(name, input, options = {}) {
  if (!useProductionScoringOperations(options.env || process.env)) {
  ...
  const { productionScoringOperationsRpc } = await import("./production-scoring-operations-server.js");
  return productionScoringOperationsRpc(name, input, options);
}

export async function claimFutureMatchGoogleCompatibility(workerId = randomUUID(), options = {}) {
  return productionOnlyScoringOperationsRpc(
    "claim_production_future_match_google_compatibility_v1",
    {
      contract_version: "production-future-google-match-provisioning-v1",
      target_tournament_id: clean(options.targetTournamentId),
      worker_id: workerId,
