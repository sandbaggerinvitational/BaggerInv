...
function useProductionScoringOperations(env = process.env) {
  const production = clean(env.VERCEL_ENV).toLowerCase() === "production";
  const selected = clean(env.SCORING_AUTHORITY || "google").toLowerCase() === "supabase";
  const activated = /^(?:1|true|yes|on|enabled)$/i.test(clean(env.PRODUCTION_CUTOVER_ACTIVATION_ENABLED));
  const workers = /^(?:1|true|yes|on|enabled)$/i.test(clean(env.PRODUCTION_SUPABASE_WORKERS_ENABLED));
  return production && (selected || activated || workers);
}

async function scoringOperationsRpc(productionName, input, options, previewName, previewBody) {
  const env = options?.env || process.env;
  if (!useProductionScoringOperations(env)) {
    return scoringShadowRpc(previewName, previewBody, options);
  }
  // Keep this legacy/Preview-capable module importable in plain Node contract
  // tests; load the server-only Production transport only when Production has
  // explicitly requested the cutover operations.
  const { productionScoringOperationsRpc } = await import("./production-scoring-operations-server.js");
  return productionScoringOperationsRpc(productionName, input, options);
}

export async function readCanonicalScoringAuthority(input, options = {}) {
  return scoringOperationsRpc("read_production_scoring_authority", input, {
    ...options, timeoutMs: options.timeoutMs || 12_000,
  }, "read_preview_scoring_authority", { input });
}
...
export async function readPreviewScoringParticipantContext(input, options = {}) {
  return scoringOperationsRpc("read_production_scoring_participant_context", input, {
    ...options, timeoutMs: options.timeoutMs || 12_000,
  }, "read_preview_scoring_participant_context", { input });
}

export async function submitCanonicalHoleScore(input, options = {}) {
  return scoringOperationsRpc("submit_production_hole_score", input, options,
    "submit_hole_score_authoritative", { input });
}

export async function finalizeCanonicalMatch(input, options = {}) {
  return scoringOperationsRpc("finalize_production_match", input, options,
    "finalize_match_authoritative", { input });
}

export async function reopenCanonicalMatch(input, options = {}) {
  return scoringOperationsRpc("reopen_production_match", input, options,
    "reopen_match_authoritative", { input });
}

export async function mutateCanonicalMatchControl(input, options = {}) {
  if (!useProductionScoringOperations(options.env || process.env)) {
    const error = new Error("This control mutation is not supported by the Preview scoring adapter.");
    error.code = "OPERATION_NOT_SUPPORTED_UNDER_SUPABASE_AUTHORITY";
    error.status = 409;
    throw error;
  }
  return scoringOperationsRpc("mutate_production_match_control", input, options,
    "", { input });
}
...
export async function claimGoogleOutbox(workerId = randomUUID(), options = {}) {
  const input = { worker_id: workerId, lease_seconds: options.leaseSeconds || 30 };
  return scoringOperationsRpc("claim_production_google_outbox", input, options,
    "claim_preview_google_outbox", input);
}

export async function completeGoogleOutbox(input, options = {}) {
  return scoringOperationsRpc("complete_production_google_outbox", input, options,
    "complete_preview_google_outbox", { input });
}

export async function failGoogleOutbox(input, options = {}) {
  return scoringOperationsRpc("fail_production_google_outbox", input, options,
    "fail_preview_google_outbox", { input });
}

export async function inspectScoringMirrorOperations(input = {}, options = {}) {
  return scoringShadowRpc("inspect_preview_scoring_mirror_operations", { input }, {
    ...options,
    timeoutMs: options.timeoutMs || 12_000,
  });
}

export async function claimGoogleOutboxEvent(input, options = {}) {
  return scoringOperationsRpc("claim_production_google_outbox_event", input, options,
    "claim_preview_google_outbox_event", { input });
}
...
export async function claimScorecardArchiveJob(workerId = randomUUID(), options = {}) {
  ...
  return scoringOperationsRpc("claim_production_scorecard_archive_job", input, options,
    "claim_preview_scorecard_archive_job", input);
}

export async function completeScorecardArchiveJob(input, options = {}) {
  return scoringOperationsRpc("complete_production_scorecard_archive_job", input, options,
    "complete_preview_scorecard_archive_job", { input });
}

export async function failScorecardArchiveJob(input, options = {}) {
  return scoringOperationsRpc("fail_production_scorecard_archive_job", input, options,
    "fail_preview_scorecard_archive_job", { input });
}

export async function claimFutureMatchGoogleCompatibility(workerId = randomUUID(), options = {}) {
  if (!useProductionScoringOperations(options.env || process.env)) {
    const error = new Error("This operation is available only to the active Production Supabase worker runtime.");
    error.code = "PRODUCTION_SCORING_WORKER_RUNTIME_REQUIRED";
    error.status = 404;
    throw error;
  }
  const { productionFutureGoogleWriterRpc } = await import(
    "./production-future-google-writer-server.js"
  );
  return productionFutureGoogleWriterRpc("claim", {
    worker_id: workerId,
    lease_seconds: options.leaseSeconds || 90,
  }, options);
}

export async function completeFutureMatchGoogleCompatibility(input, options = {}) {
  const { productionFutureGoogleWriterRpc } = await import(
    "./production-future-google-writer-server.js"
  );
  return productionFutureGoogleWriterRpc("complete", input, options);
}

export async function failFutureMatchGoogleCompatibility(input, options = {}) {
  const { productionFutureGoogleWriterRpc } = await import(
    "./production-future-google-writer-server.js"
  );
  return productionFutureGoogleWriterRpc("fail", input, options);
}

export async function resolveFutureMatchGoogleCompatibilityContext(
  targetTournamentId,
  options = {},
) {
  if (!useProductionScoringOperations(options.env || process.env)) {
  ...
  const { resolveProductionFutureGoogleWriterContext } = await import(
    "./production-future-google-writer-server.js"
  );
  return resolveProductionFutureGoogleWriterContext({
    targetTournamentId,
    ...options,
  });
}
...
export async function inspectScorecardArchiveState(input = {}, options = {}) {
  return scoringOperationsRpc("inspect_production_scorecard_archive_state", input, options,
    "inspect_preview_scorecard_archive_state", { input });
}
