...
export function buildCanonicalScoringAuthorityImport({ sheets = {}, sourceWorkbookId, requestedBy = "Phase 2 import" } = {}) {
  ...
  for (const current of liveMatches) {
    ...
    const historical = historicalScoringSnapshotForMatch(current, archivedMatches.get(matchId)) || current;
    ...
    // A permanent Final archive is written by the supported Google finalization
    // transaction. The supported reopen transaction clears that archive in the
    // same operation, so a matching Final archive also owns lifecycle even when
    // the mutable Live Matches row has regressed to Live. This is the same
    // authority rule used by the public Google tournament model.
    const state = canonicalMatchState(historical, matchHoles, fixture);
    ...
    matches.push({
      ...
      scoring_snapshot_id: snapshotId, status: state.status,
      scoring_locked: state.status === "FINAL" || truthy(current["Scoring Locked"]),
    ...
    for (const side of [1, 2]) for (const player of fixture.scoring_snapshot.participants[`team_${side}`]) {
      ...
      permissions.push({ match_id: matchId, player_id: player.id, can_score: truthy(current["Access Active"]) && state.status !== "FINAL",
  ...
  return { payload, fingerprint: canonicalAuthorityFingerprint(payload),
