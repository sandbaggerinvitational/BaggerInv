...
export function myMatchReadEnvironment(env = process.env) {
  const configured = clean(env.MY_MATCH_READ_SOURCE || "google").toLowerCase();
  ...
  const cutover = productionCutoverReadSourceEnvironment({
    env, variable: "MY_MATCH_READ_SOURCE", configuredValue: env.MY_MATCH_READ_SOURCE,
  ...
  return {
    ...
    reason: resolved === "supabase" ? (candidate.eligible ? "production-shadow-supabase-my-match" : "preview-supabase-read")
