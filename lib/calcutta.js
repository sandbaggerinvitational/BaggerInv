...
export function buildCalcuttaModel({ year, players = {}, purchases = [], ownership = [], pointStructure = [], payoutStructure = [], roundResults = [], standings = [], ownerLeaderboard = [] }) {
  ...
  for (const round of [1, 2, 3]) {
    ...
    const payoutRanks = rankWithTieAverages(eligible, (entry) => entry.net, "asc", (place, count) => awardForPlaces(payoutsForYear, round, place, count, "payout") * payoutScale);
  ...
  if (highestGuaranteed?.guaranteedWinnings) storylines.push({ icon: "🏆", title: "Largest Guaranteed Winner", subject: highestGuaranteed.player.name, detail: `${highestGuaranteed.guaranteedWinnings.toLocaleString("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 })} secured from completed rounds.` });
  if (!tournamentComplete && highestUpside?.remainingUpside) storylines.push({ icon: "↗", title: "Highest Remaining Upside", subject: highestUpside.player.name, detail: `${highestUpside.remainingUpside.toLocaleString("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 })} above guaranteed winnings.` });
  if (portfolios[0]) storylines.push({ icon: "💼", title: "Most Valuable Portfolio", subject: portfolios[0].owner.name, detail: `${portfolios[0].currentPayoutValue.toLocaleString("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 })} in projected tournament value.` });
