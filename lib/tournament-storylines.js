...
function completedRoundStories(rounds, tournament) {
  return rounds.flatMap((round) => {
    ...
    return winner ? [story(
      ...
      `The ${formatTeamPoints(one)}–${formatTeamPoints(two)} round score moved the tournament race forward.`, 4 + Number(round.number || 0),
    )] : [];
  });
}
...
function teamRaceStory(insights, tournament, completedRounds) {
  ...
  if (leader.tied) return story(
    ...
    `${leader.namesLabel} are tied at ${formatTeamPoints(leader.points)} ${pointWord(leader.points)}.`, 8,
  );
  const one = Number(tournament?.teamOne?.score) || 0;
  const two = Number(tournament?.teamTwo?.score) || 0;
  ...
  const detail = lead === 0
    ...
    ? `${winner}' lead stands at ${formatTeamPoints(lead)} points after clinching Round ${latestRoundNumber}.`
    : lastRound
  ...
  return story("team-race", "💪", "Team Race", headline, detail, 8);
}
...
function playerStories(insights, complete) {
  ...
  if (undefeated.length === 1) {
    ...
    stories.push(story("undefeated", "🏅", complete ? "Perfect Records" : "Undefeated", `${undefeated.length} players remain unbeaten.`, "The tournament's unbeaten group is still standing.", 14, `Undefeated: ${undefeated.length} players remain unbeaten: ${names}.`));
  }
  return stories;
}
