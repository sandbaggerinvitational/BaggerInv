...
function completedRoundStories(rounds, tournament) {
  return rounds.flatMap((round) => {
    ...
    if (one === two) return [story(`round-${round.number}`, "🤝", "Round Complete", `${label} finished level.`, `${formatTeamPoints(one)}–${formatTeamPoints(two)} across ${matches.length} matches.`, 18 + Number(round.number || 0))];
    const winner = one > two ? tournament?.teamOne?.name : tournament?.teamTwo?.name;
    return winner ? [story(`round-${round.number}`, "🏆", "Round Clinched", `${winner} clinched ${label}.`, `${formatTeamPoints(one)}–${formatTeamPoints(two)} round score.`, 18 + Number(round.number || 0))] : [];
  });
}
