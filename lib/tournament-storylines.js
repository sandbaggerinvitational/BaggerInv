...
function playerStories(insights, complete, now) {
  ...
  const unbeaten = insights.unbeaten;
  if (unbeaten.length === 1) {
    const player = unbeaten[0];
    stories.push(story({ id: "unbeaten", category: "player-record", icon: "🏅", label: "Unbeaten", headline: `${player.player} is the final unbeaten player.`, detail: `${player.record} through ${player.matchesPlayed} completed match${player.matchesPlayed === 1 ? "" : "es"}, with zero losses.`, basePriority: STORY_PRIORITY.unbeaten, now }));
  } else if (unbeaten.length > 1) {
    const names = unbeaten.map((player) => player.player).join(", ");
    stories.push(story({ id: "unbeaten", category: "player-record", icon: "🏅", label: "Unbeaten", headline: `${unbeaten.length} players remain unbeaten.`, detail: "Every player in the group still has zero match losses.", basePriority: STORY_PRIORITY.unbeaten, now, accessibleLabel: `Unbeaten: ${unbeaten.length} players remain unbeaten: ${names}.` }));
  }
  return stories;
}
...
export function tournamentStorylines(data = {}, options = {}) {
  ...
  const performance = playerPerformanceRows(current.leaderboard, current.scoreLeaderboard, rounds);
