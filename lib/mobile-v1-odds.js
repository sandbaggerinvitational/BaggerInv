...
export function mobileProductionOddsView(value = {}, identity = {}) {
  const publication = value.publication || {};
  const state = clean(publication.state).toUpperCase();
  requireValue(clean(value.tournament?.tournament_id) === clean(identity.tournamentId) &&
    publication.authority === "SUPABASE" && publication.google_publication_fallback === false &&
    ["UNPUBLISHED", "WITHDRAWN", "PUBLISHED"].includes(state));
  const revision = integer(publication.publication_pointer_revision);
  const current = list(value.snapshots).filter((row) => row.is_current_official === true);
  if (state === "PUBLISHED") {
    requireValue(publication.stale !== true && current.length === 1 &&
      current[0].publication_verified === true &&
      clean(publication.snapshot_id));
    const snapshotRevision = current[0].publication_state_revision ??
      (publication.adoption_kind === "LEGACY_GOOGLE_ADOPTED" &&
        current[0].authority_contract_version === "legacy-google-published-odds-v1"
        ? current[0].publication_revision : null);
    requireValue(integer(snapshotRevision, { minimum: 1 }) ===
      integer(publication.current_publication_revision, { minimum: 1 }));
  }
  return {
    tournament_id: identity.tournamentId,
    publication: {
      // Withdrawal is an explicit canonical revocation, not a failed read.
      state: state === "PUBLISHED" ? state : "UNPUBLISHED",
      revision,
      published_at: state === "PUBLISHED" ? publication.published_at : null,
      current_milestone: state === "PUBLISHED" ? current[0].milestone : null,
    },
    snapshots: state === "PUBLISHED"
      ? list(value.snapshots).filter((row) => row.publication_lifecycle !== "WITHDRAWN") : [],
  };
}

// Read-only identity enrichment. Published values and names stay in their
// immutable snapshot; team IDs come only from the current canonical tournament.
export function mobileOddsWithCanonicalTeams(view, core, identity) {
  requireValue(clean(core?.tournament?.tournament_id) === clean(identity.tournamentId));
  const teams = boundedList(core?.teams, 2, 2);
  requireValue(new Set(teams.map(t => Number(t.team_side))).size === 2 &&
    new Set(teams.map(t => clean(t.team_id))).size === 2 &&
    teams.every(t => [1, 2].includes(Number(t.team_side)) && MOBILE_ID.test(clean(t.team_id))));
  return { ...view, snapshots: view.snapshots.map(snapshot => ({ ...snapshot,
    payload: { ...snapshot.payload, teams: boundedList(snapshot.payload?.teams, 2, 2).map(team => {
      const canonical = teams.find(t => Number(t.team_side) === Number(team.side));
      requireValue(canonical);
      const existing = clean(team.id || team.teamId);
      requireValue(!existing || existing === clean(canonical.team_id));
      return existing ? team : { ...team, teamId: clean(canonical.team_id) };
    }) },
  })) };
}
...
export async function mobileOddsResult(identity, { env = process.env, now, dependencies = {} } = {}) {
  ...
  try {
    if (clean(env.VERCEL_ENV).toLowerCase() === "production") {
      const { requireMobileProductionReadContext } = await import("./mobile-v1-production-read-context.js");
      const runtime = await requireMobileProductionReadContext(identity, { env, dependencies });
      const readPublished = dependencies.readPublishedOddsView ||
        (await import("./published-odds-supabase.js")).readPublishedOddsView;
      read = await readPublished({ tournamentId: identity.tournamentId }, { env });
      requireValue(read?.payload?.ok === true);
      let view = mobileProductionOddsView(read.payload.data, identity);
      if (view.publication.state === "PUBLISHED") {
        const readCore = dependencies.readLeaderboardsCoreView ||
          (await import("./leaderboards-core-supabase.js")).readLeaderboardsCoreView;
        const core = await readCore(identity.tournamentId, { env });
        requireValue(core?.payload?.ok === true);
        view = mobileOddsWithCanonicalTeams(view, core.payload.data, identity);
      }
      read = { payload: { ok: true, data: view } };
      await requireMobileProductionReadContext(identity, { env, dependencies, expectedRuntime: runtime });
    } else read = await readProduct("ODDS", identity, { env, dependencies });
  } catch {
    throw unavailable();
  }
