  ...
  return stats.seasons
    ...
    .map((season) => {
      ...
      return {
        ...
        pointsRecorded: Number(season.overall.recordedPointMatches) > 0,
...
/**
 * Player-profile intelligence consumes canonical record-holder membership;
 * its presentation scorecards remain responsible only for Career metrics.
 */
export function buildPlayerIntelligence({
  ...
  recordsHeld = [],
  holePlayers: precomputedHolePlayers,
  matchProgression: precomputedMatchProgression,
}) {
  const completeCards = eligibleScorecards(scorecards);
  const holePlayers = Array.isArray(precomputedHolePlayers)
    ? precomputedHolePlayers
    : buildPlayerHoleByHoleAnalytics(scorecards, {
        playerNames: Object.fromEntries(allPlayerStats.map(({ player }) => [
          player["Player ID"],
          player["Display Name"],
        ])),
      });
  ...
  const progression = precomputedMatchProgression ||
    buildMatchProgressionAnalytics(scorecards, { ghostMatchExclusions });
  ...
  return {
    ...
    rankings,
    rankingRows: [
      { key: "careerPoints", label: "Career Points", rank: rankings.careerPoints },
      { key: "matchWins", label: "Match Wins", rank: rankings.matchWins },
      { key: "winPercentage", label: "Win Percentage", rank: rankings.winPercentage },
      { key: "holeDifferential", label: "Hole Differential", rank: rankings.holeDifferential },
      { key: "birdies", label: "Birdies", rank: rankings.birdies },
      { key: "averageGross", label: "Average Gross", rank: rankings.averageGross },
    ],
