...
export function buildParticipantHomePresentationImport({ liveData = {}, sourceWorkbookId, requestedBy = "Home presentation refresh" } = {}) {
  ...
  const leaderboardsPlayers = Object.fromEntries(players.map((player) => [player.id, {
    slug: clean(player.slug),
    photo: clean(player.photo),
    captain: player.captain === true,
  }]));
  const tournamentMatchDisplay = Object.fromEntries((liveData.rounds || []).flatMap((round) => round.matches || []).map((match) => [match.id, {
    team1Players: match.team1Players || [], team2Players: match.team2Players || [],
    team1PlayingHcp: match.team1PlayingHcp ?? null, team2PlayingHcp: match.team2PlayingHcp ?? null,
    team1Stroke: match.team1Stroke ?? null, team2Stroke: match.team2Stroke ?? null,
    currentHole: match.currentHole ?? 0,
  ...
  const presentation = {
    ...
    netSkinsByPlayer,
    // Player imagery is presentation data, not scoring identity. Keeping this
    // in the shared versioned projection preserves the existing Leaderboards
    // cards without putting Players sheet reads back on the foreground path.
    leaderboardsPlayers,
    // Handicap labels are Director-facing card presentation. Canonical
    // Supabase scoring continues to use the immutable snapshot and final
    // stroke allocations; this projection cannot authorize or calculate.
    tournamentMatchDisplay,
    // Calcutta remains calculated/published by the existing Google-backed
    // Director workflow. Tournament reads this imported display model lazily;
    // it is never part of live scoring authority or the primary Home payload.
    tournamentSecondary: {
      calcutta: liveData.calcutta || null,
    },
  };
  return {
    environment: "PREVIEW",
...
function homeMatch(entry, tournament, teams) {
  ...
  return {
    ...
    pointsAvailable: 3,
    finalResult: typeof data.result === "string" ? data.result : data.result?.officialResult || "",
...
export function participantHomeDataFromSupabaseView(view = {}) {
  ...
  const timelineBase = currentPresentationTimeline(presentation.timeline, {
    tournamentStatus: status,
    timeZone: tournament.timeZone,
    rounds,
  });
  const timelineEvents = timelineBase.events;
  ...
  return stable({
    ...
    rounds: (liveData.rounds || []).map((round) => ({
      ...
      matches: (round.matches || []).map((match) => ({
        ...
        currentHole: clean(match.status).toUpperCase() === "FINAL" || (match.holeResults || []).length >= 18
          ? 18
          : match.currentHole,
