...
export function participantHomeDataFromSupabaseView(view = {}) {
  ...
  const matches = (view.matches || []).map((entry) => homeMatch(entry, tournamentRow, teams));
...
export function participantHomeParityProjection(data = {}) {
  ...
  const player = (row = {}) => ({ id: row.id, name: row.name, slot: row.slot,
    playingHcp: row.playingHcp ?? null, stroke: row.stroke ?? null });
