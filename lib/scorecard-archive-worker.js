...
async function productionWorkerDispatchContext(env, dependencies) {
  if (clean(env?.VERCEL_ENV).toLowerCase() !== "production") return null;
  if (dependencies.scoringDispatchContext) return dependencies.scoringDispatchContext;
  const resolve = dependencies.resolveProductionScoringDispatchContext ||
...
async function withRoundScorecardsCredential(
  env,
  dependencies,
  ...
  const resources = dependencies.productionScoringDispatchGoogleResources ||
    (await import("./production-scoring-operations-server.js"))
  ...
  return scope({
    ...
    resources: {
      ...resources(scoringDispatchContext, { requiredPhase: "WORKERS", env }),
      scoringDispatchContext,
    },
  }, callback);
}
...
export async function processNextScorecardArchiveJob({
  ...
  const scoringDispatchContext = await productionWorkerDispatchContext(env, dependencies);
  const rpcOptions = { leaseSeconds: 90, env, scoringDispatchContext };
  const claim = await (dependencies.claimScorecardArchiveJob || claimScorecardArchiveJob)(workerId, rpcOptions);
  ...
  try {
    ...
    await (dependencies.failScorecardArchiveJob || failScorecardArchiveJob)({
      ...
      block: true,
    }, { env, scoringDispatchContext }).catch(() => {});
  ...
  try {
    ...
    const measured = await withRoundScorecardsCredential(
      env,
      dependencies,
      scoringDispatchContext,
      () =>
      measure("round-scorecards-archive", async () => {
        stage = "google-writer";
...
export async function reconcileRoundScorecardsArchives({
  ...
  const resolvedEvidenceMatchIds = scoringDispatchContext
    ? resolvedTournamentId === "2026"
      ? ["2026-R3-4", "2026-R2-1", "2026-R1-6"]
      : snapshots.map((item) => clean(item.match_id || item.payload?.match?.match_id))
  ...
  const grid = await withRoundScorecardsCredential(
    env,
    dependencies,
    scoringDispatchContext,
    () => (dependencies.inspectRoundScorecardsArchiveReadback ||
      inspectRoundScorecardsArchiveReadback)(),
  );
  const reports = [];
  ...
  const evidence = Object.fromEntries(resolvedEvidenceMatchIds.map((matchId) => {
