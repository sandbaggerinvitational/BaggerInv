...
export function netSkinsReadEnvironment(env = process.env) {
  const configured = clean(env.NET_SKINS_READ_SOURCE || "google").toLowerCase();
  ...
  const cutover = productionCutoverReadSourceEnvironment({
    env, variable: "NET_SKINS_READ_SOURCE", configuredValue: env.NET_SKINS_READ_SOURCE,
    // The canonical Production V1 read owns NOT_CONFIGURED as an explicit,
    // safe domain state.  A deployment flag must not prevent the application
    // from reading that state or make optional Net Skins presentation block
    // otherwise healthy tournament surfaces.
  ...
  return {
    ...
    reason: resolved === "supabase" ? (candidate.eligible ? "production-shadow-supabase-net-skins" : "preview-supabase-net-skins")
