...
async function supabaseRequest(path, {
  ...
  if (!gate.enabled && !candidateReadAllowed) return { skipped: true, reason: gate.reason };
...
export async function scoringShadowRpc(functionName, body = {}, options = {}) {
  ...
  const translation = candidate.eligible
  ...
  if (!candidate.eligible || !result?.ok) return result;
