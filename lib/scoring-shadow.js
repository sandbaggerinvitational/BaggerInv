...
} from "./production-cutover-read-transport.js";

// The Production-shadow candidate is read-only by construction. It may call
// only bounded projection/status RPCs needed by certified website/PWA reads;
// imports, recalculations, scoring, jobs, outbox, archive, and publication RPCs
// remain unavailable even with the Production service credential present.
export const PRODUCTION_SHADOW_CANDIDATE_READ_RPCS = Object.freeze([
  "read_production_calcutta_configuration",
  "read_production_candidate_completed_history",
...
const PRODUCTION_SHADOW_CANDIDATE_READ_RPC_SET = new Set(
...
async function supabaseRequest(path, {
  ...
  productionReadState = null,
} = {}) {
  const gate = scoringShadowEnvironment(env);
  const candidate = candidateReadOperation
    ? productionShadowCandidateReadEnvironment(env)
    : { eligible: false };
  const candidateReadAllowed = candidate.eligible &&
    PRODUCTION_SHADOW_CANDIDATE_READ_RPC_SET.has(candidateReadOperation);
  const productionReadAllowed = Boolean(productionReadState?.allowed);
  if (!gate.enabled && !candidateReadAllowed && !productionReadAllowed) {
    return { skipped: true, reason: productionReadState?.reason || gate.reason };
  }
  const url = `${String(env.SUPABASE_SCORING_MIRROR_URL).replace(/\/$/, "")}/rest/v1/${path}`;
  const startedAt = Date.now();
  recordDataAuthorityTransport("supabase", { adapter: "scoring-shadow" });
...
export async function scoringShadowRpc(functionName, body = {}, options = {}) {
  ...
  const candidateTranslation = candidate.eligible
    ? productionShadowCandidateRpcTranslation(functionName, body)
    : null;
  const productionRead = productionCutoverReadTransportEnvironment(env, functionName, body);
  if (productionRead.requested && !productionRead.allowed) {
    const error = new Error(`Production cutover read transport is unavailable (${productionRead.reason}).`);
    error.code = "PRODUCTION_CUTOVER_READ_RPC_UNAVAILABLE";
    error.status = 503;
    error.diagnostics = productionRead;
    throw error;
  }
  const currentReadDispatch = productionRead.allowed
    ? await (options.resolveProductionCurrentReadDispatch ||
      (await import("./production-current-read-dispatch.js"))
        .resolveProductionCurrentReadDispatch)(functionName, body, {
        env,
        ...(options.readProductionCurrentTournamentRuntime
          ? { readRuntime: options.readProductionCurrentTournamentRuntime }
          : {}),
      })
    : { body, annualRuntimeInput: null };
  const productionTranslation = productionRead.allowed
    ? productionCutoverReadRpcTranslation(functionName, currentReadDispatch.body, env, {
      annualRuntimeInput: currentReadDispatch.annualRuntimeInput,
      currentRuntime: currentReadDispatch.runtime,
    })
    : null;
  const translation = candidateTranslation || productionTranslation;
  const resolvedFunctionName = translation?.functionName || functionName;
  // production_cutover_surface is an application transport hint consumed by
  // the active Production translator above. It is not part of Preview's
  // read_tournament_live_view(target_tournament_id) SQL signature and must not
  // leak into an untranslated PostgREST RPC body.
  const directBody = clean(functionName) === "read_tournament_live_view"
    ? Object.fromEntries(Object.entries(body || {}).filter(([key]) => key !== "production_cutover_surface"))
    : body;
  const resolvedBody = translation?.body || directBody;
  if (candidate.eligible && !PRODUCTION_SHADOW_CANDIDATE_READ_RPC_SET.has(resolvedFunctionName)) {
    const error = new Error("The Production-shadow candidate permits certified read RPCs only.");
    error.code = "PRODUCTION_SHADOW_CANDIDATE_RPC_FORBIDDEN";
    error.status = 403;
    throw error;
  }
  const result = await supabaseRequest(`rpc/${resolvedFunctionName}`, {
    method: "POST",
    body: resolvedBody,
    timeoutMs: options.timeoutMs || 8_000,
    env,
    candidateReadOperation: resolvedFunctionName,
    productionReadState: productionRead.allowed ? productionRead : null,
  });
  if (!result?.ok) return result;
  if (candidate.eligible) {
    return { ...result, payload: adaptProductionShadowCandidatePayload(result.payload, translation) };
  }
  if (productionRead.allowed) {
    return { ...result, payload: adaptProductionCutoverReadPayload(result.payload, translation, productionRead) };
  }
  return result;
}
