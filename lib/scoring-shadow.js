...
export async function inspectScoringShadow({ sourceWorkbookId, matchId = "", holeNumber = 0 } = {}, options = {}) {
  ...
  const [events, holes, matches, runs, selected] = await Promise.all([
    ...
    readScoringShadowRows("live_match_mirror", `${scope}&select=match_id`, options),
    readScoringShadowRows("mirror_reconciliation_runs", `${scope}&select=id,status,operation,started_at&order=started_at.desc`, options),
    readScoringShadowRows("score_mirror_events", `${scope}${logical}${hole}&select=id,match_id,hole_number,google_revision,mutation_key,payload_hash,comparison_status,delivery_count,google_updated_at,observed_at&order=observed_at.desc`, options),
  ]);
  return {
    counts: {
