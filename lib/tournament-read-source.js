...
function sourceEnvironment(env, variable, label) {
  const configured = clean(env[variable] || "google").toLowerCase();
  ...
  const cutover = productionCutoverReadSourceEnvironment({
    env, variable, configuredValue: env[variable], requiredPhase: "CURRENT_READS",
  });
  if (cutover.handled) {
    return {
  ...
  return {
    ...
    reason: resolved === "supabase" ? (candidate.eligible ? `production-shadow-supabase-${label}` : `preview-supabase-${label}`)
