...
function guideRuntime(env = process.env) {
  ...
  const approvedProject = candidate.eligible || projectRef === PREVIEW_GUIDE_SUPABASE_PROJECT_REF;
  ...
  const supabaseEligible = candidate.eligible || (previewDeployment && previewWorkbook && approvedProject && approvedTournament && credentialsConfigured && serviceEnabled);
  return {
    previewDeployment,
    workbookId,
    previewWorkbook,
...
function sourceState(requested, runtime, label, cutover = null) {
  if (cutover?.handled) {
    return {
  ...
  return {
    ...
    blocked,
    productionBlocked,
    productionShadowCandidate: runtime.productionShadowCandidate,
    reason: resolved === "supabase" ? (runtime.productionShadowCandidate ? `production-shadow-supabase-${label}` : `preview-supabase-${label}`)
...
export function guideReadEnvironment(env = process.env) {
  ...
  const guideCutover = productionCutoverReadSourceEnvironment({
    env,
    variable: "GUIDE_READ_SOURCE",
    configuredValue: env.GUIDE_READ_SOURCE,
    requiredPhase: "READ_CUTOVER",
  });
  const courseConfigured = env.COURSE_PRESENTATION_READ_SOURCE || env.GUIDE_READ_SOURCE;
  const courseCutover = productionCutoverReadSourceEnvironment({
    env,
    variable: "COURSE_PRESENTATION_READ_SOURCE",
    configuredValue: courseConfigured,
    requiredPhase: "READ_CUTOVER",
  });
  const guide = sourceState(normalizedSource(env.GUIDE_READ_SOURCE), runtime, "guide", guideCutover);
  const course = sourceState(normalizedSource(courseConfigured), runtime, "course-presentation", courseCutover);
...
export function guideSyncEnvironment(env = process.env) {
  ...
  const administrativeEligible = runtime.supabaseEligible && !runtime.productionShadowCandidate;
  return {
    ...runtime,
    autoSyncRequested,
