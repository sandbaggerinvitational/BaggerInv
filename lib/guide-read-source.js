...
function sourceState(requested, runtime, label) {
  ...
  const blocked = requested === "invalid" || (requested === "supabase" && runtime.previewDeployment && !runtime.supabaseEligible);
  const resolved = requested === "supabase" && runtime.supabaseEligible ? "supabase" : "google";
  return {
    requested,
    resolved,
    blocked,
