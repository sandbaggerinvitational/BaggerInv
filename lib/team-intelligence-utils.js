...
export function buildLineupPlans({
  ...
  const make = (id, label, indexes) => {
    ...
    for (const row of singles) {
      const playerId = row.players[0]?.id;
      if (!playerId || usedSingles.has(playerId)) continue;
      usedSingles.add(playerId);
      add("SI", row, usedSingles.size - 1);
    }
