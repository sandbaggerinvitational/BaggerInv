...
import {
  calculateProductionFullNetSkins,
  FULL_NET_SKINS_ENGINE,
...
export async function configureProductionNetSkinsV1({
  ...
  if (!rounds.length || rounds.some(round => !EXACT_ROUNDS.includes(round)) || !entryRevisions
    || Array.isArray(entryRevisions) || Object.keys(entryRevisions).length !== rounds.length
    || rounds.some(round => !Number.isSafeInteger(entryRevisions[round]) || entryRevisions[round] < 1)) {
    throw netSkinsError(
      "PRODUCTION_NET_SKINS_APPROVED_ROUNDS_REQUIRED",
      "Saved explicit Round entry revisions are required.",
      400,
    );
  }
...
export async function processProductionNetSkinsV1Job({
  ...
  try {
    ...
    const calculated = calculateProductionFullNetSkins(claimed.calculation_input);
    ...
    const result = await mutationRpc("complete_production_net_skins_v1_recalculation", {
      ...
      engine_version: FULL_NET_SKINS_ENGINE,
