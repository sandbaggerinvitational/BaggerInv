...
export function playerLeadersProjectionSource(source, identity) {
  ...
  return { ...source, matches: (source.matches || []).filter((entry) => {
    ...
    requireLeadersValue(Array.isArray(entry.participants) &&
      ...
      !text(entry.match.result_winner) && !entry.match.scorecard_complete && !entry.match.clinched &&
      ["scored_holes", "team_1_holes_won", "team_2_holes_won", "unresolved_mutations"]
        .every((field) => Number(entry.match[field] ?? 0) === 0) &&
