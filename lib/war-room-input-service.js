...
import { predictionBundleParityProjection } from "./war-room-input-contract.js";
...
export async function prepareWarRoomInput({
  ...
  const prepared = selection.resolved === "supabase"
    ? await prepareSupabaseWarRoomInput({ scope, env, settingsVerification, timeoutMs })
...
function classifiedDifference(row = {}) {
  const path = clean(row.path);
  const intentional = [
    [/^bundle\.predictionSettings\.revision$/, "SOURCE_REVISION_METADATA"],
  ...
  return intentional
    ? { ...row, disposition: "INTENTIONAL_SOURCE_METADATA", reason: intentional[1] }
...
export async function captureWarRoomInputParity({
  ...
  const differences = compared.differences.map(classifiedDifference);
