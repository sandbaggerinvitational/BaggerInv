...
export async function verifyGoogleLifecycleMirror(input, dependencies = {}) {
  ...
  const verified = match && clean(match["Match Status"]).toLowerCase() === expectedStatus.toLowerCase()
  ...
  if (!verified) throw Object.assign(new Error("Google lifecycle mirror did not verify all authoritative lifecycle fields."), {
