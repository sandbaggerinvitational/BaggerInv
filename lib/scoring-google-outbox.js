...
async function withGoogleOutboxCredential(
  env,
  dependencies,
...
export async function processNextGoogleOutboxEvent({ workerId = `preview-${randomUUID()}`, actor = "Supabase Google mirror", expectedEventId = "", env = process.env, dependencies = {} } = {}) {
  ...
  const rpcOptions = { env, scoringDispatchContext };
  const claim = clean(expectedEventId)
    ...
    : await (dependencies.claimGoogleOutbox || claimGoogleOutbox)(workerId, {
      leaseSeconds: 45, ...rpcOptions,
    });
  ...
  try {
    const measured = await withGoogleOutboxCredential(env, dependencies, scoringDispatchContext, () =>
    ...
    stage = "checkpoint";
    const completion = await (dependencies.completeGoogleOutbox || completeGoogleOutbox)(delivery, rpcOptions);
