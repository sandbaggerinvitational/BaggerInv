...
import {
  filterEligibleRecordEntries,
  formatRecordUnit,
  recordEligibility,
  recordEmptyState,
  recordUnit,
} from "./scorecard-record-eligibility.js";
...
export const SCORECARD_RECORD_SLUGS = Object.freeze([
  "lowest-individual-round",
  "lowest-individual-to-par",
  "lowest-individual-front-nine",
  "lowest-individual-back-nine",
  "most-individual-birdies",
  "most-individual-eagles",
  "most-consecutive-individual-birdies",
  "best-individual-closing-stretch",
  "lowest-singles-round",
  "career-par-3-average",
  "career-par-4-average",
  "career-par-5-average",
  "lowest-scramble-round",
  "lowest-scramble-to-par",
  "lowest-scramble-front-nine",
  "lowest-scramble-back-nine",
  "lowest-best-ball-team-round",
  "lowest-best-ball-team-to-par",
  "lowest-best-ball-front-nine",
  "lowest-best-ball-back-nine",
  "most-team-birdies",
  "most-consecutive-team-birdies",
  "career-most-birdies",
  "career-most-eagles",
  "career-most-pars",
  "career-lowest-average-score",
  "career-lowest-average-net",
  "advanced-par-3-average",
  "advanced-par-4-average",
  "advanced-par-5-average",
  "most-holes-won",
  "most-holes-halved",
  "highest-hole-differential",
  "most-front-nine-holes-won",
  "most-back-nine-holes-won",
  "most-closing-holes-won",
  "hardest-historical-hole",
  "easiest-historical-hole",
]);

const SCORECARD_RECORD_SLUG_SET = new Set(SCORECARD_RECORD_SLUGS);

export function isScorecardRecordSlug(slug) {
  return SCORECARD_RECORD_SLUG_SET.has(clean(slug));
}
...
function definition({
  ...
  const metadata = recordEligibility(slug);
  const eligible = filterEligibleRecordEntries(slug, entries)
    .sort((a, b) => {
  ...
  return {
    ...
    entityType,
    aggregate,
    unit: recordUnit(slug),
    emptyState: recordEmptyState(slug),
...
export function buildScorecardRecordLeaderboards(
  ...
  const closingEntry = (card) => {
    const holes = [16, 17, 18].map((number) =>
      card.holes.find((hole) => Number(hole.holeNumber) === number && finite(hole.score))
    );
    if (holes.some((hole) => !hole)) return performanceEntry(card, null);
    const value = holes.reduce((sum, hole) => sum + Number(hole.score), 0);
    const par = holes.every((hole) => finite(hole.par))
      ? holes.reduce((sum, hole) => sum + Number(hole.par), 0)
      : null;
    return {
      ...performanceEntry(card, value),
      secondaryValue: finite(par) ? value - par : null,
    };
  };
  ...
  const records = [
    ...
    definition({ slug: "best-individual-closing-stretch", title: "Lowest Individual Score on Holes 16–18", group: "individual", direction: "lowest", entries: individuals.map(closingEntry) }),
  ...
  return {
    ...
    // Player profiles already need the same canonical career aggregates.
    // Expose the completed calculation so they do not rescan every scorecard.
...
/**
 * Resolve one detail-page record without evaluating the unrelated record
 * universe. Career aggregates and course-hole records deliberately fall back
 * to the full certified catalog because they share its cross-card analytics.
 */
export function buildScorecardRecordLeaderboard(
  slug,
  scorecards = [],
  options = {}
) {
  const requested = clean(slug);
  if (!isScorecardRecordSlug(requested)) return null;

  const complete = scorecards.filter((card) =>
    COMPLETE_STATUSES.has(clean(card.status).toUpperCase()) &&
    card.completedHoleCount === 18
  );
  const individuals = complete.filter((card) =>
    card.scoreType === "INDIVIDUAL" && card.playerId
  );
  const singles = individuals.filter((card) => card.format === "SI");
  const scramble = complete.filter((card) =>
    card.scoreType === "TEAM" && card.format === "SC" && card.teamId
  ...
  const make = (input) => definition({ slug: requested, ...input });

  switch (requested) {
    case "lowest-individual-round":
      return make({ title: "Lowest Recorded Individual Round", group: "individual", direction: "lowest", entries: individualMetric((card) => card.total) });
    case "lowest-individual-to-par":
      return make({ title: "Lowest Individual Round to Par", group: "individual", direction: "lowest", signed: true, entries: individualMetric((card) => card.totalToPar) });
    case "lowest-individual-front-nine":
      return make({ title: "Lowest Individual Front Nine", group: "individual", direction: "lowest", entries: individualMetric((card) => card.frontNine) });
    case "lowest-individual-back-nine":
      return make({ title: "Lowest Individual Back Nine", group: "individual", direction: "lowest", entries: individualMetric((card) => card.backNine) });
    case "most-individual-birdies":
      return make({ title: "Most Birdies in an Individual Round", group: "individual", direction: "highest", entries: individualMetric(birdies) });
    case "most-individual-eagles":
      return make({ title: "Most Eagles in an Individual Round", group: "individual", direction: "highest", entries: individualMetric(eagles) });
    case "most-consecutive-individual-birdies":
      return make({ title: "Most Consecutive Birdies by an Individual", group: "individual", direction: "highest", entries: individualMetric((card) => calculateScorecardMetrics(card).longestBirdieStreak.value) });
    case "best-individual-closing-stretch":
      return make({ title: "Lowest Individual Score on Holes 16–18", group: "individual", direction: "lowest", entries: individuals.map(closingEntry) });
    case "lowest-singles-round":
      return make({ title: "Lowest Singles Round", group: "individual", direction: "lowest", entries: singles.map((card) => performanceEntry(card, card.total)) });
    case "lowest-scramble-round":
      return make({ title: "Lowest Scramble Round", group: "team", direction: "lowest", entityType: "TEAM_PERFORMANCE", entries: teamMetric(scramble, (card) => card.total) });
    case "lowest-scramble-to-par":
      return make({ title: "Lowest Scramble Round to Par", group: "team", direction: "lowest", entityType: "TEAM_PERFORMANCE", signed: true, entries: teamMetric(scramble, (card) => card.totalToPar) });
    case "lowest-scramble-front-nine":
      return make({ title: "Lowest Scramble Front Nine", group: "team", direction: "lowest", entityType: "TEAM_PERFORMANCE", entries: teamMetric(scramble, (card) => card.frontNine) });
    case "lowest-scramble-back-nine":
      return make({ title: "Lowest Scramble Back Nine", group: "team", direction: "lowest", entityType: "TEAM_PERFORMANCE", entries: teamMetric(scramble, (card) => card.backNine) });
    case "most-team-birdies":
      return make({ title: "Most Team Birdies in a Round", group: "team", direction: "highest", entityType: "TEAM_PERFORMANCE", entries: teamMetric(scramble, birdies) });
    case "most-consecutive-team-birdies":
      return make({ title: "Most Consecutive Team Birdies", group: "team", direction: "highest", entityType: "TEAM_PERFORMANCE", entries: teamMetric(scramble, (card) => consecutiveBirdies(card.holes)) });
    case "lowest-best-ball-team-round":
    case "lowest-best-ball-team-to-par":
    case "lowest-best-ball-front-nine":
    case "lowest-best-ball-back-nine": {
      const bestBall = bestBallTeamPerformances(individuals);
      const definitions = {
        "lowest-best-ball-team-round": ["Lowest Best Ball Team Round", false, (card) => card.total],
        "lowest-best-ball-team-to-par": ["Lowest Best Ball Team Round to Par", true, (card) => card.totalToPar],
        "lowest-best-ball-front-nine": ["Lowest Best Ball Front Nine", false, (card) => card.frontNine],
        "lowest-best-ball-back-nine": ["Lowest Best Ball Back Nine", false, (card) => card.backNine],
      };
      const [title, signed, valueFor] = definitions[requested];
      return make({ title, group: "team", direction: "lowest", entityType: "TEAM_PERFORMANCE", signed, entries: teamMetric(bestBall, valueFor) });
    }
    default:
      return buildScorecardRecordLeaderboards(scorecards, options).bySlug[requested] || null;
  }
}
...
export function scorecardLeaderboardRows(record) {
  return record.entries.map((entry, index) => ({
    ...
    valueDisplay: `${formatRecordValue(entry.value, record)}${
      finite(entry.secondaryValue)
        ? ` · ${Number(entry.secondaryValue) === 0 ? "Even" : `${Number(entry.secondaryValue) > 0 ? "+" : ""}${entry.secondaryValue}`}`
        : ""
    }`,
    secondaryValueDisplay: finite(entry.secondaryValue)
      ? `${Number(entry.secondaryValue) > 0 ? "+" : ""}${entry.secondaryValue}`
      : "",
    year: entry.year ?? "",
...
export function formatRecordValue(value, record) {
  ...
  const displayed = record.signed && numeric > 0 ? `+${rendered}` : rendered;
  const unit = formatRecordUnit(numeric, record.unit);
