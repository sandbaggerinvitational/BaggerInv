...
export function buildFutureRuntimeMutation(action, value = {}) {
  ...
  if (selected === "grant-future-director") return {
    action: "GRANT_FUTURE_DIRECTOR",
    ...scoped,
    target_player_id: canonicalStableId(value.targetPlayerId, "Player ID"),
  };
...
function normalizeFutureRuntimeCapabilities(value = {}) {
  ...
  const enabled = (...keys) => keys.some((key) => safeBoolean(source[key]));
  return Object.freeze({
    grantFutureDirector: enabled("grantFutureDirector", "grant_future_director"),
...
export function normalizeProductionFutureRuntimePayload(payload = {}) {
  ...
  const directorGovernance = firstDefined(
    value.futureDirectorGovernance,
    value.future_director_governance,
  );
  return Object.freeze({
    contractVersion,
    currentTournament: normalizeCurrentTournament(firstDefined(value.currentTournament, value.current_tournament) || {}),
    ...
    futureDirectorGovernance: Object.freeze({
      revision: Math.max(0, safeInteger(firstDefined(
        directorGovernance?.revision,
        directorGovernance?.governance_revision,
      ))),
      directors: Object.freeze((Array.isArray(directorGovernance?.directors)
        ? directorGovernance.directors : []).map((item) => Object.freeze({
        playerId: safeStableId(firstDefined(item?.playerId, item?.player_id)),
        displayName: safeText(firstDefined(item?.displayName, item?.display_name), 160),
        status: safeCode(item?.status, "INACTIVE"),
        roleActive: safeBoolean(firstDefined(item?.roleActive, item?.role_active)),
      })).filter((item) => item.playerId)),
    }),
    readiness: normalizeReadiness(value.readiness || {}),
