import { timelineEventIcon, timelineEventStatus, tournamentDateTime, tournamentDayKey } from "./tournament-timeline.js";
...
export function directorRoundStatus(round) {
  ...
  const liveMatches = matches.filter((match) => /^(Live|Reopened)$/i.test(match.status));
  const counts = {
    ...
    scoringLocked: liveMatches.filter((match) => match.scoringEnabled === false).length,
  };
...
export function directorReadinessLifecycle(rounds = []) {
  return rounds.some((round) => ["LIVE", "FINAL"].includes(clean(round?.status).toUpperCase())) ? "operations" : "setup";
}
...
function nextTournamentEvent(data, rounds, now) {
  ...
  const scheduled = (data.timeline.events || []).map((event) => {
    ...
    const status = timelineEventStatus(event, { now, tournamentStatus: data?.tournament?.status, timeZone, rounds });
  ...
  if (scheduled) {
    ...
    return {
      ...
      icon: timelineEventIcon(scheduled.type),
...
function operationalIssues({ data, matches, rounds, automation, nextEvent, now }) {
  ...
  for (const match of matches) {
    ...
    if (live && match.scoringEnabled === false) issues.push(issue({ id: `scoring:${match.id}`, severity: "critical", title: matchLabel(match), message: "Match is LIVE but participant scoring is locked.", impact: "Assigned golfers cannot open the scorecard.", actionLabel: "Unlock Scoring →", action: "unlock-scoring", href: "#quick-actions" }));
...
function groupOperationalIssues(issues) {
  ...
  const labels = {
    ...
    conflict: "Scoring Conflicts", scoring: "Scoring Access", automation: "Automation", round: "Round Opening", sync: "Synchronization",
  };
...
function adaptivePrimaryAction({ operatingRound, tournamentComplete }) {
  ...
  if (operatingRound.scoringLocked > 0) return { kind: "action", action: "unlock-scoring", label: "Unlock Scoring", message: `${operatingRound.scoringLocked} LIVE match${operatingRound.scoringLocked === 1 ? " is" : "es are"} unavailable to participants.` };
...
export function tournamentDirectorModel(data, now = new Date()) {
  ...
  const effectiveTimelineNow = data?.timeline?.previewDateActive && data.timeline.effectiveNow ? new Date(data.timeline.effectiveNow) : now;
  const nextEvent = nextTournamentEvent(data, rounds, Number.isNaN(effectiveTimelineNow.getTime()) ? now : effectiveTimelineNow);
  ...
  const readinessLifecycle = directorReadinessLifecycle(rounds);
  return {
    ...
    readiness: data?.readiness || null, readinessLifecycle,
    health: { status: healthStatus, issueCount: issues.length, criticalCount: critical, live, upcoming, final, awaitingConfirmation, reopened, scoringLocked: matches.filter((match) => /^(Live|Reopened)$/i.test(match.status) && match.scoringEnabled === false).length, errors: issues.filter((item) => /^(players|teams|tee|hcp|conflict|status|sync|scoring):?/.test(item.id)).length, lastSynchronization: data?.tournament?.lastUpdated || recentActivity[0]?.updatedAt || now.toISOString() },
