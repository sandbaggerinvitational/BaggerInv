...
function eventTiming(minutes, startTime, roundEvent) {
  const relative = countdownLabel(minutes, startTime);
  if (minutes === null || minutes <= 0 || relative.startsWith("Tomorrow")) return relative;
  return `${roundEvent ? "Opens" : "Begins"} ${relative}`;
}
...
function nextTournamentEvent(data, rounds, now) {
  ...
  if (scheduled) {
    ...
    return {
      ...
      countdown: eventTiming(scheduled.minutes, scheduled.startTime, Boolean(roundNumber) || /round|opens/i.test(`${scheduled.type} ${scheduled.title}`)),
  ...
  return {
    ...
    minutes,
    countdown: eventTiming(minutes, nextRound.firstTeeTime, true),
...
export function tournamentDirectorModel(data, now = new Date()) {
  ...
  const healthStatus = critical
    ...
    : warnings
      ...
      : { level: "healthy", label: "Tournament Healthy", icon: "🟢", message: "No immediate action required." };
