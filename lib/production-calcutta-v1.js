...
export function productionCalcuttaV1ContractData(view = {}, {
  ...
  if (contractVersion !== PRODUCTION_CALCUTTA_V1_CONTRACT ||
      tournamentId !== clean(expectedTournamentId) ||
  ...
  if (state === "NOT_CONFIGURED" &&
      (configurationRevision !== (tournamentId === PRODUCTION_TOURNAMENT_ID ? 1 : 0) ||
...
export function productionCalcuttaV1Data(view = {}, options = {}) {
  const contract = productionCalcuttaV1ContractData(view, options);
...
export async function readProductionCalcuttaV1({
  reviewerAuthUserId = "",
  playerId = "",
  tournamentId = "",
  env = process.env,
  ...
  return scoringShadowRpc("read_production_calcutta_v1", {
    input: {...(clean(playerId) ? { player_id: clean(playerId) } : {}), ...(reviewerAuthUserId ? {observer_auth_user_id:reviewerAuthUserId} : {})},
  }, {
    env,
    timeoutMs: 8_000,
