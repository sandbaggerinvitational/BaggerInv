...
export const PREVIEW_DIRECTOR_PASSPORT_COOKIE = "sbi-preview-director-passport";
...
export function previewDirectorPassportCookie(token, maxAge = 365 * 24 * 60 * 60) {
  return {
    ...playerPassportCookie(token, maxAge),
    name: PREVIEW_DIRECTOR_PASSPORT_COOKIE,
  };
}
...
function currentDirectorToken(primaryToken, directorToken) {
  if (!directorToken) return primaryToken;
  if (!primaryToken) return directorToken;
  try {
    if (isPreviewImpersonationSession(verifyPlayerPassportSession(primaryToken))) return primaryToken;
  } catch {
    // An expired participant/impersonation pointer cannot suppress an otherwise
    // valid, separately scoped Director Passport.
  }
  return directorToken;
}

export function tournamentDirectorTokenFromRequest(request) {
  return currentDirectorToken(
    playerPassportTokenFromRequest(request),
    request.cookies.get(PREVIEW_DIRECTOR_PASSPORT_COOKIE)?.value || "",
  );
}

export function tournamentDirectorTokenFromCookieStore(cookieStore) {
  return currentDirectorToken(
