...
function ratingProjection(draft, history) {
  const ratings = history.getSandbaggerRatings().byCategory.OVERALL || [];
...
export async function getDraftAnalysis(draft, options = {}) {
  ...
  const matches = history.getTournamentMatches(draft.year);
  const leaderboard = history.getTournamentPlayerLeaderboard(draft.year);
  const snapshots = await (options.readOddsSnapshots || readOddsSnapshots)();
  const projections = oddsProjection(draft, snapshots) || ratingProjection(draft, history);
