...
import { draftValueScore, gradeForScore } from "./draft-value.js";

export { draftValueScore, gradeForScore } from "./draft-value.js";
...
function draftGrades({ state, projections, teamResults, players, winner }) {
  return projections.teams.map((projection) => {
    ...
    const averageDvs = rosterPlayers.length
      ...
      : 0;
    let score = 50 + projection.probability / 2;
    if (state === "live") {
      ...
      score = 58 + share * 34 + clamp(averageDvs, -8, 8);
    }
    if (state === "final") {
      score = winner?.id === projection.team.id ? 94 : 76;
      score += clamp(averageDvs * 1.5, -12, 6);
    }
    score = Math.round(clamp(score, 55, 99));
    return {
...
export async function getDraftAnalysis(draft) {
  ...
  return {
    ...
    projectedFavorite,
    projectedBest: {
      team: projectedFavorite.team,
      score: Math.round(50 + projectedFavorite.probability / 2),
    },
    leader,
