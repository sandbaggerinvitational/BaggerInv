...
function selectorState({ configuredValue, preview, eligible, kind }) {
  ...
  const reason = !valid ? "invalid-source"
    ...
    : eligible ? "preview-supabase-authority"
...
export function oddsCalculationEnvironment(env = process.env) {
  ...
  const input = selectorState({ configuredValue: env.ODDS_CALCULATION_INPUT_SOURCE, preview, eligible, kind: "ODDS_CALCULATION_INPUT" });
  const publication = selectorState({ configuredValue: env.ODDS_PUBLICATION_AUTHORITY, preview, eligible, kind: "ODDS_PUBLICATION" });
  return {
