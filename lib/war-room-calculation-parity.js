...
export const WAR_ROOM_CALCULATION_ENGINE_VERSIONS = Object.freeze({
  ...
  matchup: "prediction-engine.js:sbi-postgres-handicap-parity-v2",
  simulation: "match-simulator.js:v1+prediction-handicap-postgres-parity-v2",
  optimizer: "lineup-optimizer.js:prediction-engine-postgres-handicap-parity-v2",
  "team-intelligence": "team-intelligence.js:v1",
  calibration: "scorecard-calibration.js:shadow-v1+prediction-handicap-postgres-parity-v2",
});
...
const MATCHUP_FORMATS = Object.freeze(["BB", "SC", "SI"]);

function selectedMatchupFormats(formats = []) {
  const requested = new Set(list(formats).map((format) => clean(format).toUpperCase()).filter((format) => MATCHUP_FORMATS.includes(format)));
  return requested.size ? MATCHUP_FORMATS.filter((format) => requested.has(format)) : [...MATCHUP_FORMATS];
}

function buildMatchupScenarios(data = {}, { formats = [] } = {}) {
  const rows = [];
  for (const requestedFormat of selectedMatchupFormats(formats)) {
...
export function runMatchupParitySource(prepared, { repeat = 2, formats = [] } = {}) {
  const startedAt = performance.now();
  const runs = [];
  const timings = [];
  const selectedFormats = selectedMatchupFormats(formats);
  for (let attempt = 0; attempt < repeat; attempt += 1) {
    const attemptStartedAt = performance.now();
    runs.push(buildMatchupScenarios(prepared.consumerData, { formats: selectedFormats }));
    timings.push(Math.max(0, performance.now() - attemptStartedAt));
  }
  ...
  return {
    ...
    selectedFormats,
    rows,
    counts: Object.fromEntries(selectedFormats.map((format) => [format, rows.filter((row) => row.format === format).length])),
...
export function runSimulationParitySource(prepared, { iterations = 10_000, repeat = 2, scenarioIds = [], formats = [] } = {}) {
  const selectedFormats = selectedMatchupFormats(formats);
  const allRows = buildMatchupScenarios(prepared.consumerData, { formats: selectedFormats });
  ...
  return { source: prepared.source, selectedFormats, rows, totalExecutionMs: Math.max(0, performance.now() - startedAt) };
}
...
export function runLineupParitySource(prepared, { repeat = 2, formats = [] } = {}) {
  const formatRuns = {};
  const startedAt = performance.now();
  const selectedFormats = selectedMatchupFormats(formats);
  ...
  return { source: prepared.source, selectedFormats, formats: formatRuns, totalExecutionMs: Math.max(0, performance.now() - startedAt) };
}

export function compareLineupParity(expected, actual) {
  const formats = {};
  let unexplained = 0;
  const selectedFormats = MATCHUP_FORMATS.filter((format) => expected.formats?.[format] || actual.formats?.[format]);
  for (const format of selectedFormats) {
