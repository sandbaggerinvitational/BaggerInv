...
import { predictionInputFingerprint } from "./prediction-input-bundle-contract.js";
import {
  allocateStrokes,
  courseHandicap,
  formatCode,
...
export function runChampionshipParitySource(prepared, { phase = "", iterations = 10_000, repeat = 2 } = {}) {
  ...
  const run = repeatResult(() => logicalOddsOutput(simulateTournamentOdds({
    sheets: prepared.consumerData.sheets,
  ...
  return {
    ...
    domainFingerprints: championshipDomainFingerprints(prepared.consumerData, selectedPhase),
    output: run.result,
