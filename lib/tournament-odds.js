...
export function simulateTournamentOdds({ sheets, historical = {}, phase = "Pre-Tournament", iterations = 10_000, contractVersion = ODDS_PUBLICATION_CONTRACT_VERSION }) {
  ...
  const year = currentTournamentYear(sheets), teams = getTeamContext(sheets, year), deterministicSeed = `${year}|${phase}|${ODDS_SIMULATION_SEED_VERSION}`, random = rng(deterministicSeed);
  ...
  if (contractVersion === ODDS_LEGACY_PUBLICATION_CONTRACT_VERSION) {
    ...
    return { year, phase, phaseOrder:ODDS_PHASES.indexOf(phase), publishedAt:new Date().toISOString(), iterations, totalPointsAvailable, teams, players };
  }
  const teamRows = rawTeamRows;
  const playerRows = rankPlayerOddsRows(rawPlayerRows);
  return { year, phase, phaseOrder:ODDS_PHASES.indexOf(phase), publishedAt:new Date().toISOString(), iterations, totalPointsAvailable,
