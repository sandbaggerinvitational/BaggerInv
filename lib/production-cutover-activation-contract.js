...
export const PRODUCTION_MAINTENANCE_DEPLOYMENT_CAPABILITY_CONTRACT =
  "production-maintenance-single-deployment-capability-v1";
export const PRODUCTION_MAINTENANCE_DEPLOYMENT_CAPABILITY_CEILING =
  "OBSERVATION";
...
export const PRODUCTION_CUTOVER_CONFIGURATION_PLAN = Object.freeze({
  ...
  workersMirrors: Object.freeze([
    ...
    "SCORING_GOOGLE_OUTBOX_WORKER_SECRET",
    "ROUND_SCORECARDS_ARCHIVE_WORKER_SECRET",
  ]),
  odds: Object.freeze([
    "ODDS_CALCULATION_INPUT_SOURCE", "WAR_ROOM_INPUT_SOURCE",
    "PREDICTION_SETTINGS_READ_SOURCE",
    "WAR_ROOM_PREDICTION_SETTINGS_SOURCE_FINGERPRINT",
    "WAR_ROOM_PREDICTION_SETTINGS_EFFECTIVE_FINGERPRINT",
    "ODDS_PUBLICATION_AUTHORITY",
    "PRODUCTION_SUPABASE_ODDS_CALCULATION_ENABLED",
    "PRODUCTION_SUPABASE_ODDS_PUBLICATION_ENABLED",
    "PRODUCTION_SUPABASE_ODDS_GOOGLE_MIRROR_ENABLED",
  ]),
...
export function productionCutoverActivationEnvironment(env = process.env) {
  ...
  const capabilityContractValue = clean(
    env.PRODUCTION_MAINTENANCE_DEPLOYMENT_CAPABILITY_CONTRACT,
  );
  const capabilityCeiling = clean(
    env.PRODUCTION_MAINTENANCE_DEPLOYMENT_CAPABILITY_CEILING,
  ).toUpperCase();
  const capabilityRequested = Boolean(capabilityContractValue || capabilityCeiling);
  const capabilityContractApproved = capabilityContractValue ===
    PRODUCTION_MAINTENANCE_DEPLOYMENT_CAPABILITY_CONTRACT;
  const capabilityCeilingApproved = capabilityCeiling ===
    PRODUCTION_MAINTENANCE_DEPLOYMENT_CAPABILITY_CEILING;
  const maintenanceDeploymentCapabilityApproved = !capabilityRequested ||
    (capabilityContractApproved && capabilityCeilingApproved &&
      phase === "SCORING_COMMIT");
  const capabilityCeilingIndex = capabilityContractApproved &&
      capabilityCeilingApproved
    ? phaseIndex(capabilityCeiling)
    : -1;
  const authorizationPhaseIndex = maintenanceDeploymentCapabilityApproved &&
      capabilityRequested
    ? capabilityCeilingIndex
    : phaseIndex(phase);
  ...
  const allowed = requested && deploymentApproved && foundationEnabled && projectRefApproved &&
    ...
    maintenanceDeploymentCapabilityApproved;
  const reason = allowed ? "production-cutover-activation-ready"
    ...
    : !maintenanceDeploymentCapabilityApproved
      ? "maintenance-deployment-capability-invalid"
  ...
  return {
    ...
    authorizationPhaseIndex,
    maintenanceDeploymentCapability: Object.freeze({
      requested: capabilityRequested,
      allowed: capabilityRequested && maintenanceDeploymentCapabilityApproved,
      contract: capabilityContractApproved ? capabilityContractValue : "",
      ceiling: capabilityCeilingApproved ? capabilityCeiling : "",
      ceilingIndex: capabilityCeilingIndex,
      databasePhaseAuthoritative: capabilityRequested &&
        maintenanceDeploymentCapabilityApproved,
    }),
...
export function assertProductionCutoverActivation({ env = process.env, requiredPhase } = {}) {
  ...
  if (!state.allowed ||
      (requiredIndex >= 0 && state.authorizationPhaseIndex < requiredIndex)) {
...
export function productionCutoverPhaseAtLeast(env, phase) {
  ...
  return state.allowed && requiredIndex >= 0 &&
    state.authorizationPhaseIndex >= requiredIndex;
}
