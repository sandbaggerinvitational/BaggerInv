...
function stateFor(variable, env = process.env) {
  ...
  return { requested, resolved, blocked: requested === "supabase" && preview && !eligible,
    reason: resolved === "supabase" ? "preview-supabase" : !preview ? "production-hard-block" : !isolated ? "preview-workbook-required" : !configured ? "credentials-missing" : "application-configured" };
}
