...
function oddsModel(odds = null) {
  ...
  if (odds.publication_revision !== undefined || odds.publicationRevision !== undefined) {
    ...
    const history = (Array.isArray(odds.snapshots) ? odds.snapshots : []).map((snapshot) => ({
      revision: number(stateValue(snapshot, "publication_state_revision", "publication_revision")),
      ...
      provenance: upper(stateValue(snapshot, "adoption_kind", "origin_authority") ||
        (number(stateValue(snapshot, "publication_state_revision")) === currentRevision
          ? stateValue(odds, "adoption_kind")
          : "")),
