...
export function buildProductionDirectorOverview({
  ...
  return {
    ...
    tournament: {
      id: clean(currentTournamentContext?.tournamentId || live.tournament?.id || view.tournament?.tournament_id || PRODUCTION_TOURNAMENT_ID),
...
export async function readProductionDirectorOverview({
  ...
  const settled = await Promise.allSettled([
    ...
    legacy2026Runtime ? adapters.readNetSkins({ env }) : Promise.resolve(null),
    legacy2026Runtime ? adapters.readCalcutta({ env }) : Promise.resolve(null),
