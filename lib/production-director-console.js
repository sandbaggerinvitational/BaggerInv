...
export const PRODUCTION_DIRECTOR_SECTIONS = Object.freeze([
  ...
  Object.freeze({ id: "tournaments", label: "Tournaments", href: "/admin/director?section=tournaments" }),
...
const FROZEN_2026_DIRECTOR_SECTION_IDS = new Set([
  "players-access", "handicaps", "tournament-setup",
]);

export function productionDirectorNavigationForTournament(tournamentId) {
  const selected = clean(tournamentId || PRODUCTION_TOURNAMENT_ID);
  return selected === PRODUCTION_TOURNAMENT_ID
    ? PRODUCTION_DIRECTOR_SECTIONS
    : Object.freeze(PRODUCTION_DIRECTOR_SECTIONS.filter(
      (section) => !FROZEN_2026_DIRECTOR_SECTION_IDS.has(section.id),
    ));
}
...
function positivePublicationRevision(value) {
...
export function productionDirectorOddsPublicationRevision(snapshot = {}, publication = {}) {
  const explicitKeys = ["publication_state_revision", "publicationStateRevision"];
  const explicitKey = explicitKeys.find((key) => Object.hasOwn(snapshot, key));
  if (explicitKey) {
    const explicit = positivePublicationRevision(snapshot[explicitKey]);
    if (explicit > 0) return explicit;
    // Migration 088 permits the legacy fallback only when the explicit field
    // is absent or JSON null. Any other malformed explicit value fails closed.
    if (snapshot[explicitKey] !== null) return 0;
  }

  if (upper(stateValue(publication, "adoption_kind", "adoptionKind")) !==
      "LEGACY_GOOGLE_ADOPTED" ||
      clean(stateValue(snapshot, "authority_contract_version", "authorityContractVersion")) !==
      "legacy-google-published-odds-v1") {
    return 0;
  }
  return positivePublicationRevision(
    stateValue(snapshot, "publication_revision", "publicationRevision"),
  );
}
...
function oddsModel(odds = null) {
  ...
  if (odds.publication_revision !== undefined || odds.publicationRevision !== undefined) {
    ...
    const currentRevision = number(stateValue(odds, "publication_revision", "publicationRevision"));
    const historyCandidates = (Array.isArray(odds.snapshots) ? odds.snapshots : []).map((snapshot) => {
      const revision = productionDirectorOddsPublicationRevision(snapshot, odds);
      return {
        revision,
        milestone: clean(stateValue(snapshot, "milestone")),
        publishedAt: clean(stateValue(snapshot, "published_at")),
        provenance: upper(stateValue(snapshot, "adoption_kind", "origin_authority") ||
          (revision === currentRevision ? stateValue(odds, "adoption_kind") : "")),
        lifecycle: upper(stateValue(snapshot, "publication_lifecycle") ||
          (snapshot.is_current_official === true ? "PUBLISHED" : "HISTORICAL")),
        withdrawnAt: clean(snapshot.withdrawal?.withdrawn_at),
        withdrawalReason: upper(snapshot.withdrawal?.reason_code),
      };
    }).filter((item) => item.revision > 0).sort((left, right) =>
      right.revision - left.revision ||
      right.publishedAt.localeCompare(left.publishedAt) ||
      left.milestone.localeCompare(right.milestone));
    const seenHistoryRevisions = new Set();
    const history = historyCandidates.filter((item) => {
      if (seenHistoryRevisions.has(item.revision)) return false;
      seenHistoryRevisions.add(item.revision);
      return true;
    });
    return {
      available: true,
      state,
      label: state === "PUBLISHED" ? "Published" : state === "WITHDRAWN" ? "No current publication" : state === "STALE" ? "Needs review" : "Not published",
      publishedAt: clean(odds.published_at || odds.publishedAt || odds.last_published_at),
      revision: currentRevision,
      snapshotId: clean(stateValue(odds, "published_snapshot_id", "snapshot_id", "snapshotId")),
      ...(stateValue(odds, "publication_predecessor_snapshot_id", "predecessor_snapshot_id") !== undefined
        ? { predecessorSnapshotId: clean(stateValue(odds, "publication_predecessor_snapshot_id", "predecessor_snapshot_id")) }
        : {}),
      ...(stateValue(odds, "publication_pointer_revision") !== undefined
        ? { pointerRevision: number(stateValue(odds, "publication_pointer_revision")) }
        : {}),
...
export function buildProductionDirectorOverview({
  ...
  const currentTournamentId = clean(
    currentTournamentContext?.tournamentId ||
    live.tournament?.id ||
    view.tournament?.tournament_id ||
    PRODUCTION_TOURNAMENT_ID,
  );
  const legacy2026Runtime = currentTournamentId === PRODUCTION_TOURNAMENT_ID;
  return {
    mode: "production",
    navigation: productionDirectorNavigationForTournament(currentTournamentId),
    tournament: {
      id: currentTournamentId,
      year: number(currentTournamentContext?.tournamentYear || live.tournament?.year || view.tournament?.tournament_year),
    ...
    capabilities: {
      ...
      handicapManagement: Boolean(handicaps),
      playersAccessManagement: legacy2026Runtime && Boolean(enrollment),
      tournamentSetupManagement: legacy2026Runtime,
...
async function defaultDependencies() {
  const [tournament, identity, readControl, scoring, odds, oddsPublication, netSkins, calcutta, handicaps,
    mutationAuthority, prediction, draft, guide, guideAdapter, privateOperations, currentRuntime] = await Promise.all([
    ...
    import("./production-current-tournament-runtime.js"),
  ]);
  return {
    ...
    readCurrentTournamentRuntime: currentRuntime.readProductionCurrentTournamentRuntime,
  };
}
...
export async function readProductionDirectorOverview({
  ...
  const injectedComplete = completeDependencies(dependencies);
  const adapters = injectedComplete
    ? dependencies
    : { ...(await defaultDependencies()), ...dependencies };
  const production = clean(env.VERCEL_ENV).toLowerCase() === "production";
  const currentTournamentContext = typeof adapters.readCurrentTournamentRuntime === "function"
    ? await adapters.readCurrentTournamentRuntime({}, { env })
    : production && !injectedComplete
      ? (() => { throw productionDirectorDataError({ code: "PRODUCTION_CURRENT_TOURNAMENT_RUNTIME_REQUIRED" }); })()
      : { tournamentId: PRODUCTION_TOURNAMENT_ID, tournamentYear: Number(PRODUCTION_TOURNAMENT_ID), lifecycle: "ACTIVE" };
  const currentTournamentId = clean(currentTournamentContext.tournamentId);
  const legacy2026Runtime = currentTournamentId === PRODUCTION_TOURNAMENT_ID;
  const settled = await Promise.allSettled([
    adapters.readTournamentLiveView(currentTournamentId, { env }),
    adapters.inspectReadState({ env }),
    legacy2026Runtime ? adapters.inspectEnrollment({ env }) : Promise.resolve(null),
    adapters.inspectWorkers({ env }),
    adapters.readOdds({ tournamentId: currentTournamentId, sourceWorkbookId: PRODUCTION_GOOGLE_WORKBOOK_ID }, { env }),
    adapters.readNetSkins({ env, tournamentId: currentTournamentId }),
    adapters.readCalcutta({ env }),
    legacy2026Runtime && typeof adapters.readHandicaps === "function"
    ...
    typeof adapters.readPredictionSettings === "function"
      ? adapters.readPredictionSettings(currentTournamentId, { env })
      : Promise.resolve(null),
    typeof adapters.readDraft === "function"
      ? adapters.readDraft({ env, tournamentId: currentTournamentId })
      : Promise.resolve(null),
    typeof adapters.readGuide === "function"
      ? adapters.readGuide({ env, tournamentId: currentTournamentId })
      : Promise.resolve(null),
    legacy2026Runtime && typeof adapters.readPrivateOperations === "function"
  ...
  if (clean(live.tournament?.id || view.tournament?.tournament_id) !== currentTournamentId) {
    throw productionDirectorDataError({ code: "CURRENT_RUNTIME_READ_TRANSPORT_NOT_POINTER_AWARE" });
  }
