import {mobilePublicGuideGolfData} from './mobile-v1-guide.js';
...
import {mobileHistoryDetailData,mobilePublicHistorySummary} from './mobile-v1-history.js';
  ...
  function pick(rule, source, depth=0) {
    ...
    if(rule===true){
      if(!['string','number','boolean'].includes(typeof source) ||
        ...
        (typeof source==='string'&&source.length>(resource==='guide'?20000:2000))) throw Error('SPECTATOR_UNAVAILABLE');
      return source;
    }
...
// Published golf editorial text can contain embedded contact or money-game text.
// Do not treat an allowed string field as permission to disclose that content.
export function publicGuideGolf(read) {
  const data=publicGolfAllowlist('guide',mobilePublicGuideGolfData(read));
  const excluded=/calcutta|net\s*skins|auction|holdings|payout|purchase|settlement|buy[- ]?in|\$|\bUSD\b|[\w.+-]+@[\w.-]+\.[a-z]{2,}|(?:\+?1[- .]?)?\(?\d{3}\)?[- .]\d{3}[- .]\d{4}/i;
  const unsafe=value=>excluded.test(JSON.stringify(value));
  data.overview=data.overview.filter(row=>!unsafe(row));
  data.rules.items=data.rules.items.filter(row=>!unsafe(row));
  function clean(v){if(typeof v==='string')return excluded.test(v)?null:v;if(Array.isArray(v))return v.map(clean);if(v&&typeof v==='object')return Object.fromEntries(Object.entries(v).map(([k,x])=>[k,clean(x)]));return v;}
  return clean(data);
}

export function publicHistoryArchiveGolf(views) {
  if(!Array.isArray(views)||views.length>10||views.some(v=>v.source!=='supabase'||!Number.isInteger(v.year)||v.year<2017||v.year>2026)||new Set(views.map(v=>v.year)).size!==views.length)throw Error('SPECTATOR_UNAVAILABLE');
  return publicGolfAllowlist('archive',{tournaments:views.map(mobilePublicHistorySummary).sort((a,b)=>b.year-a.year)});
}
