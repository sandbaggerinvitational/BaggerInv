...
export const MATCH_PROGRESSION_RECORD_SLUGS = Object.freeze([
  "largest-match-victory",
  "largest-comeback",
  "largest-lead",
  "largest-lead-blown",
  "most-lead-changes",
  ...
  "most-holes-won-one-match",
  "most-holes-lost-one-match",
  "best-front-nine-match",
  "best-back-nine-match",
  "best-closing-stretch-match",
]);

const MATCH_PROGRESSION_RECORD_SLUG_SET = new Set(MATCH_PROGRESSION_RECORD_SLUGS);

export function isMatchProgressionRecordSlug(slug) {
  return MATCH_PROGRESSION_RECORD_SLUG_SET.has(clean(slug));
}
...
function sideEntry(match, side, value, {
  ...
  const eligible = negative
    ...
    : participants;
  if (!eligible.length || !Number.isFinite(Number(value))) return null;
  const singles = match.format === "SI";
  return {
    entityType: singles ? "PLAYER" : "TEAM_PERFORMANCE",
    value: Number(value),
    valueDisplay,
    ...
    playerId: singles ? eligible[0]?.id : undefined,
    playerName: singles ? eligible[0]?.name : undefined,
...
function record(slug, title, entries, { signed = false, formatter = null } = {}) {
  ...
  const eligible = filterEligibleRecordEntries(slug, entries.filter(Boolean)).sort((a, b) =>
    b.value - a.value ||
...
function buildRecords(matches, ghostMatchExclusions) {
  ...
  return [
    ...
    record("best-front-nine-match", "Most Front-Nine Holes Won in a Match", sideRows((match, side) => match.frontNine[side].won)),
    record("best-back-nine-match", "Most Back-Nine Holes Won in a Match", sideRows((match, side) => match.backNine[side].won)),
...
export function matchProgressionLeaderboardRows(recordDefinition) {
  return recordDefinition.entries.map((entry, index) => ({
    ...
    name: entry.entityType === "PLAYER"
      ? entry.playerName
      : entry.teamName || entry.name,
    subtitle: entry.entityType === "TEAM_PERFORMANCE"
      ? (entry.playerNames || []).join(" & ")
      : "",
    value: entry.value,
    valueDisplay: entry.valueDisplay || `${recordDefinition.signed && entry.value > 0 ? "+" : ""}${entry.value}${
      formatRecordUnit(entry.value, recordDefinition.unit)
        ? ` ${formatRecordUnit(entry.value, recordDefinition.unit)}`
        : ""
    }`,
    year: entry.year ?? "",
