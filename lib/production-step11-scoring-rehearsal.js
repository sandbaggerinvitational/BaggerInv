...
export function productionStep11ScoringRehearsalEnvironment(env = process.env) {
  ...
  const legacyAuthorityPreserved = clean(env.SCORING_AUTHORITY || "google").toLowerCase() === "google" &&
    clean(env.PARTICIPANT_IDENTITY_AUTHORITY || "passport").toLowerCase() === "passport";
  ...
  const allowed = requested && previewDeployment && shaApproved && hostnameApproved && projectApproved &&
    workbookApproved && runSecretConfigured && s3FingerprintApproved && legacyAuthorityPreserved && liveFeaturesDormant &&
  ...
  const reason = allowed ? "production-step11-scoring-rehearsal-ready"
    ...
    : !legacyAuthorityPreserved ? "live-google-passport-authority-required"
