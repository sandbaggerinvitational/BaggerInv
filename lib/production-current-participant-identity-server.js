import { PRODUCTION_PHONE_IDENTITY_OPERATIONS } from "./production-phone-identity-operations.js";
import "server-only";
...
export function productionCurrentParticipantIdentityRpcResolution({
  ...
  if (tournamentId === "2026" && Number(runtime?.tournamentYear) === 2026 &&
    ...
    return Object.freeze({
      functionName: PRODUCTION_PHONE_IDENTITY_OPERATIONS[clean(logicalFunctionName)] || clean(frozenFunctionName),
      body,
      tournamentId,
  ...
  const functionName = PRODUCTION_PHONE_IDENTITY_OPERATIONS[clean(logicalFunctionName)] || FUTURE_RPC_NAMES[clean(logicalFunctionName)];
  if (!functionName) {
