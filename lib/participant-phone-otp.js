...
export function participantPhoneOtpErrorMessage(code) {
  return ({
    ...
    PHONE_LOGIN_DIRECTOR_ENTITLEMENT_PRESENT: "The controlled participant phone login cannot continue while this Auth account has Director access.",
...
export function verifyParticipantPhoneLoginProof(token, secret, { now = Date.now() } = {}) {
  ...
  if (payload.version !== 1 || payload.issuedAt > current + 30 || payload.expiresAt <= current ||
