...
export function participantPhoneOtpErrorMessage(code) {
  return ({
    ...
    PHONE_OTP_CAPTCHA_FAILED: "We couldn't verify this request. Try again.",
    PHONE_LOGIN_PROOF_REQUIRED: "The controlled Preview phone-login proof is unavailable. Email sign-in remains available.",
    PHONE_LOGIN_PROOF_USED: "This controlled phone login proof has already requested a code. Reload the controlled Preview test to start over.",
...
export function participantPhoneOtpIdentifierFingerprint(value, secret) {
  const key = clean(secret);
  if (key.length < 32) {
    throw new ParticipantPhoneOtpError(
      "PHONE_OTP_CONFIGURATION_REQUIRED",
  ...
  return createHmac("sha256", key).update(`phone-login|${clean(value)}`).digest("hex");
}
...
export function normalizeParticipantAuthCaptchaToken(value, { required = true } = {}) {
  const token = clean(value);
  if (!token && !required) return "";
  if (token.length < 20 || token.length > 4096 || /\s/.test(token)) {
    throw new ParticipantPhoneOtpError("PHONE_OTP_CAPTCHA_FAILED", participantPhoneOtpErrorMessage("PHONE_OTP_CAPTCHA_FAILED"), 400);
  }
  return token;
}
...
export async function requestExistingParticipantPhoneLogin({ authClient, phone, captchaToken }) {
  const result = await authClient.auth.signInWithOtp({
    phone,
    options: { shouldCreateUser: false, channel: "sms", ...(captchaToken ? { captchaToken } : {}) },
  });
  if (result.error) throw result.error;
...
export function classifyParticipantPhoneOtpProviderFailure(error, phase = "send") {
  ...
  if (authErrorCode.includes("captcha") || message.includes("captcha")) {
    return {
