...
async function fetchSheetAliases(sheetNames, optional = false) {
  ...
  for (const name of names) {
    try {
      const rows = await fetchSheet(name, false);
      // A valid but empty tab is still a successful lookup. Return it rather
      // than silently trying a differently named tab.
      return rows;
    } catch (error) {
      lastError = error;
    }
...
export const loadPredictionSheets = cache(async () => {
  const entries = await Promise.all(Object.entries(SHEETS).map(async ([key, names]) => [
    key,
    await fetchSheetAliases(names, ["scorecards", "holes", "settings"].includes(key)),
  ]));
