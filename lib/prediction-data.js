...
function csvUrlByGid(gid) {
  return `https://docs.google.com/spreadsheets/d/${SPREADSHEET_ID}/gviz/tq?tqx=out:csv&gid=${encodeURIComponent(gid)}`;
}
...
async function fetchSheetDetailed(config) {
  ...
  for (const name of config.aliases) {
    try {
      const result = await fetchCsv(csvUrlByName(name), name);
  ...
  try {
    ...
    for (const name of config.aliases) {
      ...
      try {
        const result = await fetchCsv(csvUrlByGid(gid), `${name} (gid ${gid})`);
