...
function normalizeHistoryEntry(entry = {}) {
  ...
  return {
    ...
    receipt: {
      ...
      operationRequestId: clean(firstDefined(receipt.operationRequestId, receipt.operation_request_id)),
      payloadHash: clean(firstDefined(
        receipt.payloadHash,
        receipt.payload_hash,
        receipt.requestPayloadHash,
        receipt.request_payload_hash,
      )),
    },
  };
}

export function weeklyHandicapApprovalReceipt(payload = {}, {
  expectedOperationRequestId = "",
  expectedRevisionId = "",
} = {}) {
  const value = payload.data || payload.result || payload;
  const receipt = value.receipt || value.auditReceipt || value.audit_receipt || {};
  const result = {
    receiptId: clean(firstDefined(receipt.receiptId, receipt.receipt_id)),
    operationRequestId: clean(firstDefined(
    ...
    approvedAt: clean(firstDefined(receipt.approvedAt, receipt.approved_at)),
    revisionId: clean(firstDefined(value.revisionId, value.revision_id)),
  };
  const expectedOperation = clean(expectedOperationRequestId);
  const expectedRevision = clean(expectedRevisionId);
  const responsePayloadHash = clean(firstDefined(
    value.requestPayloadHash,
    value.request_payload_hash,
  ));
  const responseCode = clean(value.code).toUpperCase();
  if (!result.receiptId || !/^[0-9a-f]{64}$/i.test(result.payloadHash) ||
      !result.operationRequestId || result.status !== "APPROVED" ||
      !Number.isFinite(Date.parse(result.approvedAt)) ||
      (responseCode && responseCode !== "HANDICAP_REVISION_APPROVED") ||
      (expectedOperation && result.operationRequestId !== expectedOperation) ||
      (expectedRevision && result.revisionId !== expectedRevision) ||
      (responsePayloadHash && responsePayloadHash !== result.payloadHash)) {
    throw new Error("The approved revision did not return an authoritative receipt.");
  }
  return result;
}

export function currentApprovedWeeklyHandicapRevision(data = {}, revisionId = "") {
  const targetRevisionId = clean(revisionId);
  if (!targetRevisionId) return null;
  const historyEntry = (data.history || []).find((entry) =>
    entry.revisionId === targetRevisionId && entry.status === "APPROVED");
  const currentRevisionId = clean(data.currentRevisionId);
  const currentStatus = clean(data.currentRevisionStatus).toUpperCase();
  if (currentRevisionId) {
    if (currentRevisionId !== targetRevisionId || currentStatus !== "APPROVED") return null;
    return historyEntry || {
      revisionId: currentRevisionId,
      revision: Number(data.revision) || null,
      status: currentStatus,
      approvedAt: clean(data.currentRevisionApprovedAt),
      receipt: null,
    };
  }
  return historyEntry && Number(historyEntry.revision) === Number(data.revision)
    ? historyEntry
    : null;
}
...
export function normalizeWeeklyHandicapPayload(payload = {}) {
  ...
  const currentRevision = current.revision && typeof current.revision === "object"
    ? current.revision
    : {};
  ...
  return {
    ...
    currentRevisionId: clean(firstDefined(
    ...
    currentRevisionStatus: clean(firstDefined(
      value.currentRevisionStatus,
      value.current_revision_status,
      current.status,
      currentRevision.status,
    )).toUpperCase(),
    currentRevisionApprovedAt: clean(firstDefined(
      value.currentRevisionApprovedAt,
      value.current_revision_approved_at,
      current.approvedAt,
      current.approved_at,
