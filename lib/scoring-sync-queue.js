import {
  ...
  sameScoringMutationAuthorityContract,
...
export function sameScoringIntent(left = {}, right = {}) {
  return Number(left.holeNumber) === Number(right.holeNumber) &&
    JSON.stringify(parseGrossScores(left.team1GrossScores).map(Number)) === JSON.stringify(parseGrossScores(right.team1GrossScores).map(Number)) &&
    JSON.stringify(parseGrossScores(left.team2GrossScores).map(Number)) === JSON.stringify(parseGrossScores(right.team2GrossScores).map(Number)) &&
    sameScoringMutationAuthorityContract(left.scoringAuthorityContract, right.scoringAuthorityContract);
}

export function classifyScoringSyncFailure(error = {}) {
  const statusCode = Number(error?.status || 0);
  const message = String(error?.message || "");
  if (isScoringAuthorityTransitionFailure(error)) {
    return {
      status: "action-required",
      kind: "authority-transition",
      message: "Scoring authority changed. Refresh the match before syncing this saved score.",
    };
  }
  if (isScoringMutationReconciliationFailure(error)) {
    return {
      status: "action-required",
      kind: "operator-reconciliation",
      message: "This score needs Tournament Director review before it can be synchronized.",
    };
  }
...
export function createScoringSyncQueue({
  ...
  return {
    ...
    async enqueue(input) {
      ...
      const equivalent = matching
        .filter((entry) => sameScoringIntent(entry, input))
        .sort((a, b) => Number(b.sequence || 0) - Number(a.sequence || 0))[0];
      if (equivalent) {
        await emit("coalesced", { entry: equivalent, unchanged: true });
        process().catch(() => {});
        return { ...clone(equivalent), coalesced: true, unchanged: true };
      }
      if (input.authoritativeHole && sameGrossScores(input, input.authoritativeHole)) {
        const confirmed = {
          ...clone(input),
          id: "",
          status: "confirmed",
          clientMutationId: cleanMutationIdentity(input.authoritativeHole),
          noChange: true,
          unchanged: true,
        };
        await emit("no-change", { entry: confirmed, unchanged: true });
        return confirmed;
      }
      ...
      const generatedIdentity = globalThis.crypto?.randomUUID?.() || `${now()}-${Math.random().toString(36).slice(2)}`;
      const clientMutationId = input.clientMutationId || generatedIdentity;
      const operationRequestId = input.operationRequestId ||
        (/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(clientMutationId)
          ? clientMutationId
          : globalThis.crypto?.randomUUID?.() || generatedIdentity);
      const entry = {
        ...clone(input),
        id: "",
        version,
    ...
    async reconcile(matchId, serverHoles = [], serverMatch = {}) {
      ...
      for (const entry of await activeForMatch(matchId)) {
        if (["authority-transition", "operator-reconciliation"].includes(entry.failureKind)) {
          continue;
        }
    ...
    retryEntry(id) {
      ...
      return store.list().then(async (entries) => {
        ...
        if (entry && !["authority-transition", "operator-reconciliation"].includes(entry.failureKind) &&
            (["retryable", "action-required"].includes(entry.status) || scoringSyncIssueKind(entry) === "retryable")) {
          await store.put({
            ...entry,
            status: "queued",
            failureKind: null,
...
function cleanMutationIdentity(hole = {}) {
  return String(hole.clientMutationId || hole.mutationKey || hole["Client Mutation ID"] || "").trim();
}
