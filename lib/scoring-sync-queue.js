...
export function createScoringSyncQueue({
  ...
  return {
    ...
    async resolveConflict(id, resolution) {
      const entries = await store.list();
      const entry = entries.find((item) => item.id === id && item.status === "conflict");
      if (!entry) return false;
      if (resolution === "server") {
        await store.delete(entry.id);
        await emit("confirmed", { entry, result: { hole: entry.authoritativeHole, updatedAt: entry.authoritativeMatchUpdatedAt }, reconciled: true, resolution: "server" });
        process().catch(() => {});
        return true;
      }
