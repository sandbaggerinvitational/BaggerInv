...
const DB_VERSION = 1;
const TERMINAL = new Set(["confirmed", "conflict"]);
...
export function scoringSyncSummary(entries = [], online = true) {
  ...
  if (conflicts) return { state: "attention", text: `${conflicts} score${conflicts === 1 ? "" : "s"} need${conflicts === 1 ? "s" : ""} attention` };
  ...
  const retrying = active.some((entry) => entry.status === "retryable");
  return retrying
    ? { state: "retry", text: `Score${active.length === 1 ? "" : "s"} saved on this device · Tap to retry sync` }
    : { state: "syncing", text: `Syncing ${active.length} hole${active.length === 1 ? "" : "s"}…` };
}
...
export function createScoringSyncQueue({
  ...
  const runMutation = async (entry) => {
    ...
    try {
      ...
      await emit("confirmed", { entry: syncing, result, stale: newer });
      return true;
    } catch (error) {
      const statusCode = Number(error?.status || 0);
      const conflict = statusCode === 409;
      const blocked = [401, 403].includes(statusCode) || (statusCode === 400 && /final|locked|reopen|access/i.test(error?.message || ""));
      const next = {
        ...syncing,
        status: conflict || blocked ? "conflict" : "retryable",
        lastError: String(error?.message || "Synchronization failed."),
        nextRetryAt: conflict || blocked ? null : now() + Math.min(30_000, 750 * (2 ** Math.min(syncing.attempts, 5))),
      };
    ...
    async reconcile(matchId, serverHoles = [], serverMatch = {}) {
      ...
      for (const entry of entries) {
        ...
        if (serverHole && sameGrossScores(entry, serverHole)) {
          ...
          await store.put({ ...entry, status: "conflict", lastError: "The authoritative score changed on another device." });
