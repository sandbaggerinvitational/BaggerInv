...
export function readParticipantInitializationCache(now = Date.now()) {
  ...
  try {
    ...
    if (!cached?.payload?.player?.id || !shell?.id || cached.payload.player.id !== shell.id || cached.expiresAt <= now) {
...
export function writeParticipantInitializationCache(payload, now = Date.now()) {
  ...
  window.sessionStorage.setItem(CACHE_KEY, JSON.stringify({ payload, expiresAt: now + CACHE_TTL_MS }));
}
