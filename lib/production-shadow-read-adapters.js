...
export const PRODUCTION_SHADOW_CANDIDATE_PROJECTION_CONTRACTS = Object.freeze({
  ...
  DRAFT: Object.freeze({
    rpc: "read_production_draft_view_v1",
...
function currentSurface(surface, extra = {}, annualRuntimeInput = null) {
...
export function productionShadowCandidateRpcTranslation(functionName, body = {}, {
  ...
  switch (clean(functionName)) {
    case "read_tournament_live_view":
      return currentSurface("TOURNAMENT_LIVE", target, annualRuntimeInput);
    case "read_leaderboards_core_view":
      return currentSurface("LEADERBOARDS", target, annualRuntimeInput);
    case "read_preview_2026_historical_view":
      return currentSurface("HISTORY_2026");
    case "read_participant_home_view":
      return currentSurface("PARTICIPANT_HOME", { ...target, player_id: clean(body.target_player_id) }, annualRuntimeInput);
    case "read_my_match_view":
      return currentSurface("MY_MATCH", { ...target, player_id: clean(body.target_player_id) }, annualRuntimeInput);
    case "read_game_center_view":
      return currentSurface("GAME_CENTER", { ...target, match_id: clean(body.target_match_id) }, annualRuntimeInput);
    case "read_match_authorization_matrix":
      return currentSurface("MATCH_AUTHORIZATION", target, annualRuntimeInput);
    case "read_net_skins_input_view":
      return currentSurface("NET_SKINS_INPUT", target, annualRuntimeInput);
    case "read_net_skins_result_view":
      return currentSurface("NET_SKINS_RESULT", target, annualRuntimeInput);
    case "read_calcutta_configuration_view":
      return currentSurface("CALCUTTA_CONFIGURATION", target, annualRuntimeInput);
    case "read_published_odds_view":
      return currentSurface("PUBLISHED_ODDS", target, annualRuntimeInput);
    case "read_championship_odds_inputs":
      return currentSurface("ODDS_INPUT", target, annualRuntimeInput);
    case "read_participant_identity_context":
      return currentSurface("PARTICIPANT_IDENTITY", { ...target, player_id: clean(body.target_player_id || input.player_id) }, annualRuntimeInput);
    ...
    case "read_current_guide_projection":
      if (targetTournamentId && targetTournamentId !== PRODUCTION_TOURNAMENT_ID) {
        return {
          ...currentSurface("GUIDE_PROJECTION", target, annualRuntimeInput),
    ...
    case "read_preview_draft_view":
      {
        ...
        return {
          ...
          body: { input: {
            ...projectionInput("DRAFT"),
            target_scope: request.scope,
            target_year: request.year,
            target_player_id: request.playerId || null,
            ...target,
            ...(annualRuntimeInput || {}),
          } },
...
function adaptDraftProjection(payload = {}, request = {}) {
  if (!payload.ok || !payload.data) return payload;
  const data = payload.data;
  const canonicalCurrentRows = Array.isArray(data.drafts);
  const all = canonicalCurrentRows
    ? data.drafts
    : Array.isArray(data.payload?.drafts) ? data.payload.drafts : [];
  ...
  const selected = all.filter((row) => {
    ...
    // The canonical V1 RPC already selected CURRENT under the annual pointer.
    // The max-year behavior remains only for dormant legacy payload adaptation.
    if (scope === "CURRENT") return canonicalCurrentRows || integer(row.tournament_year) === currentYear;
    ...
    contract_version: clean(row.contract_version || data.contract_version),
    revision_id: row.revision_id || data.revision_id,
    revision_number: row.revision_number ?? data.revision_number,
    previous_revision_id: row.previous_revision_id ?? data.previous_revision_id,
    synchronized_at: row.synchronized_at || data.imported_at,
    synchronized_by: row.synchronized_by || data.imported_by,
  }));
  return {
    ...payload,
    data: {
