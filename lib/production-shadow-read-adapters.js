...
function currentSurface(surface, extra = {}) {
  return {
    functionName: "read_production_candidate_current_view",
...
export function productionShadowCandidateRpcTranslation(functionName, body = {}) {
  ...
  switch (clean(functionName)) {
    case "read_tournament_live_view":
      return currentSurface("TOURNAMENT_LIVE");
    case "read_leaderboards_core_view":
      return currentSurface("LEADERBOARDS");
    case "read_preview_2026_historical_view":
      return currentSurface("HISTORY_2026");
    case "read_participant_home_view":
      return currentSurface("PARTICIPANT_HOME", { player_id: clean(body.target_player_id) });
    case "read_my_match_view":
      return currentSurface("MY_MATCH", { player_id: clean(body.target_player_id) });
    case "read_game_center_view":
      return currentSurface("GAME_CENTER", { match_id: clean(body.target_match_id) });
    case "read_match_authorization_matrix":
      return currentSurface("MATCH_AUTHORIZATION");
    case "read_net_skins_input_view":
      return currentSurface("NET_SKINS_INPUT");
    case "read_net_skins_result_view":
      return currentSurface("NET_SKINS_RESULT");
    case "read_calcutta_configuration_view":
      return currentSurface("CALCUTTA_CONFIGURATION");
    case "read_published_odds_view":
      return currentSurface("PUBLISHED_ODDS");
    case "read_championship_odds_inputs":
      return currentSurface("ODDS_INPUT");
    case "read_participant_identity_context":
      return currentSurface("PARTICIPANT_IDENTITY", { player_id: clean(body.target_player_id || input.player_id) });
