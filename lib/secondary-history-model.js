...
function completedTournamentLegacyRow(view = {}) {
  ...
  return {
    ...
    // The established deterministic statistics contract compares these
    // fields with the season's presentation name. Stable IDs remain on the
    // team rows and richer completed-History view; using IDs here would make
    // every losing finalist look merely "Completed" instead of Runner-Up.
    "Winning Team": clean(champion?.name || champion?.id),
    "Runner-Up Team": clean(runnerUp?.name || runnerUp?.id),
