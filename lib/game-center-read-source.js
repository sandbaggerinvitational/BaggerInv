...
export function gameCenterReadEnvironment(env = process.env) {
  const configured = clean(env.GAME_CENTER_READ_SOURCE || "google").toLowerCase();
  ...
  const cutover = productionCutoverReadSourceEnvironment({
    env, variable: "GAME_CENTER_READ_SOURCE", configuredValue: env.GAME_CENTER_READ_SOURCE,
  ...
  return {
    ...
    reason: resolved === "supabase" ? (candidate.eligible ? "production-shadow-supabase-game-center" : "preview-supabase-read")
