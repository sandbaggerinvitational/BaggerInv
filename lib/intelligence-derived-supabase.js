...
async function productionIntelligenceWorkerContext(options = {}) {
...
async function intelligenceWorkerRpc(operationName, input, options = {}) {
  const env = options.env || process.env;
  const context = await productionIntelligenceWorkerContext(options);
...
export const writeIntelligenceDerivedBundle = (input, options = {}) =>
  intelligenceWorkerRpc("write_intelligence_derived_bundle", input, {
    ...options, timeoutMs: options.timeoutMs || 15_000,
  });
export const claimIntelligenceDerivedBundle = (input, options = {}) =>
  intelligenceWorkerRpc("claim_intelligence_derived_bundle", input, {
    ...options, timeoutMs: options.timeoutMs || 8_000,
  });

export async function recalculateIntelligenceDerivedTournament(tournamentId, {
  calculatedBy = "Intelligence derived worker", env = process.env,
  workerId = "intelligence-derived-current", ...transportOptions
} = {}) {
  const scoringDispatchContext = await productionIntelligenceWorkerContext({
    env, ...transportOptions,
  });
  const resolvedTournamentId = clean(
    scoringDispatchContext?.runtime?.tournamentId || tournamentId,
  );
  const workerOptions = {
    env, workerId, scoringDispatchContext, ...transportOptions,
  };
  const inputs = await loadIntelligenceCanonicalInputs(resolvedTournamentId);
  const calculated = calculateIntelligenceDerivedFromData(inputs);
  const engineKeys = ["TOURNAMENT_INTELLIGENCE", "PROJECTION_EDITORIAL",
    ...(calculated.recap.gate.eligible ? ["TOURNAMENT_FINAL_RECAP"] : [])];
  const claim = await claimIntelligenceDerivedBundle({
    environment: "PREVIEW", tournament_id: calculated.tournamentId,
    engine_keys: engineKeys, requested_by: calculatedBy,
    worker_id: workerId, lease_seconds: 90,
  }, workerOptions);
  if (!claim.payload?.ok) throw Object.assign(new Error("Intelligence work could not be claimed."), { code: claim.payload?.code || "INTELLIGENCE_CLAIM_FAILED" });
  const payload = { environment: "PREVIEW", tournament_id: calculated.tournamentId,
    calculated_by: calculatedBy, worker_id: workerId,
    claim_token: clean(claim.payload.claim_token),
    source_fingerprint: calculated.sourceFingerprint, dependency: calculated.dependency,
    engines: [
      { key: "TOURNAMENT_INTELLIGENCE", version: TOURNAMENT_INTELLIGENCE_ENGINE_VERSION, result: calculated.intelligence.result, payload_hash: calculated.intelligence.payloadHash, claim_started_at: claim.payload.claim_started_at },
      { key: "PROJECTION_EDITORIAL", version: PROJECTION_EDITORIAL_ENGINE_VERSION, result: calculated.editorial.result, payload_hash: calculated.editorial.payloadHash, claim_started_at: claim.payload.claim_started_at },
      ...(calculated.recap.gate.eligible ? [{ key: "TOURNAMENT_FINAL_RECAP", version: TOURNAMENT_FINAL_RECAP_ENGINE_VERSION, result: calculated.recap.result, payload_hash: calculated.recap.payloadHash, claim_started_at: claim.payload.claim_started_at }] : []),
    ], final_gate: calculated.recap.gate, duration_ms: calculated.calculationMs };
  const write = await writeIntelligenceDerivedBundle(payload, workerOptions);
...
export async function currentIntelligenceDerivedState(tournamentId, options = {}) {
  const read = await readCompetitionDerivedState(tournamentId, INTELLIGENCE_ENGINE_KEYS, { env: options.env });
