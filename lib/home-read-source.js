...
export function homeReadEnvironment(env = process.env) {
  const configured = clean(env.HOME_READ_SOURCE || "google").toLowerCase();
  ...
  const cutover = productionCutoverReadSourceEnvironment({
    env, variable: "HOME_READ_SOURCE", configuredValue: env.HOME_READ_SOURCE,
  ...
  return {
    ...
    reason: resolved === "supabase" ? (candidate.eligible ? "production-shadow-supabase-home" : "preview-supabase-home")
