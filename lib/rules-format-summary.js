...
export function formatRuleSummary(formatCode, sources, points) {
  ...
  const defaultHandicap = { BB: "90%", SC: "35% / 15% Team Handicap", SI: "100%" }[formatCode] || "";
  const defaultScoring = { BB: "Nassau Match Play", SC: "Nassau Match Play", SI: "18-Hole Match Play" }[formatCode] || "";
  const handicapSummary = String(handicap || defaultHandicap)
    .replace(/\s+Handicap Allocation$/i, "")
    .trim();
  return [
    `Points: ${points !== null && points !== undefined && points !== "" ? points : 3}`,
    `Handicap: ${handicapSummary}`,
    `Scoring: ${scoring || defaultScoring}`,
  ];
}

export function formatRuleHeading(value) {
  const heading = String(value || "").replace(/^(?:Fourball|Best Ball|Scramble|Singles)\s+/i, "").trim();
  if (/handicap allocation/i.test(heading)) return "Handicap";
  if (/^(?:tournament|match) scoring$/i.test(heading)) return "Scoring";
  if (/mulligans?$/i.test(heading)) return "Mulligans";
  return heading;
}
