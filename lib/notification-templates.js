...
const definitions = [
  { id: "test", required: [], title: () => "🏌️ Test Notification", body: () => "Notifications are configured successfully.", url: () => "/admin/director" },
  { id: "tee-time-reminder", required: ["round", "format", "teeTime", "tee", "course"], title: () => "⛳ Tee Time Reminder", body: (c) => lines(roundFormat(c), teeDetails(c), c.course), url: () => "/my-match" },
  { id: "match-ready", required: ["round", "format", "opponent"], title: () => "⛳ Your Match Is Ready", body: (c) => lines(roundFormat(c), `vs ${c.opponent}`, "Tap to begin scoring."), url: () => "/my-match" },
  { id: "match-finalized", required: ["round", "format", "year", "match", "matchId", "finalizedBy"], title: () => "✅ Match Finalized", body: (c) => lines(roundFormat(c), `Finalized by ${scoringActor(c, "finalizedBy")}.`, "Tap to view the official scorecard."), url: officialScorecardUrl },
  { id: "match-reopened", required: ["round", "format", "reopenedBy"], title: () => "🔄 Match Reopened", body: (c) => lines(roundFormat(c), `Reopened by ${scoringActor(c, "reopenedBy")}.`, "Tap to resume scoring."), url: () => "/my-match" },
  { id: "singles-pairing", required: ["round", "format", "opponent", "teeTime", "tee", "course"], title: () => "👤 Singles Pairing Released", body: (c) => lines(roundFormat(c), `vs ${c.opponent}`, teeDetails(c), c.course), url: () => "/my-match" },
  { id: "timeline-event", required: ["event", "eventLead"], title: (c) => `${c.eventIcon || "📅"} ${c.event}`, body: (c) => lines(`Begins in ${c.eventLead}`, c.location), url: () => "/home" },
  { id: "round-started", required: ["round"], title: (c) => `🔴 Round ${c.round} Is LIVE`, body: () => lines("Every match is underway.", "Tap to follow the action live."), url: () => "/live" },
  { id: "round-clinched", required: ["round", "format", "team", "teamOnePoints", "teamTwoPoints"], title: (c) => `🏆 Round ${c.round} Clinched`, body: (c) => lines(`${c.team} won ${roundFormat(c)}`, `${c.teamOnePoints}–${c.teamTwoPoints}.`, "Tap to view updated standings."), url: () => "/live" },
  { id: "round-tied", required: ["round", "team", "otherTeam"], title: (c) => `🤝 Round ${c.round} Ends in a Tie`, body: (c) => lines(`${c.team} and ${c.otherTeam}`, "finished tied.", "Tap to view updated standings."), url: () => "/live" },
  ...
  { id: "tournament-champions", required: ["team", "year"], title: () => "🏆 Champions Crowned", body: (c) => lines("Congratulations to", `${c.team},`, `${c.year} Sandbagger Invitational Champions!`), url: () => "/live" },
  { id: "net-skins-results", required: ["round", "skins", "winnings"], title: (c) => `💰 Round ${c.round} Net Skins Final`, body: (c) => Number(c.skins) > 0 ? lines(`You won ${c.skins} ${Number(c.skins) === 1 ? "skin" : "skins"} • ${c.winnings}`, "Tap to view standings and payouts.") : lines(`Round ${c.round} Net Skins are official.`, "Tap to view standings and payouts."), url: () => "/live?view=leaderboards&tab=net-skins" },
...
export function notificationTemplate(id, context = {}) {
  ...
  const missing = (selected.required || []).filter((field) => context[field] === undefined || context[field] === null || context[field] === "");
  if (missing.length) throw new Error(`Notification template ${selected.id} is missing runtime fields: ${missing.join(", ")}`);
  const title = selected.title(context);
  return { id: selected.id, label: title, title, body: selected.body(context), url: selected.url(context) };
}

export function previewNotificationTemplate(id, context = {}) {
  return notificationTemplate(id, { ...PREVIEW_NOTIFICATION_CONTEXT, ...context });
}

export function previewNotificationTemplateOptions(context = {}) {
  const values = { ...PREVIEW_NOTIFICATION_CONTEXT, ...context };
  return definitions.map(({ id }) => ({ id, label: notificationTemplate(id, values).title }));
}

export const NOTIFICATION_TEMPLATE_OPTIONS = previewNotificationTemplateOptions();
