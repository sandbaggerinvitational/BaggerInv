...
export async function mobileV1ScoringResponse(request, loader, capability = "scoring") {
  let result;
  try {
    const identity = await resolveMobileBearerIdentity({ request, capability });
    result = await loader(identity);
    if (capability === "reads") await recheckMobileNativeIdentity(identity, "reads");
  } catch (error) {
