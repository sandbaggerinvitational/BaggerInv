...
const enabled = (value) => /^(?:1|true|yes|on|enabled)$/i.test(clean(value));

async function withProductionGoogleAuthorityWriteBoundary(input, operation, env = process.env) {
  // Keep the ordinary Google-authority adapter importable by mobile/contract
  // tests and by the legacy deployment. The server-only Production lease
  // implementation is loaded only when its explicit cutover gate is requested;
  // a requested but malformed gate still imports and fails closed.
  if (!enabled(env.PRODUCTION_GOOGLE_INGRESS_LEASE_GATE_ENABLED)) return operation();
...
function canonicalAuthorization(current, matchId, verified = {}) {
  const verifiedIdentity = verified.identity || verified.director || {};
  return {
...
export function canonicalPersistenceError(payload = {}) {
  ...
  error.status = /CONFLICT/.test(payload.code || "") ? 409 : /UNAUTHORIZED|PERMISSION_STALE/.test(payload.code || "") ? 403 : 400;
  error.authoritativeDiagnostics = payload;
  return error;
}

export async function persistDirectorMatchLifecycle({ action, matchId, updatedBy, authUserId, playerId }) {
  const authority = requireScoringAuthority();
