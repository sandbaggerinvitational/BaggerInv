...
export function canonicalPersistenceError(payload = {}) {
  const message = ({
    ...
    SCORING_READINESS_REFRESH_REQUIRED: "Refresh match authority and retry in a fresh transaction.",
    PRODUCTION_MATCH_NOT_RESUME_READY: "The existing scoring context or history could not be verified for resume. Keep the match paused and review its authority; do not reinitialize or erase scores.",
    PRODUCTION_MATCH_NOT_SCORING_READY: "Complete Tournament Setup and prepare a current scoring snapshot before granting scoring access or marking this match Live.",
  })[payload.code] || "The authoritative score transaction could not be completed.";
  const error = new Error(message);
  error.code = payload.code;
  error.status = /CONFLICT|PRODUCTION_MATCH_NOT_RESUME_READY|PRODUCTION_MATCH_NOT_SCORING_READY|SCORING_READINESS_REFRESH_REQUIRED/.test(payload.code || "")
    ? 409
