...
export async function persistParticipantScore({
  ...
  const serverBoundMobileV1 = Boolean(suppliedCanonicalContext && includeCanonicalAcknowledgement &&
...
export async function persistDirectorMatchLifecycle({ action, matchId, updatedBy, authUserId, playerId, operationRequestId }) {
  const authority = requireScoringAuthority();
