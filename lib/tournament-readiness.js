...
export function tournamentReadiness({ tournamentId, players = [], passports = [], devices = [], handicaps = [], matches = [] } = {}) {
  ...
  const rows = players.map((player) => {
    ...
    return {
      ...
      notificationsEnabled: playerDevices.some((record) => truthy(record["Notifications Enabled"])),
  ...
  const items = definitions.map(([id, label, field]) => {
    ...
    return { id, label, complete: rows.length - missing.length, total: rows.length, missing };
  });
