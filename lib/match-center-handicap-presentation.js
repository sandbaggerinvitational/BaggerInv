...
export function matchCenterScrambleHandicaps(view = {}) {
  ...
  for (const entry of list(view.matches)) {
    ...
    // Empty per-match holes are valid before scoring preparation. The shared
    // read-only course projection still requires 18 complete, agreeing holes
    // for this exact round/course/tee. Partial or conflicting records poison
    // that projection; neither this label nor its holes are persisted.
    if (course?.state !== "READY" || completeAdvisoryHoles(course.holes, s.par).length !== 18) continue;
    const exact = [];
