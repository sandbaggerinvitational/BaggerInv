...
import credentialConfinementEvidenceArtifact from
  "../docs/evidence/step11-6-production-google-credential-confinement-v4.json" with {
    type: "json",
  };
import {
...
export const PRODUCTION_GOOGLE_CREDENTIAL_CONFINEMENT_RECORD_COUNT = 1_292;
...
const CANDIDATE_BRANCH = "feature/mock-tournament-qa-integration";
const REVIEWED_SHADOWED_PROJECT_WIDE_PREVIEW_NAMES = Object.freeze([
  "GOOGLE_SHEETS_ID",
...
const REVIEWED_UNSHADOWED_NONSECRET_PREVIEW_NAMES = Object.freeze([
...
const exactCandidatePreviewRecord = (name) => [
  name, ["preview"], CANDIDATE_BRANCH,
];
const V3_ENVIRONMENT_SCOPE_CONTRACT = deepFreeze({
  broadLegacyNames: ["GOOGLE_PRIVATE_KEY", "GOOGLE_SERVICE_ACCOUNT_EMAIL"],
  broadLegacyTargets: ["preview", "production"],
  dedicatedAndProductionResourcePreviewScope: "EXACT_FEATURE_BRANCH_REQUIRED",
  ...
  reviewedProjectWidePreviewException: {
    ...
    shadowedProjectWideRecords:
      REVIEWED_SHADOWED_PROJECT_WIDE_PREVIEW_NAMES.map(projectWidePreviewRecord),
    requiredSameNameExactCandidateOverrides:
      REVIEWED_SHADOWED_PROJECT_WIDE_PREVIEW_NAMES.map(exactCandidatePreviewRecord),
    unshadowedNonsecretProjectWideRecords:
      REVIEWED_UNSHADOWED_NONSECRET_PREVIEW_NAMES.map(projectWidePreviewRecord),
...
function v4EnvironmentScopeContract(review) {
  if (!validVercelEnvironmentResourceReview(review)) return null;
  return deepFreeze({
    ...V3_ENVIRONMENT_SCOPE_CONTRACT,
    providerEnvironmentResourceReview: {
      ...
      recordsFingerprint: review.recordsFingerprint,
      reviewFingerprint: review.reviewFingerprint,
      ownerCertifiedContinuityBaselineFingerprint:
        review.ownerCertifiedContinuityBaselineFingerprint,
...
function validVersionedEvidence(evidence, classificationTotal, base, {
  evidenceSchema,
  inventorySchema,
  providerEnvironmentReviewRequired,
}) {
  ...
  const topLevelKeys = [
    ...
    ...(providerEnvironmentReviewRequired ? ["providerEnvironmentResourceReview"] : []),
    "evidenceFingerprint",
  ];
  return exactKeys(evidence, topLevelKeys) &&
    evidence.schemaVersion === evidenceSchema &&
    evidence.originInventorySchemaVersion === inventorySchema &&
    ...
    (evidenceSchema !== "step11-6-production-google-credential-confinement-v4" ||
      evidence.dynamicCandidateContract?.rehearsalTarget === "PREVIEW" &&
      evidence.dynamicCandidateContract?.cutoverTarget === "PREVIEW" &&
      JSON.stringify(
        evidence.dynamicCandidateContract?.permittedAdditionScopeClasses,
      ) === JSON.stringify(["PROJECT_PREVIEW"])) &&
    ...
    (providerEnvironmentReviewRequired
      ? validVercelEnvironmentResourceReview(
        evidence.providerEnvironmentResourceReview,
      ) && JSON.stringify(evidence.environmentScopeContract) ===
        JSON.stringify(v4EnvironmentScopeContract(
          evidence.providerEnvironmentResourceReview,
        ))
      : evidence.environmentScopeContract
        ?.duplicateUnscopedDedicatedPreviewRecordAllowed === false);
...
function validV2Evidence(evidence, classificationTotal, base) {
  return validVersionedEvidence(evidence, classificationTotal, base, {
    evidenceSchema: "step11-6-production-google-credential-confinement-v2",
    inventorySchema: "step11-6-production-origin-inventory-v3",
    providerEnvironmentReviewRequired: false,
  });
}

function validV3Evidence(evidence, classificationTotal, base) {
  return validVersionedEvidence(evidence, classificationTotal, base, {
    evidenceSchema: "step11-6-production-google-credential-confinement-v3",
    inventorySchema: "step11-6-production-origin-inventory-v4",
    providerEnvironmentReviewRequired: false,
  });
}

function validV4Evidence(evidence, classificationTotal, base) {
  return validVersionedEvidence(evidence, classificationTotal, base, {
    evidenceSchema: "step11-6-production-google-credential-confinement-v4",
    inventorySchema: "step11-6-production-origin-inventory-v4",
    providerEnvironmentReviewRequired: true,
  });
}
...
export function productionGoogleCredentialConfinementEvidence() {
  ...
  const valid = {
    "step11-6-production-google-credential-confinement-v1": validV1Evidence,
    "step11-6-production-google-credential-confinement-v2": validV2Evidence,
    "step11-6-production-google-credential-confinement-v3": validV3Evidence,
    "step11-6-production-google-credential-confinement-v4": validV4Evidence,
  }[PRODUCTION_GOOGLE_CREDENTIAL_CONFINEMENT_SCHEMA]?.(
    evidence, classificationTotal, base,
  ) === true;
