...
export async function mobileMatchesResult(identity, { env = process.env, now, dependencies = {} } = {}) {
  ...
  const {data, revision} = matchesGolfData(successfulRead(liveRead), guideRead, identity.playerId, dependencies);
  const result = response(data, scoringShadowPayloadHash({
    product: "mobile-matches-v1",
    sourceRevision: clean(revision),
    data,
  }), now);
...
export function mobilePublicMatchesGolfData(raw, guideRead) {
  return matchesGolfData(raw, guideRead, "", {}).data;
}
function matchesGolfData(raw, guideRead, playerId, dependencies) {
  let live = (dependencies.tournamentLiveDataFromSupabaseView || tournamentLiveDataFromSupabaseView)(raw, { mobileContract: true });
  ...
  const matches = (live.rounds || []).flatMap((round) => (round.matches || []).map((match) =>
    matchDto({ ...match, team1Name: live.tournament?.teamOne?.name, team2Name: live.tournament?.teamTwo?.name },
      round, playerId, tournament.timeZone, {
  ...
  return {data, revision: live.revision};
}
...
export async function mobileLeadersResult(identity, { env = process.env, now, dependencies = {} } = {}) {
  ...
  const data = leadersGolfData(successfulRead(read), identity, dependencies, leaders);
  const result = response(data, scoringShadowPayloadHash({
...
// Same canonical golf projections for an explicitly bounded public read surface.
// No authentication, admission, money-game or personalized source is read here.
export function mobileLeadersGolfData(raw, identity = { tournamentId: "2026" }, dependencies = {}) {
  const leaders = (dependencies.leaderboardsCoreDataFromSupabaseView || leaderboardsCoreDataFromSupabaseView)(raw);
  return leadersGolfData(raw, identity, dependencies, leaders);
}

function leadersGolfData(raw, identity, dependencies, leaders) {
  ...
  const source = playerLeadersProjectionSource(raw, identity);
