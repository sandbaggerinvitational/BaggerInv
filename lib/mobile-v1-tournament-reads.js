...
function matchDto(match = {}, round = {}, authenticatedPlayerId = "", zone = "America/Chicago") {
  ...
  return {
    ...
    teams: [
      { side: 1, name: clean(match.team1Name) || null, participants: teamOne },
      { side: 2, name: clean(match.team2Name) || null, participants: teamTwo },
    ],
    authenticatedPlayer: {
...
export async function mobileMatchesResult(identity, { env = process.env, now, dependencies = {} } = {}) {
  ...
  const matches = (live.rounds || []).flatMap((round) => (round.matches || []).map((match) =>
    matchDto({ ...match, team1Name: live.tournament?.teamOne?.name, team2Name: live.tournament?.teamTwo?.name },
      round, identity.playerId, tournament.timeZone)))
    .sort((left, right) => (left.round.roundNumber ?? 0) - (right.round.roundNumber ?? 0)
      || clean(left.matchId).localeCompare(clean(right.matchId)));
  return response({ tournament, matches }, live.revision, now);
}
...
export async function mobileLeadersResult(identity, { env = process.env, now, dependencies = {} } = {}) {
  ...
  const teams = (dependencies.teamStandings || teamStandings)(leaders.rounds || [], leaders.tournament || {}, "overall")
    .map((row) => ({ rank: numeric(row.rank), teamId: clean(row.side === 1 ? leaders.tournament?.teamOne?.id : leaders.tournament?.teamTwo?.id),
      name: clean(row.name), points: numeric(row.points), record: clean(row.record), remainingMatches: numeric(row.remaining) }));
  ...
  return response({ tournament: tournamentDto(leaders.tournament), teamStandings: teams, playerStandings: players }, leaders.revision, now);
}
