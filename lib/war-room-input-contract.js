...
function handicapRows(sheets = {}, year = null, players = [], matches = [], history = {}) {
  ...
  return { historical, current, frozenMatchFacts: frozen, courseHandicap: { kind: "DERIVED", contract: "prediction-engine-course-handicap-v2-postgres-parity" } };
}
...
export function legacyPredictionSheetsFromBundle(bundle = {}, { scope = "full" } = {}) {
  ...
  // The canonical Supabase composition can retain a current-year handicap in
  // provenance history while bundle.players owns the active roster value.
  // Project only completed-year rows here so the legacy compatibility shape
  // never duplicates current players or revives an older current-year value.
  const historicalHandicaps = list(bundle.handicaps.historical).filter((row) => Number(row.year) !== Number(year)).map((row) => {
  ...
  const liveTournaments = [{ Year: year, "Tournament ID": bundle.tournament.id, "Tournament Name": bundle.tournament.name,
  ...
  const scorecards = bundle.courses.filter((row) => row.year === year).map((row) => ({ "Course ID": row.stableCourseId, Course: row.name, Tee: row.tee,
    "Course Rating": row.rating, "Slope Rating": row.slope, Par: row.par, Yardage: row.yardage }));
  if (scope === "team-intelligence") {
    return { players, liveTournaments, teamNames, handicaps, courses, scorecards, settings };
  }
  ...
  return {
    ...
    liveMatches, teamNames, liveTournaments,
...
export function buildWarRoomConsumerData({ bundle = {}, calculations, scorecardAnalytics = {}, scope = "war-room" } = {}) {
  const teamIntelligence = scope === "team-intelligence";
  const sheets = legacyPredictionSheetsFromBundle(bundle, {
    scope: teamIntelligence ? "team-intelligence" : "full",
  });
  ...
  // Team Intelligence consumes the scorecards only while deriving these
  // aggregates on the server. Shipping them again (plus unused compatibility
  // tabs) added more than a megabyte to the client payload without powering a
  // visible control.
  return {
    sheets,
    historical,
