...
function currentMatchRows(sheets = {}, year = null, teams = [], rounds = []) {
  ...
  return sourceRows.map((row, sourceOrder) => {
    ...
    const participants = [1, 2].flatMap((side) => [1, 2]
      .map((slot, index) => matchParticipant(row, teams, side, slot, side * 2 + index))
      .filter(Boolean));
    ...
    return {
      ...
      courseId: clean(pick(row, "Course ID")) || clean(roundConfig.courseId),
      tee: clean(pick(row, "Tee", "Tee Played")),
      lifecycle: state,
...
function tournamentState(sheets = {}, year = null, matches = [], rounds = []) {
  ...
  return {
    ...
    name: clean(pick(live, "Tournament Name", "Name")), lifecycle: upper(pick(live, "Tournament Status", "Status")) || "LIVE",
    currentRound: integer(pick(live, "Current Round"), Math.max(1, ...matches.filter((row) => row.lifecycle !== "UPCOMING").map((row) => row.round))),
