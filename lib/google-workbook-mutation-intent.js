...
export function currentGoogleWorkbookMutationIntent() {
  return mutationContext.getStore() || null;
}
...
export function withGoogleWorkbookMutationIntent(options, operation) {
  ...
  const store = {
    ...
    beforeFirstWrite: typeof options?.beforeFirstWrite === "function" ? options.beforeFirstWrite : null,
...
/**
 * Last-line workbook-write guard. It is called immediately before credentials
 * and network access by the sole Sheets transport. Exact Production workbook
 * writes cannot proceed without one of the three explicit intents. Canonical
 * legacy writes additionally require a live v2 admission and a durable
 * WRITE_STARTED marker before the first Google request is sent.
 */
export async function prepareGoogleWorkbookMutation({
  ...
  if (productionWorkbook && store.intent === GOOGLE_WORKBOOK_MUTATION_INTENTS.CANONICAL_LEGACY) {
    if (!store.admission?.enabled || !clean(store.admission?.admissionId || store.admission?.leaseId)) {
      throw mutationIntentError(
        ...
        "Production canonical Google writes require an active v2 admission.",
        { intent: store.intent, operation: store.operation, productionWorkbook: true },
      );
    }
    if (!store.beforeFirstWrite) {
      throw mutationIntentError(
        "PRODUCTION_CANONICAL_GOOGLE_WRITE_MARKER_REQUIRED",
        "Production canonical Google writes require a durable write-start marker.",
        { intent: store.intent, operation: store.operation, productionWorkbook: true },
      );
    }
    if (!store.writeStartPromise) {
      store.writeStartPromise = Promise.resolve().then(() => store.beforeFirstWrite({
...
export function markGoogleWorkbookMutationAmbiguous() {
  const store = mutationContext.getStore();
  if (store) store.ambiguousWrites += 1;
}
