import {publicMatchesGolf,publicMatchGolf,publicLeadersGolf,publicPlayerGolf,publicHistoryGolf,publicGuideGolf,publicHistoryArchiveGolf} from './spectator-golf-projection.js';
import {publicMatchNavigationScope,publicRoundMatchNavigation} from './spectator-match-navigation.js';

export function publicGolfRequest(parts) {
  if(!Array.isArray(parts))return null;
  const [resource,id,...rest]=parts;
  if(rest.length)return null;
  if(['leaders','matches','guide','archive'].includes(resource)&&parts.length===1)return {resource};
...
export function createPublicGolfReader(reads,{now=Date.now}={}) {
  ...
  return async parts=>{
    ...
    await reads.guard();
    if(resource==='archive')return {contract:'bagger-public-golf-v1',resource,data:await cached('public-golf-v1:archive',60000,async()=>publicHistoryArchiveGolf(await reads.archive()))};
    if(resource==='guide')return {contract:'bagger-public-golf-v1',resource,data:publicGuideGolf(await reads.guide())};
    if(resource==='matches') {
    ...
    const scope=await cached('public-golf-v1:scope',15000,async()=>{
      ...
      // Public golfer/match IDs and bounded golf navigation facts only; never
      // cache the upstream envelope or participant/access authority.
      const navigation=publicMatchNavigationScope(raw.matches||[]);
      return {players:(raw.players||[]).map(p=>p.player_id),matches:navigation.map(r=>r.id),navigation};
    });
    if(resource==='match') {
      ...
      const value=await cached('public-golf-v1:match:'+id,15000,async()=>{
        ...
        return publicMatchGolf({...raw,navigation:publicRoundMatchNavigation(raw,scope.navigation)});
      });
      return {contract:'bagger-public-golf-v1',resource,data:value};
    }
