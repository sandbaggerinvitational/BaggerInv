...
async function readProductionCutoverParticipantAuthTransport(env) {
  const { productionCutoverParticipantAuthTransport } = await import("./production-cutover-participant-auth-server.js");
  return productionCutoverParticipantAuthTransport(env);
}

async function readProductionCurrentIdentityRpcResolution(functionName, frozenFunctionName, body, env) {
  const { resolveProductionCurrentParticipantIdentityRpc } = await import("./production-current-participant-identity-server.js");
  return resolveProductionCurrentParticipantIdentityRpc({
    logicalFunctionName: functionName,
    frozenFunctionName,
    body,
  }, { env });
}
...
export async function participantIdentityRpc(functionName, body = {}, {
  env = process.env,
  timeoutMs = 12_000,
  resolveProductionIdentityRpc = readProductionCurrentIdentityRpcResolution,
} = {}) {
  ...
  const productionCutoverRpcNames = {
    authorize_single_participant_otp_request: "authorize_production_participant_otp_request",
    record_single_participant_otp_delivery: "record_production_participant_otp_delivery",
    authorize_single_participant_otp_verification: "authorize_production_participant_otp_verification",
    record_single_participant_otp_verification: "record_production_participant_otp_verification",
    recover_single_participant_otp_verification: "recover_production_participant_otp_verification",
    record_single_participant_auth_logout: "record_production_participant_logout",
    read_participant_identity_context_for_auth: "read_production_participant_context_for_auth",
    read_participant_identity_context: "read_production_participant_player_context",
    read_production_director_entitlement: "read_production_cutover_director_entitlement",
  };
  const frozenProductionFunctionName = productionCutoverRpcNames[functionName] || functionName;
  const productionResolution = authority.productionCutoverIdentity
    ? await resolveProductionIdentityRpc(functionName, frozenProductionFunctionName, body, env)
    : null;
  const selectedFunctionName = authority.productionCutoverIdentity
    ? productionResolution.functionName
    : authority.productionShadowCandidate
    ? productionRpcNames[functionName] || functionName
    : functionName;
  const selectedBody = productionResolution?.body || body;
  assertParticipantIdentityAdministrativeEnvironment(env, { operation: selectedFunctionName });
  const transport = authority.productionCutoverIdentity
    ? await readProductionCutoverParticipantAuthTransport(env)
    : authority.productionShadowCandidate
  ...
  return {
    ...
    ...(productionResolution ? {
      productionRuntime: Object.freeze({
        tournamentId: productionResolution.tournamentId,
        pointerRevision: productionResolution.pointerRevision,
        futureGeneration: productionResolution.futureGeneration === true,
      }),
    } : {}),
  };
}
...
export const inspectProductionParticipantIdentityEnrollment = (options) => participantIdentityRpc(
