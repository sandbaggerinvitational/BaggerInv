...
export async function verifyMobileSupabaseAccessToken(token, options = {}) {
  ...
  return { status: result.status, authUserId: result.authUserId };
}
...
export async function resolveMobileBearerIdentity({
  ...
  try {
    verifyCertification({
      ...
      productionContext: admission ? { runtime: admission.authority.runtime, revocationGeneration: admission.controls.revocationGeneration, participant: context } : null,
      env,
    });
  } catch (error) {
    if (error instanceof MobileApiError) throw error;
