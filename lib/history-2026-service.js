...
// The merge helper reads only the bounded tournament_source_payload handicap
// field and captain presentation metadata from that existing batch projection.
import { mergeHistoryTournamentPlayerMetadata } from "./history-team-metadata.js";

export { mergeHistoryTournamentPlayerMetadata } from "./history-team-metadata.js";
...
function cacheKey(aggregate, guideProjection) {
  // The bounded tournament-player read augments the base History aggregate
  // with year-scoped handicaps and captain identity. Those presentation facts
  // must participate in the cache key; otherwise an overview adaptation can
  // incorrectly satisfy a later Team History request with an unaugmented view.
 ...
 */
async function loadHistory2026ViewUncached(options = {}) {
  const startedAt = performance.now();
...
const loadCachedHistory2026View = cache((
  env,
  year,
  tournamentId,
  includeTournamentPlayerMetadata,
  timeoutMs,
) => loadHistory2026ViewUncached({
  env,
  year,
  tournamentId,
  includeTournamentPlayerMetadata,
...
export async function loadHistory2026View(options = {}) {
  if (options.dependencies) return loadHistory2026ViewUncached(options);
  return loadCachedHistory2026View(
    options.env || process.env,
    number(options.year ?? PREVIEW_HISTORY_2026_TOURNAMENT_YEAR),
    clean(options.tournamentId || PREVIEW_HISTORY_2026_TOURNAMENT_ID),
    options.includeTournamentPlayerMetadata === true,
    options.timeoutMs || 0,
  );
}
