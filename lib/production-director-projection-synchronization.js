...
function specFor(domain) {
  ...
  if (normalized === "PREDICTION_SETTINGS") {
    throw syncError(
      "PRODUCTION_PREDICTION_SETTINGS_GOOGLE_AUTHORING_RETIRED",
      "Production Prediction Settings are managed in the Director Console.",
      410,
    );
  }
  if (normalized === "DRAFT") {
    throw syncError(
      ...
      "Production Drafts are managed in the Director Console.",
      410,
    );
  }
  if (normalized === "GUIDE") {
    throw syncError(
      ...
      "Production Tournament Guide content is managed in the Director Console.",
      410,
    );
  }
...
function annualTarget(targetTournamentId, targetTournamentYear) {
  const tournamentId = clean(targetTournamentId || PRODUCTION_TOURNAMENT_ID);
  const tournamentYear = Number(targetTournamentYear || tournamentId);
  const current = tournamentId === PRODUCTION_TOURNAMENT_ID &&
    tournamentYear === PRODUCTION_TOURNAMENT_YEAR;
  if (!/^\d{4}$/.test(tournamentId) || !Number.isSafeInteger(tournamentYear) ||
      tournamentId !== String(tournamentYear) || tournamentYear < PRODUCTION_TOURNAMENT_YEAR ||
      tournamentYear > 2200) {
    throw syncError("PRODUCTION_DIRECTOR_SYNC_TOURNAMENT_INVALID", "Select a valid Production tournament year.", 400);
  }
  return Object.freeze({ tournamentId, tournamentYear, current, future: !current });
}
...
function exactScope({ activation, domain, actorAuthUserId, actorPlayerId, target, extra = {} }) {
  const selectedTarget = target || annualTarget();
  return {
    ...extra,
    ...
    target_tournament_id: selectedTarget.tournamentId,
    target_tournament_year: selectedTarget.tournamentYear,
    ...
    authorization: {
      auth_user_id: clean(actorAuthUserId).toLowerCase(),
...
function normalizePredictionSettingsSheet(sheet = {}, target = annualTarget()) {
  const headers = Array.isArray(sheet.headers) ? sheet.headers.map(clean) : [];
  let normalized;
  if (headers[0] === "Setting" && headers[1] === "Value") normalized = records(sheet);
  else {
  ...
  const first = Object.fromEntries(canonicalHeaders.map((header, index) => [header,
    ...
    normalized = [first, ...(sheet.records || []).map((entry) => {
  ...
  const scoped = normalized.filter((row) => {
    const rowId = clean(row?.["Tournament ID"] || row?.Tournament);
    const rowYear = clean(row?.["Tournament Year"] || row?.Year);
    if (!rowId && !rowYear) return target.current;
    return (!rowId || rowId === target.tournamentId) &&
      (!rowYear || Number(rowYear) === target.tournamentYear);
  });
  if (target.future && (!scoped.length || scoped.some((row) =>
    !clean(row?.["Tournament ID"] || row?.Tournament) &&
    !clean(row?.["Tournament Year"] || row?.Year)))) {
    throw syncError(
      "PRODUCTION_FUTURE_PREDICTION_SETTINGS_YEAR_SCOPE_REQUIRED",
      "Future Prediction Settings require explicit tournament or year scope in Google.",
      422,
    );
  }
  return scoped;
...
export function buildProductionDirectorProjectionEnvelope({
  ...
  targetTournamentYear = PRODUCTION_TOURNAMENT_YEAR,
} = {}) {
  const selected = specFor(domain);
  const target = annualTarget(targetTournamentId, targetTournamentYear);
  if (selected.domain === "GUIDE") {
    ...
    const projection = buildGuideProjection({
      sheets: guideSheets,
      tournament: { id: target.tournamentId, year: target.tournamentYear },
      approvedTournamentId: target.tournamentId,
  ...
  if (selected.domain === "PREDICTION_SETTINGS") {
    const projection = buildPredictionSettingsProjection({
      ...
      tournamentYear: target.tournamentYear,
      sourceWorkbookId: PRODUCTION_GOOGLE_WORKBOOK_ID,
      rows: normalizePredictionSettingsSheet(sheets[PREDICTION_SETTINGS_SOURCE_TAB], target),
  ...
  const drafts = projection.drafts.map((draft) => Number(draft.tournament_year) < PRODUCTION_TOURNAMENT_YEAR && reason
    ...
    : draft).filter((draft) => !target.future ||
      Number(draft.tournament_year) === target.tournamentYear);
  if (target.future && drafts.length !== 1) {
    throw syncError(
      "PRODUCTION_FUTURE_DRAFT_YEAR_SCOPE_REQUIRED",
      "Google Draft Settings must contain exactly one configuration for the selected future tournament.",
      422,
    );
  }
...
export async function inspectProductionDirectorProjectionSynchronization({
  ...
  const contextInput = exactScope({ activation, domain: selected.domain, actorAuthUserId, actorPlayerId, target });
  const context = await rpc(target.future
    ? "read_production_future_annual_projection_v1"
    : "read_production_director_sync_context", contextInput, { env, ...dependencies });
  ...
  const source = buildProductionDirectorProjectionEnvelope({
    ...
    actorPlayerId,
    correctionReason,
    targetTournamentId: target.tournamentId,
    targetTournamentYear: target.tournamentYear,
  });
  return {
    ok: true,
    domain: selected.domain,
...
export async function synchronizeProductionDirectorProjection(options = {}) {
  ...
  const storedProjection = inspected.context?.current_projection;
  const storedSourceRevision = Math.max(0, Number(
    storedProjection?.source_revision ?? storedProjection?.sourceRevision ?? 0,
  ) || 0);
  const exactSourceReplay = Boolean(storedProjection &&
    clean(storedProjection.source_fingerprint ?? storedProjection.sourceFingerprint) ===
      inspected.source.source_fingerprint &&
    clean(storedProjection.payload_fingerprint ?? storedProjection.payloadFingerprint) ===
      inspected.source.payload_fingerprint);
  const sourceRevision = inspected.target.future
    ? (exactSourceReplay ? storedSourceRevision : storedSourceRevision + 1)
    : undefined;
  const input = exactScope({
    ...
    extra: {
      ...inspected.source,
      ...(inspected.target.future ? { source_revision: sourceRevision } : {}),
      expected_activation_revision: inspected.activationRevision,
      expected_setup_revision: Number(inspected.context?.setup_revision || 0),
      expected_runtime_revision: Number(inspected.context?.runtime_revision || 0),
    },
  });
  const imported = await rpc(inspected.target.future
    ? "synchronize_production_future_annual_projection_v1"
    : "synchronize_production_director_projection", input, {
  ...
  const readback = await rpc(inspected.target.future
    ? "read_production_future_annual_projection_v1"
    : selected.spec.readRpc, readbackInput, {
    env: options.env || process.env,
    ...(options.dependencies || {}),
  });
