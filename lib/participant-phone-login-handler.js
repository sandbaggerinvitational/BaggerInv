...
async function POST(request, parsedInput) {
  ...
  try {
    ...
    if (native) {
      ...
      // Signed-out Text verification has no bearer yet. Use only the session
      // just verified by Supabase, never a client-supplied session substitute.
      const certificationHeaders = new Headers(request.headers);
      certificationHeaders.set('authorization', `Bearer ${session.access_token}`);
      const certificationRequest = new Request(request.url, {method:'POST',headers:certificationHeaders});
      const certification = await certifyParticipant({channel:"phone",request:certificationRequest, authUserId:verified.userId, playerId:proof.playerId, tournamentId:proof.tournamentId, env:process.env, dependencies:{...certificationDependencies,nativeAdmission}});
