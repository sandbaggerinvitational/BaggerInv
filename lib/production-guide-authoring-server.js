...
async function freshStoredDraftProjection(values, options, { requireValidated = false, privatePreview = false } = {}) {
  ...
  const normalized = normalizeProductionGuideAuthoring({
    ...
    validationLevel: privatePreview && draft.status !== "VALIDATED" ? "DRAFT" : "PUBLICATION",
  });
...
export async function previewProductionGuideDraft(values = {}, options = {}) {
  ...
  const fresh = await freshStoredDraftProjection(values, options, { privatePreview: true });
  const requestPayloadHash = productionGuideAuthoringPayloadHash({
    operation: "PREVIEW",
    ...operation,
    authoringContentFingerprint: fresh.normalized.authoringContentFingerprint,
