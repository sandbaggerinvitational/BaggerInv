...
export const PRODUCTION_GUIDE_AUTHORING_DOMAINS = Object.freeze([
  ...
  domain({
    ...
    required: ["Section ID", "Section Name", "Section Slug", "Description", "Display Order"],
...
function normalizeRow(sourceValue, spec, index, targetTournamentId, targetYear, draft = false) {
  ...
  const row = Object.fromEntries(spec.fields.map(({ key }) => {
    ...
    if (ORDER_FIELDS.has(key) && (value || draft || ["dining", "localGuide"].includes(spec.key))) {
  ...
  for (const requirement of spec.required) {
    // IDs/references and ordering remain structural. Missing editorial content
    // belongs to publication validation, not private working-draft persistence.
    if (draft && !["Section ID", "Section Slug", "Event ID", "Rule ID", "Format ID", "Round", "Format", "Course ID", "Display Order", "Sort Order"].includes(requirement)) continue;
...
function normalizeTournament(sourceValue, targetTournamentId, targetYear, draft = false) {
  ...
  row.Year = String(targetYear);
  for (const requirement of spec.required) {
    if (draft && Array.isArray(requirement) && requirement.includes("Tournament Name")) continue;
    if (!requiredValue(row, requirement)) {
      const fields = Array.isArray(requirement) ? requirement : [requirement];
      throw guideError(
        "GUIDE_REQUIRED_FIELD_MISSING",
  ...
  if (!draft) {
    const issues = guideOverviewPublicationIssues(row);
    if (issues.length) throw guideError("GUIDE_REQUIRED_FIELD_MISSING", "Tournament Guide overview needs attention.",
      issues.map(({ field, message }) => issue(spec.source, field, message, targetTournamentId)));
  }
...
export function normalizeProductionGuideAuthoring({
  ...
  canonicalRounds = [],
  validationLevel = "PUBLICATION",
} = {}) {
  if (!["DRAFT", "PUBLICATION"].includes(validationLevel)) throw guideError("GUIDE_VALIDATION_LEVEL_INVALID", "Unsupported Guide validation level.");
  const draft = validationLevel === "DRAFT";
  ...
  const authoringContent = {
    tournament: normalizeTournament(source.tournament, tournamentId, tournamentYear, draft),
  };
  for (const spec of PRODUCTION_GUIDE_AUTHORING_DOMAINS.filter((value) => value.cardinality === "many")) {
    ...
    authoringContent[spec.key] = Object.freeze(source[spec.key].map((value, index) =>
      normalizeRow(value, spec, index, tournamentId, tournamentYear, draft)));
    duplicateIdentity(authoringContent[spec.key], spec);
    const orderKey = spec.fields.find(({ key }) => ORDER_FIELDS.has(key))?.key;
    if (orderKey) {
      const orders = authoringContent[spec.key].map((row) => row[orderKey]);
      if (new Set(orders).size !== orders.length) throw guideError("GUIDE_ORDER_DUPLICATE", `${spec.label} contains duplicate order values.`, [issue(spec.source, orderKey, "Use a distinct order for each item.")]);
    }
  }
  ...
  if (draft) {
    // This projection is Director-private. It deliberately includes unfinished
    // and item-Draft rows, and can never be used as a validated publication.
    const privateContent = Object.fromEntries(Object.entries(authoringContent).map(([key, value]) => [key,
      Array.isArray(value) ? value.map(({ itemId, ...row }) => row) : value]));
    privateContent.tournamentIdentity = { id: tournamentId, year: tournamentYear };
    const projectionPayload = { schemaVersion: GUIDE_PROJECTION_SCHEMA_VERSION, content: privateContent };
    const authoringCanonicalJson = canonicalGuideJson(authoringContent);
    if (byteLength(authoringCanonicalJson) > MAX_AUTHORING_BYTES) throw guideError("GUIDE_CONTENT_TOO_LARGE", "Tournament Guide content exceeds the bounded authoring limit.");
    const contentCanonicalJson = canonicalGuideJson(privateContent);
    const projectionPayloadCanonicalJson = canonicalGuideJson(projectionPayload);
    return {
      contractVersion: PRODUCTION_GUIDE_AUTHORING_CONTRACT, tournamentId, tournamentYear,
      authoringContent, authoringCanonicalJson, authoringContentFingerprint: guideProjectionHash(authoringCanonicalJson),
      projectionPayload, projectionPayloadCanonicalJson, projectionPayloadHash: guideProjectionHash(projectionPayloadCanonicalJson),
      contentCanonicalJson, contentFingerprint: guideProjectionHash(contentCanonicalJson),
      validation: { valid: true, validated: false, level: "DRAFT" },
    };
  }
  ...
  try {
    projection = buildGuideProjection({
      ...
      participantContentPolicy: GUIDE_PARTICIPANT_CONTENT_POLICIES.REQUIRE_ITINERARY,
    });
  } catch (error) {
    if (!(error instanceof GuideProjectionValidationError)) throw error;
    throw guideError(
      "GUIDE_VALIDATION_FAILED",
      "Tournament Guide content needs review before publication.",
