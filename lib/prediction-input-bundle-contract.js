...
function currentMatchRows(currentState = {}, live = {}) {
  ...
  return list(currentState.matches).map((entry, sourceIndex) => {
    ...
    const participants = list(entry.participants).map((participant, participantIndex) => ({
      ...
      teamId: clean(participant.team_id),
...
function evidenceContract(scorecards = [], holes = []) {
...
export function assemblePredictionInputBundle({
  ...
  const bundle = {
    ...
    handicaps,
    predictionSettings,
    evidence: evidenceContract(scorecards, holes),
    ordering,
    provenance,
  };
...
export function buildPredictionInputBundle({
  ...
  const matches = currentMatchRows(currentState, live);
...
export function championshipOddsInputFromPredictionBundle(bundle = {}) {
  ...
  const matches = list(bundle.matches).map((match) => {
    const row = {
      ...
      Format: match.format,
      "Match ID": match.id,
      "Team 1 Points": match.points?.current?.teamOne,
      "Team 2 Points": match.points?.current?.teamTwo,
    };
...
export function validatePredictionInputBundle(bundle = {}, { allowUnknownSettingsFreshness = true } = {}) {
  ...
  for (const player of list(bundle.players)) {
    ...
    if (player.active && player.tournamentHandicap === null) issue(issues, "CURRENT_HANDICAP_UNAVAILABLE", `players.${player.id}.tournamentHandicap`);
  }
