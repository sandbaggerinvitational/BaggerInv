...
function handicapRows(secondaryHistory = {}, currentPlayers = [], currentMatches = [], completedMatches = []) {
  ...
  return {
    ...
    courseHandicap: { kind: "DERIVED", contract: "prediction-engine-course-handicap-v2-postgres-parity" },
  };
}
...
function completePairingKey(match = {}) {
  const participants = list(match.participants);
  const expectedTeamSize = integer(match.teamSize, formatCode(match.format) === "SI" ? 1 : 2);
  if (expectedTeamSize < 1 || participants.length !== expectedTeamSize * 2) return "";
  const teamOne = participants.filter((row) => integer(row.teamSide, 0) === 1);
  const teamTwo = participants.filter((row) => integer(row.teamSide, 0) === 2);
  const playerIds = participants.map((row) => clean(row.playerId));
  if (teamOne.length !== expectedTeamSize || teamTwo.length !== expectedTeamSize || playerIds.some((id) => !id)) return "";
  return `${match.round}:${playerIds.sort().join("|")}`;
}
...
export function validatePredictionInputBundle(bundle = {}, { allowUnknownSettingsFreshness = true } = {}) {
  ...
  for (const match of list(bundle.matches)) {
    ...
    const key = completePairingKey(match);
    if (key) {
