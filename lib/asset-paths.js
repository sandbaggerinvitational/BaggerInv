import { playerPortraitAssets } from "./player-portrait-assets.js";

const portraitKeys = new Set(playerPortraitAssets);
const BASE = "/images";
...
export function optimizedAssetUrl(source, width = 64, quality = 75) {
  ...
  const safeWidth = [32, 48, 64, 96, 128, 256, 384, 640, 750, 828, 1080, 1200].includes(Number(width)) ? Number(width) : 64;
...
export function playerPhoto(filename) {
  // Historical player rows preserve punctuation in surnames while the
  // canonical static asset keys are URL-safe. Keep the data identity intact
  // and normalize only the asset lookup boundary.
  const value = String(filename ?? "").trim();
  // Explicit remote presentation assets remain remote, never fabricated paths.
  if (/^https:\/\//i.test(value)) {
    try { const url = new URL(value); return url.username || url.password ? null : url.href; }
    catch { return null; }
  }
  const key = cleanFilename(value.replace(/^\/images\/players\//, "").replace(/[\u2018\u2019']/g, ""));
  return portraitKeys.has(key) ? asset("players", key) : null;
}
