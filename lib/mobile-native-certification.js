...
export function verifyMobileNativeCertification({
  ...
  if (!Number.isSafeInteger(current) || issuedAt > current + MAX_CLOCK_SKEW_SECONDS ||
      (expiresAt <= current && !(allowExpiredForRenewal && (extended || (production && certificationVersion === "v2p")))) || expiresAt - issuedAt !== (extended ? MOBILE_NATIVE_EXTENDED_CERTIFICATION_SECONDS : MOBILE_NATIVE_CERTIFICATION_SECONDS)) {
    throw new MobileApiError("AUTH_CERTIFICATION_FAILED");
  }
