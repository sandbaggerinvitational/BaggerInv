...
import { normalizeTournamentTimeline, publishedTournamentTimeZone } from "./tournament-timeline.js";

const clean = (value) => String(value ?? "").trim();

export const GUIDE_PROJECTION_SCHEMA_VERSION = "guide-projection-v1";
export const PREVIEW_GUIDE_TOURNAMENT_ID = "2026";
export const GUIDE_PARTICIPANT_CONTENT_POLICIES = Object.freeze({
  REQUIRE_PUBLISHED: "REQUIRE_PUBLISHED",
  REQUIRE_ITINERARY: "REQUIRE_ITINERARY",
  ALLOW_VALID_EMPTY_PRE_TOURNAMENT: "ALLOW_VALID_EMPTY_PRE_TOURNAMENT",
});
...
function sourceSheetDescriptor(sheets, name, issues) {
  if (!Object.hasOwn(sheets || {}, name)) {
    issues.push(`${name} source is missing`);
    return { rows: [], headers: [] };
  }
  const source = sheets[name];
  if (Array.isArray(source)) {
    return { rows: source, headers: Object.keys(source[0] || {}) };
  }
  if (source && typeof source === "object" && Array.isArray(source.records)) {
    const rows = source.records.map((item) => item?.record || item).filter(Boolean);
    return {
      rows,
      headers: Array.isArray(source.headers) ? source.headers.map(clean) : Object.keys(rows[0] || {}),
    };
  }
  issues.push(`${name} source is invalid`);
  return { rows: [], headers: [] };
}

function headerIssues(name, rows, declaredHeaders = []) {
  const orderedHeaders = (declaredHeaders.length ? declaredHeaders : Object.keys(rows?.[0] || {})).map(clean);
  const headers = new Set(orderedHeaders);
  const duplicates = orderedHeaders.filter((header, index) => header && orderedHeaders.indexOf(header) !== index);
  return [
    ...[...new Set(duplicates)].map((header) => `${name} headers contain duplicate ${header}`),
    ...(HEADER_REQUIREMENTS[name] || []).flatMap((requirement) => {
...
export function buildGuideProjection({
  ...
  participantContentPolicy = GUIDE_PARTICIPANT_CONTENT_POLICIES.REQUIRE_PUBLISHED,
} = {}) {
  const issues = [];
  ...
  const validParticipantContentPolicies = new Set(Object.values(GUIDE_PARTICIPANT_CONTENT_POLICIES));
  if (!validParticipantContentPolicies.has(participantContentPolicy)) {
    issues.push(`Guide participant content policy ${clean(participantContentPolicy) || "(missing)"} is invalid`);
  }
  const descriptors = Object.fromEntries(GUIDE_PROJECTION_SHEETS.map((name) => [name, sourceSheetDescriptor(sheets, name, issues)]));
  const source = Object.fromEntries(Object.entries(descriptors).map(([name, descriptor]) => [name, descriptor.rows]));
  for (const [name, descriptor] of Object.entries(descriptors)) {
    issues.push(...headerIssues(name, descriptor.rows, descriptor.headers));
  }
  ...
  const participantContentCollections = {
    "Guide Sections": overview,
    "Tournament Itinerary": schedule,
    "Tournament Timeline": timelineRows,
    "Rule Book": ruleBook,
    Dining: dining,
    "Local Guide": localGuide,
    "Important Contacts": importantContacts,
  };
  const participantContentEmpty = Object.values(participantContentCollections).every((rows) => rows.length === 0);
  const validEmptyParticipantContent = participantContentEmpty
    && participantContentPolicy === GUIDE_PARTICIPANT_CONTENT_POLICIES.ALLOW_VALID_EMPTY_PRE_TOURNAMENT;
  if (participantContentPolicy === GUIDE_PARTICIPANT_CONTENT_POLICIES.REQUIRE_ITINERARY) {
    if (!schedule.length) issues.push("Tournament Itinerary needs at least one published participant event");
  } else if (!validEmptyParticipantContent) {
  ...
  return {
    ...
    validation: {
      valid: true,
      issues: [],
      participantContentState: participantContentEmpty ? "VALID_EMPTY" : "PUBLISHED",
      emptyParticipantContentAccepted: validEmptyParticipantContent,
      participantContentPolicy,
      activeAuthoringCounts: Object.fromEntries([
        "Guide Sections", "Tournament Itinerary", "Tournament Timeline", "Rule Book",
        "Dining", "Local Guide", "Important Contacts",
      ].map((name) => [name, activeRecords(source[name], canonicalTournament).length])),
      participantContentCounts: Object.fromEntries(Object.entries(participantContentCollections).map(([name, rows]) => [name, rows.length])),
      structuralHeaders: Object.fromEntries(Object.entries(descriptors).map(([name, descriptor]) => [name, [...descriptor.headers]])),
    },
...
export function timelineFromGuideProjection(projection, options = {}) {
  ...
  return normalizeTournamentTimeline({
    ...
    timeZone: publishedTournamentTimeZone(content, options.timeZone || tournament.timeZone),
