...
const FIELDS = Object.freeze({
  tournaments: [
    ...
    "Tournament Logo", "Tournament Logo Filename", "Logo Filename",
  ],
  overview: [
...
const ASSET_FIELDS = new Set(["Course Logo", "Course Profile Image", "Tournament Logo", "Tournament Logo Filename", "Logo Filename"]);
...
const HEADER_REQUIREMENTS = Object.freeze({
  ...
  "Tournament Timeline": ["Year", "Tournament Day", "Event Date", "Start Time", "End Time", "Event Type", "Title", "Subtitle", "Location", "Display on Home", "Notification Minutes", "Sort Order", "Status Override"],
  ...
  "Local Guide": ["Year", "Section", "Title", "Description", "Address", "Phone", "Website", "Sort Order"],
...
function tournamentIdentity(row, canonicalTournament) {
  ...
  return {
    ...
    logoFileName: optionalAsset(
      row["Tournament Logo"] || row["Tournament Logo Filename"] || row["Logo Filename"] || canonicalTournament.logoFileName
    ),
  };
}
...
export function buildGuideProjection({
  ...
  const schedule = sortBy(
    publishedRecords(source["Tournament Itinerary"], canonicalTournament).map((row) => {
  ...
  const timelineRows = sortBy(
    activeRecords(source["Tournament Timeline"], canonicalTournament).map((row) => {
  ...
  const dining = sortBy(
    activeRecords(source.Dining, canonicalTournament).map((row) => {
  ...
  const localGuide = sortBy(
    activeRecords(source["Local Guide"], canonicalTournament).map((row) => projectRow(row, FIELDS.localGuide)),
    "Sort Order",
  ...
  for (const [name, rows, fields] of [
    ...
    ["Dining", dining, ["Year", "Day", "Meal", "Start Time", "Location", "Sort Order"]],
  ...
  for (const event of [...schedule, ...timelineRows]) {
    if (!event["Event Date"] || numericYear(event["Event Date"]) !== canonicalTournament.year) {
      issues.push(`${event["Event ID"] || event.Title || "event"} has an invalid approved-tournament date`);
    }
    if (!validTime(event["Start Time"])) issues.push(`${event["Event ID"] || event.Title || "event"} has an invalid start time`);
    if (event["End Time"] && !validTime(event["End Time"])) issues.push(`${event["Event ID"] || event.Title || "event"} has an invalid end time`);
  }
  ...
  for (const sourceCourse of courseRows) {
    ...
    if (!canonical) {
      issues.push(`Courses contains unknown canonical assignment ${key}`);
      continue;
    }
    const sourceFormat = formatCode(sourceCourse.Format);
    if (sourceFormat && sourceFormat !== canonical.Format) issues.push(`Courses ${key} format does not match canonical configuration`);
    const sourceTee = clean(sourceCourse["Tee Played"]);
    if (sourceTee && sourceTee.toUpperCase() !== clean(canonical["Tee Played"]).toUpperCase()) {
      issues.push(`Courses ${key} tee does not match canonical scoring configuration`);
    }
    for (const field of ["Slope", "Rating", "Par", "Yardage"]) {
      ...
      if (clean(sourceCourse[field]) && Number.isFinite(sourceValue) && Number.isFinite(canonicalValue) && sourceValue !== canonicalValue) {
        issues.push(`Courses ${key} ${field} does not match canonical scoring configuration`);
      }
    }
  }
  for (const assignment of canonicalAssignments) {
    ...
    if (count !== 1) issues.push(`canonical course assignment ${key} requires exactly one presentation row; found ${count}`);
  }
  for (const event of schedule) {
    if (event["Course ID"] && !knownCourseIds.has(event["Course ID"].toUpperCase())) {
      issues.push(`Tournament Itinerary references unknown canonical Course ID ${event["Course ID"]}`);
    }
  }
  for (const context of canonicalCourseContext) {
  ...
  if (issues.length) {
    const error = new GuideProjectionValidationError([...new Set(issues)]);
