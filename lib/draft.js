...
function buildDraft(settings, allPicks, history = {
  getPlayerMap,
  getTournament,
  getTournamentHandicap,
  getTournaments,
  ...
  const tournament = history.getTournament(year);
...
export async function getDrafts(options = {}) {
  const history = options.history || { getPlayerMap, getTournament, getTournamentHandicap, getTournaments };
  const sheets = await loadDraftSheets();
  return (sheets.settings || [])
    .map((settings) => buildDraft(settings, sheets.picks || [], history))
    .filter(Boolean)
    .sort((a, b) => b.year - a.year);
}

export async function getDraftByYear(year, options = {}) {
  return (await getDrafts(options)).find((draft) => draft.year === Number(year)) || null;
}

export async function getCurrentDraft(options = {}) {
  const history = options.history || { getTournaments };
  const currentYear = history.getTournaments()[0]?.year;
  return currentYear ? getDraftByYear(currentYear, options) : null;
}
