...
function rejectedWafDispatchEvidence(row) {
  const rejectedDispatchId = clean(row?.rejected_dispatch_id).toLowerCase();
  const rejectedDispatchResultId = clean(
    row?.rejected_dispatch_result_id,
  ).toLowerCase();
  const providerResponseStatus = Number(row?.provider_response_status);
  if (!uuid(rejectedDispatchId) || !uuid(rejectedDispatchResultId) ||
...
export function productionGoogleWriterProviderFenceControlDependencies(
  ...
  const recoverRejectedCriticalWafEpoch = async ({
    ...
    const row = await receiptRpc(
      "inspect_production_vercel_writer_critical_waf_epoch",
      {
        ...exactScope(),
        operation: "RECOVER_PRODUCTION_VERCEL_WRITER_REJECTED_WAF_EPOCH",
        vercel_project_id: PRODUCTION_VERCEL_PROJECT_ID,
        expected_status: expectedId && expectedRetirementRequest
          ? "ACTIVATION_PENDING_OR_REJECTED_RETIRED"
          : "ACTIVATION_PENDING",
        expected_purpose: "REHEARSAL",
        expected_transition_mode: "REHEARSAL",
        expected_epoch_id: expectedId || null,
        retirement_request_id: expectedRetirementRequest || null,
      },
      options,
    );
    ...
    const rejectedDispatch = rejectedWafDispatchEvidence(row);
    const epoch = await receiptRpc(
      "inspect_production_vercel_writer_critical_waf_epoch",
      {
        ...exactScope(),
