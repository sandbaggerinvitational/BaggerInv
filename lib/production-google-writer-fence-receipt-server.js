...
const RECOVERY_TABLE_SELECTS = Object.freeze({
  vercel_writer_critical_waf_epochs: [
    "epoch_id", "status", "purpose", "transition_mode", "vercel_project_id",
    ...
    "run_owned_rule_name", "run_owned_rule_nonce", "run_owned_rule_fingerprint",
    "run_owned_insert_document_fingerprint", "retirement_request_id",
    "retirement_candidate_deployment_id",
    "retirement_candidate_deployment_commit", "retirement_reason", "retired_at",
  ].join(","),
  vercel_writer_critical_waf_dispatches: [
    "dispatch_id", "epoch_id", "dispatch_step", "status",
    "provider_dispatch_started_at", "provider_dispatch_result_id",
    "provider_result_observation_id",
  ].join(","),
  vercel_writer_critical_waf_dispatch_results: [
    "result_id", "dispatch_id", "epoch_id", "dispatch_step", "outcome_status",
    "provider_response_observed", "provider_response_status",
    "provider_readback_fingerprint", "provider_assigned_rule_id",
  ].join(","),
});

async function recoveryTableRows(table, parameters, {
  env = process.env,
  fetchImpl = globalThis.fetch,
} = {}) {
  const select = RECOVERY_TABLE_SELECTS[table];
  if (!select || !(parameters instanceof URLSearchParams)) {
    throw receiptError(
      "STEP11_6_VERCEL_WAF_RECOVERY_QUERY_INVALID",
      "The durable WAF recovery query was invalid.", {}, 500,
    );
  }
  const secret = clean(env.PRODUCTION_SUPABASE_SECRET_KEY);
  if (secret.length < 20) {
    throw receiptError(
      "STEP11_6_WRITER_FENCE_RECEIPT_CREDENTIAL_REQUIRED",
      "The Production control-plane credential is unavailable.",
    );
  }
  const url = new URL(`${PRODUCTION_SUPABASE_URL}/rest/v1/${table}`);
  ...
  try {
    response = await fetchImpl(url, {
      ...
      headers: {
        ...
        "accept-profile": "production_control",
      },
      cache: "no-store",
      signal: AbortSignal.timeout(15_000),
    });
    ...
    throw receiptError(
      "STEP11_6_VERCEL_WAF_RECOVERY_RESPONSE_UNKNOWN",
      "The authoritative active-WAF recovery response was not received.",
    );
  }
  const payload = await response.json().catch(() => null);
  if (!response.ok || !Array.isArray(payload)) {
    throw receiptError(
      "STEP11_6_VERCEL_WAF_RECOVERY_READ_UNAVAILABLE",
      "The authoritative active-WAF recovery read was unavailable.",
      { providerStatus: response.status },
      [400, 401, 403, 406].includes(response.status) ? response.status : 503,
    );
  }
  return payload;
}

async function recoverableWafEpochRows(status, {
  env = process.env,
  fetchImpl = globalThis.fetch,
} = {}, epochId = "") {
  const selectedStatus = clean(status).toUpperCase();
  if (!new Set(["ACTIVATION_PENDING", "REJECTED_RETIRED"]).has(
    selectedStatus,
  ) || (epochId && !uuid(epochId))) {
    throw receiptError(
      "STEP11_6_VERCEL_WAF_RECOVERY_QUERY_INVALID",
      "The durable WAF recovery query was invalid.", {}, 500,
    );
  ...
  const parameters = new URLSearchParams({
    status: `eq.${selectedStatus}`,
    purpose: "eq.REHEARSAL",
    transition_mode: "eq.REHEARSAL",
    vercel_project_id: `eq.${PRODUCTION_VERCEL_PROJECT_ID}`,
    order: "created_at.asc",
  });
  if (epochId) parameters.set("epoch_id", `eq.${clean(epochId).toLowerCase()}`);
  return recoveryTableRows(
    "vercel_writer_critical_waf_epochs", parameters, { env, fetchImpl },
  );
}

async function assertRejectedWafDispatch(epochId, options) {
  const dispatches = await recoveryTableRows(
    "vercel_writer_critical_waf_dispatches",
    new URLSearchParams({
      epoch_id: `eq.${epochId}`,
      order: "created_at.asc",
  ...
  const resultId = clean(dispatch?.provider_dispatch_result_id).toLowerCase();
  if (dispatches.length !== 1 || !uuid(dispatch?.dispatch_id) ||
      clean(dispatch?.epoch_id).toLowerCase() !== epochId ||
      clean(dispatch?.dispatch_step).toUpperCase() !== "CRITICAL_RULE_INSERT" ||
      clean(dispatch?.status).toUpperCase() !== "PROVIDER_REJECTED" ||
      !clean(dispatch?.provider_dispatch_started_at) || !uuid(resultId) ||
      clean(dispatch?.provider_result_observation_id)) {
    throw receiptError(
      "STEP11_6_VERCEL_WAF_RECOVERY_DISPATCH_INVALID",
      "The active WAF epoch was not a provider-rejected recoverable epoch.",
      {}, 409,
    );
  }
  const results = await recoveryTableRows(
    "vercel_writer_critical_waf_dispatch_results",
    new URLSearchParams({
      result_id: `eq.${resultId}`,
      epoch_id: `eq.${epochId}`,
    }),
    options,
  );
  const result = results[0];
  const status = Number(result?.provider_response_status);
  if (results.length !== 1 || clean(result?.result_id).toLowerCase() !== resultId ||
      clean(result?.dispatch_id).toLowerCase() !==
        clean(dispatch.dispatch_id).toLowerCase() ||
      clean(result?.epoch_id).toLowerCase() !== epochId ||
      clean(result?.dispatch_step).toUpperCase() !== "CRITICAL_RULE_INSERT" ||
      clean(result?.outcome_status).toUpperCase() !== "PROVIDER_REJECTED" ||
      result?.provider_response_observed !== true ||
      !Number.isInteger(status) || status < 400 || status > 599 ||
      clean(result?.provider_readback_fingerprint) ||
      clean(result?.provider_assigned_rule_id)) {
    throw receiptError(
      "STEP11_6_VERCEL_WAF_RECOVERY_RESULT_INVALID",
      "The active WAF epoch lacked exact synchronous provider-rejection evidence.",
      {}, 409,
    );
  }
  return Object.freeze({
    rejectedDispatchId: clean(dispatch.dispatch_id).toLowerCase(),
    rejectedDispatchResultId: resultId,
...
export function productionGoogleWriterQuiesceReceiptDependencies(
  ...
  async function criticalWafQuiesceBinding(input, environment) {
    const candidate = challengeCandidate(environment);
    const epochId = exactUuid(
      input.criticalWafEpochId,
      "STEP11_6_WRITER_QUIESCE_CRITICAL_WAF_EPOCH_INVALID",
    );
    const observationId = exactUuid(
      input.criticalWafObservationId,
      "STEP11_6_WRITER_QUIESCE_CRITICAL_WAF_OBSERVATION_INVALID",
    );
    const stage = clean(input.criticalWafQuiesceStage).toUpperCase();
    if (!new Set(["INSTALL", "RESTORE_REATTEST"]).has(stage)) {
      throw receiptError(
        "STEP11_6_WRITER_QUIESCE_CRITICAL_WAF_STAGE_INVALID",
        "The critical-WAF quiesce stage was invalid.", {}, 400,
      );
    }
    const epoch = await receiptRpc(
      "inspect_production_vercel_writer_critical_waf_epoch",
      {
        ...exactScope(),
    ...
    const status = clean(epoch.status).toUpperCase();
    const actualObservationId = clean(stage === "INSTALL"
      ? value(epoch, "critical_active_observation_id", "criticalActiveObservationId")
      : value(epoch, "latest_critical_reattest_observation_id",
        "latestCriticalReattestObservationId")).toLowerCase();
    if (value(epoch, "epoch_id", "epochId").toLowerCase() !== epochId ||
        clean(epoch.purpose).toUpperCase() !==
          clean(input.quiescePurpose).toUpperCase() ||
        (epoch.critical_window_active !== true &&
          epoch.criticalWindowActive !== true) ||
        (stage === "INSTALL" && status !== "ACTIVE_UNBOUND") ||
        (stage === "RESTORE_REATTEST" && status !== "FENCE_BOUND") ||
        actualObservationId !== observationId) {
      throw receiptError(
        "STEP11_6_WRITER_QUIESCE_CRITICAL_WAF_BINDING_INVALID",
        "The quiesce request did not bind the exact current critical-WAF observation.",
        {},
        409,
      );
    }
    return Object.freeze({
      ...
      critical_waf_observation_id: observationId,
      critical_waf_quiesce_stage: stage,
    });
  }
  ...
  return Object.freeze({
    ...
    begin: async ({ input, environment, normalized, probes, providerReservation }) => {
      const candidate = quiesceCandidate(environment, env);
      const criticalWafBinding = await criticalWafQuiesceBinding(
        input,
        environment,
      );
      const evidenceRequestId = exactUuid(
...
export function productionGoogleWriterProviderFenceControlDependencies(
  ...
  const recoverRejectedCriticalWafEpoch = async ({
    environment,
    expectedEpochId = "",
    retirementRequestId = "",
  }) => {
    const candidate = candidateWafIdentity(environment, env);
    const expectedId = clean(expectedEpochId).toLowerCase();
    const expectedRetirementRequest = clean(retirementRequestId).toLowerCase();
    if ((expectedId && !uuid(expectedId)) ||
        (expectedRetirementRequest && !uuid(expectedRetirementRequest))) {
      throw receiptError(
        "STEP11_6_VERCEL_WAF_RECOVERY_IDENTITY_INVALID",
        "The rejected WAF recovery identity was invalid.", {}, 400,
      );
    }
    expectedControl(await inspectAuthority(options));
    const activeRows = await recoverableWafEpochRows(
      "ACTIVATION_PENDING", options,
    );
    if (activeRows.length > 1) {
      throw receiptError(
        "STEP11_6_VERCEL_WAF_RECOVERY_CONFLICT",
        "More than one active recoverable WAF epoch was found.", {}, 409,
      );
    }
    let row = activeRows[0] || null;
    if (row && expectedId && clean(row.epoch_id).toLowerCase() !== expectedId) {
      throw receiptError(
        "STEP11_6_VERCEL_WAF_RECOVERY_EPOCH_MISMATCH",
        "The caller identity did not match the authoritative active WAF epoch.",
        {}, 409,
      );
    }
    if (!row && expectedId && expectedRetirementRequest) {
      const retiredRows = await recoverableWafEpochRows(
        "REJECTED_RETIRED", options, expectedId,
      );
      if (retiredRows.length > 1) {
        throw receiptError(
          "STEP11_6_VERCEL_WAF_RECOVERY_CONFLICT",
          "More than one terminal WAF recovery record was found.", {}, 409,
        );
      }
      row = retiredRows[0] || null;
    }
    if (!row) {
      throw receiptError(
        "STEP11_6_VERCEL_WAF_RECOVERY_NOT_FOUND",
        "No active recoverable WAF epoch was found.", {}, 409,
      );
    }
    const epochId = clean(row.epoch_id).toLowerCase();
    const status = clean(row.status).toUpperCase();
    const teamId = clean(env[VERCEL_PROVIDER_ATTESTATION_TEAM_ID_ENV]);
    const historicalAliasOrigin = clean(row.candidate_alias_origin).toLowerCase();
    const historicalImmutableOrigin = clean(
      row.candidate_immutable_origin,
    ).toLowerCase();
    if (!uuid(epochId) || !new Set([
      "ACTIVATION_PENDING", "REJECTED_RETIRED",
    ]).has(status) || clean(row.purpose).toUpperCase() !== "REHEARSAL" ||
        clean(row.transition_mode).toUpperCase() !== "REHEARSAL" ||
        clean(row.vercel_project_id) !== PRODUCTION_VERCEL_PROJECT_ID ||
        clean(row.vercel_team_id) !== teamId ||
        clean(row.candidate_deployment_target).toUpperCase() !== "PREVIEW" ||
        !/^dpl_[A-Za-z0-9]{8,64}$/.test(clean(row.candidate_deployment_id)) ||
        !/^[0-9a-f]{40}$/.test(clean(row.candidate_deployment_commit)) ||
        !/^https:\/\/[a-z0-9.-]+\.vercel\.app$/.test(historicalAliasOrigin) ||
        !/^https:\/\/[a-z0-9.-]+\.vercel\.app$/.test(
          historicalImmutableOrigin,
        ) || historicalAliasOrigin === historicalImmutableOrigin) {
      throw receiptError(
        "STEP11_6_VERCEL_WAF_RECOVERY_SCOPE_INVALID",
        "The active WAF epoch did not match the exact Production recovery scope.",
        {}, 409,
      );
    }
    if (status === "REJECTED_RETIRED" &&
        (clean(row.retirement_request_id).toLowerCase() !==
          expectedRetirementRequest ||
         clean(row.retirement_candidate_deployment_id) !== candidate.deploymentId ||
         clean(row.retirement_candidate_deployment_commit).toLowerCase() !==
          candidate.commit ||
         clean(row.retirement_reason) !== "PROVIDER_REJECTED_NO_MUTATION" ||
         !clean(row.retired_at))) {
      throw receiptError(
        "STEP11_6_VERCEL_WAF_RECOVERY_TERMINAL_STATE_INVALID",
        "The terminal WAF epoch did not match the durable retirement retry.",
        {}, 409,
      );
    }
    const historicalInsert =
      buildProductionGoogleWriterCriticalWindowVercelRuleInsert({
        candidateAliasOrigin: historicalAliasOrigin,
        candidateImmutableOrigin: historicalImmutableOrigin,
        runOwnedRuleName: clean(row.run_owned_rule_name),
        runOwnedRuleNonce: clean(row.run_owned_rule_nonce).toLowerCase(),
      });
    ...
    if (historicalInsert.runOwnedRuleFingerprint !==
          clean(row.run_owned_rule_fingerprint).toLowerCase() ||
        historicalInsert.runOwnedInsertDocumentFingerprint !==
          clean(row.run_owned_insert_document_fingerprint).toLowerCase() ||
        historicalContract.candidateControlHostsFingerprint !==
          clean(row.candidate_control_hosts_fingerprint).toLowerCase()) {
      throw receiptError(
        "STEP11_6_VERCEL_WAF_RECOVERY_CONTRACT_INVALID",
        "The durable WAF epoch did not match its exact historical rule contract.",
        {}, 409,
      );
    }
    const rejectedDispatch = await assertRejectedWafDispatch(epochId, options);
    const epoch = await receiptRpc(
      "inspect_production_vercel_writer_critical_waf_epoch",
      {
        ...exactScope(),
        operation: "INSPECT_PRODUCTION_VERCEL_WRITER_CRITICAL_WAF_EPOCH",
        epoch_id: epochId,
        candidate_deployment_id: clean(row.candidate_deployment_id),
        candidate_deployment_commit:
          clean(row.candidate_deployment_commit).toLowerCase(),
      },
      options,
    );
    if (epoch.found !== true || clean(epoch.epoch_id).toLowerCase() !== epochId ||
        clean(epoch.status).toUpperCase() !== status ||
        clean(epoch.run_owned_rule_name) !== historicalInsert.runOwnedRuleName ||
        clean(epoch.run_owned_rule_nonce).toLowerCase() !==
          historicalInsert.runOwnedRuleNonce ||
        clean(epoch.run_owned_rule_fingerprint).toLowerCase() !==
          historicalInsert.runOwnedRuleFingerprint ||
        clean(epoch.run_owned_insert_document_fingerprint).toLowerCase() !==
          historicalInsert.runOwnedInsertDocumentFingerprint) {
      throw receiptError(
        "STEP11_6_VERCEL_WAF_RECOVERY_READBACK_INVALID",
        "The recovered WAF epoch failed authoritative readback verification.",
        {}, 409,
      );
    }
    return Object.freeze({
      ...epoch,
      vercel_project_id: clean(row.vercel_project_id),
      vercel_team_id: clean(row.vercel_team_id),
      candidate_deployment_id: clean(row.candidate_deployment_id),
      candidate_deployment_commit:
        clean(row.candidate_deployment_commit).toLowerCase(),
      candidate_deployment_target: "PREVIEW",
      candidate_alias_origin: historicalAliasOrigin,
      candidate_immutable_origin: historicalImmutableOrigin,
      candidate_control_hosts_fingerprint:
        clean(row.candidate_control_hosts_fingerprint).toLowerCase(),
      recoverable_rejected: true,
      provider_mutation_performed: false,
      ...rejectedDispatch,
    });
  };
  return Object.freeze({
    recoverRejectedCriticalWafEpoch,
    ...
    retireRejectedCriticalWafEpoch: async (details) => {
      ...
      if (evidence.stage !== "BASELINE_CAPTURE") {
        throw receiptError(
          "STEP11_6_VERCEL_WAF_RETIRE_BASELINE_EVIDENCE_REQUIRED",
          "A fresh signed baseline observation was required to retire the rejected WAF epoch.",
          {},
          409,
        );
      }
      ...
      const retirementRequestId = exactUuid(
        details.retirementRequestId,
        "STEP11_6_VERCEL_WAF_RETIREMENT_REQUEST_ID_INVALID",
      );
      const freshBaselineObservationRequestId = exactUuid(
        details.freshBaselineObservationRequestId,
        "STEP11_6_VERCEL_WAF_OBSERVATION_REQUEST_ID_INVALID",
      );
      const recovered = await recoverRejectedCriticalWafEpoch({
        environment: details.environment,
        expectedEpochId: epochId,
        retirementRequestId,
      });
      if (evidence.vercelProjectId !== PRODUCTION_VERCEL_PROJECT_ID ||
          evidence.vercelTeamId !== clean(
            env[VERCEL_PROVIDER_ATTESTATION_TEAM_ID_ENV],
          ) || evidence.candidateAliasOrigin !==
            clean(recovered.candidate_alias_origin).toLowerCase() ||
          evidence.candidateImmutableOrigin !==
            clean(recovered.candidate_immutable_origin).toLowerCase() ||
          evidence.candidateDeploymentId !== candidate.deploymentId ||
          evidence.candidateCommitSha !== candidate.commit ||
          evidence.candidateDeploymentTarget !== "PREVIEW" ||
          evidence.runOwnedRuleName !== clean(recovered.run_owned_rule_name) ||
          evidence.runOwnedRuleNonce !==
            clean(recovered.run_owned_rule_nonce).toLowerCase() ||
          evidence.runOwnedRuleFingerprint !==
            clean(recovered.run_owned_rule_fingerprint).toLowerCase() ||
          evidence.runOwnedInsertDocumentFingerprint !==
            clean(recovered.run_owned_insert_document_fingerprint).toLowerCase()) {
        throw receiptError(
          "STEP11_6_VERCEL_WAF_RETIRE_RECOVERY_SCOPE_MISMATCH",
          "The retirement evidence did not match the authoritative recovered WAF epoch.",
          {},
          409,
        );
      }
      if (evidence.wafEpochId !== epochId) {
        throw receiptError(
          "STEP11_6_VERCEL_WAF_RETIRE_EPOCH_MISMATCH",
          "The signed baseline evidence did not match the rejected WAF epoch.",
          {},
          409,
        );
      }
      const requestFingerprint = providerRequestFingerprint(
        "production-vercel-writer-rejected-waf-epoch-retire-v1",
        epochId,
        retirementRequestId,
      ...
      return receiptRpc(
        "retire_production_vercel_writer_rejected_waf_epoch",
        {
          ...
          fresh_baseline_observation_request_id:
            freshBaselineObservationRequestId,
          request_fingerprint: requestFingerprint,
          candidate_deployment_id: candidate.deploymentId,
          candidate_deployment_commit: candidate.commit,
