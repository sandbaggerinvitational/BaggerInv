...
async function initialize(session) {
  ...
  const tournamentDataMs = Date.now() - tournamentStartedAt;
  let personalizedTiming = {};
  const personalized = await readPlayerPassportMatches(session, {
    onTiming: (timing) => { personalizedTiming = timing; },
  });
  return {
    ...
    timing: {
      tournamentDataMs,
      passportLookupMs: personalizedTiming.passportLookupMs || 0,
      personalizedDataMs: personalizedTiming.personalizedDataMs || 0,
      initializationMs: Date.now() - initializedStartedAt,
      cacheHit: false,
    },
  };
}
...
export async function initializeParticipantTournament(session) {
  ...
  if (cached && Date.now() - cached.cachedAt < CACHE_TTL) return {
    ...cached.value,
    timing: { ...cached.value.timing, cacheHit: true },
  };
