...
function individualPlayFormat(value) {
  return ["BB", "BEST BALL", "SI", "SINGLES"].includes(clean(value).toUpperCase());
}

export function playerPerformanceRows(leaderboard = [], scoreLeaderboard = [], rounds = []) {
  const completedScores = new Map();
  const eligibleOfficialRounds = new Set();
  for (const round of rounds) {
    if (!individualPlayFormat(round.format)) continue;
    for (const match of round.matches || []) {
      if (!isOfficialMatchResult(match)) continue;
      for (const player of [...(match.team1Players || []), ...(match.team2Players || [])]) {
        if (clean(player?.id)) eligibleOfficialRounds.add(`${Number(round.number)}:${clean(player.id)}`);
      }
    }
  }
  for (const row of scoreLeaderboard) {
    const playerId = clean(row.id || row.playerIds?.[0]);
    if (row.entityType === "PAIRING" || Number(row.holes) < 18 || !eligibleOfficialRounds.has(`${Number(row.round)}:${playerId}`)) continue;
    const values = completedScores.get(playerId) || { gross: [], net: [] };
    if (numeric(row.gross) !== null) values.gross.push(Number(row.gross));
    if (numeric(row.net) !== null) values.net.push(Number(row.net));
    completedScores.set(playerId, values);
  }
...
export function teamStandings(rounds = [], tournament = {}, selectedRound = "overall", {
  ...
  for (const match of selected.flatMap((round) => round.matches || [])) {
    const final = requireOfficialResults ? isOfficialMatchResult(match) :
...
function rowMatch(row, matches = []) {
  const ids = (row.playerIds || [row.id]).map(String);
  return matches.find((match) => {
    const participants = [...(match.team1Players || []), ...(match.team2Players || [])].map((player) => String(player.id));
    return ids.every((id) => participants.includes(id));
  });
}

export function roundCompetitionRows(rows = [], round, format = "", officialRows = [], matches = []) {
  const pairing = ["SC", "SCRAMBLE"].includes(clean(format).toUpperCase());
  const pointsByPlayer = new Map(officialRows.map((row) => [String(row.id), numeric(row.points)]));
  const eligible = roundScoreRows(rows, round, format).map((row, stableIndex) => {
    const match = rowMatch(row, matches);
    ...
    const playerIds = (row.playerIds || [row.id]).map(String);
    const playerPoints = playerIds.map((id) => pointsByPlayer.get(id));
    const pointsComplete = final && playerPoints.length && playerPoints.every((value) => value !== null && value !== undefined);
    const points = pointsComplete
      ? pairing ? playerPoints.reduce((sum, value) => sum + value, 0) : playerPoints[0]
      : null;
    return { ...row, points, playerPoints: pointsComplete ? playerIds.map((id, index) => ({ playerId: id, points: playerPoints[index] })) : [], officialFinal: final, stableIndex };
  }).sort((left, right) => {
    const leftPoints = numeric(left.points);
    const rightPoints = numeric(right.points);
    if (leftPoints === null && rightPoints !== null) return 1;
    if (rightPoints === null && leftPoints !== null) return -1;
    if (leftPoints !== null && rightPoints !== null && rightPoints !== leftPoints) return rightPoints - leftPoints;
    const leftNet = numeric(left.net);
    const rightNet = numeric(right.net);
    if (leftNet === null && rightNet !== null) return 1;
    if (rightNet === null && leftNet !== null) return -1;
    if (leftNet !== null && rightNet !== null && leftNet !== rightNet) return leftNet - rightNet;
  ...
  return eligible.map((row, index) => {
    ...
    const tied = index > 0 && points === previousPoints && net === previousNet;
    const displayRank = tied ? previousRank : index + 1;
    previousPoints = points;
    previousNet = net;
    previousRank = displayRank;
    const { stableIndex: _stableIndex, ...result } = row;
    return { ...result, displayRank };
  });
}
