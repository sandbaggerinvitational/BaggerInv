...
export async function replaceNetSkinsConfiguration(input, options = {}) {
  return scoringShadowRpc("replace_preview_net_skins_configuration", { input }, { ...options, timeoutMs: options.timeoutMs || 20_000 });
}
    ...
    for (const side of [1, 2]) {
      ...
      if (format === "SC") {
        rows.push({
          ...
          scorecard: (entry.scores || []).map((score) => {
        ...
        sidePlayers.forEach((participant, index) => rows.push({
          ...
          scorecard: (entry.scores || []).map((score) => {
            ...
            const strokes = array(score[`team_${side}_strokes`])[index];
            const hole = holeByNumber.get(number(score.hole_number)) || {};
...
export function calculateNetSkinsFromSupabaseView(view = {}) {
  ...
  return {
    ...
    canonicalInputVerification: {
      ...
      usesStoredIndividualGrossAndAppliedStrokes: true,
