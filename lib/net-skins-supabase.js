...
function individualStrokeAllocation(sheets = {}, { year, round, match, format, playerId } = {}) {
  ...
  for (const side of [1, 2]) for (const slot of [1, 2]) {
    ...
    const allocation = nullableNumber(source[`Team ${side} Player ${slot} Stroke`]);
    if (allocation === null) throw Object.assign(new Error(`Net Skins stroke allocation is missing for ${playerId}.`), { code: "NET_SKINS_INDIVIDUAL_ALLOCATION_REQUIRED" });
    return allocation;
  }
