...
function individualStrokeAllocation(sheets = {}, { year, round, match, format, playerId } = {}) {
  if (format === "SC") return null;
  const candidates = rowRecords(sheets["Live Matches"]).filter((row) =>
    number(row.Year) === number(year) && number(row.Round) === number(round) &&
    (!clean(match) || clean(row.Match || row["Match Number"]) === clean(match)) &&
    normalizeSkinsFormat(row.Format) === format && [1, 2].some((side) => [1, 2].some((slot) =>
      clean(row[`Team ${side} Player ${slot}`]) === clean(playerId))));
  if (candidates.length !== 1) {
    throw Object.assign(new Error(`Net Skins requires one authoritative Round ${round} match allocation for ${playerId}.`), { code: "NET_SKINS_INDIVIDUAL_ALLOCATION_REQUIRED" });
  }
  const source = candidates[0];
  for (const side of [1, 2]) for (const slot of [1, 2]) {
    if (clean(source[`Team ${side} Player ${slot}`]) !== clean(playerId)) continue;
    // Preserve the existing leaderboard/Net Skins adapter contract exactly:
    // an explicit Player Stroke wins, otherwise Player Playing HCP supplies the
    // round allocation, and an empty pair is a zero-stroke allocation.
    const configuredStroke = clean(source[`Team ${side} Player ${slot} Stroke`]);
    const playingHandicap = clean(source[`Team ${side} Player ${slot} Playing HCP`]);
    return nullableNumber(configuredStroke || playingHandicap) ?? 0;
  }
  throw Object.assign(new Error(`Net Skins stroke allocation is missing for ${playerId}.`), { code: "NET_SKINS_INDIVIDUAL_ALLOCATION_REQUIRED" });
}

function canonicalEntry(row = {}, { year, sheets } = {}) {
  ...
  const individualAllocation = individualStrokeAllocation(sheets, { year: rowYear, round, match, format, playerId: playerId1 });
  ...
  return {
    ...
    individual_stroke_allocation: individualAllocation,
    ...
    source_payload: {
      ...
      "Individual Stroke Allocation": individualAllocation,
...
export function buildNetSkinsConfigurationImport({ sheets = {}, tournamentId, tournamentYear, sourceWorkbookId, requestedBy } = {}) {
  ...
  const entries = rows.map((row) => canonicalEntry(row, { year, sheets })).filter(Boolean)
...
export function netSkinsScoreRowsFromSupabaseView(view = {}) {
  const rows = [];
  const individualAllocations = new Map((view.configurations || []).flatMap((item) => item.entries || [])
    .filter((entry) => clean(entry.format).toUpperCase() !== "SC")
    .map((entry) => [`${number(entry.round_number)}:${clean(entry.player_id_1)}`, nullableNumber(entry.individual_stroke_allocation)]));
    ...
    for (const side of [1, 2]) {
      ...
      if (format === "SC") {
        ...
        sidePlayers.forEach((participant, index) => {
          const fullRoundStrokes = individualAllocations.get(`${round}:${clean(participant.player_id)}`);
          if (fullRoundStrokes === null || fullRoundStrokes === undefined) return;
          const scorecard = (entry.scores || []).map((score) => {
...
export function calculateNetSkinsFromSupabaseView(view = {}) {
  ...
  return {
    ...
    canonicalInputVerification: {
      ...
      usesStoredIndividualGrossAndExplicitConfiguredAllocation: true,
...
export async function currentNetSkinsOperationalResult(tournamentId, { recalculatePending = true, calculatedBy, env } = {}) {
  if (productionShadowCandidateReadEnvironment(env || process.env).eligible) recalculatePending = false;
  let read = await readNetSkinsResultView(tournamentId, { env });
  ...
  if (recalculatePending && (data.stale || !data.netSkins.rounds.length)) {
    try {
      ...
      read = await readNetSkinsResultView(tournamentId, { env });
