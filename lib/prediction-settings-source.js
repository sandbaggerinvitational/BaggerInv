...
export function predictionSettingsEnvironment(env = process.env) {
  ...
  const cutover = productionCutoverReadSourceEnvironment({
    env,
    variable: "PREDICTION_SETTINGS_READ_SOURCE",
    configuredValue: env.PREDICTION_SETTINGS_READ_SOURCE,
    requiredPhase: "ODDS_WAR_ROOM",
  });
  if (cutover.handled) return {
    requestedSource: cutover.requested,
    source: cutover.resolved,
    preview: false,
    isolated: false,
    configured: cutover.activation.serviceCredentialConfigured,
    eligible: cutover.resolved === "supabase",
    ...
    productionHardBlock: cutover.blocked,
  };
