...
function accountSession(entitlement, authUserId) {
  const revision = Number(entitlement.revision || 1);
  return {
    type: "preview-director-entitlement",
...
function accountIdentity(entitlement, authUserId, details = {}) {
  ...
  const session = accountSession(entitlement, authUserId);
  return {
...
async function verifiedAccount(cookieStore, env, dependencies) {
  ...
  const readEntitlement = dependencies.readEntitlement || readPreviewDirectorEntitlement;
  try {
