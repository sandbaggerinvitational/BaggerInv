...
export function completedHistoryReadEnvironment(env = process.env) {
  ...
  const projectApproved = approvedProject(env);
  const credentialsConfigured = Boolean(
    clean(env.SUPABASE_SCORING_MIRROR_URL) &&
    clean(env.SUPABASE_SCORING_MIRROR_SECRET_KEY)
  );
  const supabaseEligible = preview && projectApproved && credentialsConfigured;
  ...
  const resolved = requested === "supabase" && supabaseEligible
    ? "supabase"
    : "google";
  const reason = resolved === "supabase" ? "preview-supabase-completed-history"
  ...
  return {
    ...
    preview,
    projectApproved,
    projectRef: projectApproved ? PREVIEW_COMPLETED_HISTORY_SUPABASE_PROJECT_REF : "",
...
export function isSupabaseCompletedHistoryYear(year, env = process.env) {
  const state = completedHistoryReadEnvironment(env);
  return isCompletedHistoryYear(year) && state.preview && state.requested !== "google";
}
