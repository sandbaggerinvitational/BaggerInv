...
import {
  calculateProductionFullNetCalcutta,
  FULL_NET_CALCUTTA_ENGINE,
} from "./production-full-net.js";
import {
...
export async function readProductionCalcuttaManagement({ actorAuthUserId, actorPlayerId, expectedAuctionRevision } = {}, options = {}) {
  return mutationRpc("read_production_calcutta_management_v1", {
    contract_version: PRODUCTION_CALCUTTA_V1_CONTRACT,
    authorization: directorAuthorization({ actorAuthUserId, actorPlayerId }),
    predecessor_auction_revision: expectedAuctionRevision == null ? null : exactInteger(expectedAuctionRevision, "PRODUCTION_CALCUTTA_AUCTION_REVISION_REQUIRED"),
    request_fingerprint: serviceOperationFingerprint("DIRECTOR_MANAGEMENT_READ"),
  }, options);
}
...
export async function clearProductionCalcuttaV1AuctionEntry({
  actorAuthUserId,
  actorPlayerId,
  expectedConfigurationRevision,
  expectedConfigurationFingerprint,
  ...
  if (expectedTournamentId !== PRODUCTION_TOURNAMENT_ID)
    throw calcuttaError("PRODUCTION_CALCUTTA_CLEAR_SCOPE_DENIED", "Clear is supported only for the current 2026 tournament.", 400);
  return mutationRpc("clear_production_calcutta_v1_auction_entry", {
    contract_version: PRODUCTION_CALCUTTA_V1_CONTRACT,
    expected_configuration_revision: exactInteger(
      expectedConfigurationRevision,
      "PRODUCTION_CALCUTTA_CONFIGURATION_REVISION_REQUIRED",
    ...
    player_id: exactPlayerId(playerId),
    expected_tournament_id: expectedTournamentId,
    authorization: directorAuthorization({ actorAuthUserId, actorPlayerId }),
    request_fingerprint: operationFingerprint(requestFingerprint, "CLEAR_ENTRY"),
  }, options);
}
...
export async function processProductionCalcuttaV1Job({
  ...
  try {
    ...
    const calculated = calculateProductionFullNetCalcutta(configurationView, coreView);
    const result = await mutationRpc("complete_production_calcutta_v1_recalculation", {
      ...
      engine_version: FULL_NET_CALCUTTA_ENGINE,
