...
export function participantIdentityAuthorityEnvironment(env = process.env) {
  ...
  const eligible = candidate.requested ? productionShadowCandidate : previewEligible;
  const blocked = previewDeployment && (candidate.requested
    ? (!valid || !productionShadowCandidate)
    : (!valid || (requested === "supabase" && !previewEligible)));
  const resolved = !previewDeployment ? "passport"
    : blocked ? "unavailable"
  ...
  const participantAuthEnabled = productionShadowCandidate || (previewDeployment && previewWorkbook && serverIdentityConfigured &&
