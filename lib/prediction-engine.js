...
export function predict({ format, players, historical, partnership, headToHead, handicap, settings, teamNames = ["Team 1", "Team 2"] }) {
  ...
  const factors=[
    {category:"Handicap edge", detail:strokeDiff === 0 ? "Even" : `${strokeDiff > 0 ? teamNames[0] : teamNames[1]} (+${Math.abs(strokeDiff)} strokes)`, side:strokeDiff===0?"neutral":strokeDiff>0?"A":"B"},
    {category:`${formatLabel} history`, detail:Math.abs(pa-pb)<0.5?"Even":`${pa>=pb?teamNames[0]:teamNames[1]} (${Math.max(pa,pb).toFixed(0)}–${Math.min(pa,pb).toFixed(0)} rating)`,side:Math.abs(pa-pb)<0.5?"neutral":pa>=pb?"A":"B"},
    {category:"Head-to-head", detail:Math.abs(oa-ob)<0.5?"Even":`${oa>=ob?teamNames[0]:teamNames[1]} (${Math.max(oa,ob).toFixed(0)}–${Math.min(oa,ob).toFixed(0)} rating)`,side:Math.abs(oa-ob)<0.5?"neutral":oa>=ob?"A":"B"},
  ];
  if(code!=="SI") factors.push({category:"Partnership history",detail:ta===50&&tb===50?"No prior partnership history":Math.abs(ta-tb)<0.5?"Even":`${ta>=tb?teamNames[0]:teamNames[1]} (${Math.max(ta,tb).toFixed(0)}–${Math.min(ta,tb).toFixed(0)} rating)`,side:ta===tb?"neutral":ta>tb?"A":"B"});

  const componentMetrics = { handicap:[ha,hb], player:[pa,pb], team:[ta,tb], opponent:[oa,ob], tournament:[50,50] };
  const categoryLabels = { handicap:"Handicap", player:"Player strength", team:code === "SI" ? "Partnership (N/A)" : "Partnership", opponent:"Head-to-head", tournament:"Tournament" };
  ...
  return {teamA:Math.round(a),teamB:Math.round(b),tie:Math.round(100-a-b),confidence,factors,components:componentMetrics,contributions,rawScoreA:rawA,model:setting(settings,"Prediction Model","SBI v1.0")};
}
