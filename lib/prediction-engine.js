...
export function predict({ format, players, historical, partnership, headToHead, handicap, settings }) {
  const code = formatCode(format);
  ...
  const factors=[
    {label:`Net stroke edge: ${Math.abs(strokeDiff)} to ${strokeDiff>=0?"Team A":"Team B"}`, side:strokeDiff===0?"neutral":strokeDiff>0?"A":"B"},
    {label:`${code === "BB" ? "Best Ball" : code === "SC" ? "Scramble" : "Singles"} history favors ${pa>=pb?"Team A":"Team B"}`,side:pa>=pb?"A":"B"},
    {label:`Head-to-head favors ${oa>=50?"Team A":"Team B"}`,side:oa>=50?"A":"B"},
  ];
  if(code!=="SI") factors.push({label: ta===50&&tb===50?"No prior partnership history":"Partnership history favors "+(ta>=tb?"Team A":"Team B"),side:ta===tb?"neutral":ta>tb?"A":"B"});
