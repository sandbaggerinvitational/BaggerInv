...
export function predict({ format, players, historical, partnership, headToHead, handicap, settings, teamNames = ["Team 1", "Team 2"] }) {
  ...
  const pa = playerScore(teamA), pb = playerScore(teamB);const averageAppearances = (team) =>
  average(
    team.map(
      (player) => historical[player.id]?.appearances?.length || 0
    )
  );

const appearancesA = averageAppearances(teamA);
const appearancesB = averageAppearances(teamB);

// The +4 prevents one or two appearances from creating an oversized edge.
// This scales naturally as the tournament accumulates more history.
const experienceA = Math.max(
  0,
  Math.min(
    100,
    50 +
      (50 * (appearancesA - appearancesB)) /
        (appearancesA + appearancesB + 4)
  )
);
