...
export function predict({ format, players, historical, partnership, headToHead, handicap, settings, teamNames = ["Team 1", "Team 2"] }) {
  ...
  if(code!=="SI") factors.push({category:"Partnership history",detail:ta===50&&tb===50?"No prior partnership history":Math.abs(ta-tb)<0.5?"No measurable edge":`${ta>=tb?teamNames[0]:teamNames[1]} · ${edgePhrase(ta-tb)}`,side:ta===tb?"neutral":ta>tb?"A":"B"});
