...
// Advisory outputs only; independent of Prediction Settings and SQL scoring.
export const ADVISORY_PREDICTION_ENGINE_VERSION = "prediction-engine.js:team-symmetry-v3";
...
// PostgreSQL numeric round(value, 0) rounds exact halves away from zero.
// JavaScript's Math.round differs for negative halves, so every prediction
// calculation that mirrors the scoring contract must use this helper.
export function roundPostgresNumeric(value) {
  const numeric = Number(value);
  if (!Number.isFinite(numeric)) return NaN;
  const absolute = Math.abs(numeric);
  const whole = Math.floor(absolute);
  const magnitude = whole + (absolute - whole >= 0.5 ? 1 : 0);
...
export function playingHandicaps(format, handicaps) {
  ...
  if (code === "SC") {
    const team = (pair) => roundPostgresNumeric(Math.min(...pair) * .35 + Math.max(...pair) * .15);
  ...
  if (code === "SI") {
    ...
    const playerStrokes = handicaps.map((hcp) => roundPostgresNumeric(hcp - low));
    return {
      raw: handicaps,
      teamA: roundPostgresNumeric(handicaps[0]),
      teamB: roundPostgresNumeric(handicaps[1]),
      strokesA: playerStrokes[0],
      strokesB: playerStrokes[1],
      playerStrokes,
    };
  }
  const low = Math.min(...handicaps);
  const playerStrokes = handicaps.map((hcp) => roundPostgresNumeric((hcp - low) * .9));
...
export function allocateStrokes(strokes, holes) {
  const count = Math.max(0, roundPostgresNumeric(n(strokes, 0)));
  ...
  for (let pass = 0; pass < Math.ceil(count / 18); pass += 1) {
    for (let i = 0; i < Math.min(18, count - pass * 18); i += 1) {
      const hole = n(pick(ordered[i], "Hole Number"), i + 1) - 1;
...
export const PREDICTION_SETTING_DEFAULTS = Object.freeze({
  ...
  "Scorecard Influence Enabled": false,
  "Scorecard Category Weight": 10,
  "Maximum Scorecard Adjustment": 6,
...
export function predict({ format, players, historical, partnership, headToHead, handicap, settings, teamNames = ["Team 1", "Team 2"] }) {
  ...
  // Center the partnership difference on neutral. Equal strengths cancel,
  // including two unknown pairs; swapping teams negates the same signal.
  // Keep the absolute evidence in teamVibes for presentation, not weighting.
  const ta = 50 + (vibesA.score - vibesB.score) / 2, tb = 100 - ta;
  ...
  const center = (100 - tie) / 2;
  const probabilityBeforeSkillAdjustment = center+edge*(100-tie)/100;
  ...
  // Apply both teams' caps about the same center, never privileging Team A.
  const radius = Math.max(0, Math.min(center - min, max - center, center));
  const cappedEdge = clamp(probabilityAfterSkillAdjustment - center, -radius, radius);
  const a = center + cappedEdge, b = center - cappedEdge;
  ...
  // Remove sub-1e-10 percentage-point arithmetic noise before display rounding
  // so an exact half cannot land on opposite sides after a pure team swap.
  const roundedA = Math.round(Number(a.toFixed(10)));
  const roundedB = Math.round(Number(b.toFixed(10)));
  return {
    engineVersion: ADVISORY_PREDICTION_ENGINE_VERSION,
    teamA:roundedA,
    teamB:roundedB,
