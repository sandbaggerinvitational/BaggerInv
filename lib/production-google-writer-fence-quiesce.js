...
import productionOriginInventoryArtifact from
  "../docs/evidence/step11-6-production-origin-inventory-v4.json" with {
    type: "json",
  };
...
} from "./production-google-historical-writer-scope.js";

export const PRODUCTION_GOOGLE_WRITER_QUIESCE_VERSION =
  "production-google-writer-quiesce-noncanonical-host-v2";
...
export const PRODUCTION_LEGACY_DEPLOYMENT_INVENTORY_COUNT = 1_292;
...
export const PRODUCTION_LEGACY_DEPLOYMENT_PROVIDER_INVENTORY_COUNT = 1_292;
...
export const PRODUCTION_GOOGLE_WRITER_QUIESCE_EDGE_PROBE_FIXED_ORIGINS =
  Object.freeze(PRODUCTION_GOOGLE_WRITER_QUIESCE_FIXED_ORIGINS.filter((origin) =>
    new URL(origin).hostname !== PRODUCTION_CANONICAL_HOSTNAME));
...
export function productionGoogleWriterAllMethodFenceHosts() {
  const evidence = productionGoogleHistoricalProviderFenceScope();
  const origins = Array.isArray(evidence?.executableAllMethodHostOrigins)
    ? evidence.executableAllMethodHostOrigins.map((origin) =>
      normalizedOrigin(origin, { requireVercel: true }))
    : [];
  const sorted = [...origins].sort(codepointCompare);
  const fingerprint = clean(
    evidence?.executableAllMethodHostOriginsFingerprint,
  ).toLowerCase();
  if (origins.length !== PRODUCTION_GOOGLE_EXECUTABLE_ALL_METHOD_HOST_ORIGIN_COUNT ||
      origins.some((origin) => !origin) ||
      new Set(origins).size !== origins.length || !exactJson(origins, sorted) ||
      Number(evidence?.executableAllMethodHostOriginCount) !== origins.length ||
      !/^[0-9a-f]{64}$/.test(fingerprint) ||
      fingerprint !== PRODUCTION_GOOGLE_EXECUTABLE_ALL_METHOD_HOST_ORIGINS_FINGERPRINT ||
  ...
  return Object.freeze({
    ...
    fingerprint,
    policy: "SOURCE_UNRESOLVED_IMMUTABLE_EIGHT_PLUS_EXACT_UNSAFE_ALIAS_ALL_METHOD_DENY",
  });
}
...
function normalizeRoutingRule(value) {
  if (!exactKeys(value, [
    ...
    "hostnameOperator", "canonicalHostname",
  ...
  if (clean(value.projectId) !== PRODUCTION_VERCEL_PROJECT_ID ||
      ...
      clean(value.hostnameOperator).toUpperCase() !== "DOES_NOT_EQUAL" ||
      clean(value.canonicalHostname).toLowerCase() !== PRODUCTION_CANONICAL_HOSTNAME ||
...
function attestedLiveInventory(providerAttestation, retained, candidate) {
  ...
  if (providerAttestation?.signatureVerified !== true || !Array.isArray(records) ||
      ...
      providerAttestation?.routingRuleHostnameOperator !== "DOES_NOT_EQUAL" ||
      providerAttestation?.routingRuleCanonicalHostname !==
        PRODUCTION_CANONICAL_HOSTNAME ||
      providerAttestation?.routingRuleEarlierActiveBypassRuleCount !== 0 ||
      ...
      providerAttestation?.routingRuleAllMethodFenceRequiredPathsFingerprint !==
        allMethodPaths.fingerprint ||
      credentialConfinement.originInventorySchemaVersion !==
  ...
  const normalized = records.map((tuple) => {
    ...
    const allowedScope = scope === "PROJECT_PREVIEW";
...
function attestedAliasInventory(providerAttestation, candidate) {
  const records = providerAttestation?.aliasInventoryRecords;
  const count = Number(providerAttestation?.aliasInventoryCount);
  const fingerprint = clean(providerAttestation?.aliasInventoryFingerprint).toLowerCase();
  const candidateAliasHostname = new URL(candidate.aliasOrigin).hostname;
  const candidateImmutableHostname = new URL(candidate.immutableOrigin).hostname;
  const courseAlias = Object.freeze([
  ...
  if (providerAttestation?.signatureVerified !== true || !Array.isArray(records) ||
      count !== 56 || records.length !== count ||
      !/^[0-9a-f]{64}$/.test(fingerprint) ||
      sha256(JSON.stringify(records)) !== fingerprint ||
      records.filter((record) => exactJson(record, courseAlias)).length !== 1 ||
      records.filter((record) => Array.isArray(record) && record.length === 5 &&
        record[0] === candidateAliasHostname &&
        record[1] === candidate.deploymentId &&
        record[2] === candidateImmutableHostname &&
        record[3] === null && record[4] === null).length !== 1 ||
      Number(providerAttestation?.aliasPaginationPageCount) < 1 ||
      !/^[0-9a-f]{64}$/.test(clean(
        providerAttestation?.aliasPaginationFingerprint,
      ).toLowerCase())) {
    throw quiesceError(
      "STEP11_6_WRITER_QUIESCE_PROVIDER_ALIAS_ATTESTATION_INVALID",
      "The signed live Vercel alias inventory was unavailable or invalid.",
      {},
      409,
    );
  }
  return Object.freeze({
    ...
    count,
    fingerprint,
    paginationPageCount: Number(providerAttestation.aliasPaginationPageCount),
    paginationFingerprint:
      clean(providerAttestation.aliasPaginationFingerprint).toLowerCase(),
  });
}
...
export function normalizeProductionWriterQuiesceEvidenceInput(
  ...
  const aliasInventory = attestedAliasInventory(providerAttestation, {
    deploymentId: candidateDeploymentId,
    immutableOrigin: candidateImmutableOrigin,
    aliasOrigin: candidateAliasOrigin,
  });
  ...
  const probeOrigins = [...new Set([
    ...
    ...PRODUCTION_GOOGLE_WRITER_QUIESCE_EDGE_PROBE_FIXED_ORIGINS,
    candidateAliasOrigin,
  ])].sort(codepointCompare);
  const expectedProbeOriginCount = liveInventory.count +
    PRODUCTION_GOOGLE_WRITER_QUIESCE_EDGE_PROBE_FIXED_ORIGINS.length + 1;
  ...
  return Object.freeze({
    ...
    aliasPaginationPageCount: aliasInventory.paginationPageCount,
    aliasPaginationFingerprint: aliasInventory.paginationFingerprint,
