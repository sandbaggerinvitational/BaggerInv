...
export async function loadMobileCareerAuthority(
  ...
  if ((!playerId && !isCanonicalReviewer(identity.context)) || !tournamentId || identity.context?.membership?.active === false) {
    throw unavailable();
  }
  ...
  try {
    if (clean(env.VERCEL_ENV).toLowerCase() === "production") {
      ...
      const loadModel = dependencies.loadSecondaryHistoryModel ||
        (await import("./secondary-history-service.js")).loadSecondaryHistoryModel;
      const model = await loadModel({ env, dependencies: dependencies.secondaryHistoryDependencies });
      if (model?.source !== "supabase" || !model.calculations ||
          !Array.isArray(model.scorecardAnalytics?.canonicalCareerScorecards)) throw unavailable();
