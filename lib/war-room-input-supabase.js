...
import { buildAdvisoryCourseProjections } from "./advisory-match-simulation.js";
...
export async function prepareSupabaseWarRoomInput({
  ...
  const prepared = await preparePredictionInputBundle({
    ...
    dependencies: {
      readOddsInputBundle: async (...args) => {
        const read = await readOddsInputBundle(...args);
        currentState = (read?.payload || read)?.data?.current_state;
        return read;
      },
  ...
  if (scope === "war-room") consumerData.advisoryCourses = buildAdvisoryCourseProjections(currentState);
