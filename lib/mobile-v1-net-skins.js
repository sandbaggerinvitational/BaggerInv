...
import { productionNetSkinsPresentation } from './mobile-v1-net-skins-presentation.js';
...
export function mobileNetSkinsDataFromProductionView(view = {}, identity = {}) {
  ...
  requireValue(playerId || isCanonicalReviewer(identity.context));
  ...
  const presentation = productionNetSkinsPresentation(view, data, playerId || null);
  if(presentation && data.observer) presentation.observer = data.observer;
  return { ...data, presentation };
}

export async function readMobileProductionNetSkinsV1({ tournamentId, playerId, context } = {}, {
  env = process.env,
  dependencies = {},
} = {}) {
