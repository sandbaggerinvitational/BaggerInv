...
export function mobileNetSkinsDataFromProductionView(view = {}, identity = {}) {
  ...
  requireValue(contractVersion === MOBILE_NET_SKINS_CONTRACT_VERSION &&
    tournamentId === clean(identity.tournamentId) && tournamentId === PRODUCTION_TOURNAMENT_ID &&
...
export async function mobileNetSkinsResult(identity, {
  ...
  let read;
  try {
    read = await (dependencies.readProductionNetSkinsV1 || readMobileProductionNetSkinsV1)({
  ...
  return {
    ...
    body: {
      ...
      data,
      meta: { generatedAt, revision: data.revision },
    },
  };
}
