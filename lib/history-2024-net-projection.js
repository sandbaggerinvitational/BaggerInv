...
function recordFormatName(value) {
  return ({ BB: "Best Ball", SC: "Scramble", SI: "Singles" })[clean(value).toUpperCase()] || "";
}
...
function uniqueHolders(scorecards = []) {
  ...
  return scorecards.map((scorecard) => {
    const name = clean(scorecard.playerName || scorecard.playerId);
    return {
      ...
      team: clean(scorecard.teamName),
      formatName: recordFormatName(scorecard.format),
