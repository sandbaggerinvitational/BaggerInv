...
export function publishedOddsFreshness(view = {}) {
  ...
  const publicationState = clean(control.state || "UNPUBLISHED").toUpperCase();
  const intentionallyWithdrawn = publicationState === "WITHDRAWN" &&
    !clean(control.snapshot_id);
  ...
  if (!officialRows.length && !intentionallyWithdrawn) {
    reasons.push("CURRENT_OFFICIAL_SNAPSHOT_MISSING");
  }
  ...
  if (control.state && publicationState !== "PUBLISHED" &&
      !intentionallyWithdrawn) {
  ...
  const status = intentionallyWithdrawn
    ? "UNPUBLISHED"
    : reasons.length ? (current ? "STALE" : "UNAVAILABLE") : "CURRENT_OFFICIAL";
  ...
  return {
    ...
    stale: intentionallyWithdrawn ? false : reasons.length > 0,
    reasons: intentionallyWithdrawn
      ? ["CURRENT_PUBLICATION_WITHDRAWN"]
      : reasons,
    ...
    publicationState: publicationState || (current ? "PUBLISHED" : "UNPUBLISHED"),
    publishedSnapshotId: clean(control.snapshot_id) || null,
    publicationPointerRevision:
      number(control.publication_pointer_revision) ?? null,
    predecessorSnapshotId: clean(control.predecessor_snapshot_id) || null,
