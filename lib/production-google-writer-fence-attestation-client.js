...
const RETAINED_BEGIN_ATTEMPT_KEYS = new Set([
  "baseline",
  "canonicalValues",
  "routingRuleId",
  "routingRuleRevision",
...
function exactRetainedAttemptState(recoveryInput) {
  ...
  if (!state || Object.keys(state).some(
    (key) => !RETAINED_BEGIN_ATTEMPT_KEYS.has(key),
  ) || DURABLE_ATTEMPT_MARKERS.some((key) => clean(state[key])) ||
    state.rehearsalCertified === true) {
    fail(
      "PROVIDER_ATTESTATION_RETAINED_ATTEMPT_UNSAFE",
      "The retained BEGIN attempt included durable or unknown recovery state.",
    );
  }
  return state;
}
...
export function canAbandonRetainedProviderAttestationChallenge(
  ...
  try {
    const state = exactRetainedAttemptState(recoveryInput);
    const retained = validateRetainedProviderAttestationChallenge(
      state.beginProviderChallenge, state, purposeInput,
    );
    const inspection = validateRetainedProviderAttestationChallenge(
      inspectionInput, state, purposeInput,
    );
    if (retained.status !== "ISSUED" ||
    ...
    if (inspection.status === "ABANDONED") {
      discardAbandonedProviderAttestationAttempt(
        state, inspection, purposeInput,
      );
      return true;
    }
    return inspection.status === "ISSUED" && inspection.abandonEligible === true &&
      clean(inspection.abandonmentCode).toUpperCase() === "ELIGIBLE" &&
...
export function discardAbandonedProviderAttestationAttempt(
  ...
  const receipt = validateRetainedProviderAttestationChallenge(
    receiptInput, state, purposeInput,
  );
  if (receipt.status !== "ABANDONED" ||
      ...
      exactUuid(receipt.abandonRequestId,
        ...
        exactUuid(state.beginAbandonRequestId,
          "PROVIDER_ATTESTATION_ABANDON_RECEIPT_INVALID") ||
      ...
      Date.parse(clean(receipt.abandonedAt)) < Date.parse(clean(receipt.expiresAt)) ||
    ...
    fail(
      "PROVIDER_ATTESTATION_ABANDON_RECEIPT_INVALID",
      "The authoritative abandoned challenge receipt did not match this BEGIN attempt.",
    );
  }
  const next = {};
  for (const key of ["baseline", "canonicalValues"]) {
    const value = clean(recoveryInput[key]);
...
export function buildRetainedProviderAttestationChallengePayload(
  ...
  if (challenge.status !== "ISSUED") {
    fail(
      "PROVIDER_ATTESTATION_RETAINED_CHALLENGE_NOT_ISSUED",
      "Only a retained ISSUED BEGIN challenge can be inspected or abandoned.",
    );
  }
  return Object.freeze({
    quiescePurpose: clean(purposeInput).toUpperCase(),
    evidenceRequestId: state.evidenceRequestId,
    challengeRequestId: state.beginChallengeRequestId,
