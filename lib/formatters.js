...
// Explicit null means unavailable, not permission to fall back to rounded PH.
// Legacy/history DTOs without this field retain their original PWA value.
export function playerDisplayHandicap(player) {
  return Object.hasOwn(player || {}, "displayHandicap") ? player.displayHandicap : player?.playingHcp;
}
...
export function formatMeaningfulNumber(value) {
  const points = parseNumericValue(value);
  if (points === null) return "—";
...
export function formatCalcuttaPoints(value) {
  return formatMeaningfulNumber(value);
}
