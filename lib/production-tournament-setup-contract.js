...
export function buildTournamentSetupMutation(action, value = {}) {
  ...
  if (selected === "replace-round-pairings") {
    if (!Array.isArray(value.matches) || !value.matches.length || value.matches.length > 12 ||
        new Set(value.matches.map((match) => match.matchId)).size !== value.matches.length) {
      throw contractError("TOURNAMENT_SETUP_ROUND_MATCH_SET_INVALID", "Review every match in this round exactly once.");
    }
    return {
      operation: "REPLACE_ROUND_PAIRINGS", ...common,
      expected_handicap_revision_id: canonicalTournamentSetupOperationId(value.expectedHandicapRevisionId),
      round_number: boundedInteger(value.roundNumber, "round number", 1, 3),
      matches: value.matches.map((match) => {
        const normalized = buildTournamentSetupMutation("replace-pairings", { ...match,
          expectedRevision: value.expectedRevision, operationRequestId: value.operationRequestId });
        if (!normalized.participants.length) throw contractError("TOURNAMENT_SETUP_PAIRING_COUNT_INVALID", `${match.matchId}: complete every slot before saving the round.`);
        return { match_id: normalized.match_id, format: normalized.format, participants: normalized.participants };
...
function normalizeCourse(value = {}, retainedNames = new Map()) {
  const roundNumbers = Array.isArray(firstDefined(value.roundNumbers, value.round_numbers))
    ...
    : [];
  const courseId = safeText(firstDefined(value.courseId, value.course_id), 96);
  const tee = safeText(value.tee, 100);
  const name = safeText(firstDefined(value.name, value.courseName, value.course_name, value.displayName, value.display_name), 160);
  const retainedName = retainedNames.get(JSON.stringify([courseId, tee]));
  return Object.freeze({
    roundNumber: safeInteger(firstDefined(value.roundNumber, value.round_number, roundNumbers[0])),
    courseId,
    name: (!name || name === courseId) && retainedName ? retainedName : name,
    ...
    locked: safeBoolean(value.locked),
    setupManaged: safeBoolean(firstDefined(value.setupManaged, value.setup_managed)),
    setupRevision: safeInteger(firstDefined(value.setupRevision, value.setup_revision)),
...
function normalizeMatch(value = {}) {
  ...
  return Object.freeze({
    detailsManaged: safeBoolean(value.detailsManaged),
    contextFingerprint: safeText(value.contextFingerprint, 64),
...
export function normalizeProductionTournamentSetupPayload(payload = {}) {
  ...
  // Reuse this response's retained presentation, indexed once by exact identity.
  // Conflicting labels do not select an arbitrary name or affect scoring facts.
  const retainedCourseNames = new Map();
  for (const match of value.matches) {
    const courseId = safeText(firstDefined(match.courseId, match.course_id), 96);
    const tee = safeText(match.tee, 100);
    const name = safeText(firstDefined(match.courseName, match.course_name), 160);
    if (!courseId || !tee || !name || name === courseId) continue;
    const key = JSON.stringify([courseId, tee]);
    if (!retainedCourseNames.has(key)) retainedCourseNames.set(key, name);
    else if (retainedCourseNames.get(key) !== name) retainedCourseNames.set(key, null);
  }
  return Object.freeze({
    contractVersion,
    revision,
    approvedHandicapRevisionId: safeText(value.approvedHandicapRevisionId, 80),
    approvedHandicapRevisionNumber: safeInteger(value.approvedHandicapRevisionNumber),
    actor: Object.freeze({
    ...
    courses: Object.freeze(value.courses.map((course) => normalizeCourse(course, retainedCourseNames)).filter((item) => item.roundNumber > 0).sort((a, b) => a.roundNumber - b.roundNumber)),
...
export function normalizeProductionTournamentSetupMutation(payload = {}) {
  ...
  return Object.freeze({
    ...
    warnings: Object.freeze(Array.isArray(value.warnings) && value.warnings.includes("ODDS_PUBLICATION_REVIEW_REQUIRED")
      ? ["Published Odds remain unchanged and operationally stale. Review and replace them after real pairings are configured."]
      : []),
