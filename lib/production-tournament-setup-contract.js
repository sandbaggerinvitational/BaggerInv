...
export function canonicalTournamentSetupParticipants(value, format) {
  ...
  if (!Array.isArray(value) || value.length !== expected) {
    throw contractError(
      ...
      `${format} requires exactly ${expected} participants.`,
    );
  }
...
function normalizeMatch(value = {}) {
  ...
  return Object.freeze({
    ...
    participants: Object.freeze((Array.isArray(value.participants) ? value.participants : []).map(normalizeParticipant)),
    snapshot: Object.freeze({
      ...
      prepared: safeBoolean(firstDefined(value.snapshot?.prepared, value.snapshotPrepared, value.snapshot_prepared, value.prepared)),
    ...
    scoringReady: safeBoolean(firstDefined(
      value.scoringReady,
      value.scoring_ready,
