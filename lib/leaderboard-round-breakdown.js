...
export function participantRoundBreakdown(round, playerIds, points, tournament = {}) {
  const ids = (Array.isArray(playerIds) ? playerIds : [playerIds]).filter(Boolean);
  const match = (round?.matches || []).find((candidate) => ids.every((id) => includesPlayer(candidate, id)));
  ...
  if (!match || (!final && !live)) return { state: "pending", label: "Upcoming", segments: [], points: null };
  ...
  ].filter(([label]) => !singles || label === "Overall").map(([label, start, end, official]) => ({
    label,
  ...
  return {
    ...
    segments,
    points: final && points !== null && points !== undefined ? points : null,
  };
}

export function playerRoundBreakdown(round, playerId, officialPlayerResult, tournament = {}) {
  return participantRoundBreakdown(round, [playerId], officialPlayerResult?.points, tournament);
}
