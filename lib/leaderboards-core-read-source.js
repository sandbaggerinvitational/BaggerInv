...
export function leaderboardsCoreReadEnvironment(env = process.env) {
  const configured = clean(env.LEADERBOARDS_CORE_READ_SOURCE || "google").toLowerCase();
  ...
  const cutover = productionCutoverReadSourceEnvironment({
    env, variable: "LEADERBOARDS_CORE_READ_SOURCE", configuredValue: env.LEADERBOARDS_CORE_READ_SOURCE,
  ...
  return {
    ...
    reason: resolved === "supabase" ? (candidate.eligible ? "production-shadow-supabase-leaderboards-core" : "preview-supabase-leaderboards-core")
