...
export async function initializeTournamentWorkbook({
  ...
  let optional = {};
  if (optionalNames.length && readOptionalBatch) {
    try {
      optional = await readOptionalBatch(optionalNames);
    } catch {
      // A missing optional worksheet can invalidate a Google batch range. Fall
      // back to isolated reads so one unconfigured module never blocks startup.
    }
  }
  const unresolvedOptionalNames = optionalNames.filter((name) => !Object.hasOwn(optional, name));
  const optionalPairs = await Promise.all(unresolvedOptionalNames.map(async (name) => {
    try {
  ...
  optional = { ...optional, ...Object.fromEntries(optionalPairs) };
  const checks = {
