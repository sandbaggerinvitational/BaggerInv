...
export function mobileHistoryDetailData(view = {}) {
  ...
  const standingRows = boundedList(view.leaderboardRows, 128);
  if (standingRows.some((row) => !Object.hasOwn(row, "rank"))) {
    requireValue(standingRows.every((row, index) => number(row.points) !== null &&
      (index === 0 || number(standingRows[index - 1].points) >= number(row.points))));
  }
  const standings = addTournamentRanks(standingRows, "points")
    .map(({ tournamentRank, ...row }) => standingDto({ ...row,
      rank: Object.hasOwn(row, "rank") ? row.rank : Number(tournamentRank.replace(/^T/, "")) }));
...
export async function mobileHistoryResult(identity, { env = process.env, now, dependencies = {} } = {}) {
  if (clean(env.VERCEL_ENV).toLowerCase() === "production") {
    const runtime = await requireMobileProductionReadContext(identity, { env, dependencies, historical2026: true });
    const loadCompleted = dependencies.loadCompletedHistoryYears ||
      (await import("./completed-history-service.js")).loadCompletedHistoryYears;
    const loadCurrent = dependencies.loadHistory2026View ||
      (await import("./history-2026-service.js")).loadHistory2026View;
    let completed, current;
    try {
      [completed, current] = await Promise.all([
        loadCompleted({ env, dependencies: dependencies.completedHistoryDependencies }),
        loadCurrent({ env, tournamentId: identity.tournamentId, dependencies: dependencies.currentHistoryDependencies }),
      ]);
      requireValue(completed?.source === "supabase" && completed.views?.length === 9 &&
        completed.views.every((view, index) => view.source === "supabase" && Number(view.year) === 2017 + index) &&
        current?.source === "supabase" && Number(current.year) === 2026);
      const tournaments = [...completed.views, current].map(summaryFromView).sort((a, b) => b.year - a.year);
      await requireMobileProductionReadContext(identity, { env, dependencies, historical2026: true, expectedRuntime: runtime });
...
export function mobilePublicHistorySummary(view) { return summaryFromView(view); }
