...
/**
 * Keeps the accepted Round 2 scorecard evidence intact while replacing raw
 * team-source labels in participant-facing scorecard rows with canonical
 * year-scoped History team names.
 */
export function canonicalize2025ScrambleScorecardPresentation({
  ...
  return scorecards.map((scorecard) => {
    ...
    const summaryStrokes = canonicalMatch && resolvedSide
      ...
      : null;
    const summaryNetTotals = summaryStrokes !== null && Array.isArray(scorecard.holes)
      ? calculateNetTotals(scorecard.holes.map((hole) => {
    ...
    return {
      ...
      participantPlayerIds: context.participantIds.length
        ? context.participantIds
        : scorecard.participantPlayerIds,
      participantNames: context.participantNames.length
