...
function buildCompletedHistoryTournamentRecords({ year, scorecards = [], matches = [], teams = [] } = {}) {
  ...
  const birdiePopulation = individualEvidence.filter(statusAllowsEvidence);
  const birdiePopulationHoles = birdiePopulation.reduce((total, scorecard) =>
    total + (scorecard.holes || []).filter((hole) => finite(hole?.score) && finite(hole?.par)).length, 0);
  const birdieSample = targetYear === 2023
    ? `${birdiePopulation.length} individual rounds · ${birdiePopulationHoles} holes`
    : birdies.winners.length && birdieWinnerRounds.length === 1 && birdieWinnerHoles.length === 1
