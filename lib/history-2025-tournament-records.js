...
/**
 * Participant-facing Birdie provenance follows the complete canonical
 * individual population supplied to the Birdie calculation. The caller owns
 * the format/population selection; this helper only reports the same recorded
 * rounds and finite gross-hole/par observations without introducing another
 * scoring definition.
 */
export function buildHistoricalBirdiePopulationProvenance(scorecards = []) {
  const population = scorecards
    .filter(statusAllowsEvidence)
    .map((scorecard) => (scorecard?.holes || []).filter((hole) => finite(hole?.score) && finite(hole?.par)))
    .filter((holes) => holes.length > 0);
  const holes = population.reduce((total, scorecardHoles) => total + scorecardHoles.length, 0);
  return {
    rounds: population.length,
    holes,
    label: `${population.length} individual rounds · ${holes} holes`,
  };
}
...
function buildCompletedHistoryTournamentRecords({ year, scorecards = [], matches = [], teams = [] } = {}) {
  const targetYear = Number(year);
  const yearScorecards = deduplicateScorecards(
    scorecards.filter((scorecard) => Number(scorecard?.year) === targetYear)
  );
  ...
  const birdiePopulation = buildHistoricalBirdiePopulationProvenance(individualEvidence);
  const birdieSample = birdiePopulation.label;
  const birdieRecord = {
    key: "birdie-leader",
    label: "Birdie Leader",
    ...
    sampleSize: birdiePopulation.holes,
...
/** Frozen 2025 record contract. */
export function build2025TournamentRecords({ scorecards = [], matches = [], teams = [] } = {}) {
  return buildCompletedHistoryTournamentRecords({
    year: TARGET_YEAR,
    scorecards,
    matches,
    teams,
...
/**
 * Applies the frozen completed-year record populations to one explicitly
 * selected historical year without copying factual values across years.
 */
export function buildHistoricalTournamentRecords({ year, scorecards = [], matches = [], teams = [] } = {}) {
  return buildCompletedHistoryTournamentRecords({ year, scorecards, matches, teams });
}
