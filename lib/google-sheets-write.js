...
export async function updateDirectorNetSkins(input, updatedByValue) {
  ...
  if (!Number.isFinite(round) || round < 1) throw new Error("A configured tournament round is required.");
  const sheet = await requireTabHeaders("Net Skins", ["Year", "Round", "Player ID 1", "Player ID 2", "Eligible"]);
  const found = sheet.records.filter(({ record }) => String(record.Year) === year && Number(record.Round) === round && [record["Player ID 1"], record["Player ID 2"]].some((id) => String(id) === playerId));
  if (!found.length) throw new Error(`No Net Skins entry exists for this player in Round ${round}.`);
  ...
  await appendAdminAudit({ resource: "net-skins", recordId: `${playerId}:R${round}`, action: input.eligible ? "Eligible" : "Ineligible", previous: {}, next: { year, round, playerId, eligible: Boolean(input.eligible) }, updatedBy, summary: "Director Net Skins eligibility" });
  return true;
}
