...
export async function updatePlayerReadiness(session, input = {}) {
  ...
  if (typeof input.notificationsEnabled === "boolean") {
    updates["Notifications Enabled"] = input.notificationsEnabled ? "TRUE" : "FALSE";
  ...
  return {
    ...
    notificationsEnabled: truthy(updates["Notifications Enabled"] ?? current.record["Notifications Enabled"]),
  };
}
...
export async function readPlayerPassportMatches(session) {
  ...
  return {
    readiness: {
      ...
      notificationsEnabled: truthy(identity.device?.["Notifications Enabled"]),
    },
    player: {
      ...identity.player,
