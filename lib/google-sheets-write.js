...
import {
  ...
  recordGoogleWorkbookReadback,
  rejectGoogleWorkbookMutation,
...
function googleSheetsApiTarget() {
  const credential = googleServiceAccountCredentialDiagnostics();
  const scopedProductionWorkbook = ["production-worker", "legacy-canonical"].includes(
    credential?.credentialSource,
  )
    ? String(credential?.resources?.googleWorkbookId || "").trim()
    : "";
  const spreadsheetId = scopedProductionWorkbook || resolveSpreadsheetId();
  return {
    spreadsheetId,
    apiBase: `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}`,
  };
}
const archiveClean = (value) => String(value ?? "").replace(/\u0000/g, "").trim();
const cachedGoogleTokens = new Map();
const pendingGoogleTokens = new Map();
...
function mutationSheetNames(path, options) {
  const explicit = Array.isArray(options?.mutationSheets)
    ? options.mutationSheets.map((name) => String(name || "").trim()).filter(Boolean)
    : [];
  try {
    ...
    return [...new Set(explicit.map(rangeSheetName).filter(Boolean))];
  }
}
...
function credentials() {
  try {
    ...
    if (error?.code === "LEGACY_GOOGLE_CREDENTIALS_MISSING") {
      throw new Error("Google Sheets write credentials are not configured.");
    }
    throw error;
  }
}
...
async function accessToken() {
  ...
  const cached = cachedGoogleTokens.get(cacheKey);
  if (cached?.token && cached.expiresAt > Date.now() + 60_000) return cached.token;
  if (pendingGoogleTokens.has(cacheKey)) return pendingGoogleTokens.get(cacheKey);
  const pendingGoogleToken = (async () => {
  ...
  metric("httpRequests");
  recordDataAuthorityTransport("google", { adapter: "google-sheets-write", transport: "oauth" });
  ...
  return token;
  })().finally(() => { pendingGoogleTokens.delete(cacheKey); });
  pendingGoogleTokens.set(cacheKey, pendingGoogleToken);
...
async function google(path, options = {}) {
  ...
  delete transportOptions.mutationSheets;
  const diagnostics = writeDiagnostics.getStore();
  const perform = async () => {
    // Fail the exact attempted operation before credentials or network access.
    // Module imports remain side-effect free so Supabase-only candidates can
    // compile without weakening Preview workbook isolation.
    const target = googleSheetsApiTarget();
    const affectedSheets = method === "GET" ? [] : mutationSheetNames(path, options);
    const mutation = method === "GET" ? null : await prepareGoogleWorkbookMutation({
      spreadsheetId: target.spreadsheetId,
      method,
      path,
      affectedSheets,
    });
    if (target.spreadsheetId === PRODUCTION_GOOGLE_WORKBOOK_ID && mutation) {
      try {
        assertProductionGoogleServiceAccountMutationBinding({
    ...
    const credential = googleServiceAccountCredentialDiagnostics();
    const expectedCredentialSource = mutation?.intent ===
        GOOGLE_WORKBOOK_MUTATION_INTENTS.CANONICAL_LEGACY
      ? "legacy-canonical"
      : "production-worker";
    if (mutation?.productionWorkbook &&
        [GOOGLE_WORKBOOK_MUTATION_INTENTS.CANONICAL_LEGACY,
          GOOGLE_WORKBOOK_MUTATION_INTENTS.AUTHORING,
          GOOGLE_WORKBOOK_MUTATION_INTENTS.MIRROR_ARCHIVE].includes(mutation.intent) &&
        credential?.credentialSource !== expectedCredentialSource) {
      rejectGoogleWorkbookMutation();
      const error = new Error(mutation.intent === GOOGLE_WORKBOOK_MUTATION_INTENTS.CANONICAL_LEGACY
        ? "Production canonical legacy Google writes require the admission-bound legacy credential."
        : "Production authoring and mirror/archive writes require the dedicated Production credential.");
      error.code = mutation.intent === GOOGLE_WORKBOOK_MUTATION_INTENTS.CANONICAL_LEGACY
        ? "PRODUCTION_GOOGLE_CANONICAL_LEGACY_CREDENTIAL_REQUIRED"
        : "PRODUCTION_GOOGLE_DEDICATED_CREDENTIAL_REQUIRED";
      error.status = 503;
      throw error;
    }
    let token;
    try {
      ...
      if (method !== "GET") rejectGoogleWorkbookMutation();
      throw error;
    }
    metric("httpRequests");
    ...
    recordDataAuthorityTransport("google", {
      adapter: "google-sheets-write",
      transport: "sheets",
      writer: method !== "GET",
    });
    try {
      // OAuth completes before the durable WRITE_STARTED boundary. A request
      // paused during token acquisition therefore remains merely ADMITTED and
      // can be closed/expired without becoming an invisible provider writer.
      await mutation?.prepareDispatch?.();
    } catch (error) {
      if (method !== "GET") {
        if (error?.capabilityDiagnostics?.writeStarted === true ||
            error?.authorityDiagnostics?.writeStartOutcomeUnknown === true) {
    ...
    let responsePromise;
    try {
      // No await may be introduced between these final synchronous assertions
      // and invoking fetch. A detached request whose wrapper revoked its
      // capability while token acquisition or WRITE_STARTED was pending must
      // never dispatch.
      if (mutation?.productionWorkbook) {
        assertProductionGoogleServiceAccountMutationBinding({
          intent: mutation.intent,
      ...
      mutation?.assertDispatch?.();
      responsePromise = fetch(`${target.apiBase}${path}`, { ...transportOptions, headers: { authorization: `Bearer ${token}`, "content-type": "application/json", ...(transportOptions.headers || {}) }, cache: "no-store", signal: transportOptions.signal || AbortSignal.timeout(10_000) });
    } catch (error) {
      if (method !== "GET") rejectGoogleWorkbookMutation();
      throw error;
    }
    ...
    try {
      ...
      if (method !== "GET") markGoogleWorkbookMutationAmbiguous(error);
      throw error;
    }
    if (!response.ok) {
    ...
    if (method === "GET") recordGoogleWorkbookReadback({ path, payload });
    return payload;
  };
...
/**
 * Read-only Google-native recovery metadata. Editor identities, cell values,
 * credentials, and protected-range principals are deliberately excluded.
 */
export async function readWorkbookNativeMetadataSnapshot() {
  const book = await readWorkbookMetadata();
  return {
    spreadsheetId: String(book.spreadsheetId || ""),
    properties: {
      title: String(book.properties?.title || ""),
      locale: String(book.properties?.locale || ""),
      timeZone: String(book.properties?.timeZone || ""),
    },
    namedRanges: (book.namedRanges || []).map((entry) => ({
      namedRangeId: String(entry.namedRangeId || ""),
      name: String(entry.name || ""),
      range: safeGridRange(entry.range),
    })),
    sheets: (book.sheets || []).map((sheet) => ({
      properties: {
        ...
        gridProperties: {
          ...
          frozenColumnCount: Number(sheet.properties?.gridProperties?.frozenColumnCount || 0),
          hideGridlines: sheet.properties?.gridProperties?.hideGridlines === true,
        },
      },
      protectedRanges: (sheet.protectedRanges || []).map((entry) => ({
        protectedRangeId: Number(entry.protectedRangeId),
        description: String(entry.description || ""),
        warningOnly: entry.warningOnly === true,
        range: safeGridRange(entry.range),
      })),
      filterViews: (sheet.filterViews || []).map((entry) => ({
        filterViewId: Number(entry.filterViewId),
        title: String(entry.title || ""),
        range: safeGridRange(entry.range),
      })),
      basicFilter: sheet.basicFilter ? { range: safeGridRange(sheet.basicFilter.range) } : null,
      merges: (sheet.merges || []).map(safeGridRange),
    })),
  };
}
...
export async function initializePreviewParticipantIdentityConfiguration(updatedByValue, { tournamentId, players = [] } = {}) {
  ...
  if (!before.sheetExists) {
    await google(":batchUpdate", { method: "POST", mutationSheets: ["Participant Identity Configuration"], body: JSON.stringify({ requests: [{
  ...
  await google(":batchUpdate", { method: "POST", mutationSheets: ["Participant Identity Configuration"], body: JSON.stringify({ requests: [{ addProtectedRange: { protectedRange: {
...
export async function migratePreviewLiveMatchScoringLock({ expectedFingerprint = "" } = {}) {
  ...
  if (!before.lockPresent) {
    ...
    await google(":batchUpdate", { method: "POST", mutationSheets: ["Live Matches"], body: JSON.stringify({ requests, includeSpreadsheetInResponse: false }) });
  }
...
async function provisionRoundScorecardsFormulas(grid, assignments) {
  ...
  metric("workbookWrites");
  await google(":batchUpdate", { method: "POST", mutationSheets: ["Round Scorecards"], body: JSON.stringify({ requests, includeSpreadsheetInResponse: false }) });
...
const FUTURE_MATCH_COMPATIBILITY_CONTRACT = "production-future-match-google-compatibility-v2";
const FUTURE_MATCH_LIVE_CONFIGURATION_FIELDS = Object.freeze([
  "Year", "Round", "Format", "Match", "Course ID", "Tee Time", "Starting Hole",
  ...LIVE_PAIRING_FIELDS,
]);
const FUTURE_MATCH_ARCHIVE_CONFIGURATION_FIELDS = Object.freeze([
  "Year", "Round", "Format", "Match", ...LIVE_PAIRING_FIELDS,
  "Course ID", "Tee Time", "Starting Hole",
]);
const FUTURE_MATCH_RESULT_FIELDS = Object.freeze([
  "Matchup Winner", "Front 9 Winner", "Back 9 Winner", "18-Hole Winner",
  "Team 1 Points", "Team 2 Points", "Current Hole", "Team 1 Holes Won",
  "Team 2 Holes Won", "Holes Remaining", "Match Status Text", "Final Result",
  "Winner", "Completed At", "Finalized At", "Finalized By",
]);
const FUTURE_MATCH_ACCESS_SECRET_FIELDS = Object.freeze([
  "Access Code Hash", "Access Token Hash", "Access Selector", "Access Expires At",
]);

const firstCompatibilityValue = (...values) => values.find((value) =>
  value !== undefined && value !== null && archiveClean(value) !== "");

function futureMatchPostgresJsonbKeyOrder(left, right) {
  const leftBytes = Buffer.from(left);
  const rightBytes = Buffer.from(right);
  return leftBytes.length - rightBytes.length || Buffer.compare(leftBytes, rightBytes);
}

/** Exact PostgreSQL `jsonb::text` for this bounded compatibility manifest. */
export function futureMatchGoogleCompatibilityPostgresJsonbText(value) {
  if (value === null) return "null";
  if (Array.isArray(value)) {
    return `[${value.map(futureMatchGoogleCompatibilityPostgresJsonbText).join(", ")}]`;
  }
  if (typeof value === "object") {
    return `{${Object.entries(value)
      .sort(([left], [right]) => futureMatchPostgresJsonbKeyOrder(left, right))
      .map(([key, item]) => `${JSON.stringify(key)}: ${futureMatchGoogleCompatibilityPostgresJsonbText(item)}`)
      .join(", ")}}`;
  }
  ...
  throw new TypeError("Future match compatibility evidence must be an exact JSON value.");
}

function futureMatchCompatibilityHash(value) {
  return createHash("sha256")
    .update(futureMatchGoogleCompatibilityPostgresJsonbText(value))
    .digest("hex");
}

/** Fingerprint the exact database-owned provisioning manifest. */
export function futureMatchGoogleCompatibilityManifestFingerprint(value) {
  return futureMatchCompatibilityHash(value);
}

/**
 * The audited Production Live Matches workbook does not derive Match ID with a
 * formula. Future-match provisioning is the sole exception that may seed this
 * protected stable identifier, and only into a physically empty cell.
 */
export function futureMatchGoogleCompatibilityLiveMatchIdValue(currentValue, matchId) {
  if (currentValue !== undefined && currentValue !== null && currentValue !== "") {
    throw Object.assign(new Error("The future Live Matches Match ID cell is not exactly blank."), {
      code: "FUTURE_MATCH_GOOGLE_COMPATIBILITY_MATCH_ID_NOT_BLANK",
    });
  }
  const canonicalMatchId = archiveClean(matchId);
  if (!/^\d{4}-R\d{1,2}-\d{1,2}$/.test(canonicalMatchId)) {
    throw Object.assign(new Error("The future Live Matches Match ID is not canonical."), {
      code: "FUTURE_MATCH_GOOGLE_COMPATIBILITY_MATCH_ID_INVALID",
    });
  }
  return canonicalMatchId;
}

function futureMatchParticipantId(source, side, slot) {
  const participants = Array.isArray(source.participants) ? source.participants
    : Array.isArray(source.match_participants) ? source.match_participants
    : Array.isArray(source.matchParticipants) ? source.matchParticipants : [];
  const participant = participants.find((item) =>
    Number(item?.team_side ?? item?.teamSide) === side &&
    Number(item?.player_slot ?? item?.playerSlot) === slot);
  return archiveClean(firstCompatibilityValue(
    participant?.player_id,
    participant?.playerId,
    source[`team_${side}_player_${slot}`],
    source[`team${side}Player${slot}`],
    source[`Team ${side} Player ${slot}`],
  ));
}

/**
 * Normalize the database-owned compatibility manifest into the only fields the
 * downstream workbook provisioner may write. Result, finalization, access
 * secret, and score fields are intentionally absent.
 */
export function futureMatchGoogleCompatibilityProjection(input = {}) {
  const embeddedConfiguration = input.liveMatch && typeof input.liveMatch === "object"
    ? input.liveMatch
    : input.live_match && typeof input.live_match === "object" ? input.live_match : {};
  const source = input.match && typeof input.match === "object"
    ? { ...input, ...embeddedConfiguration, ...input.match }
    : { ...input, ...embeddedConfiguration };
  const tournament = input.tournament && typeof input.tournament === "object" ? input.tournament : {};
  const year = Number(firstCompatibilityValue(
    source.tournament_year, source.tournamentYear, source.year, source.Year,
    tournament.tournament_year, tournament.tournamentYear, tournament.year,
  ));
  const round = Number(firstCompatibilityValue(
    source.round_number, source.roundNumber, source.round, source.Round,
  ));
  const matchNumber = Number(firstCompatibilityValue(
    source.match_number, source.matchNumber, source.display_number, source.displayNumber,
    source.match, source.Match,
  ));
  const matchId = archiveClean(firstCompatibilityValue(source.match_id, source.matchId, input.match_id, input.matchId));
  const format = archiveClean(firstCompatibilityValue(
    source.format, source.Format, source.round_format, source.roundFormat,
  )).toUpperCase();
  const courseId = archiveClean(firstCompatibilityValue(
    source.course_id, source.courseId, source["Course ID"], input.course_id, input.courseId,
  ));
  const teeTime = archiveClean(firstCompatibilityValue(
    source.tee_time, source.teeTime, source["Tee Time"],
  ));
  const startingHole = Number(firstCompatibilityValue(
    source.starting_hole, source.startingHole, source["Starting Hole"], 1,
  ));
  const templateMatchId = archiveClean(firstCompatibilityValue(
    input.template_match_id, input.templateMatchId,
    input.google_template_match_id, input.googleTemplateMatchId,
  ));
  const templateLiveMatchId = archiveClean(firstCompatibilityValue(
    input.template_live_match_id, input.templateLiveMatchId, templateMatchId,
  ));
  const templateArchiveMatchId = archiveClean(firstCompatibilityValue(
    input.template_archive_match_id, input.templateArchiveMatchId, templateMatchId,
  ));
  const writerGenerationId = archiveClean(firstCompatibilityValue(
    input.writer_generation_id, input.writerGenerationId,
  )).toLowerCase();
  const destinationWorkbookId = archiveClean(firstCompatibilityValue(
    input.destination_workbook_id, input.destinationWorkbookId,
  ));
  const targetContractFingerprint = archiveClean(firstCompatibilityValue(
    input.target_contract_fingerprint, input.targetContractFingerprint,
  )).toLowerCase();
  const structuralFingerprint = archiveClean(firstCompatibilityValue(
    input.structural_fingerprint, input.structuralFingerprint,
  )).toLowerCase();
  const runtimeRevision = Number(firstCompatibilityValue(
    input.runtime_revision, input.runtimeRevision,
  ));
  if (!Number.isInteger(year) || year < 2000 || year > 9999 ||
      !Number.isInteger(round) || round < 1 || round > 99 ||
      !Number.isInteger(matchNumber) || matchNumber < 1 || matchNumber > 99 ||
      !["BB", "SC", "SI"].includes(format) || !courseId ||
      !Number.isInteger(startingHole) || startingHole < 1 || startingHole > 18) {
    throw Object.assign(new Error("The future match compatibility manifest is incomplete."), {
      code: "FUTURE_MATCH_GOOGLE_COMPATIBILITY_MANIFEST_INVALID",
    });
  }
  if (matchId !== `${year}-R${round}-${matchNumber}`) {
    throw Object.assign(new Error("The future Match ID does not match its canonical year, round, and match number."), {
      code: "FUTURE_MATCH_GOOGLE_COMPATIBILITY_MATCH_ID_INVALID",
    });
  }
  if (!templateLiveMatchId || !templateArchiveMatchId ||
      templateLiveMatchId === matchId || templateArchiveMatchId === matchId) {
    throw Object.assign(new Error("Certified Google formula-template Match IDs are required."), {
      code: "FUTURE_MATCH_GOOGLE_COMPATIBILITY_TEMPLATE_REQUIRED",
    });
  }
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/.test(writerGenerationId) ||
      !destinationWorkbookId || !/^[0-9a-f]{64}$/.test(targetContractFingerprint) ||
      !/^[0-9a-f]{64}$/.test(structuralFingerprint) ||
      !Number.isInteger(runtimeRevision) || runtimeRevision < 1) {
    throw Object.assign(new Error("The certified future Google writer binding is incomplete."), {
      code: "FUTURE_MATCH_GOOGLE_WRITER_BINDING_INVALID",
    });
  }
  const players = Object.fromEntries(LIVE_PAIRING_FIELDS.map((field) => {
    const match = field.match(/^Team (\d) Player (\d)$/);
    return [field, futureMatchParticipantId(source, Number(match[1]), Number(match[2]))];
  }));
  const requiredPlayerFields = format === "SI"
    ? ["Team 1 Player 1", "Team 2 Player 1"] : LIVE_PAIRING_FIELDS;
  if (requiredPlayerFields.some((field) => !players[field]) ||
      (format === "SI" && (players["Team 1 Player 2"] || players["Team 2 Player 2"])) ||
      new Set(Object.values(players).filter(Boolean)).size !== Object.values(players).filter(Boolean).length) {
    throw Object.assign(new Error("The future match compatibility pairing is not complete and unique for its format."), {
      code: "FUTURE_MATCH_GOOGLE_COMPATIBILITY_PAIRING_INVALID",
    });
  }
  const configuration = {
    Year: year,
  ...
  const projection = {
    contractVersion: FUTURE_MATCH_COMPATIBILITY_CONTRACT,
    tournamentId: archiveClean(firstCompatibilityValue(
      source.tournament_id, source.tournamentId, tournament.tournament_id, tournament.tournamentId, String(year),
    )),
    tournamentYear: year,
    matchId,
    templateLiveMatchId,
    ...
    runtimeRevision,
    liveMatch: Object.fromEntries(
      FUTURE_MATCH_LIVE_CONFIGURATION_FIELDS.map((field) => [field, configuration[field] ?? ""]),
    ),
    archiveMatch: Object.fromEntries(
      FUTURE_MATCH_ARCHIVE_CONFIGURATION_FIELDS.map((field) => [field, configuration[field] ?? ""]),
    ),
  };
  return Object.freeze({ ...projection, manifestFingerprint: futureMatchCompatibilityHash(projection) });
}

function compatibilityFieldEqual(field, actual, expected) {
  if (["Year", "Round", "Match", "Starting Hole", "Access Version"].includes(field)) {
    return Number(actual) === Number(expected);
  }
  if (["Scoring Locked", "Access Active"].includes(field)) return truthy(actual) === truthy(expected);
  return archiveClean(actual).toLowerCase() === archiveClean(expected).toLowerCase();
}

function compatibilityRowHasNoFacts(record = {}) {
  return [...FUTURE_MATCH_RESULT_FIELDS, ...FUTURE_MATCH_ACCESS_SECRET_FIELDS]
    .every((field) => archiveClean(record[field]) === "");
}

async function inspectFutureMatchCompatibilityGrid(tab) {
  const book = await readWorkbookMetadata();
  const metadata = (book.sheets || []).find((item) => archiveClean(item?.properties?.title) === tab);
  if (!metadata) throw Object.assign(new Error(`Workbook schema is missing required sheet '${tab}'.`), {
    code: "FUTURE_MATCH_GOOGLE_COMPATIBILITY_SHEET_MISSING",
  });
  ...
  const computed = computedResponse.values || [];
  const formulas = formulaResponse.values || [];
  const width = Math.max(computed[0]?.length || 0, formulas[0]?.length || 0);
  const headers = Array.from({ length: width }, (_value, index) =>
    archiveClean(computed[0]?.[index] ?? formulas[0]?.[index]));
  const required = tab === "Live Matches"
    ? ["Match ID", ...FUTURE_MATCH_LIVE_CONFIGURATION_FIELDS, "Match Status", "Scoring Locked", "Access Active", "Access Version"]
    : ["Match ID", ...FUTURE_MATCH_ARCHIVE_CONFIGURATION_FIELDS];
  validateSheetSchema(tab, headers, required);
  const physicalRowCount = Math.max(
    Number(metadata.properties?.gridProperties?.rowCount || 0), computed.length, formulas.length, 2,
  );
  const protectedSet = new Set(protectedFields(tab));
  const writable = writableFields(tab).filter((field) => headers.includes(field));
  const rows = [];
  for (let rowNumber = 2; rowNumber <= physicalRowCount; rowNumber += 1) {
    const computedRow = computed[rowNumber - 1] || [];
    const formulaRow = formulas[rowNumber - 1] || [];
    const record = Object.fromEntries(headers.flatMap((header, index) =>
      header ? [[header, computedRow[index] ?? ""]] : []));
    const rowFormulas = Object.fromEntries(headers.flatMap((header, index) =>
      header && protectedSet.has(header) && /^=/.test(archiveClean(formulaRow[index]))
        ? [[header, archiveClean(formulaRow[index])]] : []));
    const rawValues = Object.fromEntries(headers.flatMap((header, index) =>
      header ? [[header, formulaRow[index] ?? ""]] : []));
    rows.push({
      rowNumber,
      record,
      formulas: rowFormulas,
      rawValues,
      writableBlank: writable.every((field) => archiveClean(record[field]) === ""),
    });
  }
  return { tab, headers, sheetId: Number(metadata.properties.sheetId), rows };
}

function certifiedFormulaTemplate(grid, templateMatchId) {
  const matches = grid.rows.filter((row) => archiveClean(row.record["Match ID"]) === templateMatchId);
  if (matches.length !== 1) throw Object.assign(new Error(`Certified ${grid.tab} formula template was not unique.`), {
    code: "FUTURE_MATCH_GOOGLE_COMPATIBILITY_TEMPLATE_INVALID",
  });
  const template = matches[0];
  const formulaPurposes = new Set([
    COLUMN_PURPOSE.FORMULA,
    COLUMN_PURPOSE.ARRAY_FORMULA,
    COLUMN_PURPOSE.LOOKUP,
  ]);
  const requiredFormulas = grid.headers.filter((field) =>
    formulaPurposes.has(columnPurpose(grid.tab, field)));
  if (requiredFormulas.some((field) => !/^=/.test(template.formulas[field] || ""))) {
    throw Object.assign(new Error(`Certified ${grid.tab} formula template is incomplete.`), {
      code: "FUTURE_MATCH_GOOGLE_COMPATIBILITY_TEMPLATE_FORMULA_MISSING",
    });
  }
  return { template, formulaFields: requiredFormulas };
}

function compatibilityNonProjectionFieldsBlank(tab, row, expected) {
  return writableFields(tab)
    .filter((field) => Object.hasOwn(row.record, field) && !Object.hasOwn(expected.fields, field))
    .every((field) => archiveClean(row.record[field]) === "");
}

function compatibilityRowVerification(tab, row, expected, formulaFields) {
  if (!row || archiveClean(row.record["Match ID"]) !== expected.matchId ||
      !compatibilityRowHasNoFacts(row.record) ||
      !compatibilityNonProjectionFieldsBlank(tab, row, expected) ||
      formulaFields.some((field) => !/^=/.test(row.formulas[field] || ""))) return false;
  return Object.entries(expected.fields).every(([field, value]) =>
    compatibilityFieldEqual(field, row.record[field], value));
}

function compatibilityPartialRowCanResume(tab, row, expected) {
  return row && archiveClean(row.record["Match ID"]) === expected.matchId &&
    compatibilityRowHasNoFacts(row.record) &&
    compatibilityNonProjectionFieldsBlank(tab, row, expected) &&
    Object.entries(expected.fields).every(([field, value]) =>
      archiveClean(row.record[field]) === "" || compatibilityFieldEqual(field, row.record[field], value));
}

async function writeFutureMatchCompatibilityLiveMatchId(grid, target, matchId) {
  if (grid.tab !== "Live Matches") {
    throw Object.assign(new Error("The protected Match ID seed is restricted to Live Matches."), {
      code: "FUTURE_MATCH_GOOGLE_COMPATIBILITY_MATCH_ID_WRITE_FORBIDDEN",
    });
  }
  const columnIndex = grid.headers.indexOf("Match ID");
  if (columnIndex < 0) throw Object.assign(new Error("Live Matches is missing Match ID."), {
    code: "FUTURE_MATCH_GOOGLE_COMPATIBILITY_SHEET_MISSING",
  });
  const value = futureMatchGoogleCompatibilityLiveMatchIdValue(
    target.rawValues?.["Match ID"],
    matchId,
  );
  metric("workbookWrites");
  await google("/values:batchUpdate", {
    method: "POST",
    mutationSheets: ["Live Matches"],
    body: JSON.stringify({
      valueInputOption: "RAW",
      data: [{
        range: `Live Matches!${columnName(columnIndex)}${target.rowNumber}`,
        values: [[value]],
      }],
    }),
  });
  invalidateWorkbookReadSnapshots(["Live Matches"]);
  const readback = await verifyDirectorReadBack({
    delays: [0, 250, 750, 1500],
    invalidate: async () => invalidateWorkbookReadSnapshots(["Live Matches"]),
    read: () => inspectFutureMatchCompatibilityGrid("Live Matches"),
    verify: (fresh) => {
      const row = fresh.rows.find((item) => item.rowNumber === target.rowNumber);
      return row?.rawValues?.["Match ID"] === value &&
        archiveClean(row?.record?.["Match ID"]) === value &&
        compatibilityRowHasNoFacts(row?.record);
    },
    summarize: (fresh) => ({
      targetRowMatches: fresh.rows.filter((item) =>
        item.rowNumber === target.rowNumber && archiveClean(item.record["Match ID"]) === value).length,
    }),
  });
  if (!readback.success) {
    throw Object.assign(new Error("Live Matches Match ID seed did not verify."), {
      code: "FUTURE_MATCH_GOOGLE_COMPATIBILITY_MATCH_ID_READBACK_MISMATCH",
      verificationAttempts: readback.attempts,
    });
  }
  return readback.data.rows.find((item) => item.rowNumber === target.rowNumber);
}

async function copyFutureMatchCompatibilityFormulas(grid, template, target, formulaFields) {
  const requests = formulaFields.map((field) => {
    const columnIndex = grid.headers.indexOf(field);
    return { copyPaste: {
      source: {
        sheetId: grid.sheetId,
        startRowIndex: template.rowNumber - 1,
        endRowIndex: template.rowNumber,
  ...
  if (!requests.length) throw Object.assign(new Error(`${grid.tab} has no certified protected formulas to copy.`), {
    code: "FUTURE_MATCH_GOOGLE_COMPATIBILITY_TEMPLATE_FORMULA_MISSING",
  });
  metric("workbookWrites");
  await google(":batchUpdate", {
    method: "POST",
    mutationSheets: [grid.tab],
    body: JSON.stringify({ requests, includeSpreadsheetInResponse: false }),
  });
}

async function provisionFutureMatchCompatibilityRow({ tab, expected, templateMatchId }) {
  let grid = await inspectFutureMatchCompatibilityGrid(tab);
  const { template, formulaFields } = certifiedFormulaTemplate(grid, templateMatchId);
  const existing = grid.rows.filter((row) => archiveClean(row.record["Match ID"]) === expected.matchId);
  if (existing.length > 1) throw Object.assign(new Error(`${tab} contains duplicate future Match IDs.`), {
    code: "FUTURE_MATCH_GOOGLE_COMPATIBILITY_DUPLICATE",
  });
  if (existing.length) {
    if (compatibilityRowVerification(tab, existing[0], expected, formulaFields)) {
      return { row: existing[0], formulaFields, idempotent: true };
    }
    if (!compatibilityPartialRowCanResume(tab, existing[0], expected)) {
      throw Object.assign(new Error(`${tab} already contains a conflicting future match row.`), {
        code: "FUTURE_MATCH_GOOGLE_COMPATIBILITY_EXISTING_ROW_CONFLICT",
      });
    }
    const target = existing[0];
    await copyFutureMatchCompatibilityFormulas(grid, template, target, formulaFields);
    invalidateWorkbookReadSnapshots([tab]);
    await writeSheetFields(tab, grid.headers, target.rowNumber, expected.fields);
  } else {
    let target = grid.rows.find((row) => row.rowNumber !== template.rowNumber && row.writableBlank &&
      compatibilityRowHasNoFacts(row.record) && (tab !== "Live Matches" ||
        (row.rawValues?.["Match ID"] === "" && !row.formulas["Match ID"])) &&
      ["", "-R-"].includes(archiveClean(row.record["Match ID"])));
    if (!target) throw Object.assign(new Error(`${tab} has no safe empty physical row for future match compatibility.`), {
      code: "FUTURE_MATCH_GOOGLE_COMPATIBILITY_CAPACITY_EXHAUSTED",
    });
    if (tab === "Live Matches") {
      target = await writeFutureMatchCompatibilityLiveMatchId(grid, target, expected.matchId);
      grid = await inspectFutureMatchCompatibilityGrid(tab);
      target = grid.rows.find((row) => row.rowNumber === target.rowNumber);
    }
    await copyFutureMatchCompatibilityFormulas(grid, template, target, formulaFields);
    invalidateWorkbookReadSnapshots([tab]);
    await writeSheetFields(tab, grid.headers, target.rowNumber, expected.fields);
  }
  const verified = await verifyDirectorReadBack({
    delays: [0, 250, 750, 1500, 3000],
    invalidate: async () => invalidateWorkbookReadSnapshots([tab]),
    read: () => inspectFutureMatchCompatibilityGrid(tab),
    verify: (fresh) => {
      const matches = fresh.rows.filter((row) => archiveClean(row.record["Match ID"]) === expected.matchId);
      return matches.length === 1 && compatibilityRowVerification(tab, matches[0], expected, formulaFields);
    },
    summarize: (fresh) => ({
      matchRows: fresh.rows.filter((row) => archiveClean(row.record["Match ID"]) === expected.matchId).length,
    }),
  });
  if (!verified.success) throw Object.assign(new Error(`${tab} future match compatibility readback did not verify.`), {
    code: "FUTURE_MATCH_GOOGLE_COMPATIBILITY_READBACK_MISMATCH",
    verificationAttempts: verified.attempts,
  });
  const row = verified.data.rows.find((item) => archiveClean(item.record["Match ID"]) === expected.matchId);
  return { row, formulaFields, idempotent: false };
}

/**
 * Provision the downstream, non-authoritative Google compatibility rows for a
 * canonical Supabase future match. Partial provider success is intentionally
 * not rolled back: an exact retry recognizes the certified row and continues.
 */
export async function provisionFutureMatchGoogleCompatibility(input = {}) {
  requireIsolatedScoringSheet();
  const projection = futureMatchGoogleCompatibilityProjection(input);
  const live = await provisionFutureMatchCompatibilityRow({
    tab: "Live Matches",
    expected: { matchId: projection.matchId, fields: projection.liveMatch },
    templateMatchId: projection.templateLiveMatchId,
  });
  const archive = await provisionFutureMatchCompatibilityRow({
    tab: "Matches",
    expected: { matchId: projection.matchId, fields: projection.archiveMatch },
    templateMatchId: projection.templateArchiveMatchId,
  });
  const readback = {
    contractVersion: projection.contractVersion,
    tournamentId: projection.tournamentId,
    tournamentYear: projection.tournamentYear,
    ...
    liveMatches: {
      rowNumber: live.row.rowNumber,
      formulaFields: [...live.formulaFields].sort(),
      projection: projection.liveMatch,
    },
    matches: {
      rowNumber: archive.row.rowNumber,
      formulaFields: [...archive.formulaFields].sort(),
      projection: projection.archiveMatch,
    },
  };
  return {
    pass: true,
    idempotent: live.idempotent && archive.idempotent,
    ...readback,
    googleReadbackFingerprint: futureMatchCompatibilityHash(readback),
  };
}
...
function requireIsolatedScoringSheet() {
  const credential = googleServiceAccountCredentialDiagnostics();
  if (["production-worker", "legacy-canonical"].includes(credential.credentialSource)) {
    const productionOperation = credential.credentialSource === "legacy-canonical"
      ? credential.operation === "CANONICAL_LEGACY_V2" && credential.policy?.canonicalLegacy === true
      : [
        "GOOGLE_DIRECTOR_AUTHORING",
        "SCORING_GOOGLE_OUTBOX",
        "FUTURE_MATCH_GOOGLE_COMPATIBILITY",
        "ROUND_SCORECARDS_ARCHIVE",
      ].includes(credential.operation);
    if (!credential.allowed || !credential.policy?.googleWrite || !productionOperation) {
      const error = new Error("The active Production Google credential context is not authorized for scoring writes.");
      error.code = "PRODUCTION_GOOGLE_WRITE_CONTEXT_FORBIDDEN";
      throw error;
    }
    return credential;
  }
...
/** @deprecated Retained for external Preview operator compatibility only. */
...
/** @deprecated Active routes use participant authorization contracts. */
...
async function performLiveHoleScoreSave(matchIdValue, input = {}, updatedByValue) {
  ...
  const verifiedHole = verification.data["Live Hole Scores"].records.find(({ record }) =>
    String(record["Match ID"] || "").trim() === matchId && Number(record["Hole Number"]) === holeNumber
  )?.record;
  const verifiedMatch = verification.data["Live Matches"].records.find(({ record }) =>
    String(record["Match ID"] || "").trim() === matchId
  )?.record;
  const scoreProof = ({ holeRecord, matchRecord }) => ({
    matchId,
    holeNumber,
    team1GrossScores: grossScoresFromCell(holeRecord?.["Team 1 Gross Scores"]),
    team2GrossScores: grossScoresFromCell(holeRecord?.["Team 2 Gross Scores"]),
    team1NetScore: Number(holeRecord?.["Team 1 Net Score"]),
    team2NetScore: Number(holeRecord?.["Team 2 Net Score"]),
    winner: String(holeRecord?.["Hole Winner"] || ""),
    revision: Number(holeRecord?.Revision || 0),
    updatedAt: String(holeRecord?.["Updated At"] || ""),
    matchUpdatedAt: String(matchRecord?.["Updated At"] || ""),
  });
  certifyGoogleWorkbookMutationReadback({
    proofType: "LIVE_HOLE_SCORE",
    before: scoreProof({ holeRecord: existingRecord, matchRecord: match }),
    expectedAfter: scoreProof({ holeRecord: next, matchRecord: { "Updated At": updatedAt } }),
    providerReadback: scoreProof({ holeRecord: verifiedHole, matchRecord: verifiedMatch }),
  });
...
export async function confirmLiveMatchScorecard(matchIdValue, updatedByValue) {
  requireIsolatedScoringSheet();
  const matchId = String(matchIdValue || "").trim();
  const sheets = await readSheets(["Live Matches", "Live Hole Scores", "Matches"]);
  const matches = sheets["Live Matches"];
  const holes = sheets["Live Hole Scores"];
  requireHeaders(holes, "Live Hole Scores", LIVE_HOLE_HEADERS);
  const match = findUniqueMatch(matches, matchId).record;
  ...
  const verifiedSheets = await readSheets(["Live Matches", "Matches"]);
  ...
  const previousArchive = sheets.Matches.records.find(({ record }) =>
    String(record["Match ID"] || "").trim() === matchId
  )?.record || null;
  const liveProof = (record) => ({
    matchId: String(record?.["Match ID"] || ""),
    status: String(record?.["Match Status"] || ""),
    matchupWinner: String(record?.["Matchup Winner"] || ""),
    frontWinner: String(record?.["Front 9 Winner"] || ""),
    backWinner: String(record?.["Back 9 Winner"] || ""),
    overallWinner: String(record?.["18-Hole Winner"] || ""),
    team1Points: Number(record?.["Team 1 Points"]),
    team2Points: Number(record?.["Team 2 Points"]),
    finalizedAt: String(record?.["Finalized At"] || ""),
  });
  const expectedArchive = {
    ...verifiedArchive,
    "Match ID": matchId,
    "Match Status": "Final",
    "Matchup Winner": finalized["Matchup Winner"],
    "Front 9 Winner": finalized["Front 9 Winner"],
    "Back 9 Winner": finalized["Back 9 Winner"],
    "18-Hole Winner": finalized["18-Hole Winner"],
    "Team 1 Points": finalized["Team 1 Points"],
    "Team 2 Points": finalized["Team 2 Points"],
    "Finalized At": finalized["Finalized At"],
  };
  certifyGoogleWorkbookMutationReadback({
    proofType: "FINALIZE_SCORECARD",
    before: { live: liveProof(match), archive: previousArchive ? liveProof(previousArchive) : null },
    expectedAfter: { live: liveProof(finalized), archive: liveProof(expectedArchive) },
    providerReadback: { live: liveProof(verifiedLive), archive: liveProof(verifiedArchive) },
  });
  return {
    ...finalized,
    _shadow: {
...
/**
 * Applies the narrow, non-canonical Google representation of a completed
 * Supabase control mutation. This intentionally cannot create or rotate
 * scoring credentials; it only mirrors the already-committed canonical
 * lifecycle/lock/access projection and verifies a fresh readback.
 */
export async function mirrorCanonicalLiveMatchControl(matchIdValue, projection = {}, updatedByValue) {
  requireIsolatedScoringSheet();
  const matchId = String(matchIdValue ?? "").trim();
  if (!matchId) throw Object.assign(new Error("Match ID is required."), { code: "GOOGLE_CONTROL_MATCH_REQUIRED" });
  const updatedBy = editorName(updatedByValue);
  const sheet = await readSheet("Live Matches");
  requireHeaders(sheet, "Live Matches", [
    "Match ID", "Match Status", "Scoring Locked", "Access Active", "Access Version",
    "Updated At", "Updated By",
  ]);
  const current = findUniqueMatch(sheet, matchId);
  const expectedUpdatedAt = String(projection.expectedUpdatedAt || "").trim();
  const updates = {};
  if (projection.status != null) {
    const status = String(projection.status).trim();
    if (!/^(Upcoming|Scheduled|Live|Reopened|Final)$/i.test(status)) {
      throw Object.assign(new Error("The canonical match status is not mirrorable."), { code: "GOOGLE_CONTROL_STATUS_INVALID" });
    }
    updates["Match Status"] = /^upcoming$/i.test(status) ? "Scheduled"
      : status.charAt(0).toUpperCase() + status.slice(1).toLowerCase();
  }
  if (projection.scoringLocked != null) updates["Scoring Locked"] = projection.scoringLocked ? "TRUE" : "FALSE";
  if (projection.accessActive != null) updates["Access Active"] = projection.accessActive ? "TRUE" : "FALSE";
  if (projection.accessVersion != null) {
    const version = Number(projection.accessVersion);
    if (!Number.isSafeInteger(version) || version < 0) {
      throw Object.assign(new Error("The canonical access revision is invalid."), { code: "GOOGLE_CONTROL_ACCESS_VERSION_INVALID" });
    }
    updates["Access Version"] = version;
  }
  if (!Object.keys(updates).length) {
    throw Object.assign(new Error("A canonical control projection is required."), { code: "GOOGLE_CONTROL_PROJECTION_REQUIRED" });
  }
  const equalsProjection = (record) => Object.entries(updates).every(([field, value]) => {
    if (field === "Scoring Locked" || field === "Access Active") return truthy(record[field]) === truthy(value);
    if (field === "Access Version") return Number(record[field] || 0) === Number(value);
    return String(record[field] || "").trim().toLowerCase() === String(value).trim().toLowerCase();
  });
  if (equalsProjection(current.record)) {
    return { match: current.record, updatedAt: String(current.record["Updated At"] || "").trim(), idempotent: true };
  }
  if (expectedUpdatedAt && String(current.record["Updated At"] || "").trim() !== expectedUpdatedAt) {
    throw Object.assign(new Error("The Google match changed after the last verified mirror checkpoint."), {
      code: "GOOGLE_CONTROL_CHECKPOINT_CONFLICT",
    });
  }
  if (updates["Access Version"] != null && Number(current.record["Access Version"] || 0) > Number(updates["Access Version"])) {
    throw Object.assign(new Error("The Google access revision is newer than the canonical mirror event."), {
      code: "GOOGLE_CONTROL_ACCESS_VERSION_CONFLICT",
    });
  }
  const updatedAt = new Date().toISOString();
  await writeSheetFields("Live Matches", sheet.headers, current.rowNumber, {
  ...
  if (!equalsProjection(verified) || String(verified["Updated At"] || "").trim() !== updatedAt) {
    throw Object.assign(new Error("The Google control mirror did not verify after write."), {
      code: "GOOGLE_CONTROL_READBACK_MISMATCH",
    });
  }
  return { match: verified, updatedAt, idempotent: false };
}
