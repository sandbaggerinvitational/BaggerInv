...
export async function readDirectorOperationsData(activeYear) {
  const sheets = await readSheets([
    "Live Matches", "Players", "Courses", "Course Scorecards", "Course Holes", "Live Round Handicaps",
    "Calcutta Purchases", "Calcutta Ownership", "Net Skins",
  ]);
  const year = String(activeYear ?? "").trim();
  const yearScoped = new Set(["Live Matches", "Courses", "Live Round Handicaps", "Calcutta Purchases", "Calcutta Ownership", "Net Skins"]);
