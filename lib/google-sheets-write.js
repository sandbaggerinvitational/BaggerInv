...
export async function inspectPreviewParticipantIdentityConfiguration() {
  ...
  if (!identitySheet) return { exists: false, headers: [...PARTICIPANT_IDENTITY_CONFIGURATION_HEADERS], records: [], sheetId: null };
...
export async function initializePreviewParticipantIdentityConfiguration(updatedByValue) {
  requireIsolatedScoringSheet();
  if (process.env.VERCEL_ENV !== "preview") throw new Error("Participant identity configuration is Preview-only.");
  const before = await inspectPreviewParticipantIdentityConfiguration();
  if (before.exists) return { changed: false, ...before };
  ...
  await google(`/values/${encodeURIComponent("Participant Identity Configuration!A1:I1")}`, {
    method: "PUT",
    body: JSON.stringify({ range: "Participant Identity Configuration!A1:I1", majorDimension: "ROWS", values: [[...PARTICIPANT_IDENTITY_CONFIGURATION_HEADERS]] }),
  });
