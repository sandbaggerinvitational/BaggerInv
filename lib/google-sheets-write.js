...
export async function inspectRoundScorecardsArchiveReadback() {
  ...
  const book = await readWorkbookMetadata();
  const metadata = (book.sheets || []).find((item) => clean(item?.properties?.title) === "Round Scorecards");
  ...
  const headers = Array.from({ length: ROUND_SCORECARDS_HEADERS.length }, (_, index) => clean(computed[0]?.[index] ?? formulas[0]?.[index]));
  ...
  for (let rowNumber = 2; rowNumber <= physicalRowCount; rowNumber += 1) {
    ...
    rows.push({
      rowNumber,
      record,
      formula: clean(formulaRow[0]),
      writableBlank: headers.slice(1).every((_header, index) => clean(computedRow[index + 1]) === ""),
    });
  }
...
async function provisionRoundScorecardsFormulas(grid, assignments) {
  ...
  for (const assignment of assignments) {
    ...
    if (clean(physical.record["Match ID"]) && clean(physical.record["Match ID"]) !== "-R-") {
...
function roundScorecardArchivePayloadIntegrity(snapshot, expectedPayloadHash) {
  const snapshotHash = clean(snapshot?.payload_hash || snapshot?.archive_payload_hash);
  const expected = clean(expectedPayloadHash || snapshotHash);
...
export async function upsertRoundScorecardsArchive(snapshot, { expectedPayloadHash } = {}) {
  ...
  const actualRows = readback.rows.filter((row) => clean(row.record["Match ID"]) === plan.matchId && !row.writableBlank);
...
export async function invalidateRoundScorecardsArchive(snapshot, { expectedPayloadHash } = {}) {
  ...
  const expectedIdentities = canonicalRows.map((row) => {
    const type = clean(row["Score Type"]).toUpperCase() === "TEAM" ? "TEAM" : "PLAYER";
    return `${clean(row["Match ID"])}:${type}:${clean(type === "TEAM" ? row["Team ID"] : row["Player ID"])}`;
  }).sort();
  const matchId = clean(canonicalRows[0]?.["Match ID"]);
  const grid = await inspectRoundScorecardsArchiveReadback();
  const currentRows = grid.rows.filter((row) => clean(row.record["Match ID"]) === matchId && !row.writableBlank);
  ...
  invalidateWorkbookReadSnapshots(["Round Scorecards"]);
  const readback = await inspectRoundScorecardsArchiveReadback();
  const remaining = readback.rows.filter((row) => clean(row.record["Match ID"]) === matchId && !row.writableBlank);
  const invalid = remaining.filter((row) => clean(row.record["Scorecard Status"]).toLowerCase() === ROUND_SCORECARDS_COMPLETE_STATUS.toLowerCase());
...
export async function finalizeLiveMatch(matchIdValue, updates, updatedByValue, options = {}) {
  ...
  const lifecycleUpdates = lifecycleMirrorUpdates(updates);
  ...
  try {
    ...
    await replaceRuntimeRecordSets([
      ...
      { tab: "Odds Control", sheet: sheets["Odds Control"], records: [{ Year: snapshot.year, "Current Official Phase": snapshot.phase, "Updated At": snapshot.publishedAt }] },
      {
        ...
        records: all.flatMap((snap) => snap.teams.map((row) => normalizeWorkbookProducerRecord("Odds Team Results", {
      ...
      },
      {
        ...
        records: all.flatMap((snap) => snap.players.map((row) => normalizeWorkbookProducerRecord("Odds Player Results", {
