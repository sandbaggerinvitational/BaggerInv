...
import { accessCodeMatches, calculateLiveHole, calculateLiveMatchStatus, calculateMatchPoints } from "./live-hole-scoring.js";
...
const PRODUCTION_SHEET_ID = "1umqPxiQxN9_jwmsD7IcVTzqxPmMycYLlrY_gm31l5U4";
...
function requireIsolatedScoringSheet() {
  ...
  if (!process.env.GOOGLE_SHEETS_ID || SHEET_ID === PRODUCTION_SHEET_ID) {
...
export async function initializeLiveScoringTestSchema() {
  ...
  await ensureTabHeaders("Live Matches", [
    "Match ID", "Access Code Hash", "Current Hole", "Team 1 Holes Won", "Team 2 Holes Won",
...
export async function readLiveScoringMatch(matchIdValue) {
  ...
  const [matches, holes, courseHoles] = await Promise.all([
    readSheet("Live Matches"),
    ensureTabHeaders("Live Hole Scores", LIVE_HOLE_HEADERS),
    readSheet("Course Holes"),
  ]);
  ...
  const scores = holes.records.map(({ record }) => record)
    ...
    .sort((a, b) => Number(a["Hole Number"]) - Number(b["Hole Number"]));
  return {
    match: Object.fromEntries(Object.entries(match).filter(([key]) => key !== "Access Code Hash")),
...
export async function saveLiveHoleScore(matchIdValue, input = {}, updatedByValue) {
  ...
  if (points.team1Points !== null) {
    matchRecord["Front 9 Winner"] = points.frontWinner;
    matchRecord["Back 9 Winner"] = points.backWinner;
    matchRecord["18-Hole Winner"] = points.overallWinner;
    matchRecord["Matchup Winner"] = points.overallWinner;
    matchRecord["Team 1 Points"] = points.team1Points;
    matchRecord["Team 2 Points"] = points.team2Points;
  }
  const liveMatchRow = findUniqueMatch(matches, matchId);
  await writeSheetRow("Live Matches", matches.headers, liveMatchRow.rowNumber, matchRecord);
  const matchComplete = liveStatus.complete || points.team1Points !== null;
  let finalizedMatch = null;
  if (matchComplete) {
    finalizedMatch = await finalizeLiveMatch(matchId, {
  ...
  return { hole: next, points, liveStatus, finalizedMatch };
}
...
export async function updateLiveMatchPairing(matchIdValue, updates, updatedByValue) {
  ...
  await writeSheetRow("Live Matches", liveSheet.headers, current.rowNumber, next);
