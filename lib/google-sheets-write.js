...
export async function restorePreviewScoringBenchmarkRows({ matches = [], holes = [], archives = [] } = {}) {
  ...
  for (const snapshot of holes) {
    ...
    if (snapshot.record) {
      ...
      await writeSheetFields("Live Hole Scores", sheet.headers, row.rowNumber, Object.fromEntries(
      ...
      await clearSheetFields("Live Hole Scores", sheet.headers, row.rowNumber, writableFields("Live Hole Scores").filter((field) => sheet.headers.includes(field)));
    }
  }
  for (const [tab, snapshots] of [["Live Matches", matches], ["Matches", archives]]) {
    ...
    for (const snapshot of snapshots) {
      ...
      await writeSheetFields(tab, sheet.headers, row.rowNumber, Object.fromEntries(
        writableFields(tab)
          .filter((field) => sheet.headers.includes(field) && Object.hasOwn(snapshot.record, field))
          .map((field) => [field, snapshot.record[field] ?? ""])
      ));
