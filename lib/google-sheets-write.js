...
async function google(path, options = {}) {
  ...
  return method === "GET" ? withTransientGoogleRetry(perform, { onRetry: () => metric("retryLoops") }) : perform();
}
...
export async function publishOddsSnapshot(snapshot) {
  ...
  await replaceRuntimeRecords("Odds Snapshots", sheets["Odds Snapshots"], all.map((row) => ({ Year: row.year, Phase: row.phase, "Published At": row.publishedAt, "Snapshot JSON": JSON.stringify(row) })));
  await replaceRuntimeRecords("Odds Control", sheets["Odds Control"], [{ Year: snapshot.year, "Current Official Phase": snapshot.phase, "Updated At": snapshot.publishedAt }]);
  await replaceRuntimeRecords("Odds Team Results", sheets["Odds Team Results"], all.flatMap((snap) => snap.teams.map((row) => ({ Year: snap.year, Phase: snap.phase, Team: row.name, "Win Probability": row.probability, "American Odds": row.americanOdds, "Expected Points": row.expectedPoints }))));
  await replaceRuntimeRecords("Odds Player Results", sheets["Odds Player Results"], all.flatMap((snap) => snap.players.map((row) => ({ Year: snap.year, Phase: snap.phase, "Player ID": row.id, Player: row.name, "Top Player Probability": row.probability, "American Odds": row.americanOdds, "Expected Points": row.expectedPoints, "Expected Record": row.expectedRecord, "Average Finish": row.averageFinish }))));
  return snapshot;
}
