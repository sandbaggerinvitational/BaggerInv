...
async function replaceScopedRuntimeRecordSets(sets) {
  const data = [];
  const counts = {};
  for (const { tab, sheet, records, belongsToScope } of sets) {
    ...
    for (let index = 0; index < Math.max(scopedRows.length, records.length); index += 1) {
      ...
      for (const [field, value] of Object.entries(updates)) {
        const column = columnName(sheet.headers.indexOf(field));
        data.push({ range: `${tab}!${column}${rowNumber}`, values: [[value]] });
      }
    }
...
export async function updateDirectorNetSkins(input, updatedByValue) {
  ...
  const sheet = await requireTabHeaders("Net Skins", ["Year", "Player ID 1", "Player ID 2", "Eligible"]);
  const found = sheet.records.filter(({ record }) => String(record.Year) === year && [record["Player ID 1"], record["Player ID 2"]].some((id) => String(id) === playerId));
  if (!found.length) throw new Error("No Net Skins entry exists for this player.");
  await Promise.all(found.map((row) => writeSheetFields("Net Skins", sheet.headers, row.rowNumber, { Eligible: input.eligible ? "TRUE" : "FALSE" })));
  await appendAdminAudit({ resource: "net-skins", recordId: playerId, action: input.eligible ? "Eligible" : "Ineligible", previous: {}, next: { year, playerId, eligible: Boolean(input.eligible) }, updatedBy, summary: "Director Net Skins eligibility" });
  return true;
}
