...
export async function repairFinalizedLiveMatchParity(matchIdValue, input = {}, updatedByValue) {
  ...
  let sheet = await readSheet("Live Matches");
  if (!sheet.headers.includes("Scoring Locked")) {
    const headerColumn = columnName(sheet.headers.length);
    metric("workbookWrites");
    await google("/values:batchUpdate", {
      method: "POST",
      body: JSON.stringify({
        valueInputOption: "RAW",
        data: [{ range: `Live Matches!${headerColumn}1`, values: [["Scoring Locked"]] }],
      }),
    });
    invalidateWorkbookReadSnapshots(["Live Matches"]);
    sheet = await readSheet("Live Matches");
  }
  requireHeaders(sheet, "Live Matches", ["Match ID", "Match Status", "Scoring Locked", "Access Active", "Access Version", "Updated At", "Updated By"]);
  const current = findUniqueMatch(sheet, matchId);
