...
import {
  ...
  currentGoogleWorkbookMutationIntent,
...
function googleSheetsApiTarget() {
  ...
  const dedicatedProductionWorkbook = credential?.credentialSource === "production-worker"
    ? String(credential?.resources?.googleWorkbookId || "").trim()
    : "";
  const spreadsheetId = dedicatedProductionWorkbook || resolveSpreadsheetId();
  return {
    spreadsheetId,
...
async function google(path, options = {}) {
  ...
  const perform = async () => {
    ...
    const mutationScope = method === "GET" ? null : currentGoogleWorkbookMutationIntent();
    if (target.spreadsheetId === PRODUCTION_GOOGLE_WORKBOOK_ID && mutationScope) {
      try {
        assertProductionGoogleServiceAccountMutationBinding({
          intent: mutationScope.intent,
          operation: mutationScope.operation,
          admission: mutationScope.admission,
        });
      } catch (error) {
        rejectGoogleWorkbookMutation();
        throw error;
    ...
    if (mutation?.productionWorkbook &&
        ...
        credential?.credentialSource !== "production-worker") {
      rejectGoogleWorkbookMutation();
      const error = new Error("Production Google writes require the dedicated Production credential.");
      error.code = "PRODUCTION_GOOGLE_DEDICATED_CREDENTIAL_REQUIRED";
      error.status = 503;
      throw error;
    }
    let token;
    ...
    try {
      ...
      // capability while token acquisition was pending must never dispatch.
      if (mutation?.productionWorkbook) {
        assertProductionGoogleServiceAccountMutationBinding({
    ...
    let response;
    try {
      response = await fetch(`${target.apiBase}${path}`, { ...transportOptions, headers: { authorization: `Bearer ${token}`, "content-type": "application/json", ...(transportOptions.headers || {}) }, cache: "no-store", signal: transportOptions.signal || AbortSignal.timeout(10_000) });
    } catch (error) {
      if (method !== "GET") markGoogleWorkbookMutationAmbiguous();
      throw error;
    }
    if (!response.ok) {
...
function requireIsolatedScoringSheet() {
  const credential = googleServiceAccountCredentialDiagnostics();
  if (credential.credentialSource === "production-worker") {
