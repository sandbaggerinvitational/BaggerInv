...
function filterCmsRows(records, schema, { tournament, year } = {}) {
  ...
  if (schema.filter === "tournament" && tournament) return records.filter(({ record }) => String(record["Tournament ID"] ?? "").trim() === String(tournament));
  return records;
}
...
export async function readAdminDashboard({ tournament, year } = {}) {
  ...
  const tournamentRow = tournaments.records.find(({ record }) => String(record["Tournament ID"] ?? record.Year ?? "") === String(tournament))?.record
    || tournaments.records.find(({ record }) => String(record.Year ?? "") === String(year))?.record || {};
  const forYear = (sheet) => sheet.records.map(({ record }) => record).filter((record) => !year || String(record.Year ?? "") === String(year));
  const yearMatches = forYear(matches), yearLive = forYear(live), yearTeams = forYear(teams), yearCourses = forYear(courses);
  const finalMatches = yearMatches.filter((match) => match["Match Status"] === "Final");
  ...
  const lastOdds = odds.records.map(({ record }) => record).filter((record) => !year || String(record.Year ?? "") === String(year)).at(-1) || {};
  ...
  return {
    tournamentStatus: tournamentRow["Tournament Status"] || tournamentRow.Status || "Not set",
    currentRound: tournamentRow["Current Round"] || "—",
    liveMatches: yearLive.filter((match) => match["Match Status"] === "Live" || match["Match Status"] === "Reopened").length,
    matchesRemaining: Math.max(0, yearMatches.length - finalMatches.length),
...
export async function readAdminStandings({ year } = {}) {
  ...
  const rows = matches.records.map(({ record }) => record).filter((match) => String(match.Year ?? "") === String(year) && match["Match Status"] === "Final");
