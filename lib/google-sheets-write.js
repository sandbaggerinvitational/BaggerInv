...
import { googleResponseError, withTransientGoogleRetry } from "./google-api-reliability.js";
...
import { calcuttaPublicationRecords } from "./calcutta.js";
...
export async function withWorkbookWriteDiagnostics(label, operation) {
  const diagnostics = { label, httpRequests: 0, sheetsApiCalls: 0, workbookWrites: 0, retryLoops: 0 };
  try {
    const result = await writeDiagnostics.run(diagnostics, operation);
    return { result, diagnostics };
  } catch (error) {
    error.workbookDiagnostics = { ...diagnostics };
    throw error;
  }
}
...
export async function synchronizeNetSkinsResults(yearValue, roundValue) {
  requireIsolatedScoringSheet();
...
async function synchronizeNetSkinsAfterMatch(record) {
  try {
    return await synchronizeNetSkinsResults(record.Year, record.Round);
  } catch (error) {
  ...
  for (const { tab, sheet, records, fields = writableFields(tab) } of sets) {
    ...
    for (let index = 0; index < rowCount; index += 1) {
      ...
      for (const [field, value] of Object.entries(updates)) {
        ...
        data.push({ range: `${tab}!${column}${index + 2}`, values: [[value]] });
      }
    }
  }
  if (!data.length) return;
...
async function replaceScopedRuntimeRecords(tab, sheet, records, belongsToScope) {
  const fields = writableFields(tab).filter((field) => sheet.headers.includes(field));
  validateSheetSchema(tab, sheet.headers, fields);
  ...
  for (let index = 0; index < Math.max(scopedRows.length, records.length); index += 1) {
    ...
    if (!record) {
      await clearSheetFields(tab, sheet.headers, rowNumber, fields);
      continue;
    }
    await writeSheetFields(tab, sheet.headers, rowNumber, Object.fromEntries(
...
const CALCUTTA_STANDINGS_HEADERS = ["Year", "Rank", "Player ID", "Purchase Price", "Round 1 Points", "Round 2 Points", "Round 3 Points", "Total Points", "Round 1 Payout %", "Round 2 Payout %", "Round 3 Payout %", "Overall Payout %", "Total Payout %", "Current Payout Value", "ROI", "Updated At"];

export async function publishOfficialCalcutta(yearValue) {
  requireIsolatedScoringSheet();
  const year = Number(yearValue);
  if (!year) return { published: false, reason: "invalid-year" };
  const tabs = ["Round Results", "Live Round Handicaps", "Handicaps", "Calcutta Purchases", "Calcutta Ownership", "Calcutta Point Structure", "Calcutta Payout", "Calcutta Round Results", "Calcutta Standings"];
  ...
  const output = calcuttaPublicationRecords({
    year,
    roundResults: rows("Round Results"),
  ...
  if (!output.roundResults.length) return { published: false, reason: "no-completed-rounds", year };
  const completedRounds = new Set(output.roundResults.map((record) => Number(record.Round)));
  await replaceScopedRuntimeRecords(
    "Calcutta Round Results",
    sheets["Calcutta Round Results"],
    output.roundResults,
    (record) => Number(record.Year) === year && completedRounds.has(Number(record.Round))
  );
  await replaceScopedRuntimeRecords(
    "Calcutta Standings",
    sheets["Calcutta Standings"],
    output.standings,
    (record) => Number(record.Year) === year
  );
  return { published: true, year, completedRounds: [...completedRounds].sort(), roundResults: output.roundResults.length, standings: output.standings.length };
}

async function synchronizeCalcuttaAfterOfficialUpdate(record) {
  try {
    return await publishOfficialCalcutta(record?.Year);
  } catch (error) {
    console.error("Calcutta publication failed", { year: String(record?.Year || ""), reason: error?.message || String(error), stack: error?.stack });
...
export async function finalizeLiveMatch(matchIdValue, updates, updatedByValue) {
  ...
  await synchronizeNetSkinsAfterMatch(nextLive);
  await synchronizeCalcuttaAfterOfficialUpdate(nextLive);
  return nextLive;
}
