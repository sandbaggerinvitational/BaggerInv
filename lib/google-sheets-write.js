...
async function performLiveHoleScoreSave(matchIdValue, input = {}, updatedByValue) {
  ...
  if (String(input.expectedUpdatedAt || "") !== String(match["Updated At"] || "") || Number(input.expectedRevision || 0) !== currentRevision) {
    ...
    const matchAlreadyReflectsSubmission = submittedMatchesExisting &&
      ...
      Number(match["Current Hole"]) === Number(persistedStatus.currentHole) &&
      Number(match["Holes Remaining"]) === Number(persistedStatus.holesRemaining);
    if (matchAlreadyReflectsSubmission) {
      return {
        hole: existingRecord,
        points: persistedPoints,
        liveStatus: persistedStatus,
        matchComplete: persistedStatus.complete || persistedPoints.team1Points !== null,
        updatedAt: match["Updated At"],
  ...
  const verification = await verifyDirectorReadBack({
    ...
    invalidate: async () => invalidateWorkbookReadSnapshots(["Live Hole Scores", "Live Matches"]),
    ...
    verify: (verifiedSheets) => {
      ...
      return Boolean(verified) &&
        ...
        Number(verifiedMatch?.["Holes Remaining"]) === Number(liveStatus.holesRemaining);
    },
    summarize: (verifiedSheets) => {
      ...
      return {
        holeRevision: holeRecord?.Revision,
        team1Gross: holeRecord?.["Team 1 Gross Scores"],
        team2Gross: holeRecord?.["Team 2 Gross Scores"],
        team1Net: holeRecord?.["Team 1 Net Score"],
        team2Net: holeRecord?.["Team 2 Net Score"],
        holeWinner: holeRecord?.["Hole Winner"],
        matchUpdatedAt: matchRecord?.["Updated At"],
        currentHole: matchRecord?.["Current Hole"],
        holesRemaining: matchRecord?.["Holes Remaining"],
      };
    },
  });
