...
async function performLiveHoleScoreSave(matchIdValue, input = {}, updatedByValue) {
  ...
  const verification = await verifyDirectorReadBack({
    ...
    summarize: (verifiedSheets) => ({
      holeRevision: verifiedSheets["Live Hole Scores"].records.find(({ record }) => String(record["Match ID"] || "").trim() === matchId && Number(record["Hole Number"]) === holeNumber)?.record?.Revision,
      matchUpdatedAt: verifiedSheets["Live Matches"].records.find(({ record }) => String(record["Match ID"] || "").trim() === matchId)?.record?.["Updated At"],
    }),
  });
