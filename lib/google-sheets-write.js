...
import { playerAppearsInMatch } from "./player-passport.js";
...
export async function activatePlayerPassport({ reference, code, deviceLabel = "" }) {
  ...
  const current = passports.records.find(({ record }) =>
    String(record["Tournament ID"]) === String(context.tournamentId) &&
    String(record["Invite Reference"]) === String(reference || "")
  );
  const record = current?.record;
  if (!record || !truthy(record["Activation Active"]) ||
      accessExpired({ "Access Expires At": record["Activation Expires At"] }) ||
      !accessCodeMatches(code, record["Activation Code Hash"], accessSalt())) {
    throw new Error("Unable to activate Player Passport.");
  }
  const player = context.players.find((item) => item.id === String(record["Player ID"]));
  if (!player) throw new Error("Unable to activate Player Passport.");
  ...
  return { player, tournamentId: context.tournamentId, deviceId, sessionVersion: device["Session Version"], expiresAt };
}
...
export async function validatePlayerPassport(session) {
  ...
  return { session, device, passport, player: { id: session.playerId, name: player["Display Name"] || player.Name || session.playerId } };
}
...
export async function readPlayerPassportMatches(session) {
  ...
  const courseNames = Object.fromEntries(courses.records.map(({ record }) => [
    String(record["Course ID"] || ""), String(record["Course Name"] || record.Name || ""),
  ]));
  const teamRecords = teams.records.map(({ record }) => record)
    .filter((record) => String(record.Year || "") === String(context.year));
  ...
  const participantIds = (match, side) =>
    [match[`Team ${side} Player 1`], match[`Team ${side} Player 2`]]
  ...
  const matchModels = context.liveMatches.filter((match) => playerAppearsInMatch(match, session.playerId)).map((match) => {
    const side = participantIds(match, 1).includes(String(session.playerId)) ? 1 : 2;
    const otherSide = side === 1 ? 2 : 1;
    const teammateIds = participantIds(match, side).filter((id) => id !== String(session.playerId));
    ...
    return {
      ...
      course: courseNames[String(match["Course ID"] || "")] || "",
      courseId: match["Course ID"] || "", teeTime: match["Tee Time"] || "",
      ...
      opponentNames: participantIds(match, otherSide).map((id) => playerNames[id] || id),
      participantNames: participantIds(match, side).map((id) => playerNames[id] || id),
      result: isFinal ? {
