...
import { calcuttaPublicationReadiness, calcuttaPublicationRecords } from "./calcutta.js";
...
export async function publishOfficialCalcutta(yearValue) {
  ...
  if (!output.roundResults.length) {
    ...
    trace.stopReason = readiness.purchasedPlayers.length
      ? "No round contains official Round Results for every purchased golfer."
  ...
  trace.rowsSuccessfullyWritten.roundResults = await replaceScopedRuntimeRecords(
  ...
  trace.stage = "standings-write";
  trace.rowsSuccessfullyWritten.standings = await replaceScopedRuntimeRecords(
    "Calcutta Standings",
...
async function synchronizeCalcuttaAfterOfficialUpdate(record) {
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    ...
    try {
      result = await publishOfficialCalcutta(record?.Year);
      if (result.published || result.reason !== "no-completed-rounds") break;
...
export async function finalizeLiveMatch(matchIdValue, updates, updatedByValue, options = {}) {
  ...
  const calcuttaPublication = await synchronizeCalcuttaAfterOfficialUpdate(nextLive);
