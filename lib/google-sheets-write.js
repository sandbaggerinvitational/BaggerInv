...
async function google(path, options = {}) {
  ...
  return method === "GET" ? withTransientGoogleRetry(perform) : perform();
}
...
async function replaceTab(tab, headers, rows) {
  await google(`/values/${encodeURIComponent(`${tab}!A:ZZ`)}:clear`, { method: "POST", body: "{}" });
...
export async function resetPreviewTournament(reference, updatedByValue) {
  ...
  for (const row of scopedLiveMatches) {
    await writeSheetRow("Live Matches", liveMatches.headers, row.rowNumber, resetPreviewMatchRecord(row.record, now));
  }
  ...
  if (existingTabs.has("Matches")) {
    ...
    for (const row of matches.records.filter(({ record }) => recordInPreviewTournament(record, scope))) {
      await writeSheetRow("Matches", matches.headers, row.rowNumber, resetPreviewMatchRecord(row.record, now));
      finalizedMatches += 1;
    }
  }
  ...
  for (const tab of PREVIEW_RESET_ROW_TABS.filter((name) => existingTabs.has(name))) {
    ...
    const preserved = sheet.records.map(({ record }) => record).filter((record) => !resetRowBelongsToTournament(tab, record, scope, matchIds));
    cleared[tab] = sheet.records.length - preserved.length;
    await replaceTab(tab, sheet.headers, preserved.map((record) => sheet.headers.map((header) => record[header] ?? "")));
  }

  if (existingTabs.has("Match Update Log")) {
    const log = await readSheet("Match Update Log");
    const preserved = log.records.map(({ record }) => record).filter((record) => !matchIds.has(String(record["Match ID"] || "").trim()));
    cleared["Match Update Log"] = log.records.length - preserved.length;
    await replaceTab("Match Update Log", log.headers, preserved.map((record) => log.headers.map((header) => record[header] ?? "")));
  }
...
export async function synchronizeNetSkinsResults(yearValue, roundValue) {
  ...
  const sheet = await ensureTabHeaders("Net Skins Result", NET_SKINS_RESULT_HEADERS);
  ...
  await replaceTab("Net Skins Result", sheet.headers, records.map((record) =>
...
async function passportTournamentContext() {
  requireIsolatedScoringSheet();
  const sheets = await readSheets(["Tournaments", "Players", "Handicaps", "Live Matches"]);
...
export async function validatePlayerPassport(session) {
  const sheets = await readSheets(["Trusted Devices", "Player Passport", "Players"]);
...
export async function appendNotificationLog(session, { type = "Test", recipient = "Authenticated device", status, failure = "", template = "" } = {}) {
  requireIsolatedScoringSheet();
  const sheet = await ensureTabHeaders("Notification Log", NOTIFICATION_LOG_HEADERS);
  await appendSheetRow("Notification Log", sheet.headers, {
...
export async function readPlayerPassportMatches(session) {
  ...
  const sheets = await readSheets(["Players", "Courses", "Team Names", "Live Hole Scores"]);
...
export async function finalizeLiveMatch(matchIdValue, updates, updatedByValue) {
  ...
  const matchesSheet = await ensureTabHeaders("Matches", FINALIZED_MATCH_ARCHIVE_HEADERS);
  ...
  if (existing.length) await writeSheetFields(
    ...
    Object.fromEntries(matchesSheet.headers.map((header) => [header, permanent[header] ?? ""]))
  );
