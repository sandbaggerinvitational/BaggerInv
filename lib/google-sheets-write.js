...
export async function readPlayerPassportMatches(session) {
  ...
  return {
    ...
    matches: context.liveMatches.filter((match) => playerAppearsInMatch(match, session.playerId)).map((match) => ({
      selector: match["Access Selector"] || "",
      matchId: match["Match ID"],
      round: match.Round, match: match.Match, format: match.Format,
      course: match["Course ID"] || "", teeTime: match["Tee Time"] || "",
      status: match["Match Status"] || "Scheduled",
      scoringEnabled: truthy(match["Access Active"]) && !accessExpired(match) &&
