...
const NOTIFICATION_LOG_HEADERS = [
  ...
  "Recipient", "Time Sent", "Delivery Status", "Failure",
];
...
export async function appendNotificationLog(session, { type = "Test", recipient = "Authenticated device", status, failure = "" } = {}) {
  requireIsolatedScoringSheet();
  ...
  await appendSheetRow("Notification Log", sheet.headers, {
    ...
    "Time Sent": new Date().toISOString(), "Delivery Status": status, "Failure": failure,
  });
}
...
export async function readNotificationLog(limit = 20) {
  ...
  return sheet.records.map(({ record }) => ({
    ...
    sentAt: record["Time Sent"], status: record["Delivery Status"], failure: record.Failure || "",
