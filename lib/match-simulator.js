...
export function simulateMatch({
  ...
  for (let simulation = 0; simulation < iterations; simulation += 1) {
    ...
    if (teamFormat) {
      ...
      expectedA += overallOutcome === "teamA" ? pointsAvailable : overallOutcome === "halve" ? pointsAvailable / 2 : 0;
      const margin = finishingMargin(closeHole < 18 ? closeLead : overall, closeHole);
      addResult(resultCounts, `${overallOutcome}|${margin}`);
    }
  }
  ...
  const maximumPoints = teamFormat ? 3 : pointsAvailable;
  const likelyResults = Object.entries(resultCounts)
    .map(([key, count]) => ({
      key,
      label: teamFormat
        ? pointsLabel(teamNames, Number(key))
        : key.startsWith("halve|")
          ? "Halved"
          : `${key.startsWith("teamA|") ? teamNames[0] : teamNames[1]} · ${key.split("|")[1]}`,
