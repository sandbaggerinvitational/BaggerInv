...
export async function certifyMobileNativeOtp({
  ...
  const nativeCertification = await certifyCanonicalNativeParticipant({request,authUserId,playerId:expectedPlayerId,tournamentId,participant:after.payload.data,env,dependencies});
  const certification = {token:nativeCertification.certificationToken,expiresInSeconds:nativeCertification.expiresInSeconds};
