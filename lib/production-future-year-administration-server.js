...
function fixedProductionScope(input, { actorAuthUserId, actorPlayerId } = {}, contractVersion = PRODUCTION_FUTURE_YEAR_ADMINISTRATION_CONTRACT) {
...
function safeProviderCode(payload) {
  for (const value of [payload?.code, payload?.message, payload?.error?.code]) {
    ...
    if (/^(?:(?:PRODUCTION_)?FUTURE_(?:YEAR|RUNTIME|TOURNAMENT|TEAM|ROSTER|ROUND|COURSE|MATCH)|GLOBAL_COURSE)_[A-Z0-9_]{3,120}$/.test(candidate)) {
      return candidate;
    }
  }
...
export async function readProductionFutureRuntime({
  ...
  }, { actorAuthUserId, actorPlayerId }, PRODUCTION_FUTURE_RUNTIME_ACTIVATION_CONTRACT);
  const rpc = options.rpc || productionFutureYearAdministrationRpc;
