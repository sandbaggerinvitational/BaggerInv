...
function resolve2025ScrambleTeamRowSides(roundScorecards, matches, teamNames) {
  const resolved = new Map();
  const matchById = new Map(matches.map((match) => [clean(pick(match, "Match ID")), match]));
  const rowsByMatch = new Map();
  for (const row of roundScorecards) {
    const identity = matchIdentity(row);
    if (identity.year !== 2025 || identity.round !== 2 || identity.format !== "SC") continue;
    if (normalizeScoreType(pick(row, "Score Type"), identity.format) !== "TEAM") continue;
    if (!rowsByMatch.has(identity.matchId)) rowsByMatch.set(identity.matchId, []);
    rowsByMatch.get(identity.matchId).push(row);
  }
  for (const [matchId, rows] of rowsByMatch) {
    const match = matchById.get(matchId);
    if (!match) continue;
    const canonicalSides = matchTeams(match, teamNames);
    const usedSides = new Set();
    const unresolvedRows = [];
    for (const row of rows) {
      const rowTeamId = clean(pick(row, "Team ID"));
      const directSide = canonicalSides.find((team) => sameId(team.teamId, rowTeamId))?.side || null;
      if (directSide && !usedSides.has(directSide)) {
        resolved.set(row, directSide);
        usedSides.add(directSide);
      } else {
        unresolvedRows.push(row);
      }
    }
    const availableSides = canonicalSides.map((team) => team.side).filter((side) => !usedSides.has(side));
    // The 2025 archive has one known team-ID spelling drift between the
    // Round Scorecards identity and the bundled Team Names context. Resolve
    // only the unambiguous one-row/one-side remainder; never use row order.
    if (unresolvedRows.length === 1 && availableSides.length === 1) {
      resolved.set(unresolvedRows[0], availableSides[0]);
    }
  }
  return resolved;
}
...
function expectedScorecards(matches, teamNames, actualByMatch) {
  ...
  for (const match of matches) {
    ...
    if (scoreType === "TEAM") {
      ...
      const actualPlayerIds = unique((actualByMatch.get(identity.matchId) || [])
        .filter((row) => normalizeScoreType(pick(row, "Score Type"), identity.format) === "INDIVIDUAL")
        .map((row) => clean(pick(row, "Player ID"))));
      const requiredPlayerCount = identity.format === "BB" ? 4 : identity.format === "SI" ? 2 : null;
      const playerIds = requiredPlayerCount && actualPlayerIds.length === requiredPlayerCount
        ? actualPlayerIds
        : matchPlayers(match);
...
export function buildScorecardAnalytics({
  ...
  const resolved2025ScrambleSides = resolve2025ScrambleTeamRowSides(
    roundScorecards,
    matches,
    teamNames
  );
  ...
  const normalized = roundScorecards.map((row) => {
    ...
    const resolvedScrambleSide = resolved2025ScrambleSides.get(row) || null;
    const matchTeam = match
      ? matchTeams(match, teamNames).find((team) => sameId(team.teamId, teamId)) ||
        matchTeams(match, teamNames).find((team) => team.side === resolvedScrambleSide)
      : null;
    ...
    const teamSide = scoreType === "TEAM" && match
      ? matchTeam?.side || resolvedScrambleSide || null
      : null;
