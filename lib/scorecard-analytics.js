...
export function officialStrokeValue(match, side, playerSlot = null) {
...
export function buildMatchNetScoring(scorecards, match, teamNames) {
...
export function buildScorecardAnalytics({
  ...
  return attachScorecardAnalyticsSelectors({
...
/**
 * Reattaches the three lightweight selector closures after a scorecard
 * analytics domain product has crossed a serialization/cache boundary.
 */
export function attachScorecardAnalyticsSelectors(analytics = {}) {
  const scorecards = Array.isArray(analytics.scorecards) ? analytics.scorecards : [];
  const usableScorecards = Array.isArray(analytics.usableScorecards)
    ? analytics.usableScorecards
    : scorecards.filter((scorecard) => scorecard.status !== "MISSING");
  const individualScorecards = Array.isArray(analytics.individualScorecards)
    ? analytics.individualScorecards
    : usableScorecards.filter((scorecard) => scorecard.scoreType === "INDIVIDUAL");
  const teamScorecards = Array.isArray(analytics.teamScorecards)
    ? analytics.teamScorecards
    : usableScorecards.filter((scorecard) => scorecard.scoreType === "TEAM");
  const expectedScorecards = Array.isArray(analytics.expectedScorecards)
    ? analytics.expectedScorecards
    : [];
  const playerSummary = (playerId) => {
    const cards = individualScorecards.filter((scorecard) => sameId(scorecard.playerId, playerId));
    const expectedCount = expectedScorecards.filter((record) => sameId(record.playerId, playerId)).length;
  ...
  const teamSummary = (teamId) => {
    ...
    const expectedCount = expectedScorecards.filter((record) => sameId(record.teamId, teamId)).length;
    return summarizeScorecards(cards, expectedCount);
  };
