 ...
 */
export function buildPlayerHoleByHoleAnalytics(
  scorecards = [],
  ...
  for (const [matchId, cards] of byMatch.entries()) {
    ...
    for (const hole of matchNet.holeWinners) {
      ...
      const apply = (ids, result) => ids.forEach((playerId) => {
        if (isPlayerExcludedFromMatchRecord(matchId, playerId, ghostMatchExclusions)) return;
