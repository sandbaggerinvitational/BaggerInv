...
import { readMobileProductionMatchDetail } from "./mobile-v1-production-match-detail.js";
...
function canonicalMatchId(value, { nullable = false } = {}) {
  return requireOpaqueMatchID(value, { nullable });
}
...
export async function readMobilePreviewMatchDetailV1(
  ...
  const matchIdentifier = canonicalMatchId(matchId);
...
export function mobileMatchDetailDataFromPreviewView(raw = {}, identity = {}) {
  const playerId = isCanonicalReviewer(identity.context) ? null : canonicalId(identity.playerId);
  return matchDetailGolfData(raw, identity, playerId);
}

// A pure presenter, not an authorization entry point. Public callers must first
// bound the match to the published tournament and then apply their field allowlist.
export function mobilePublicMatchGolfData(raw = {}) {
  const neutral = { ...raw,
    participants: list(raw.participants).map(row => ({ ...row, is_authenticated_player: false })),
    navigation: { ...raw.navigation, my_match_id: null, is_my_match: false },
  };
  return matchDetailGolfData(neutral, {
    tournamentId: raw.tournament?.tournament_id, matchId: raw.match?.match_id,
  }, null);
}

function matchDetailGolfData(raw, identity, playerId) {
  ...
  const matchId = canonicalMatchId(matchRow.match_id);
  requireValue(tournamentId === canonicalId(identity.tournamentId) && matchId === canonicalMatchId(identity.matchId));
  ...
  const participantsForSide = (side) => {
    ...
    return values.map((row, index) => {
      ...
      return {
        ...
        courseHandicap: mobileCourseHandicap(row?.course_handicap, format),
  ...
  const myMatchId = canonicalMatchId(navigationRow.my_match_id, { nullable: true });
  const isMyMatch = canonicalBoolean(navigationRow.is_my_match);
  requireValue(isMyMatch === (myMatchId === matchId));
  const navigation = {
    roundMatchIndex,
    roundMatchCount,
    previousMatchId: canonicalMatchId(navigationRow.previous_match_id, { nullable: true }),
    nextMatchId: canonicalMatchId(navigationRow.next_match_id, { nullable: true }),
    myMatchId,
    isMyMatch,
  };
  ...
  return {
    ...
    match: {
      ...
      progress: {
        ...
        statusText: status === "scheduled" ? null
          : boundedText(replaceTeamReferences(matchRow.running_result, teams), 200, { nullable: true }),
      },
      result,
      navigation,
      scorecard: {
...
export async function mobileMatchDetailResult(
  ...
  if (isCanonicalReviewer(identity?.context)) {
    return (await import('./mobile-reviewer-history.js')).mobileReviewerHistoryResult(identity,matchIdentifier,{env,now,dependencies});
  }
  const read = clean(env.VERCEL_ENV).toLowerCase() === "production"
    ? await readMobileProductionMatchDetail(identity, matchIdentifier, { env, dependencies })
    : await (dependencies.readMobilePreviewMatchDetailV1 || readMobilePreviewMatchDetailV1)(
    { tournamentId: identity?.tournamentId, playerId: identity?.playerId, matchId: matchIdentifier },
    { env, dependencies },
  );
