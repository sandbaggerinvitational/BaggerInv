...
export function mobileMatchDetailDataFromPreviewView(raw = {}, identity = {}) {
  ...
  const finalSummary = status === "completed"
    ? finalMatchSummary(legacyMatch, legacyScores, teamNames) : "";
  const clinchHole = canonicalInteger(clean(finalSummary).match(/\bHole\s+(\d+)\b/i)?.[1], 1, 18, { nullable: true });
  const clinched = canonicalBoolean(matchRow.clinched);
  requireValue(!clinched || (clinchHole !== null && resultSide !== null));
  const clinch = clinched ? {
    holeNumber: clinchHole,
    winnerSide: resultSide,
    winnerTeamId: teams[resultSide - 1].teamId,
    summary: boundedText(finalSummary, 240),
  } : null;
