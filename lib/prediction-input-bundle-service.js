...
function verifiedSettingsView(configuration = {}, verification = null) {
  ...
  if (verification.trustCurrentSupabaseConfiguration === true) {
    const sourceSettings = Array.isArray(view.source_settings) ? view.source_settings : [];
    let normalized = null;
    try { normalized = normalizePredictionSettings(sourceSettings); }
    catch { normalized = null; }
    const canonicalMatches = normalized !== null && scoringShadowPayloadHash(normalized.canonicalSettings) ===
      scoringShadowPayloadHash(view.canonical_settings || {});
    const effectiveMatches = normalized !== null && scoringShadowPayloadHash(normalized.effectiveSettings) ===
      scoringShadowPayloadHash(view.effective_settings || {});
    const expectedSettingsFingerprint = scoringShadowPayloadHash(sourceSettings);
    const expectedEffectiveFingerprint = normalized === null ? "" :
      scoringShadowPayloadHash(normalized.effectiveSettings);
    const fingerprintsMatch = clean(view.settings_fingerprint) === expectedSettingsFingerprint &&
      clean(view.effective_settings_fingerprint) === expectedEffectiveFingerprint;
    if (view.configuration_is_current !== true || view.validation_status !== "VALID" ||
        view.settings_contract_version !== PREDICTION_SETTINGS_CONTRACT_VERSION ||
        sourceSettings.length === 0 || normalized === null ||
        !canonicalMatches || !effectiveMatches || !fingerprintsMatch) {
