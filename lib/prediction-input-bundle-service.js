...
async function prepareUncached(options = {}) {
  ...
  const predictionSettings = predictionSettingsProjectionFromView(
    predictionSettingsViewFromOddsConfiguration(current.input_configuration || {})
  );
...
export async function preparePredictionInputBundle(options = {}) {
  return options.env || options.dependencies || options.timeoutMs || options.scope || options.allowUnknownSettingsFreshness === false
