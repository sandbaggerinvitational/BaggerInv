...
export async function runWarRoomCalculationParity({
  ...
  const selectedFormat = clean(format).toUpperCase();
  const options = {
    iterations: Math.max(10_000, Math.trunc(Number(iterations) || 10_000)),
    repeat: Math.max(2, Math.trunc(Number(repeat) || 2)),
    phase: pair.capture.phase,
  ...
  return {
    ok: pass,
    operation,
    diagnosticSlice: selectedFormat ? { format: selectedFormat } : { format: "ALL" },
