...
export function roundScorecardsArchiveEnvironment(env = process.env) {
  ...
  const productionAuthority = clean(env.SCORING_AUTHORITY).toLowerCase() === "supabase";
  const productionWorkers = /^(1|true|yes|on)$/i.test(clean(env.PRODUCTION_SUPABASE_WORKERS_ENABLED));
  const productionMirror = /^(1|true|yes|on)$/i.test(clean(env.PRODUCTION_SUPABASE_GOOGLE_MIRROR_ENABLED));
  const productionEnabled = requested && productionDeployment && productionAuthority &&
    productionWorkers && productionMirror && productionCutoverPhaseAtLeast(env, "WORKERS");
  const previewEnabled = requested && authority.previewDeployment && authority.resolved === "supabase";
  const enabled = previewEnabled || productionEnabled;
  return {
    requested,
    enabled,
    previewDeployment: authority.previewDeployment,
    productionDeployment,
    scoringAuthority: productionEnabled ? "supabase" : authority.resolved,
    productionBlocked: requested && productionDeployment && !productionEnabled,
    reason: previewEnabled ? "preview-supabase-archive"
      : productionEnabled ? "production-supabase-archive"
      : !requested ? "archive-disabled"
      : productionDeployment ? "production-worker-gate-closed"
      : !authority.previewDeployment ? "non-preview-environment-blocked"
