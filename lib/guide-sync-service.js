...
const DIRECTOR_VALIDATION_SOURCES = new Set([...GUIDE_PROJECTION_SHEETS, "Canonical Course Context", "Guide validation"]);
const DIRECTOR_SAFE_VALUE_FIELDS = new Map([
  ["Courses", new Set(["Course ID", "Round", "Course ID / Round", "Format", "Tee Played", "Slope", "Rating", "Par"])],
  ["Tournament Itinerary", new Set(["Event Date", "Start Time", "End Time", "Display Order", "Course ID"])],
  ["Tournament Timeline", new Set(["Event Date", "Start Time", "End Time", "Sort Order"])],
]);

function diagnosticText(value, maximum = 240) {
...
export function guideValidationIssuesForDirector(error) {
  const details = Array.isArray(error?.validationIssues) && error.validationIssues.length
    ? error.validationIssues
    : (Array.isArray(error?.issues) ? error.issues.map((message) => ({ message, reason: message })) : []);
  return details.map((detail) => {
    const requestedSource = diagnosticText(detail?.source, 64);
    const source = DIRECTOR_VALIDATION_SOURCES.has(requestedSource) ? requestedSource : "Guide validation";
    const field = diagnosticText(detail?.field, 80);
    const issue = {
      source,
      entity: diagnosticText(detail?.entity, 120),
      field,
      reason: diagnosticText(detail?.reason || detail?.message) || "Guide content did not pass publication validation.",
    };
    if (detail?.valueSafe === true && DIRECTOR_SAFE_VALUE_FIELDS.get(source)?.has(field)) {
      if (Object.hasOwn(detail, "currentValue")) issue.currentValue = diagnosticText(detail.currentValue, 160) || "(blank)";
      if (Object.hasOwn(detail, "expectedValue")) issue.expectedValue = diagnosticText(detail.expectedValue, 160) || "(blank)";
    }
    return issue;
  });
}
...
export async function synchronizeGuideContent({
  ...
  try {
    ...
    const validationIssues = failure.category === "VALIDATION" ? guideValidationIssuesForDirector(error) : [];
    await failFn({
      claimToken,
