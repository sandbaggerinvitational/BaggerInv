export function participantDestination(pathname, search = "", playerSlug = "") {
  ...
  if (pathname === "/players") return "Player";
...
export function participantAppShellRoute(pathname = "") {
  ...
  if (!route || route === "/" || route.startsWith("/admin") || route.startsWith("/participant-auth") ||
    route.startsWith("/activate") || route.startsWith("/score/access")) return false;
  if (route === "/players" || route === "/courses") return true;
  if (/^\/courses\/[^/]+(?:\/holes\/[^/]+)?$/.test(route)) return true;
  if (route === "/history" || route.startsWith("/history/")) return true;
  if (/^\/players\/[^/]+$/.test(route)) return true;
  return [
    "/home",
    "/my-match",
    "/score",
...
export function participantRouteContext(pathname = "", search = "") {
  ...
  if (/^\/players\/[^/]+$/.test(route)) return "Career Profile";
  ...
  if (/^\/courses\/[^/]+\/holes\/[^/]+$/.test(route)) return "Course Hole";
  if (route.startsWith("/courses/")) return "Course";
  if (route === "/history") return "History";
  const historyRound = route.match(/^\/history\/(\d{4})\/round\//);
  if (historyRound) return `${historyRound[1]} Round History`;
  const historyTeam = route.match(/^\/history\/(\d{4})\/team\//);
  if (historyTeam) return `${historyTeam[1]} Team History`;
  const historyYear = route.match(/^\/history\/(\d{4})/);
