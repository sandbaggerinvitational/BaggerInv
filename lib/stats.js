...
import { markDataAuthorityFallback } from "./data-authority-request.js";
import {
  loadCanonical2017To2022HistoricalData,
  loadCanonicalCareerHistoricalData,
  loadCanonical2023HistoricalData,
  loadCanonical2024HistoricalData,
  loadHistoricalData,
} from "./google-sheets-data.js";
import { formatHandicap, parseNumericValue } from "./formatters.js";
import {
  historicalCaptainReference,
  resolveHistoricalCaptain,
} from "./historical-captains.js";
import {
...
} from "./legacy-history-player-identity.js";
import { projectHistoricalAwards } from "./historical-awards.js";

// Legacy routes still use the process-level Google-backed model. Migrated
// secondary-history routes bind an immutable, request-local model through this
// AsyncLocalStorage boundary instead of mutating that shared process state.
let legacyHistoricalData = fallbackHistoricalData;
let legacyHistoricalSourceDiagnostics = Object.freeze({
  mode: "bundled-initial",
  usedBundledFallback: true,
  refreshedAt: null,
  errorCode: "",
});
const historicalDataContext = new AsyncLocalStorage();
const historicalData = new Proxy({}, {
  get(_target, property) {
    return (historicalDataContext.getStore() || legacyHistoricalData)?.[property];
  },
  has(_target, property) {
    return property in (historicalDataContext.getStore() || legacyHistoricalData || {});
  },
  ownKeys() {
    return Reflect.ownKeys(historicalDataContext.getStore() || legacyHistoricalData || {});
  },
  getOwnPropertyDescriptor(_target, property) {
    const source = historicalDataContext.getStore() || legacyHistoricalData || {};
    return Object.hasOwn(source, property)
      ? { configurable: true, enumerable: true, value: source[property] }
      : undefined;
  },
});
...
export async function refreshHistoricalData() {
  try {
    legacyHistoricalData = normalizeHistoricalData(await loadHistoricalData());
    legacyHistoricalSourceDiagnostics = Object.freeze({
      mode: "google-live",
    ...
    legacyHistoricalData = normalizeHistoricalData(fallbackHistoricalData);
    markDataAuthorityFallback("historical-data.json:legacy-history");
    legacyHistoricalSourceDiagnostics = Object.freeze({
      mode: "bundled-fallback",
      usedBundledFallback: true,
      refreshedAt: new Date().toISOString(),
      errorCode: String(error?.code || error?.name || "HISTORICAL_REFRESH_FAILED"),
    });
  }
...
/** Read-only diagnostics for protected prediction parity operations. */
export function getLegacyHistoricalSourceDiagnostics() {
  return { ...legacyHistoricalSourceDiagnostics };
}

export async function refreshCanonical2017To2022HistoricalData() {
  try {
    legacyHistoricalData = normalizeHistoricalData(await loadCanonical2017To2022HistoricalData());
  } catch (error) {
    // Do not revive the known-stale bundled 2019/2020 point populations when
    // the canonical archive is unavailable. Step 3C pages fail closed instead.
    console.error("Unable to refresh canonical 2017–2022 historical Google Sheet data.", {
      name: error?.name,
      message: error?.message || String(error),
      stack: error?.stack,
    });
...
export async function refreshCanonicalCareerHistoricalData() {
  try {
    legacyHistoricalData = normalizeHistoricalData(await loadCanonicalCareerHistoricalData());
  } catch (error) {
    // Career facts must not revive the stale bundled completed-year point
    // populations when the frozen production archive is unavailable.
    console.error("Unable to refresh canonical Career Profile historical data.", {
      name: error?.name,
      message: error?.message || String(error),
      stack: error?.stack,
    });
...
export async function refreshCanonical2024HistoricalData() {
  try {
    legacyHistoricalData = normalizeHistoricalData(await loadCanonical2024HistoricalData());
  } catch (error) {
    console.error("Unable to refresh canonical legacy Google Sheet data. Using the bundled fallback.", {
      name: error?.name,
      message: error?.message || String(error),
      stack: error?.stack,
    });
    legacyHistoricalData = normalizeHistoricalData(fallbackHistoricalData);
    markDataAuthorityFallback("historical-data.json:canonical-2024");
  }

  return historicalData;
}

export async function refreshCanonical2023HistoricalData() {
  try {
    legacyHistoricalData = normalizeHistoricalData(await loadCanonical2023HistoricalData());
  } catch (error) {
    console.error("Unable to refresh canonical 2023 historical Google Sheet data. Using the bundled fallback.", {
      name: error?.name,
      message: error?.message || String(error),
      stack: error?.stack,
    });
    legacyHistoricalData = normalizeHistoricalData(fallbackHistoricalData);
    markDataAuthorityFallback("historical-data.json:canonical-2023");
  }
...
function teamColor(year, side) {
  const row = teamRow(year, side);
  return clean(row?.["Primary Color"] || row?.["Team Color"] || row?.Color);
}

function teamResolved(year, side) {
  return Boolean(teamRow(year, side));
}
...
function ghostMatchExclusions() {
  return buildGhostMatchExclusionSet(historicalData.ghostMatches || []);
}

function includeOfficialMatchRecord(match, playerId, exclusions = ghostMatchExclusions()) {
  return !isPlayerExcludedFromMatchRecord(match?.["Match ID"], playerId, exclusions);
}
...
export function getPlayerStats(playerId) {
  ...
  const exclusions = ghostMatchExclusions();
  for (const match of historicalData.matches) {
    const side = playerSide(match, playerId);
    if (!side || !includeOfficialMatchRecord(match, playerId, exclusions)) continue;
    ...
    if (!seasons.has(year)) {
      seasons.set(year, {
        ...
        teamColor: teamColor(year, `Team ${side}`),
        teamResolved: teamResolved(year, `Team ${side}`),
        handicap: null,
        overall: emptyRecord(),
        BB: emptyRecord(),
        SC: emptyRecord(),
  ...
  for (const row of handicaps) {
    ...
    if (!seasons.has(year)) {
      seasons.set(year, {
        ...
        teamColor: teamColor(year, side),
        teamResolved: teamResolved(year, side),
      ...
      seasons.get(year).teamColor = teamColor(year, side);
      seasons.get(year).teamResolved = teamResolved(year, side);
    }
  }
  ...
  const pointsChampionYears = historicalData.tournaments
    .filter((row) =>
      clean(row["Winning Team"]) ||
      ["FINAL", "COMPLETE", "COMPLETED"].includes(clean(row.lifecycle || row.Status).toUpperCase())
    )
...
function historicalWinner(value, year) {
  const normalized = clean(value).toLowerCase();
  if (!normalized) return { side: null, label: "" };
  if (["halved", "half", "tie"].includes(normalized)) {
    return { side: 0, label: "Halved" };
  }
  if (["team 1", "1"].includes(normalized)) {
    return { side: 1, label: teamName(year, "Team 1") };
  }
  if (["team 2", "2"].includes(normalized)) {
    return { side: 2, label: teamName(year, "Team 2") };
  }
  const firstTeam = teamRow(year, "Team 1");
  const secondTeam = teamRow(year, "Team 2");
  const matched = [
    firstTeam ? { side: 1, row: firstTeam } : null,
    secondTeam ? { side: 2, row: secondTeam } : null,
  ].filter(Boolean).find(({ row }) =>
    [row["Team ID"], row["Team Side"], row["Team Names"]]
      .map((candidate) => clean(candidate).toLowerCase())
      .includes(normalized)
  );
  return matched
    ? { side: matched.side, label: clean(matched.row["Team Names"]) }
    : { side: null, label: clean(value) };
}

function historicalTeam(year, side) {
  const row = teamRow(year, `Team ${side}`);
  return {
    id: clean(row?.["Team ID"]),
    side: `Team ${side}`,
    name: clean(row?.["Team Names"]) || `Team ${side}`,
    color: clean(row?.["Primary Color"] || row?.["Team Color"] || row?.Color)
      || (side === 1 ? "#0b4335" : "#9a7422"),
    resolved: Boolean(row),
  };
}

function historicalCourse(match) {
  const courseId = clean(match["Course ID"]);
  const round = Number(match.Round);
  const course = historicalData.courses.find((row) => {
    if (Number(row.Year) !== Number(match.Year)) return false;
    if (courseId) return clean(row["Course ID"]) === courseId;
    return Number(clean(row.Round).replace(/\D/g, "")) === round;
  });
  return course
    ? {
        id: clean(course["Course ID"]),
        name: clean(course.Course || course["Course Name"]),
      }
    : null;
}

function sameRecord(one, two) {
  return ["wins", "losses", "halves", "matches"]
    .every((field) => Number(one?.[field] || 0) === Number(two?.[field] || 0));
}

export function getPlayerFormatMatchHistory(playerId, expectedRecords = null) {
  const playerMap = getPlayerMap();
  const formats = { BB: [], SC: [], SI: [] };
  ...
  for (const match of historicalData.matches) {
    ...
    const year = Number(match.Year);
    const round = Number(match.Round);
    const ownIds = playersForSide(match, side);
    const opponentIds = playersForSide(match, side === 1 ? 2 : 1);
    const winner = historicalWinner(
      match["Matchup Winner"] || match["18-Hole Winner"],
      year
    );
    const outcome = winner.side === 0
      ? "half"
      : winner.side === side
        ? "win"
        : winner.side
          ? "loss"
          : "unknown";
    const team = historicalTeam(year, side);
    const opposingTeam = historicalTeam(year, side === 1 ? 2 : 1);
    const course = historicalCourse(match);
    const matchId = clean(match["Match ID"]);
    const hasRoundLink = Number.isFinite(year) && Number.isFinite(round) && Boolean(course);
    const segment = (label, value) => {
      const result = historicalWinner(value, year);
      return clean(value) ? { label, winner: result.label || "Not recorded", side: result.side } : null;
    };

    formats[format].push({
      id: matchId || `${year}-${round}-${clean(match.Match)}-${playerId}`,
      year,
      round,
      format,
      matchNumber: Number(match.Match) || null,
      partner: ownIds
        .filter((id) => id !== playerId)
        .map((id) => ({ id, name: playerMap[id]?.["Display Name"] || id })),
      opponents: opponentIds.map((id) => ({
        id,
        name: playerMap[id]?.["Display Name"] || id,
      })),
      team,
      opposingTeam,
      winner: winner.label || "Not recorded",
      winnerSide: winner.side,
      outcome,
      course,
      segments: [
        segment("Front 9", match["Front 9 Winner"]),
        segment("Back 9", match["Back 9 Winner"]),
        segment("Overall", match["18-Hole Winner"] || match["Matchup Winner"]),
      ].filter(Boolean),
      href: hasRoundLink
        ? `/history/${year}/round/${round}${matchId ? `#match-${encodeURIComponent(matchId)}` : ""}`
        : null,
      issues: [
        !winner.label ? "Historical match missing winner" : null,
        !team.resolved || !opposingTeam.resolved ? "Historical team cannot be resolved for Year" : null,
        !hasRoundLink ? "Historical round link unavailable" : null,
      ].filter(Boolean),
    });
  }

  return Object.fromEntries(Object.entries(formats).map(([format, matches]) => {
    const ordered = matches.sort((a, b) =>
      b.year - a.year || b.round - a.round || (b.matchNumber || 0) - (a.matchNumber || 0)
    );
    const record = ordered.reduce((total, match) => {
      if (match.outcome === "unknown") return total;
      total.matches += 1;
      if (match.outcome === "win") total.wins += 1;
      if (match.outcome === "loss") total.losses += 1;
      if (match.outcome === "half") total.halves += 1;
      return total;
    }, emptyRecord());
    const expected = expectedRecords?.[format] || null;
    const years = [...new Set(ordered.map((match) => match.year))]
      .sort((a, b) => b - a)
      .map((year) => ({
        year,
        matches: ordered.filter((match) => match.year === year),
      }));
    return [format, {
      format,
      matches: ordered,
      ...
      consistent: expected ? sameRecord(record, expected) : true,
      firstYear: ordered.at(-1)?.year || null,
      latestYear: ordered[0]?.year || null,
    }];
  }));
}
...
export function getAllPlayerStats(precomputedRatings = null) {
  const ratingMap = Object.fromEntries(
    (precomputedRatings || getSandbaggerRatings()).byCategory.OVERALL.map((row) => [row.player["Player ID"], row.allRatings])
  );
...
export function getHeadToHead(playerOneId, playerTwoId, precomputed = {}) {
  ...
  const exclusions = precomputed.exclusions || ghostMatchExclusions();
  const matches = precomputed.matches || chronologicalMatches();
  for (const match of matches) {
    const sideOne = playerSide(match, playerOneId);
    const sideTwo = playerSide(match, playerTwoId);

    if (
      !sideOne || !sideTwo || sideOne === sideTwo ||
      !includeOfficialMatchRecord(match, playerOneId, exclusions) ||
      !includeOfficialMatchRecord(match, playerTwoId, exclusions)
    ) continue;
...
export function getRecords(precomputedAllPlayerStats = null) {
  const all = precomputedAllPlayerStats || getAllPlayerStats();
...
export function getPartnershipStats() {
  ...
  for (const match of chronologicalMatches()) {
    for (const side of [1, 2]) {
      ...
      if (players.some((playerId) => !includeOfficialMatchRecord(match, playerId, exclusions))) continue;
  ...
  for (const match of chronologicalMatches()) {
    ...
    for (const oneId of teamOne) {
      for (const twoId of teamTwo) {
        ...
        if (
          !includeOfficialMatchRecord(match, oneId, exclusions) ||
          !includeOfficialMatchRecord(match, twoId, exclusions)
        ) continue;
...
export function getTeamSeason(year, side) {
  ...
  const recordedHandicaps = roster.map((row) => row.handicap).filter(Number.isFinite);
  const averageHandicap = roster.length > 0 && recordedHandicaps.length === roster.length
    ? recordedHandicaps.reduce((sum, handicap) => sum + handicap, 0) /
      recordedHandicaps.length
    : null;
...
export function getHistoricalRound(yearValue, roundValue) {
  ...
  const step3CYear = year >= 2017 && year <= 2022;
  const roundPoints = recordedMatches.length && (!step3CYear || recordedMatches.length === matches.length)
    ? {
    ...
    .flatMap((tournament) => {
      ...
      try {
        ...
        return [{
          ...
          courses,
          awards: projectHistoricalAwards(historicalData.awards, year, playerMap),
        }];
      } catch (error) {
...
export function getTournamentMatches(yearValue) {
  const year = Number(yearValue);
  return historicalData.matches
    .filter((match) => Number(match.Year) === year)
    .map((match) => ({ ...match }));
}
...
export function getTournamentPlayerLeaderboard(yearValue) {
  const year = Number(yearValue);
  const playerMap = getPlayerMap();
  const rosterRows = historicalData.handicaps.filter((row) => Number(row.Year) === year);
  ...
  for (const match of historicalData.matches.filter((row) => Number(row.Year) === year)) {
    ...
    for (const side of [1, 2]) {
      for (const playerId of playersForSide(match, side)) {
        if (!includeOfficialMatchRecord(match, playerId, exclusions)) continue;
        if (!standings.has(playerId)) {
          const identity = resolveLegacyHistoryLeaderboardPlayer({
            playerId,
            year,
            playerMap,
            rosterRows,
...
export function getTournamentRules(yearValue) {
  const year = Number(yearValue);
  return historicalData.rules
    .filter((rule) => Number(rule?.Year) === year)
    .map((rule) => ({ ...rule }));
}

export function getRoundFormats() {
  return historicalData.rounds.map((round) => ({ ...round }));
}
...
function freezeHistoricalValue(value, seen = new WeakSet()) {
  if (!value || typeof value !== "object" || seen.has(value)) return value;
  seen.add(value);
  for (const child of Object.values(value)) freezeHistoricalValue(child, seen);
  return Object.freeze(value);
}

/**
 * Bind the established deterministic statistics algorithms to a strict,
 * request-local canonical fact bundle. This deliberately does not call any
 * loader and it refuses incomplete input rather than reaching for Google or
 * the bundled historical snapshot.
 */
export function createHistoricalStatsModel(data = {}) {
  for (const collection of HISTORICAL_COLLECTIONS) {
    if (!Array.isArray(data?.[collection])) {
      const error = new Error(`Canonical historical collection ${collection} is required.`);
      error.code = "CANONICAL_HISTORICAL_COLLECTION_REQUIRED";
      throw error;
    }
  }
  const normalized = Object.fromEntries(HISTORICAL_COLLECTIONS.map((collection) => [
    collection,
    data[collection].filter((row) => row && typeof row === "object").map((row) => ({ ...row })),
  ]));
  if (!normalized.tournaments.length) {
    const error = new Error("Canonical historical tournaments are required.");
    error.code = "CANONICAL_HISTORICAL_TOURNAMENTS_REQUIRED";
    throw error;
  }
  freezeHistoricalValue(normalized);
  const run = (operation) => historicalDataContext.run(normalized, operation);
  const invoke = (operation) => (...args) => run(() => operation(...args));
  const memoized = (operation) => {
    let resolved = false;
    let value;
    return () => {
      if (!resolved) {
        value = freezeHistoricalValue(run(operation));
        resolved = true;
      }
      return value;
    };
  };
  const ratings = memoized(getSandbaggerRatings);
  const allPlayerStats = memoized(() => getAllPlayerStats(ratings()));
  const records = memoized(() => getRecords(allPlayerStats()));
  const partnershipStats = memoized(getPartnershipStats);
  const rivalryStats = memoized(getRivalryStats);
  const tournaments = memoized(getTournaments);
  const chronological = memoized(chronologicalMatches);
  const exclusions = memoized(ghostMatchExclusions);
  const headToHead = (...args) => run(() => getHeadToHead(...args, {
    matches: chronological(),
    exclusions: exclusions(),
  }));
  return Object.freeze({
    source: "request-local-canonical-history",
    data: normalized,
    run,
    getPlayerMap: invoke(getPlayerMap),
    getPlayers: invoke(getPlayers),
    getPlayerBySlug: invoke(getPlayerBySlug),
    getTournamentHandicap: invoke(getTournamentHandicap),
    getPlayerStats: invoke(getPlayerStats),
    getPlayerFormatMatchHistory: invoke(getPlayerFormatMatchHistory),
    getCaptainLegacy: invoke(getCaptainLegacy),
    getAllPlayerStats: allPlayerStats,
    getHeadToHead: headToHead,
    getSandbaggerRatings: ratings,
    getRecords: records,
    getPartnershipStats: partnershipStats,
    getRivalryStats: rivalryStats,
    getHandicapStats: invoke(getHandicapStats),
    getTeamSeason: invoke(getTeamSeason),
    getTournaments: tournaments,
    getTournament: invoke(getTournament),
    getTournamentMatches: invoke(getTournamentMatches),
    getTournamentPlayerLeaderboard: invoke(getTournamentPlayerLeaderboard),
    getCourses: invoke(getCourses),
    getTournamentRules: invoke(getTournamentRules),
    getRoundFormats: invoke(getRoundFormats),
    getCourse: invoke(getCourse),
  });
}
