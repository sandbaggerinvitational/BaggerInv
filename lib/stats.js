...
import { formatHandicap } from "./formatters";
    ...
    .map((tournament) => {
      ...
      const teams = ["Team 1", "Team 2"]
        ...
        .map((team) => {
          ...
          const captainId = tournament[`Captain Team ${sideNumber}`] || team.captain?.["Player ID"] || null;

          return {
            ...team,
            captainId,
            captain: captainId ? playerMap[captainId] || team.captain : team.captain,
          };
        });
      const championTeam = teamForReference(teams, tournament["Winning Team"]);
      const runnerUpTeam = teamForReference(teams, tournament["Runner-Up Team"]);

      return {
        ...tournament,
        year,
        id: String(tournament["Tournament ID"] || tournament.Year),
