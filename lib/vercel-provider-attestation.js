...
function validateLiveInventoryRecords(value, request) {
  ...
  const tuples = value.map((record) => {
    ...
    if (!DEPLOYMENT_ID.test(tuple[0]) || !tuple[2] ||
        (!isExactRetained && (tuple[1] !== request.candidateCommitSha ||
  ...
  if (tuples.length < retained.recordCount + 1 || new Set(keys).size !== tuples.length ||
    ...
    throw attestationError(
      ...
      { expectedMinimumCount: retained.recordCount + 1, observedCount: tuples.length },
      409,
    );
  }
  return Object.freeze({
