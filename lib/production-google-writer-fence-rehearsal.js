...
const METADATA_FIELDS = [
  ...
  "editors(users,groups,domainUsersCanEdit),requestingUserCanEdit),filterViews(filterViewId,title,range),",
  "basicFilter(range),merges)",
].join("");
...
function exactFenceProtection(protection, sheetId, dedicatedEmail, descriptionPrefix) {
  const editors = protection.editors || {};
  const users = [...new Set((editors.users || []).map((value) => clean(value).toLowerCase()))].sort();
  return Boolean(descriptionPrefix) &&
    ...
    users.length === 1 && users[0] === dedicatedEmail.toLowerCase() &&
...
export async function executeProductionGoogleWriterFenceRehearsal(
  ...
  try {
    ...
    const legacyAnalysis = await freshAnalysis(legacyToken, environment, fetchImpl, fenceScope);
    assertBaseline(legacyAnalysis, input.expectedBaselineFingerprint);
...
export async function executeProductionGoogleWriterProviderFence(
  ...
  if (action === "install") {
    ...
    try {
      ...
      const legacyAnalysis = await freshAnalysis(
        legacyToken,
        environment,
        fetchImpl,
        fenceScope,
