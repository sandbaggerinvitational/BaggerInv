...
function requiredPhase(functionName, body = {}) {
  ...
  if (name === "read_production_calcutta_v1") return "OBSERVATION";
...
export function productionCutoverReadRpcTranslation(functionName, body = {}, env = process.env, {
  ...
  if (name === "authorize_match_access") {
    const targetTournamentId = clean(currentRuntime?.tournamentId);
    const pointerRevision = Number(currentRuntime?.pointerRevision);
    if (!/^\d{4}$/.test(targetTournamentId) ||
        !Number.isSafeInteger(pointerRevision) || pointerRevision < 1) {
      const error = new Error(
        "The current Production match authorization runtime is unavailable.",
      );
    ...
    return {
      functionName: "authorize_production_current_match_access_v1",
      body: { input: readScope(env, {
        target_tournament_id: targetTournamentId,
        expected_current_tournament_id: targetTournamentId,
        expected_pointer_revision: pointerRevision,
        ...(annualRuntimeInput || {}),
        target_player_id: clean(body.target_player_id),
        target_match_id: clean(body.target_match_id),
        requested_action: clean(body.requested_action).toUpperCase(),
      }) },
      requiredPhase: phase,
      adapter: "MATCH_AUTHORIZATION_DECISION",
      preserveNativePayload: true,
    };
  }
  ...
  if (name === "read_production_net_skins_v1") {
    const playerId = clean(body?.input?.player_id);
    if (annualRuntimeInput) {
      return {
        functionName: "read_production_future_current_view_v1",
        body: { input: readScope(env, {
          surface: "NET_SKINS_V1",
          ...(playerId ? { player_id: playerId } : {}),
          ...annualRuntimeInput,
          target_tournament_id: annualRuntimeInput.expected_current_tournament_id,
        }) },
        requiredPhase: phase,
        adapter: "ACTIVE_CUTOVER",
      };
    ...
    return {
      functionName: "read_production_net_skins_v1",
      body: { input: readScope(env, playerId ? { player_id: playerId } : {}) },
      requiredPhase: phase,
      adapter: "ACTIVE_CUTOVER",
    };
  }
  if (name === "read_production_calcutta_v1") {
    const playerId = clean(body?.input?.player_id);
    const observer = clean(body?.input?.observer_auth_user_id);
    const reader = {...(playerId ? {player_id:playerId} : {}), ...(observer ? {observer_auth_user_id:observer} : {})};
    if (annualRuntimeInput) {
      return {
        functionName: "read_production_future_current_view_v1",
        body: { input: readScope(env, {
    ...
    return {
      functionName: "read_production_calcutta_v1",
      body: { input: readScope(env, reader) },
      requiredPhase: phase,
      adapter: "ACTIVE_CUTOVER",
    };
  }
  ...
  const candidate = productionShadowCandidateRpcTranslation(name, body, { annualRuntimeInput });
  ...
  const activeInput = readScope(env, activeSurface);
  const completedHistoryYear = Number(activeSurface.tournament_year);
  if (name === "read_preview_completed_history" &&
      clean(activeSurface.mode).toUpperCase() === "YEAR" &&
      Number.isInteger(completedHistoryYear) &&
      completedHistoryYear >= 2017 && completedHistoryYear <= 2025) {
    // tournament_id remains the frozen current Production resource assertion;
    // tournament_year is the completed-history query selector consumed by the
    // installed Production reader.
    activeInput.tournament_year = completedHistoryYear;
  }
  return {
    ...candidate,
...
export function adaptProductionCutoverReadPayload(payload, translation, state) {
  if (translation?.preserveNativePayload === true) return payload;
