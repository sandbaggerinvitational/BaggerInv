...
export function predictionInputBundleEnvironment(env = process.env) {
  ...
  const projectApproved = projectUrl.includes(PREVIEW_COMPLETED_HISTORY_SUPABASE_PROJECT_REF);
  ...
  const available = preview && projectApproved && credentialsConfigured && secondaryHistorySelected;
