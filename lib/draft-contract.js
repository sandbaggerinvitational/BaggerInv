...
export function buildDraftPresentationSeed(settings, allPicks, history, options = {}) {
  ...
  if (strict && upper(settings["Draft Format"]) === "SNAKE") {
    ...
    const mismatch = picks.find((pick, index) => pick.player && pick.team?.id !== expected[index]);
    if (mismatch) {
      const error = new Error(`Draft Pick ${mismatch.pickNumber} does not follow the configured snake order for ${year}.`);
      error.code = "DRAFT_PICK_ORDER_INVALID";
      error.diagnostics = { year, pickNumber: mismatch.pickNumber, expectedTeamId: expected[mismatch.pickNumber - 1], actualTeamId: mismatch.team?.id || mismatch.teamId };
      throw error;
    }
  }
