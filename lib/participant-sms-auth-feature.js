...
/** Production Text requires its separately certified provider and current-authority prerequisites. */
export function participantProductionSmsConfigured(env = process.env) {
  const identity = participantIdentityAuthorityEnvironment(env);
  return clean(env.VERCEL_ENV).toLowerCase() === "production" &&
    identity.productionCutoverIdentity && identity.resolved === "supabase" &&
    enabled(env.PARTICIPANT_SMS_AUTH_ENABLED) &&
    enabled(env.PARTICIPANT_SMS_PROVIDER_CONFIGURED) &&
    enabled(env.PARTICIPANT_SMS_PRODUCTION_AUTHORITY_CONFIGURED) &&
    participantAuthCaptchaConfigured(env) && participantSmsRateLimitConfigured(env);
}

export function participantSmsAuthFeatureConfigured(env = process.env) {
  if (participantProductionSmsConfigured(env)) return true;
...
export function participantAuthExperienceConfiguration(env = process.env) {
  ...
  const smsRequested = (preview || productionCutoverIdentity) && enabled(env.PARTICIPANT_SMS_AUTH_ENABLED) &&
  ...
  const smsEnabled = productionCutoverIdentity
    ? participantProductionSmsConfigured(env)
    : smsRequested && captchaReady && rateLimitReady;
  return {
    preview,
    smsRequested,
    smsEnabled,
    ...
    defaultMethod: "email",
    productionSmsAuthority: productionCutoverIdentity && smsEnabled,
    rollout: productionCutoverIdentity
      ? (clean(env.PARTICIPANT_SMS_PRODUCTION_ROLLOUT).toUpperCase() === "VERIFIED" ? "VERIFIED" : "DESIGNATED")
      : participantSmsPreviewRollout(env),
