...
function tournamentHandicap(value) {
  ...
  let source = clean(value)
    ...
    .replace(/,/g, "")
    .replace(/[−–—]/g, "-");
  const parentheticalNegative = /^\(.+\)$/.test(source);
  if (parentheticalNegative) source = source.slice(1, -1).trim();
  const parsed = Number(source);
  if (!Number.isFinite(parsed)) return null;
  return parentheticalNegative ? -Math.abs(parsed) : parsed;
}
 ...
 * which renders negative values parenthetically. Source parenthetical notation
 * is parsed as its signed negative value, a source-authored leading plus remains
 * explicit, and History always renders negative values with a minus sign.
 */
export function formatHistoryTournamentHandicap(value) {
  ...
  const numeric = tournamentHandicap(value);
