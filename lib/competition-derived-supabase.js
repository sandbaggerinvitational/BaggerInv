...
async function productionDerivedWorkerContext(options = {}) {
  const env = options.env || process.env;
  if (clean(env.VERCEL_ENV).toLowerCase() !== "production") return null;
  if (options.scoringDispatchContext) return options.scoringDispatchContext;
  const resolve = options.resolveProductionScoringDispatchContext ||
    (await import("./production-scoring-operations-server.js"))
      .resolveProductionScoringDispatchContext;
  return resolve({ requiredPhase: "WORKERS", env });
}

async function derivedWorkerRpc(operationName, input, options = {}) {
  const env = options.env || process.env;
  const context = await productionDerivedWorkerContext(options);
  if (context?.runtime?.tournamentId) {
    const rpc = options.productionScoringOperationsRpc ||
      (await import("./production-scoring-operations-server.js"))
        .productionScoringOperationsRpc;
    const boundedInput = { ...(input || {}) };
    delete boundedInput.environment;
    delete boundedInput.tournament_id;
    if (clean(options.workerId)) boundedInput.worker_id = clean(options.workerId);
    if (options.leaseSeconds) boundedInput.lease_seconds = options.leaseSeconds;
    return rpc(operationName, boundedInput, {
      env,
      scoringDispatchContext: context,
      ...(options.fetchImpl ? { fetchImpl: options.fetchImpl } : {}),
    });
  }
  const legacyInput = { ...(input || {}) };
  delete legacyInput.worker_id;
  delete legacyInput.lease_seconds;
  delete legacyInput.claim_token;
  return scoringShadowRpc(operationName, { input: legacyInput }, {
    ...options,
    env,
...
export async function writeCompetitionDerivedSnapshot(input, options = {}) {
  return derivedWorkerRpc("write_competition_derived_snapshot", input, {
    ...options, timeoutMs: options.timeoutMs || 15_000,
  });
}

export async function requestCompetitionDerivedRecalculation(tournamentId, engineKeys, {
  requestedBy = "Derived-state worker", reason = "EXPLICIT_REBUILD", ...options
} = {}) {
  return derivedWorkerRpc("request_competition_derived_recalculation", {
...
export async function claimCompetitionDerivedJobs(tournamentId, engineKeys, options = {}) {
  return derivedWorkerRpc("claim_competition_derived_jobs", {
    environment: "PREVIEW", tournament_id: clean(tournamentId), engine_keys: engineKeys,
    worker_id: clean(options.workerId), lease_seconds: options.leaseSeconds || 90,
  }, { ...options, timeoutMs: 8_000 });
}

export async function markCompetitionDerivedJobFailed(tournamentId, claim, error, options = {}) {
  return derivedWorkerRpc("mark_competition_derived_job_failed", {
    ...
    claim_token: clean(claim?.claim_token), worker_id: clean(options.workerId),
...
function storylineFreshnessBoost(story = {}) {
  const label = clean(story.freshnessLabel);
  if (label === "Just Finalized") return 28;
  if (label === "Recently Clinched") return 24;
  if (label === "NEW") return 20;
  const updatedMinutes = label.match(/^Updated (\d+) min ago$/);
  return updatedMinutes ? Math.max(2, 14 - Math.floor(number(updatedMinutes[1]) / 5)) : 0;
}

function semanticStorylineRecords(stories = []) {
  const remaining = stories.map((story) => ({
    story,
    semanticPriority: number(story.priorityScore) - storylineFreshnessBoost(story),
  }));
  const ordered = [];
  const categoryCounts = new Map();
  while (remaining.length) {
    remaining.sort((left, right) => {
      const leftCategory = clean(left.story.category);
      const rightCategory = clean(right.story.category);
      const leftAdjusted = left.semanticPriority - (categoryCounts.get(leftCategory) || 0) * 90;
      const rightAdjusted = right.semanticPriority - (categoryCounts.get(rightCategory) || 0) * 90;
      return rightAdjusted - leftAdjusted || clean(left.story.id).localeCompare(clean(right.story.id));
    });
    const next = remaining.shift();
    const category = clean(next.story.category);
    ordered.push(storylineSemanticRecord({ ...next.story, editorialRank: ordered.length + 1 }));
...
export function calculateCompetitionDerivedFromData(data = {}, { referenceTime = Date.now(),
  ...
  const stories = semanticStorylineRecords(rawStories);
  ...
  return {
    ...
    storylines: {
      ...
      timeRelativeContract: { labelsCalculatedOnRead: true, expiringSemanticCategories: ["recent-clinch"] },
    },
...
function snapshotInput({ tournamentId, engineKey, engineVersion, configurationFingerprint,
  ...
  claimStartedAt, claimToken, workerId }) {
  ...
  return {
    ...
    claim_token: clean(claimToken), worker_id: clean(workerId),
  };
}
...
export async function recalculateCompetitionDerivedTournament(tournamentId, {
  ...
  debounceMs = 250, env = process.env, workerId = "competition-derived-current",
  ...transportOptions
} = {}) {
  const scoringDispatchContext = await productionDerivedWorkerContext({
    env, ...transportOptions,
  });
  const workerOptions = {
    env, workerId, scoringDispatchContext, ...transportOptions,
  };
  let resolvedTournamentId = clean(scoringDispatchContext?.runtime?.tournamentId || tournamentId);
  ...
  if (force) {
    const requested = await requestCompetitionDerivedRecalculation(resolvedTournamentId, requestedEngines, {
      requestedBy: calculatedBy, reason: "EXPLICIT_REBUILD", ...workerOptions,
    });
  ...
  const claimed = await claimCompetitionDerivedJobs(resolvedTournamentId, requestedEngines, workerOptions);
  ...
  try {
    ...
    const writes = await Promise.all(claims.map((claim) => {
      ...
      if (engineKey === "TEAM_MOMENTUM") return writeCompetitionDerivedSnapshot(snapshotInput({
        ...
        claimStartedAt: claim.claim_started_at, claimToken: claim.claim_token,
        workerId,
      }), workerOptions);
    ...
    await Promise.allSettled(claims.map((claim) =>
      markCompetitionDerivedJobFailed(resolvedTournamentId, claim, error, workerOptions)));
    throw error;
  }
}
...
export async function currentCompetitionDerivedState(tournamentId, options = {}) {
  const read = await readCompetitionDerivedState(tournamentId, options.engineKeys, { env: options.env });
