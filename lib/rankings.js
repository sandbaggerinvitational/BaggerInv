...
/** Numeric competition ranks for server projections (1, 2, 2, 4). */
export function addNumericTournamentRanks(rows, valueOrSelector) {
  return addTournamentRanks(rows, valueOrSelector).map(({ tournamentRank, ...row }) => ({
    ...row,
    rank: Number(String(tournamentRank).replace(/^T/, "")),
  }));
}
