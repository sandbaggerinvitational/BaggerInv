...
import {
  assertProductionFutureGoogleWriterResources,
} from "./production-future-google-writer-server.js";
import {
...
const PRODUCTION_OPERATIONS = Object.freeze({
  ...
  FUTURE_MATCH_GOOGLE_COMPATIBILITY: Object.freeze({
    googleRead: true,
    googleWrite: true,
    requiresRuntimeSupabase: true,
    authority: "SCORING",
...
function exactResources(resources = {}, {
  futureGoogleWriter = false,
  currentScoringWorker = false,
  env = process.env,
} = {}) {
  const tournamentYear = Number(resources.tournamentYear);
  let certifiedFutureWriter = false;
  if (futureGoogleWriter) {
    try {
      certifiedFutureWriter = assertProductionFutureGoogleWriterResources(resources);
    } catch {
      certifiedFutureWriter = false;
    }
  }
  let certifiedCurrentScoring = false;
  if (currentScoringWorker) {
    try {
      const expected = productionScoringDispatchGoogleResources(
        resources.scoringDispatchContext,
        { requiredPhase: "WORKERS", env },
      );
      certifiedCurrentScoring = [
      ...
      ].every((key) => String(resources[key] ?? "") === String(expected[key] ?? ""));
    } catch {
      certifiedCurrentScoring = false;
    }
  }
  return {
    ...
    tournamentId: futureGoogleWriter
      ? certifiedFutureWriter
      : certifiedCurrentScoring ||
        exact(resources.tournamentId, PRODUCTION_TOURNAMENT_ID),
    tournamentYear: futureGoogleWriter
      ? certifiedFutureWriter
      : certifiedCurrentScoring ||
...
export function productionGoogleCredentialEnvironment({
  ...
  const requestedResources = exactResources(resources, {
    futureGoogleWriter:
      normalizedOperation === "FUTURE_MATCH_GOOGLE_COMPATIBILITY",
    currentScoringWorker: [
