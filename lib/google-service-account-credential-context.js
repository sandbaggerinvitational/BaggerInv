...
export function assertProductionGoogleServiceAccountMutationBinding({
  ...
  const normalizedOperation = clean(operation).toUpperCase();
  if (!active || diagnostics?.credentialSource !== "production-worker") {
    throw credentialIntentError(
      "PRODUCTION_GOOGLE_DEDICATED_CREDENTIAL_REQUIRED",
      "Production Google writes require the dedicated Production credential.",
...
export function withProductionGoogleServiceAccountCredentials(
  ...
  return credentialContext.run({ credentials, diagnostics: state, canonicalAdmission }, () => {
