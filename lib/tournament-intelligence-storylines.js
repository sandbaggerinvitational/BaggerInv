...
export function tournamentIntelligenceStorylines({ snapshots = [], playerTeams = new Map() } = {}) {
  ...
  const priorFavoriteIndex = previous.players.findIndex((player) => String(player.id) === String(favorite.id));
  const priorFavoriteRank = priorFavoriteIndex < 0 ? 0 : publishedRank(previous.players[priorFavoriteIndex], priorFavoriteIndex + 1);
  stories.push({
    id: "current-favorite",
    icon: "🏆",
  ...
  const previousRanks = new Map(previous.players.map((player, index) => [String(player.id), publishedRank(player, index + 1)]));
  const movement = current.players.map((player, index) => ({ player, currentRank: publishedRank(player, index + 1), previousRank: previousRanks.get(String(player.id)) }))
