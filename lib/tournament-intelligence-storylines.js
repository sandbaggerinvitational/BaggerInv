...
export function tournamentIntelligenceStorylines({ snapshots = [], playerTeams = new Map() } = {}) {
  ...
  stories.push({
    ...
    support: priorFavoriteRank === 1
      ? `${favorite.name} remains the favorite at ${percent(favorite.probability)} in ${current.phase}.`
      : `${favorite.name} moved from ${ordinal(priorFavoriteRank)} to the top spot in ${current.phase}.`,
  });
