...
async function loadProductionMatchDetail(identity, matchId, {
  ...
  const decision = (await (dependencies.authorizeMatchAccess || authorizeMatchAccess)(scope, { env }))?.payload;
  if (decision?.allowed !== true || clean(decision.player_id) !== scope.playerId ||
  ...
  if (read?.payload?.ok !== true || !raw || clean(raw.tournament?.tournament_id) !== scope.tournamentId ||
      ...
      !Array.isArray(raw.participants) || !raw.participants.some((p) => clean(p.player_id) === scope.playerId)) {
