...
const RPCS = new Set([
  ...
  "mutate_production_round_pairings_v1",
  "read_production_net_skins_entries_v1",
...
function successful(payload, fallbackStatus = 409) {
  if (!payload || payload.ok === false) {
    ...
    error.matchId = /^[A-Z0-9_-]{1,64}$/.test(payload?.matchId || "") ? payload.matchId : "";
    error.playerId = /^[A-Z0-9_-]{1,64}$/.test(payload?.playerId || "") ? payload.playerId : "";
    throw error;
  }
  return payload;
}
...
// Entries only: the same fixed-resource/Director boundary, no scoring worker.
export async function productionNetSkinsEntries(actorInput, values = null, options = {}) {
  const input = fixedScope(values ? {
    round_number: values.roundNumber,
    expected_revision: values.expectedRevision,
    field_fingerprint: values.fieldFingerprint,
    operation_request_id: values.operationRequestId,
    configured: values.configured,
    entries: values.entries,
  } : {}, actorInput);
  const rpc = options.rpc || productionTournamentSetupRpc;
  const result = successful((await rpc(values ? "save_production_net_skins_entries_v1" : "read_production_net_skins_entries_v1", input, options)).payload);
  if (values) return result;
  return normalizeNetSkinsEntries(result.data);
}
...
export async function mutateProductionTournamentSetup({
  ...
  const rpc = options.rpc || productionTournamentSetupRpc;
  const result = await rpc(action === "replace-round-pairings" ? "mutate_production_round_pairings_v1" : "mutate_production_tournament_setup_v1", input, {
    env,
    activation,
    ...(options.rpcOptions || {}),
  });
