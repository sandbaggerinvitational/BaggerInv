...
async function assertCurrentProductionTournamentIdentity(data, env, dependencies) {
  if (clean(env.VERCEL_ENV).toLowerCase() !== "production") return;
  const reader = dependencies.readCurrentTournamentRuntime ||
    (await import("./production-current-tournament-runtime.js")).readProductionCurrentTournamentRuntime;
  const runtime = await reader({}, { env });
  if (clean(data?.tournament?.id) !== clean(runtime?.tournamentId) || runtime?.lifecycle !== "ACTIVE") {
    throw new ParticipantIdentityResolutionError(
      "PRODUCTION_CURRENT_TOURNAMENT_IDENTITY_NOT_POINTER_AWARE",
    );
  }
}
...
export async function resolveSupabaseParticipantIdentity({
  ...
  const readCandidate = dependencies.readCandidate || readProductionAuthCandidate;
  ...
  let candidateCertificationMs = 0;
  if (authority.productionShadowCandidate) {
    const candidateStarted = performance.now();
    const candidate = await readCandidate(tournamentId || "2026", dependencies.rpcOptions);
    candidateCertificationMs = performance.now() - candidateStarted;
    if (candidate.payload?.found !== true || candidate.payload?.status !== "VERIFIED" ||
        clean(candidate.payload?.authUserId) !== clean(verified.claims.sub)) {
      throw new ParticipantIdentityResolutionError("PRODUCTION_AUTH_CERTIFICATION_REQUIRED");
    }
  }
  const contextStarted = performance.now();
  const result = await readForAuth(
    { authUserId: verified.claims.sub, tournamentId },
    { ...(dependencies.rpcOptions || {}), env },
  );
  ...
  await assertCurrentProductionTournamentIdentity(result.payload.data, env, dependencies);
  return contextResult(result.payload.data, {
    authUserId: verified.claims.sub,
    timings: {
      sessionVerificationMs,
      candidateCertificationMs,
