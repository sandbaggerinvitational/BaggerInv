import {
  buildCalcuttaModel,
  CALCUTTA_FULL_COURSE_HANDICAP_POLICY,
...
export const CALCUTTA_ENGINE_VERSION = "calcutta-js-v2";
...
export function calculateCalcuttaFromSupabaseViews(configurationView = {}, coreView = {}) {
  ...
  const matchPlayRoundResults = calcuttaRoundResultsFromTournamentModel({
    year: configuration.year, rounds: core.rounds || [], scoreLeaderboard: core.scoreLeaderboard || [],
  });
  const frozenContextAvailable = Array.isArray(coreView.matches);
  const individualMatchPlayResults = matchPlayRoundResults.filter((row) =>
    new Set(["BB", "SI"]).has(clean(row.Format).toUpperCase()));
  if (!frozenContextAvailable && individualMatchPlayResults.length > 0) {
    throw Object.assign(new Error("Completed BB/SI Calcutta results require frozen Match scoring context."), {
      code: "CALCUTTA_FROZEN_SCORING_CONTEXT_REQUIRED",
    });
  }
  const frozenIndividualResults = frozenContextAvailable
    ? calcuttaRoundResultsFromFrozenScoringContext({
        year: configuration.year, rounds: core.rounds || [], matches: coreView.matches,
      })
    : [];
  const frozenIndividualKeys = new Set(frozenIndividualResults.map((row) =>
    `${number(row.Round)}:${clean(row.Format).toUpperCase()}:${clean(row["Player IDs"])}`));
  if (individualMatchPlayResults.some((row) =>
    !frozenIndividualKeys.has(`${number(row.Round)}:${clean(row.Format).toUpperCase()}:${clean(row["Player IDs"])}`))) {
    throw Object.assign(new Error("Completed BB/SI Calcutta results do not match frozen Match scoring context."), {
      code: "CALCUTTA_FROZEN_SCORING_CONTEXT_REQUIRED",
    });
  }
  const frozenFormats = new Set(frozenIndividualResults.map((row) => `${number(row.Round)}:${clean(row.Format).toUpperCase()}`));
  const canonicalRoundResults = [
    ...matchPlayRoundResults.filter((row) => !frozenFormats.has(`${number(row.Round)}:${clean(row.Format).toUpperCase()}`)),
    ...frozenIndividualResults,
  ].sort((left, right) => number(left.Round) - number(right.Round)
    || clean(left["Player IDs"]).localeCompare(clean(right["Player IDs"])));
  return calculateCalcuttaFromCanonicalRoundResults(configurationView, coreView, canonicalRoundResults);
}

// Financial pipeline shared by historical v2 and explicit Production Full Net.
// Inputs change; publication allocation, occupied places, ties and money do not.
export function calculateCalcuttaFromCanonicalRoundResults(configurationView, coreView, canonicalRoundResults,
  { engineVersion = CALCUTTA_ENGINE_VERSION, policy = CALCUTTA_FULL_COURSE_HANDICAP_POLICY } = {}) {
  const startedAt = performance.now();
  const core = coreView.tournament?.id ? coreView : leaderboardsCoreDataFromSupabaseView(coreView);
  const configuration = googleConfigurationFromView(configurationView);
  const players = Object.fromEntries((core.players || []).map((player) => [clean(player.id), player]));
  ...
  const sourceRevision = {
    ...
    calcuttaEngineVersion: engineVersion,
    calcuttaHandicapPolicy: policy,
  ...
  return {
    ...
    canonicalInputVerification: {
      scoreSource: "frozen Supabase full Course Handicap for BB/SI; canonical Supabase team gross/net for SC",
      handicapPolicy: CALCUTTA_FULL_COURSE_HANDICAP_POLICY,
...
export async function currentCalcuttaOperationalResult(tournamentId, { recalculatePending = true, calculatedBy, env } = {}) {
  if (productionShadowCandidateReadEnvironment(env || process.env).eligible) recalculatePending = false;
  let resolvedTournamentId = clean(tournamentId);
  if (!resolvedTournamentId) {
    const scope = await readLeaderboardsCoreView("", { env });
    if (!scope.payload?.ok) throw Object.assign(new Error("Current Calcutta scope is unavailable."), { code: scope.payload?.code || "CALCUTTA_SCOPE_UNAVAILABLE" });
    resolvedTournamentId = clean(scope.payload.data?.tournament?.tournament_id);
  }
  let read = await readCompetitionDerivedState(resolvedTournamentId, ["CALCUTTA"], { env });
  ...
  if (recalculatePending && (data.stale || !data.calcutta)) {
    try {
      ...
      read = await readCompetitionDerivedState(resolvedTournamentId, ["CALCUTTA"], { env });
