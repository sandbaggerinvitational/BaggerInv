...
// This identifier is descriptive until a Google operation begins. Resolving
// it at module evaluation made Next.js page collection execute the Preview
// workbook-isolation guard even for Production-shadow routes that never call
// Google. Every default Google transport below still calls
// resolveSpreadsheetId() immediately before constructing its request.
export const SPREADSHEET_ID = configuredSpreadsheetId() || PRODUCTION_SPREADSHEET_ID;

function assertGoogleDataAccessAllowed() {
  const candidate = productionShadowCandidateEnvironment(process.env);
  // A malformed requested candidate must fail closed too. Live Production is
  // intentionally unaffected because this boundary applies only to Vercel
  // Preview deployments.
  if (!candidate.requested || !candidate.previewDeployment) return;
  const error = new Error("Google data access is unavailable on the Production-shadow candidate.");
  error.code = "PRODUCTION_SHADOW_CANDIDATE_GOOGLE_READ_FORBIDDEN";
  throw error;
}
...
function csvUrl(sheetName, spreadsheetId) {
  assertGoogleDataAccessAllowed();
  const resolvedSpreadsheetId = spreadsheetId === undefined
    ? resolveSpreadsheetId()
    : String(spreadsheetId || "").trim();
  if (!resolvedSpreadsheetId) throw new Error("Google Sheets data access requires a spreadsheet ID.");
  return `https://docs.google.com/spreadsheets/d/${resolvedSpreadsheetId}/gviz/tq?tqx=out:csv&sheet=${encodeURIComponent(
    sheetName
  )}`;
}
...
export const loadHistoricalData = cache(async () =>
  loadHistoricalDataFromSpreadsheet(() => resolveSpreadsheetId())
);
