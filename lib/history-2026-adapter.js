...
export function history2026SourceFingerprint(aggregate = {}, { guideProjection = {} } = {}) {
  ...
  const value = {
    ...
    players: (aggregate.players || []).map((player) => ({
      ...
      tournamentHandicap: number(player.tournament_handicap),
...
function scoringContextForRound(aggregate, roundNumber) {
  const records = (aggregate.matches || []).filter((entry) =>
    integer(entry?.match?.round_number) === roundNumber
  );
  const record = records.find((entry) => entry?.scoring_snapshot?.hole_definitions?.length === 18) || records[0];
  return record?.scoring_snapshot || {};
}

function hasStartedScoringEvidence(record = {}) {
  const match = record.match || {};
  return matchLifecycle(record) !== "UPCOMING" ||
    integer(match.scored_holes, 0) > 0 ||
    match.scoring_locked === true ||
    match.scorecard_complete === true ||
    Boolean(clean(match.finalized_at)) ||
    Boolean(clean(match.result_winner)) ||
    number(match.team_1_points, 0) !== 0 ||
    number(match.team_2_points, 0) !== 0 ||
...
function buildCourseHoles(aggregate) {
  ...
  for (const record of aggregate.matches || []) {
    ...
    if (!observedContexts.has(key)) {
      observedContexts.set(key, { scoring, round: integer(record?.match?.round_number) });
    }

    // A strictly unstarted match may intentionally have no active match_holes
    // after its pairings/scoring context are cleared. That absence is not a
    // second course definition. Started, partial, and conflicting contexts
    // remain fail-closed below.
    if (definitions.length === 0 && !hasStartedScoringEvidence(record)) continue;
    if (definitions.length !== 18) {
      throw new Error(`Course ${scoring.course_id} / ${scoring.tee} does not contain 18 holes.`);
    }
  ...
  for (const [key, { scoring }] of observedContexts) {
    if (!contexts.has(key)) {
      throw new Error(`Course ${scoring.course_id} / ${scoring.tee} does not contain 18 holes.`);
    }
  }
...
function participantDirectory(aggregate) {
  ...
  return (aggregate.players || []).map((player) => {
    ...
    return {
      ...
      captain: player.captain === true || presentation.captain === true,
      "Team ID": clean(player.team_id),
      "Team Side": integer(player.team_side ?? appearance.team_side),
      "Tournament Handicap": number(player.tournament_handicap),
    };
    ...
    .map((team) => {
      ...
      const roster = players
        ...
        .map((player) => ({
          ...
          handicap: number(player["Tournament Handicap"]),
        }));
      ...
      return {
        ...
        roster,
        averageHandicap: roster.length > 0 && numericHandicaps.length === roster.length
...
export function leaderboardFromMatches(matches, players, teams) {
  const rows = new Map();
  const playerById = new Map(players.map((player) => [player["Player ID"], player]));
  const teamBySide = new Map(teams.map((team) => [team.sideNumber, team]));
  for (const player of players) {
    const id = clean(player["Player ID"]);
    const teamSide = integer(player["Team Side"]);
    if (!id || !teamSide) continue;
    rows.set(id, {
      id,
      player,
      teamSide,
      teamName: teamBySide.get(teamSide)?.name || `Team ${teamSide}`,
      wins: 0,
      losses: 0,
      halves: 0,
      points: 0,
...
function completedTournamentOutcome(matches, teams) {
  const complete = matches.length === 24 && matches.every(
    (match) => upper(match.lifecycle || match.status) === "FINAL"
  );
  if (!complete) return {
    complete: false,
  ...
  const pointPairs = matches.map((match) => [
    number(match.team1Points, null),
    number(match.team2Points, null),
  ]);
  if (pointPairs.some(([one, two]) => one === null || two === null)) return {
    complete: true,
    lifecycle: "FINAL",
    teamOnePoints: null,
    teamTwoPoints: null,
  ...
  const teamOnePoints = pointPairs.reduce((sum, [value]) => sum + value, 0);
  const teamTwoPoints = pointPairs.reduce((sum, [, value]) => sum + value, 0);
  const championTeam = teamOnePoints === teamTwoPoints
    ? null
    : teamOnePoints > teamTwoPoints ? teams[0] : teams[1];
  const runnerUpTeam = !championTeam
    ? null
    : championTeam.id === teams[0]?.id ? teams[1] : teams[0];
  return {
    complete: true,
    lifecycle: "FINAL",
    teamOnePoints,
...
function applyTournamentOutcome(tournament, matches, teams) {
  const outcome = completedTournamentOutcome(matches, teams);
  const score = outcome.teamOnePoints === null || outcome.teamTwoPoints === null
    ? ""
    : `${outcome.teamOnePoints} - ${outcome.teamTwoPoints}`;
  return {
    ...tournament,
    complete: outcome.complete,
    lifecycle: outcome.lifecycle,
    "Final Score": score,
    championTeamId: outcome.championTeam?.id || null,
    runnerUpTeamId: outcome.runnerUpTeam?.id || null,
    championTeam: outcome.championTeam,
    runnerUpTeam: outcome.runnerUpTeam,
  };
}
...
export function history2026TeamPageModel(view = {}, requestedSide = "") {
  ...
  const roundGroups = roundRows.map((row) => {
    ...
    const lifecycle = matches.length && matches.every((match) => matchLifecycle(match) === "FINAL")
      ? "FINAL"
      : matches.some((match) => matchLifecycle(match) === "LIVE" || matchLifecycle(match) === "FINAL")
        ? "IN PROGRESS"
        : "UPCOMING";
    return {
...
export function buildHistory2026Adapter(aggregate = {}, { guideProjection = {} } = {}) {
  ...
  const tournament = applyTournamentOutcome(
    buildTournament(aggregate, courses, teams, guideProjection),
    publicMatches,
    teams
  );
