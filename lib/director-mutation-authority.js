...
const directorPolicy = Object.freeze({
  ...
  "match-unlock-scoring": googleDirect(
    "SCORING_PERMISSION",
    ["enableLiveMatchAccess"],
    "Enables legacy scoring access for one match.",
  ),
  "match-lock-scoring": googleDirect(
    "SCORING_PERMISSION",
    ["disableLiveMatchAccess"],
    "Disables legacy scoring access for one match.",
  ),
  "match-mark-live": googleDirect(
    "MATCH_LIFECYCLE",
    ["markLiveMatch"],
...
const adminCmsPolicy = Object.freeze({
  ...
  "draft-settings": googleDirectorAuthoring("DRAFT_CONFIGURATION_AUTHORING", ["saveCmsRecord", "synchronizeDraftProjection"], "Google remains Director authority and the supported write synchronizes an immutable Draft projection."),
  "draft-picks": googleDirectorAuthoring("DRAFT_PICK_AUTHORING", ["saveCmsRecord", "synchronizeDraftProjection"], "Google remains Director authority and the supported write synchronizes an immutable Draft projection."),
  schedule: googleDirectorAuthoring("GUIDE_SCHEDULE_AUTHORING", ["saveCmsRecord"], "Google remains Director authoring authority for schedule presentation before targeted Guide synchronization."),
  ...
  "prediction-settings": googleDirectorAuthoring("PREDICTION_CONFIGURATION_AUTHORING", ["saveCmsRecord"], "Google remains Director authority; calculation use remains blocked until the versioned settings projection is current."),
});
...
export function directorMutationAuthorityDiagnostics({
  ...
  const allowed = Boolean(policy) && (googleAuthority || (supabaseAuthority && policy.supabaseAllowed === true));
  const code = allowed ? ""
...
export class DirectorMutationAuthorityError extends Error {
  constructor(diagnostics) {
    ...
    this.status = unavailable ? 503 : unknown ? 400 : 409;
