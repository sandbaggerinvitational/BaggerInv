...
function playerAt(snapshot, playerId) {
  ...
  return index < 0 ? null : { ...snapshot.players[index], rank: index + 1 };
}
...
export function buildTournamentRecapIntelligence({ snapshots = [], tournament = {}, leaderboard = [] } = {}) {
  ...
  const openingPlayers = (opening?.players || []).map((player, index) => {
    ...
    return actual ? { id: id(player.id), name: player.name, openingRank: index + 1, openingProbability: number(player.probability), openingOdds: player.americanOdds, projectedFinish: number(player.averageFinish, index + 1), actualRank: actual.rank, actualPoints: actual.points, rankDelta: index + 1 - actual.rank, finishError: Math.abs(number(player.averageFinish, index + 1) - actual.rank) } : null;
  }).filter(Boolean);
