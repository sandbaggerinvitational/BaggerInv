...
export function rememberParticipantAuthNavigation(routeFrom, routeTo) {
  if (participantAuthDiagnosticsEnabled()) localStorage.setItem(NAVIGATION, JSON.stringify({ routeFrom, routeTo, startedAt: performance.now() }));
}
export function finishParticipantAuthNavigation(routeTo) {
  ...
  if (pending?.routeTo && new URL(pending.routeTo, location.origin).pathname === routeTo) {
    recordParticipantAuthDiagnostic("ROUTE_NAVIGATION", { routeFrom: pending.routeFrom, routeTo, durationMs: performance.now() - pending.startedAt });
