...
export function reconcileScoringShadowRecords(authoritative = [], mirrored = [], events = [], matches = [], mirroredMatches = [], observations = []) {
  ...
  for (const observation of observations) {
    const current = expectedMatchState.get(observation.match_id);
    if (!current || number(observation.google_revision) >= number(current.google_revision)) expectedMatchState.set(observation.match_id, observation);
  }
...
export async function rebuildAndReconcileScoringShadow({ sourceWorkbookId, matches, holes, requestedBy }, options = {}) {
  ...
  const report = reconcileScoringShadowRecords(authoritative, read.payload || [], events.payload || [], matches, mirroredMatches.payload || [], observations);
  const reconciliationDurationMs = Date.now() - reconciliationAt;
  await recordScoringShadowReconciliation({
    sourceWorkbookId, tournamentId, tournamentYear: number(matches[0]?.Year), report, requestedBy,
    operation: "RECONCILE", durationMs: reconciliationDurationMs,
  }, options);
  return { rebuilt: rebuilt.payload, report, timings: {
