...
function participantIdentity(match = {}) {
  return [1, 2].flatMap((side) => [1, 2].map((slot) => clean(match[`Team ${side} Player ${slot}`]))).filter(Boolean).join("|");
}

export function historicalScoringSnapshotForMatch(current = {}, archived = null) {
  const archivedFinal = archived && /^(final|finalized)$/i.test(clean(archived["Match Status"]));
  const sameParticipants = archivedFinal && participantIdentity(current) && participantIdentity(current) === participantIdentity(archived);
  return sameParticipants ? archived : current;
}

export function calculateScoringShadowHoleFromSnapshot(match = {}, hole = {}) {
...
export function scoringShadowObservationsFromWorkbook({ sourceWorkbookId, matches = [], holes = [], matchSnapshots = [] } = {}) {
  const matchById = new Map(matches.map((match) => [clean(match["Match ID"]), match]));
  const snapshotById = new Map(matchSnapshots.map((match) => [clean(match["Match ID"]), match]));
  ...
  return holes.map((hole) => {
    const currentMatch = matchById.get(clean(hole["Match ID"])) || {};
    const match = historicalScoringSnapshotForMatch(currentMatch, snapshotById.get(clean(hole["Match ID"])));
    ...
    const calculated = calculateScoringShadowHoleFromSnapshot(match, hole);
...
export async function rebuildAndReconcileScoringShadow({ sourceWorkbookId, matches, holes, matchSnapshots = [], requestedBy, googleReadDurationMs = 0 }, options = {}) {
  const startedAt = Date.now();
  const normalizedAt = Date.now();
  const observations = scoringShadowObservationsFromWorkbook({ sourceWorkbookId, matches, holes, matchSnapshots });
