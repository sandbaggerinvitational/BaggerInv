...
export function productionMaintenancePrecommitDeploymentEnvironment(
  ...
  const outboxWorkerSecretConfigured =
    clean(env.SCORING_GOOGLE_OUTBOX_WORKER_SECRET).length >= 32;
  const archiveWorkerSecretConfigured =
    clean(env.ROUND_SCORECARDS_ARCHIVE_WORKER_SECRET).length >= 32;
  const predictionSettingsSourceFingerprint = lower(
    env.WAR_ROOM_PREDICTION_SETTINGS_SOURCE_FINGERPRINT,
  );
  const predictionSettingsEffectiveFingerprint = lower(
    env.WAR_ROOM_PREDICTION_SETTINGS_EFFECTIVE_FINGERPRINT,
  );
  const oddsPublicationAuthority = lower(env.ODDS_PUBLICATION_AUTHORITY);
  const supabaseOddsPublicationEnabled = truthy(
    env.PRODUCTION_SUPABASE_ODDS_PUBLICATION_ENABLED,
  );
  const supabaseOddsGoogleMirrorEnabled = truthy(
    env.PRODUCTION_SUPABASE_ODDS_GOOGLE_MIRROR_ENABLED,
  );
  const legacyGoogleOddsPublication = oddsPublicationAuthority === "google" &&
    !supabaseOddsPublicationEnabled && !supabaseOddsGoogleMirrorEnabled;
  const canonicalSupabaseOddsPublication =
    oddsPublicationAuthority === "supabase" &&
    supabaseOddsPublicationEnabled && !supabaseOddsGoogleMirrorEnabled;
  const exactOddsPublicationConfiguration = legacyGoogleOddsPublication ||
    canonicalSupabaseOddsPublication;
  const exactRuntimeConfiguration =
    ...
    truthy(env.ROUND_SCORECARDS_ARCHIVE_ENABLED) &&
    outboxWorkerSecretConfigured && archiveWorkerSecretConfigured &&
    truthy(env.PRODUCTION_SUPABASE_ODDS_CALCULATION_ENABLED) &&
    lower(env.WAR_ROOM_INPUT_SOURCE) === "supabase" &&
    lower(env.PREDICTION_SETTINGS_READ_SOURCE) === "supabase" &&
    lower(env.ODDS_CALCULATION_INPUT_SOURCE) === "supabase" &&
    fingerprint(predictionSettingsSourceFingerprint) &&
    fingerprint(predictionSettingsEffectiveFingerprint) &&
    exactOddsPublicationConfiguration &&
  ...
  return Object.freeze({
    ...
    exactRuntimeConfiguration,
    capabilityContract: allowed ? capability.contract : "",
    capabilityCeiling: allowed ? capability.ceiling : "",
    outboxWorkerSecretConfigured,
    archiveWorkerSecretConfigured,
    predictionSettingsSourceFingerprint: allowed
      ? predictionSettingsSourceFingerprint
      : "",
    predictionSettingsEffectiveFingerprint: allowed
      ? predictionSettingsEffectiveFingerprint
      : "",
    oddsPublicationAuthority: allowed ? oddsPublicationAuthority : "",
    supabaseOddsPublicationEnabled: allowed
      ? supabaseOddsPublicationEnabled
      : false,
    supabaseOddsGoogleMirrorEnabled: allowed
      ? supabaseOddsGoogleMirrorEnabled
      : false,
    legacyGoogleOddsPublication,
    canonicalSupabaseOddsPublication,
...
function exactOperatorInput(value, runtime, actorId, now) {
  ...
  return {
    ...
    runtime_deployment_capability_contract: runtime.capabilityContract,
    runtime_deployment_capability_ceiling: runtime.capabilityCeiling,
    ...
    runtime_outbox_worker_secret_configured:
      runtime.outboxWorkerSecretConfigured,
    runtime_archive_worker_secret_configured:
      runtime.archiveWorkerSecretConfigured,
    ...
    runtime_prediction_settings_source_fingerprint:
      runtime.predictionSettingsSourceFingerprint,
    runtime_prediction_settings_effective_fingerprint:
      runtime.predictionSettingsEffectiveFingerprint,
    runtime_odds_publication_authority:
      runtime.oddsPublicationAuthority.toUpperCase(),
    runtime_supabase_odds_publication_enabled:
      runtime.supabaseOddsPublicationEnabled,
    runtime_supabase_odds_google_mirror_enabled:
      runtime.supabaseOddsGoogleMirrorEnabled,
