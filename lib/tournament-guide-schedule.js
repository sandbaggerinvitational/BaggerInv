...
export function itineraryViewModel({ records = [], tournament = {}, rounds = [], courses = [], now = new Date() } = {}) {
  ...
  const base = records.map((record, index) => {
    ...
    return {
      ...
      day: date || clean(record["Day Label"]) || "Tournament Day",
      dayHeading: date ? new Intl.DateTimeFormat("en-US", { weekday: "long", timeZone: "UTC" }).format(new Date(`${date}T12:00:00Z`)) : clean(record["Day Label"]) || "Tournament Day",
      startTime,
      endTime,
