...
export function todaysSchedule(schedule = [], {
  ...
  return events.map((event, index) => ({
    ...
    state: clean(event.status).toLowerCase() === "completed" ? "complete"
      : ["live", "delayed", "cancelled"].includes(clean(event.status).toLowerCase()) ? clean(event.status).toLowerCase()
      : index < nextIndex || (nextIndex === -1 && events.length) ? "complete"
