...
const ERROR_DEFINITIONS = Object.freeze({
  NATIVE_READS_DISABLED: Object.freeze({ status: 503, message: "Native participant reads are suspended." }),
  NATIVE_AUTH_DISABLED: Object.freeze({ status: 503, message: "Native sign-in requests are suspended." }),
  NATIVE_CERTIFICATION_DISABLED: Object.freeze({ status: 503, message: "Native certification is suspended." }),
  NATIVE_SCORING_DISABLED: Object.freeze({ status: 503, message: "Native scoring is suspended." }),
  AUTHORITY_INCOMPATIBLE: Object.freeze({ status: 503, message: "The native authority contract is unavailable." }),
  CLIENT_UPDATE_REQUIRED: Object.freeze({ status: 503, message: "A compatible application version is required." }),
  MAINTENANCE: Object.freeze({ status: 503, message: "Scoring is temporarily suspended for maintenance." }),
...
export function mobileSessionData(identity) {
  ...
  if (identity?.kind === "observer") {
    if(!isCanonicalReviewer(context)) throw new MobileApiError("PARTICIPANT_NOT_FOUND");
    return {observer:{subjectId:context.authUserId,historicalFixture:context.historicalFixture},tournament:{tournamentId:context.tournament.id,name:context.tournament.name,year:context.tournament.year}};
  }
