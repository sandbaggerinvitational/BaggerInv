...
export async function validateAuthoritativeParticipantSession(request, session, {
  ...
  if (session?.scope === "admin") {
    ...
    // authorizePreviewDirector has already selected and revalidated the
    // server-owned Production current pointer. Bind the legacy admin session
    // to that authoritative tournament without freezing the comparison at
    // 2026; a predecessor session consequently fails after a pointer switch.
    if (!/^\d{4}$/.test(directorTournamentId) || Number(directorTournamentId) < 2026 ||
        (clean(session.tournamentId) && clean(session.tournamentId) !== directorTournamentId)) {
