...
export async function mobileScoringCurrentResult(identity, {
  ...
  if(isCanonicalReviewer(identity.context)) {
    if(matchId) throw new MobileApiError("PARTICIPANT_NOT_FOUND");
    return success({scoring:null},now);
  }
...
export async function mobileScoringHoleResult(identity, value, {
  ...
  if (identity.kind === "observer" || isCanonicalReviewer(identity.context)) throw new MobileApiError("SCORING_NOT_AUTHORIZED");
  await recheckMobileNativeIdentity(identity, "scoring", env);
  requireMobileScoringAvailable(env, dependencies);
