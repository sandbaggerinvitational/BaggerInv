...
function normalizedMatchRow(entry = {}) {
  ...
  for (const participant of entry.participants || []) {
    ...
    row[`Team ${side} Player ${slot} Playing HCP`] = participant.playing_handicap;
    row[`Team ${side} Player ${slot} Stroke`] = participant.final_strokes;
  }
  const teamConfiguration = snapshot.team_configuration || {};
  for (const side of [1, 2]) {
    row[`Team ${side} Playing HCP`] = teamConfiguration[`team_${side}_playing_handicap`] ?? null;
    row[`Team ${side} Stroke`] = teamConfiguration[`team_${side}_strokes`] ?? null;
  }
  return row;
}
...
export function validateLeaderboardsCoreAttribution(view = {}, scoreLeaderboard = []) {
  const issues = [];
    ...
    for (const side of [1, 2]) {
      ...
      for (const score of entry.scores || []) {
        ...
        comparisonPlayers.forEach((participant, index) => {
          ...
          const canonicalNet = format === "SC" ? displayNumber(score[`team_${side}_net_score`]) : gross[index] - strokes[index];
          if (!derived || number(derived.gross, NaN) !== gross[index] || number(derived.net, NaN) !== canonicalNet ||
              number(derived.strokes, NaN) !== strokes[index]) {
            issues.push({ code: "SCORE_ATTRIBUTION", matchId, hole: number(score.hole_number), side,
              playerId: clean(participant?.player_id), expected: { gross: gross[index], strokes: strokes[index], net: canonicalNet },
  ...
  return { pass: issues.length === 0, issues, participantSlots, scoredHoles, formats: [...formats].sort() };
}

export function leaderboardsCoreDataFromSupabaseView(view = {}) {
  const calculationStartedAt = performance.now();
  const live = tournamentLiveDataFromSupabaseView(view);
  const entries = view.matches || [];
  const playerMap = playerMapFrom(view);
  const teams = teamMapFrom(live);
  const matchRows = entries.map(normalizedMatchRow);
  const matchMap = new Map(matchRows.map((row) => [clean(row["Match ID"]), row]));
  const holeRows = normalizedHoleRows(entries);
  const scoreLeaderboard = buildScoreLeaderboard(holeRows, matchMap, normalizedCourseHoles(entries), playerMap);
  ...
  const slotVerification = validateLeaderboardsCoreAttribution(view, scoreLeaderboard);
