...
export function strokeAllocation(published, snapshot, hasRecordedScores) {
  // Null/absence is not zero. A legitimate published value wins; otherwise
  // preserve the Match snapshot allocation, including its authoritative zero.
  const value = published ?? snapshot;
  if (Number.isSafeInteger(value)) return value;
  if (!hasRecordedScores && value == null) return null;
  // Recorded Supabase scores must never reach the generic legacy engine's
  // Playing-Handicap fallback because allocation authority is unavailable.
  const error = new Error("Canonical Leaderboards stroke authority is unavailable.");
  error.code = "LEADERBOARDS_STROKE_AUTHORITY_UNAVAILABLE";
  throw error;
}
