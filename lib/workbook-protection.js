import { PARTICIPANT_IDENTITY_CONFIGURATION_HEADERS } from "./participant-identity-workbook.js";
...
export const PROTECTED_COLUMN_MAP = Object.freeze({
  ...
  "Live Matches": merge(
    ...
    columns(WRITABLE, ["Year", "Round", "Format", "Match", "Course ID", "Tee Time", "Starting Hole", "Team 1 Player 1", "Team 1 Player 2", "Team 2 Player 1", "Team 2 Player 2"]),
  ...
  "Participant Identity Configuration": merge(columns(WRITABLE, PARTICIPANT_IDENTITY_CONFIGURATION_HEADERS)),
...
// Google Sheets RAW writes preserve the JSON value type. Keep this map beside
// the Protected Column Map so every writer uses the workbook's field purpose,
// never a column letter or a caller's incidental form-string representation.
const NUMERIC_WRITE_FIELDS = Object.freeze({
  Tournaments: ["Year", "Annual", "Team Size"],
  "Live Matches": ["Year", "Round", "Match", "Starting Hole", "Team 1 Player 1 Playing HCP", "Team 1 Player 1 Stroke", "Team 1 Player 2 Playing HCP", "Team 1 Player 2 Stroke", "Team 1 Playing HCP", "Team 1 Stroke", "Team 2 Player 1 Playing HCP", "Team 2 Player 1 Stroke", "Team 2 Player 2 Playing HCP", "Team 2 Player 2 Stroke", "Team 2 Playing HCP", "Team 2 Stroke", "Team 1 Points", "Team 2 Points", "Current Hole", "Team 1 Holes Won", "Team 2 Holes Won", "Holes Remaining", "Access Version"],
  Matches: ["Year", "Round", "Match", "Starting Hole", "Match Number", "Team 1 Player 1 Playing HCP", "Team 1 Player 1 Stroke", "Team 1 Player 2 Playing HCP", "Team 1 Player 2 Stroke", "Team 1 Playing HCP", "Team 1 Stroke", "Team 2 Player 1 Playing HCP", "Team 2 Player 1 Stroke", "Team 2 Player 2 Playing HCP", "Team 2 Player 2 Stroke", "Team 2 Playing HCP", "Team 2 Stroke", "Team 1 Points", "Team 2 Points"],
  "Live Hole Scores": ["Hole Number", "Stroke Index", "Team 1 Net Score", "Team 2 Net Score", "Revision"],
  "Net Skins Result": ["Year", "Round", "Hole", "Skin Value", "Round Pot", "Winning Net Score", "Match"],
  "Odds Control": ["Year"],
  "Odds Snapshots": ["Year"],
  "Odds Team Results": ["Year", "Win Probability", "American Odds", "Expected Points"],
  "Odds Player Results": ["Year", "Top Player Probability", "American Odds", "Expected Points", "Average Finish"],
  Players: ["First Year", "Last Year"],
  Handicaps: ["Year", "Tournament Handicap"],
  "Team Names": ["Year"],
  Courses: ["Year", "Round", "Slope", "Rating", "Yardage", "Par", "Year Opened"],
  Awards: ["Year"],
  "Draft Settings": ["Year", "Total Picks"],
  "Draft Picks": ["Year", "Pick Number"],
  "Tournament Itinerary": ["Display Order"],
  "Guide Sections": ["Display Order"],
  "Rule Book": ["Display Order", "Effective Year"],
  "Guide Information": ["Display Order"],
  "Tournament Rules": ["Year", "Round", "Team Size", "Points Available", "Front 9 Points", "Back 9 Points", "Overall Points"],
  Rounds: ["Team Size"],
  "Course Holes": ["Hole Number", "Yardage", "Par", "Stroke Index"],
  "Round Scorecards": ["Year", "Round", "Match", ...Array.from({ length: 18 }, (_, index) => `Hole ${index + 1}`)],
  "Course Scorecards": ["Rating", "Slope", "Par"],
  "Net Skins": ["Year", "Round", "Buy-In"],
  "Live Round Handicaps": ["Year", "Round", "Handicap Index", "Low Handicap Index"],
  "Participant Identity Configuration": ["Configuration Revision"],
  Dining: ["Year", "Sort Order"],
  "Important Contacts": ["Year", "Sort Order"],
  "Local Guide": ["Year", "Sort Order"],
  "Tournament Timeline": ["Year", "Notification Minutes", "Sort Order"],
  "Calcutta Purchases": ["Year", "Purchase Price"],
  "Calcutta Ownership": ["Year", "Ownership %"],
  ...
  "Calcutta Round Results": ["Year", "Round", "Gross Score", "Net Score", "Full Course Handicap", "Place", "Calcutta Points"],
  "Calcutta Standings": ["Year", "Rank", "Purchase Price", "Round 1 Points", "Round 2 Points", "Round 3 Points", "Total Points", "Round 1 Payout %", "Round 2 Payout %", "Round 3 Payout %", "Total Payout %", "Current Payout Value", "ROI"],
});

const NUMERIC_WRITE_FIELD_SETS = Object.freeze(Object.fromEntries(
  Object.entries(NUMERIC_WRITE_FIELDS).map(([tab, fields]) => [tab, new Set(fields)]),
));

const STRUCTURED_SCORE_FIELDS = new Set(["Team 1 Gross Scores", "Team 2 Gross Scores"]);

export function normalizeWorkbookWriteValue(tab, field, value) {
  if (value === null || value === undefined || value === "") return "";
  if (tab === "Live Hole Scores" && STRUCTURED_SCORE_FIELDS.has(field)) {
    if (typeof value === "number" && Number.isFinite(value)) return value;
    if (Array.isArray(value) && value.length && value.every((score) => typeof score === "number" && Number.isFinite(score))) return JSON.stringify(value);
    throw new Error(`${tab}.${field} must contain native numeric score values.`);
  }
  if (!NUMERIC_WRITE_FIELD_SETS[tab]?.has(field)) return value;
  if (typeof value === "string" && /^\s*'/.test(value)) {
    throw new Error(`${tab}.${field} cannot be written as apostrophe-prefixed text.`);
  }
  if (typeof value !== "number" || !Number.isFinite(value)) {
    const error = new Error(`${tab}.${field} must be written as a native number.`);
    error.workbookContract = { worksheet: tab, field, value, runtimeType: typeof value, expectedType: "number" };
    throw error;
  }
  return value;
}

// Producer-boundary helper. Callers use this while constructing a workbook
// mutation; the protected writer remains a strict assertion boundary.
export function normalizeWorkbookProducerRecord(tab, record = {}) {
  return Object.fromEntries(Object.entries(record).map(([field, value]) => {
    if (value === null || value === undefined || value === "" || !NUMERIC_WRITE_FIELD_SETS[tab]?.has(field)) return [field, value ?? ""];
    if (typeof value === "number" && Number.isFinite(value)) return [field, value];
    if (typeof value === "string" && /^\s*'/.test(value)) throw new Error(`${tab}.${field} cannot be written as apostrophe-prefixed text.`);
    const numeric = typeof value === "string" && value.trim() !== "" ? Number(value.trim().replace(/^\+/, "")) : NaN;
    if (!Number.isFinite(numeric)) {
      const error = new Error(`${tab}.${field} could not be normalized to its required native number.`);
      error.workbookContract = { worksheet: tab, field, value, runtimeType: typeof value, expectedType: "number" };
      throw error;
    }
...
export function validateFieldWrite(tab, headers, updates) {
  ...
  for (const [field, value] of entries) {
    ...
    normalizeWorkbookWriteValue(tab, field, value);
  }
