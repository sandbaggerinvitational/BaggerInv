import { mobileCourseHandicap } from "./mobile-course-handicap.js";
...
export function tournamentLiveDataFromSupabaseView(view = {}, { mobileContract = false, matchCenterHandicapPresentation = false } = {}) {
  ...
  const matches = entries.map((entry) => {
    ...
    const displayed = applyMatchDisplayProjection(canonical, matchDisplay[canonical.id] || {}, mobileContract);
    return { ...withMatchCenterDisplayHandicaps(
      displayed,
      playerDisplays.get(entry.match?.match_id)),
      ...scrambleDisplay.get(entry.match?.match_id),
      ...(mobileContract ? Object.fromEntries([1, 2].map(side => [`team${side}Players`,
        (canonical[`team${side}Players`] || []).map(player => ({
          ...displayed[`team${side}Players`].find(p => p.id === player.id),
          courseHandicap: mobileCourseHandicap((entry.participants || []).find(p => p.player_id === player.id && Number(p.team_side) === side)?.course_handicap, entry.match?.format),
        }))])) : {}),
