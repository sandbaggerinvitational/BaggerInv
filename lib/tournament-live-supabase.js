...
function liveMatch(entry, tournament, teams) {
  ...
  return {
    ...match,
    // The legacy Game Center adapter formats display text. The collection ID
    // must instead retain the exact canonical database string, including edge
    // whitespace, before display lookup or the mobile Match DTO can see it.
    id: entry.match?.match_id,
    currentHole,
...
function applyMatchDisplayProjection(match, projected = {}) {
  const players = (canonical, display) => canonical.map((player) => {
    ...
    return { ...player,
      // The legacy display projection always includes these nullable keys. A
      // blank display cell must not mask the canonical scoring snapshot (most
      // importantly numeric zero, which participant UI renders as No strokes).
      playingHcp: exact.playingHcp ?? player.playingHcp,
      stroke: exact.stroke ?? player.stroke,
    };
  });
  return {
    ...match,
    ...
    team1Stroke: projected.team1Stroke ?? match.team1Stroke,
    team2Stroke: projected.team2Stroke ?? match.team2Stroke,
  };
}
...
export function tournamentLiveDataFromSupabaseView(view = {}) {
  ...
  const rounds = (view.rounds || []).map((round) => {
    // `read_tournament_live_view` is the ordering authority. Its JSON aggregate
    // is already ordered by Round, game-center `match_sort_order`, and the
    // canonical Match-ID tie-break. Filtering preserves that participant-safe
    // order without promoting display Match numbers into authority.
