...
import { matchCenterScrambleHandicaps } from "./match-center-handicap-presentation.js";
import { matchCenterPlayerHandicapDisplays, withMatchCenterDisplayHandicaps } from "./match-center-player-handicap-display.js";
import { mobileScrambleHandicapContext } from "./mobile-scramble-handicap-context.js";
...
function liveMatch(entry, tournament, teams, mobileContract) {
  ...
  return {
    ...match,
    // Preserve exact canonical opaque identity before mobile DTO/path encoding.
    ...(mobileContract ? { id: entry.match?.match_id } : {}),
    currentHole,
...
function applyMatchDisplayProjection(match, projected = {}, mobileContract = false) {
  const displayValue = (display, key, fallback) => mobileContract
    ? display[key] ?? fallback : Object.hasOwn(display, key) ? display[key] : fallback;
  const players = (canonical, display) => canonical.map((player) => {
    const exact = (display || []).find((row) => clean(row.id) === clean(player.id)) || {};
    return { ...player,
      playingHcp: displayValue(exact, "playingHcp", player.playingHcp),
      stroke: displayValue(exact, "stroke", player.stroke),
    };
  });
  return {
    ...match,
    ...
    team1Stroke: displayValue(projected, "team1Stroke", match.team1Stroke),
    team2Stroke: displayValue(projected, "team2Stroke", match.team2Stroke),
  };
}

// PN-1's opaque-ID/order/null projection is opt-in for the mobile DTO only.
// Match Center's read-only handicap presentation is a separate web/PWA opt-in;
// default callers retain the existing Production projection.
export function tournamentLiveDataFromSupabaseView(view = {}, { mobileContract = false, matchCenterHandicapPresentation = false } = {}) {
  ...
  // Web/PWA Match Center only. Director, Odds, native and scoring consumers
  // retain their existing contracts. Never let retained display zeros mask it.
  const scrambleDisplay = matchCenterHandicapPresentation && !mobileContract
    ? matchCenterScrambleHandicaps(view) : new Map();
  const playerDisplays = matchCenterHandicapPresentation && !mobileContract
    ? matchCenterPlayerHandicapDisplays(view) : new Map();
  ...
  const matches = entries.map((entry) => {
    const canonical = liveMatch(entry, tournamentRow, teams, mobileContract);
    const scramble = mobileContract && entry.match?.format === "SC"
      ? mobileScrambleHandicapContext(entry.snapshot) : null;
    return { ...withMatchCenterDisplayHandicaps(
      applyMatchDisplayProjection(canonical, matchDisplay[canonical.id] || {}, mobileContract),
      playerDisplays.get(entry.match?.match_id)),
      ...scrambleDisplay.get(entry.match?.match_id),
      // Retained display labels must never override frozen mobile authority.
      ...(scramble ? {
        team1PlayingHcp: scramble.team_1_playing_handicap,
        team2PlayingHcp: scramble.team_2_playing_handicap,
        team1Stroke: scramble.team_1_strokes,
        team2Stroke: scramble.team_2_strokes,
      } : {}),
  ...
  const rounds = (view.rounds || []).map((round) => {
    // Preserve the canonical RPC aggregate order, not display-label ordering.
    const roundMatches = matches.filter((match) => number(match.round) === number(round.round_number));
    if (!mobileContract) roundMatches.sort((left, right) => number(left.match) - number(right.match)
