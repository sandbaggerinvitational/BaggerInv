...
export function publishedPlayerHistory(snapshots = [], playerId = "") {
  return snapshots.map((snapshot, snapshotIndex) => {
    ...
    return {
      phase: projectionPresentationLabel(snapshot.phase),
      sourcePhase: snapshot.phase,
      publishedAt: snapshot.publishedAt,
      rank: publishedRank(player, rankIndex + 1),
...
export function tournamentProjectionStory({ current, previous, playerTeams = new Map() } = {}) {
  ...
  const previousIndex = previous.players.findIndex((player) => String(player.id) === String(favorite.id));
  const previousRank = previousIndex < 0 ? 0 : publishedRank(previous.players[previousIndex], previousIndex + 1);
  ...
  if (leadingTeam?.[1] >= 6) return `${leadingTeam[0]} place ${leadingTeam[1]} players in the Top 10 entering ${projectionPresentationLabel(current.phase)}.`;
