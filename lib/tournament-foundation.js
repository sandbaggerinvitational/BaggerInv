...
export async function readTournamentFoundation(options = {}) {
  ...
  const currentContext = options.currentTournamentContext || null;
  let tournamentId = "";
  if (currentContext) {
    tournamentId = clean(currentContext.tournamentId);
    if (!tournamentId || currentContext.lifecycle !== "ACTIVE") {
    ...
    tournamentId = clean(options.tournamentId ||
      (clean(env.VERCEL_ENV).toLowerCase() === "production" ? "" : "2026"));
  }
  if (!tournamentId) {
    const error = new Error("The current Production tournament runtime is temporarily unavailable.");
  ...
  const [liveRead, guideRead] = await Promise.all([
    ...
    guideSource.resolved === "supabase" ? guideReader({ env, surface: "course", tournamentId }) : Promise.resolve(null),
  ]);
  ...
  const returnedTournamentId = clean(liveRead.payload.data?.tournament?.tournament_id ||
    liveRead.payload.data?.tournament?.tournamentId || liveRead.payload.data?.tournament?.id ||
    liveRead.payload.data?.tournament_id || liveRead.payload.data?.tournamentId);
  if ((currentContext && !returnedTournamentId) || (returnedTournamentId && returnedTournamentId !== tournamentId)) {
