import { currentHistory, archiveRows, oddsView } from "./fixtures/pn1-mobile.mjs";
...
test("History requires canonical standing ranks and never fabricates array-position rank", () => {
  ...
  const unranked = structuredClone(ranked);
  delete unranked.leaderboardRows[0].rank;
  assert.equal(mobileHistoryDetailData(unranked).standings[0].rank, 1);
  delete unranked.leaderboardRows[0].points;
  assert.throws(() => mobileHistoryDetailData(unranked));
  for (const rank of [null, "", "third", 0, 129]) {
...
test("History detail projects the shared competition ranks for a complete canonical 2026 view", async () => {
  ...
  // Production computes standings; the mobile adapter projects numeric tie ranks.
...
test("routes share the mobile auth/ETag boundary and reuse canonical history services", async () => {
  ...
  assert.match(historySource, /loadCompletedHistoryYears/);
  assert.match(historySource, /requireMobileProductionReadContext/);
