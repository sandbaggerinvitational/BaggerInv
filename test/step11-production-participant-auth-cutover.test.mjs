...
test("historical migration 020 remains a dormant, service-only, collision-safe baseline superseded by migration 047", async () => {
  const historicalSql = await readFile(new URL("../supabase/production_migrations/202608240020_production_participant_identity_cutover.sql", import.meta.url), "utf8");
  const effectiveSql = await readFile(new URL("../supabase/production_migrations/202608270047_production_partial_roster_participant_identity.sql", import.meta.url), "utf8");
  ...
  assert.match(historicalSql, /Applying this migration creates no Auth users and changes no authority/);
  assert.match(historicalSql, /production-participant-identity-cutover-v1/);
  assert.match(effectiveSql, /production-participant-identity-cutover-v2/);
  assert.match(effectiveSql, /'enrollmentPolicy', 'APPROVED_PARTIAL_ROSTER'/);
  assert.match(effectiveSql, /PRODUCTION_IDENTITY_APPROVED_PARTIAL_ROSTER_REQUIRED/);
  assert.doesNotMatch(effectiveSql, /production-participant-identity-cutover-v1/);
  assert.match(historicalSql, /participant_identity_authority = 'SUPABASE', auth_user_creation_enabled = true/);
  assert.match(historicalSql, /example\\\.\(com\|net\|org\)\|invalid\|test\|localhost/);
  assert.match(historicalSql, /production_participant_one_open_email_claim_idx/);
  assert.match(historicalSql, /user_player_links_auth_user_id_key|user_player_links/);
  assert.match(historicalSql, /participant_auth_identifier_current_email_unique_idx|PRODUCTION_PARTICIPANT_IDENTITY_COLLISION/);
  assert.match(historicalSql, /where email_identity_hash = email_hash and status = 'PENDING'/);
  assert.match(historicalSql, /PRODUCTION_PARTICIPANT_FIRST_LOGIN_CLAIMED/);
  assert.match(historicalSql, /rawIdentifierStoredInAudit', false/);
  assert.match(historicalSql, /grant execute on function %s to service_role/);
  assert.match(historicalSql, /revoke all on function %s from public, anon, authenticated, service_role/);
  assert.match(historicalSql, /read_production_cutover_director_entitlement/);
  assert.match(historicalSql, /assert_production_participant_identity_cutover\(\)/);
  assert.match(identityRpcSource, /read_production_director_entitlement:\s*"read_production_cutover_director_entitlement"/);
  assert.equal((historicalSql.match(/lookup_cutover_receipt\('ACTIVATE_PARTICIPANT_IDENTITY'/g) || []).length, 2);
  assert.equal((historicalSql.match(/lookup_cutover_receipt\('ROLLBACK_PARTICIPANT_IDENTITY'/g) || []).length, 2);
  assert.match(historicalSql, /store_cutover_receipt\(\s*'ACTIVATE_PARTICIPANT_IDENTITY'/);
  assert.match(historicalSql, /store_cutover_receipt\(\s*'ROLLBACK_PARTICIPANT_IDENTITY'/);
  assert.match(historicalSql, /activation\.activation_revision <> expected_revision/);
  assert.match(historicalSql, /activation_revision = activation_revision \+ 1/);
  assert.match(activationSql, /fingerprint !~ '\^\[0-9a-f\]\{64\}\$'/);
  assert.match(activationSql, /PRODUCTION_IDEMPOTENCY_CONFLICT/);
  assert.doesNotMatch(historicalSql, /auth\.admin|createUser|signInWithOtp|preview_/i);
});
