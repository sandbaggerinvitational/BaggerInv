...
function harness(){let tick=0,matchReads=0,historyReads=0;const core=structuredClone(fixture.core);core.players.push({player_id:'P1'});const reads={guard:()=>{},core:async()=>core,match:async id=>{matchReads++;return rawFixture({matchId:id,owned:false});},history:async()=>{historyReads++;return historicalView();},player:async()=>passportInput(),portraits:async()=>policy};return {reads,read:createPublicGolfReader(reads,{now:()=>tick}),tick:ms=>tick+=ms,counts:()=>({matchReads,historyReads})};}
