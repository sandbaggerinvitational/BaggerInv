...
test('phone identifiers and attempts use existing same-UUID deletion cleanup',async t=>{
 ...
 create table participant_identity.participant_auth_identifiers(auth_user_id uuid references auth.users on delete restrict,identifier_type text,normalized_value_private text);
 create table participant_identity.participant_phone_otp_attempts(auth_user_id uuid references auth.users on delete restrict,identifier_revision bigint);
 ...
 sql(`insert into participant_identity.participant_auth_identifiers values('${id(1)}','EMAIL','fixture@example.invalid'),('${id(1)}','PHONE','+12025550123');insert into participant_identity.participant_phone_otp_attempts values('${id(1)}',1);`);
 ...
 await t.test('normal participant request persists and deletion clears auth/session/link but keeps competition',()=>{
   ...
   assert.equal(sql(`select count(*) from participant_identity.participant_auth_identifiers where auth_user_id='${id(1)}';`),'0');
   assert.equal(sql(`select count(*) from participant_identity.participant_phone_otp_attempts where auth_user_id='${id(1)}';`),'0');
