...
test("custom splash is gated to a cold installed-PWA launch", async () => {
  ...
  assert.match(layout, /window\.location\.pathname==="\/home"/);
...
test("splash consumes participant Home initialization and never attaches to the public homepage", async () => {
  const [splash, bridge, publicHome, participantHome] = await Promise.all([
  ...
  assert.match(publicHome, /readHomepageCurrentTournament/);
  assert.doesNotMatch(publicHome, /PwaSplashIdentityBridge|MobileTournamentHome|getTournamentData|sheetData/);
  assert.match(participantHome, /<PwaSplashIdentityBridge tournament=\{liveData\.tournament\} \/>/);
  assert.match(participantHome, /<PwaSplashIdentityBridge tournament=\{null\} \/>/);
  ...
  assert.doesNotMatch(splash, /router|window\.location/);
});
