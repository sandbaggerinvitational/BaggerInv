...
test("eligible completed matches omit stroke copy without retaining empty stroke rows", () => {
  ...
  assert.match(matchCard, /showStroke=\{!completedHistoryMatchupCleanup\}/);
...
test("2026 keeps its approved dedicated scorecard props and presentation path", () => {
  ...
  assert.match(currentMatchRow, /<ScorecardTable scorecards=\{scorecardTableData\} compact deferClosedContent historyDensity showSummary stackPairingIdentities=\{use2026Presentation\} participantPresentation=\{participantPresentation\} \/>/);
