...
For Scramble, each side exposes required-nullable `playingHandicap` and `strokesReceived` from the final Tournament Live presentation projection used by the PWA. That projection takes non-null participant/team display overrides and otherwise uses the immutable scoring snapshot; nullable legacy display placeholders cannot mask canonical values such as zero strokes. The mobile adapter merely passes through the final values. Those values are presented as “Team Playing Handicap” and `+N team stroke(s)`; individual participant `strokesReceived` is `null` for Scramble. No handicap or stroke value is calculated by the mobile adapter.
...
Returns one strict, bounded, read-only participant Match Detail representation from the same isolated Preview Game Center/Tournament Live authority used by the participant PWA. The `matchId` path component uses the exact opaque identifier contract below, shared with `/matches`, `previousMatchId`, `nextMatchId`, and `myMatchId`. The server-resolved authenticated Player and active tournament membership remain authority: the client cannot supply a Player or tournament selector, and any active participant may read a participant-visible Match only within that exact tournament. Owned and non-owned Matches use the same participant DTO. This route requires the ordinary Supabase Bearer token plus `X-Bagger-Certification`, and it has the same fail-closed Preview authority and participant-isolation boundary as the other protected mobile reads.

#### Canonical opaque Match-ID contract

A Match ID is an exact string of **1–200 Unicode scalar values**. This is neither a UTF-16 code-unit limit nor a user-perceived grapheme-cluster limit: 200 non-BMP scalars are valid even when JavaScript reports 400 UTF-16 units, while 201 scalars are invalid. U+0000 (NUL), malformed surrogate sequences, and only the exact strings `.` and `..` are rejected. The dot exclusions prevent standard URL dot-segment normalization without changing the existing endpoint. Strings such as `...` and `match/../.` remain valid when transported as one encoded component.

Leading, trailing, internal, and whitespace-only strings are preserved. `/`, `#`, `:`, literal percent signs, and non-BMP characters are identifier text, not delimiters or semantic components. No layer may trim, Unicode-normalize, parse, split, infer Round/Match numbers, or substitute `displayMatchNumber`. Canonically equivalent NFC and NFD strings are distinct Match IDs. The value returned by `/matches` must be the exact logical value used for Match Detail lookup and returned in its response.

Clients encode the entire logical ID as one UTF-8 percent-encoded URL path component through their transport/router. For example, `match:round/2#opaque` travels as `match%3Around%2F2%23opaque`; a literal ID containing `%2F` must transport that percent as `%25`, not decode it into `/`. The server extracts the single encoded component from the request URL and decodes exactly once. It does not use Next.js-normalized dynamic parameters: Next.js 15 route matching can transform special parameter-prefix/separator values, including `nxtP`, `nxtI`, and `_NEXTSEP_`. Those values remain ordinary opaque IDs, covered by path regressions. Malformed percent encodings and raw multi-component paths do not select a Match.

`matches.schema.json` and `match-detail.schema.json` carry identical Unicode-aware `matchIdentifier` definitions. `lib/mobile-opaque-match-id.js` enforces the same scalar bound without coercion or transformation. The isolated Preview RPC migration `202609040001_preview_mobile_opaque_match_id_contract.sql` requires UTF8 PostgreSQL encoding, uses `char_length` for the scalar count, and compares Match-ID text under explicit `pg_catalog."C"` collation for exact lookup and navigation. PostgreSQL text/JSONB rejects NUL and malformed surrogate input before lookup. The repair preserves the service-role-only function grant, participant/tournament checks, and existing RLS; it does not change tables, scoring authority, or Production.

`opaque-match-id-fixtures.json` supplies shared synthetic whitespace, reserved-character, percent-literal, non-BMP, normalization-distinct, and Next.js-special-value cases. Deterministic tests additionally generate 200/201-scalar boundaries, strings whose UTF-16 length exceeds 200, and malformed surrogate sequences. These cases exercise collection projection, exact navigation, encoded path transport, Match Detail mapping, and schema agreement.

Native integration remains a separate gate: the 22 preserved Step 2J.3B working files are not changed by this prerequisite. On an explicitly authorized native resume, the existing native validator and obsolete slash-ID rejection assertion must be updated to this approved contract, with exact-string transport/cache regressions. This contract documentation does not certify remote Preview deployment or authorize resuming Swift; remote certification and the minimal native handoff are recorded separately.

#### Participant Match Detail representation
